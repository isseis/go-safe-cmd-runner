...
8. Run the critical-review procedure in `.claude/commands/_lib/review-subagent-pattern.md` with these inputs:
   ...
   - **PERSONA**: default to a **single reviewer** — an experienced senior engineer and senior SRE. A single reviewer is sufficient for routine plans, because the plan checklists below are already exhaustive and leave little room for a combined reviewer to under-cover one lens. Use **panel mode** (per `_lib/review-subagent-pattern.md`, two partitioned reviewers run in parallel) only for **high-risk or large plans** — e.g. a heavy integration-test / CI / external-resource surface, or a security-gate / migration plan (staged rollout, simultaneous behavior raises and lowers, many test updates). When panel mode is warranted, partition by **what to build vs. how to verify it** (the natural seam of a plan), NOT engineer-vs-SRE (those overlap on a plan):
     ...
     - **Reviewer B — test / test-infra (SDET).** Mandate (go deep): every AC has at least one concrete test/static verification task and an explicit AC-verification section (AC→test coverage); tests cover non-trivial logic, error paths, boundary values; creation tasks have ground-truth verification; no unjustified duplicate tests; test-helper files follow the test-organization guide; **the same English-only rule (no Japanese in comments, identifiers, or string literals) applied to Go test source** — `*_test.go`, `testutil/` helpers, AND package-internal test helpers `test_helpers.go` / `test_helpers_<category>.go` (`//go:build test`), which are Go source too and fall in B's lane not A's; AND all **Conditional checks except the phase-order item** (rerun isolation, normalization helpers, narrow linter-suppression scoping, CI scripted-logic extraction, env-skip purity, resource cleanup/leak at acquisition, repeated-CLI startup validation). Out-of-scope (defer to Reviewer A — one-line OUT-OF-LANE FLAGS): production-code task decomposition, file paths, phase ordering, reuse of production code.
   ...
   - **CRITERIA**: every item from the Technical correctness checklist, the Conditional checks section, and the Readability and consistency checklist below, copied verbatim. For Conditional checks, evaluate each item that applies to the plan; mark inapplicable items N/A (N/A is not a finding). In panel mode, give both reviewers all of them as the shared floor (option (b) in `_lib`), but each reports only items within its mandate (deferring the rest as one-line OUT-OF-LANE FLAGS); the Conditional checks are Reviewer B's, except the phase-names/order item which is Reviewer A's (it is an architecture-alignment concern in A's mandate).
   - **Synthesis** (panel mode only): after the parallel panel returns, YOU (not a subagent) merge the two outputs per `_lib` panel-mode step 3 — dedup overlapping findings, reconcile conflicting severities to the higher, reconstruct the full AC→{task,test} matrix from both reviewers' AC checks (so no AC is dropped from either the build or the verify side), and promote any confirmed OUT-OF-LANE FLAG. Then run the fix / re-review loop on the merged findings.
