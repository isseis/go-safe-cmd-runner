...
- **Verify symbols and enumerate all instances before writing.** Confirm every cited function, test, variable, or error name exists in the expected file; note which file when similar names appear in multiple locations. For any symbol or pattern the plan will change or delete, find ALL occurrences across code, comments, test names, error strings, docs, and translations. Cover all search variants: function/variable names, CLI flags, error messages, numeric values, old terminology, and comment phrases. Map each result to its enclosing function and planned action. Add explicit cleanup tasks for stale comments and documentation.
  - Example: `rg -n "func TestDefaultMessageFormatter_FormatLogFileHint" internal/logging/` to confirm the test file; `rg -n "\.FormatLogFileHint\(" -g '*.go' internal` to map every call site.
...
6. Create `03_implementation_plan.md` in the same task directory.
   ...
   **[Always required] structural and traceability rules — apply to every plan:**
   ...
   - Map each acceptance criterion to the tasks and tests that verify it. Every AC row in the Acceptance Criteria Verification section must name a **verification that will be executed**: a test location in `path::TestName` format, a `make` target, or a committed script. Do not use vague labels ("compile passes", "document review", "grep check", "none") — but do not answer vagueness with an unexecuted shell pipeline either.
   - **Put verification where it runs, not in prose.** A `static` check belongs in a guard test, a `make` target, or a script created by the same phase; the AC row names that artifact. The plan states the property that must hold; the artifact states the command. A command written only in the document is never executed until someone copies it by hand, so its defects (a `-e` list that ORs instead of ANDs, a pipe inside a Markdown table cell, a pattern that matches an identifier the same task introduces, a same-name symbol in an unrelated package, `git log` where a count comparison was meant) survive every review and reach the implementer intact.
   - If a one-off command genuinely must appear in the document, **run it while writing the plan** and record its actual output and the commit it was run against. An unexecuted command is an unverified claim.
   - Label each AC verification as `test` (executable, fails on wrong behavior), `static` (a guard test, `make` target, or committed script that greps/compiles), or `manual` (PR observation, deploy check). Every AC must have at least one `test` or `static` entry; `manual` supplements but never replaces them. A static `rg` check is valid only when the AC is purely about textual presence/absence; behavior encoded in scripts, env handling, or command routing needs an executable test (for workflow YAML declarative conditions such as `if:` expressions, manual verification on a test branch is acceptable).
   ...
   - **Do not claim what a verification would catch.** State what a test or check asserts; do not write that it "fails if the implementation were X", that a row is "the only guarantee" of something, or that two constructions are equivalent — those are claims about code that does not exist yet, and they cannot be checked by reading. The requirement that a test be able to fail for its stated reason belongs to the implementer, who confirms it by breaking the thing and records that in the commit message (see CLAUDE.md, Testing Strategy). Write the obligation as a task ("confirm this row fails when the check is reverted"), not as an assertion in prose.
   - **Claims about the existing code carry their evidence.** Any statement that a path is unreachable, that a value is the only one of its kind, or that two call sites share a value must cite `file:line` and the commit it was read at, or be written as a requirement rather than a fact.
   - Add a cross-search checklist **only** for items that `make lint` and `make test` cannot detect: deleted/renamed symbols with potential residual references, cross-package naming conflicts for generic identifiers, and doc/glossary consistency. Prefer converting such an item into a guard test when it is mechanical enough to run; keep it as a manual search only while it is not. Do not duplicate items already covered by the AC verification table — if the same check appears in both the AC table and a cross-search section, keep it in the AC table only. For purely additive tasks (new symbols with a unique prefix, single-package scope), a cross-search checklist is likely unnecessary.
   - **Renames get a ledger, not scattered edits.** When the plan renames a symbol, a file, or a test, put one table in the plan listing old name → new name, and make every other reference — task text, AC rows, cross-search checklist, and the architecture's component table — follow from that table in the same change. A rename spread across a long document leaves stragglers that no build check sees: task 0172's `ValidateGroupNames` → `ValidateIdentifiers` was flagged three separate times, in three different sections, after the rename had been "done".
   - **When the architecture is wrong, stop rather than diverge.** If planning reveals that the approved architecture is mistaken or stale, either get it corrected first or record a single blocking task saying so. Do not write the corrected version into the plan and leave the architecture saying something else — two documents that disagree generate a review finding every time either is read, and the implementer follows the design source (see `docs/dev/developer_guide/requirements_process.md`, "Editing an approved document", for which corrections need a re-approval and which do not).
   - **Keep the plan at planning altitude.** The plan says what to build and what must be verified, and where each lives. It does not write the test table row by row, the exact assertions, the shell pipeline, or the wording of an error message. Those are decided while writing the code, where they are executed and where a mistake surfaces in seconds; written a phase early they are unexecutable claims that only a careful reader can catch, and each one is a place for the plan to be wrong. As a rule of thumb, a phase whose task text runs past about 40 lines is holding detail that belongs in the code — move it there and leave the obligation behind.
   ...
   **[When applicable] apply only when the trigger matches:**
   ...
   - Text edits to a string literal, error message, or source comment: state the complete before/after string, not just the substring to change. This prevents dropped prefixes, dangling `%w`, or stray trailing spaces.
     - Bad:  "Remove ` for more details` from `FormatLogFileHint`'s no-color message."
     - Good: "Change `FormatLogFileHint(100, false)`'s return value from `"HINT: Check log file around line 100 for more details"` to `"HINT: Check log file around line 100"`."
...
8. Run the critical-review procedure in `.claude/commands/_lib/review-subagent-pattern.md` with these inputs:
   - **ARTIFACT**: the created implementation plan document (path in `_context.md`).
   - **PERSONA**: default to a **single reviewer** — an experienced senior engineer and senior SRE. A single reviewer is sufficient for routine plans, because the plan checklists below are already exhaustive and leave little room for a combined reviewer to under-cover one lens. Use **panel mode** (per `.claude/commands/_lib/review-subagent-pattern.md`, two partitioned reviewers run in parallel) only for **high-risk or large plans** — e.g. a heavy integration-test / CI / external-resource surface, or a security-gate / migration plan (staged rollout, simultaneous behavior raises and lowers, many test updates). When panel mode is warranted, partition by **what to build vs. how to verify it** (the natural seam of a plan), NOT engineer-vs-SRE (those overlap on a plan):
     - **Reviewer A — implementation (SWE).** Mandate (go deep): every AC is addressed by ≥1 implementation task (AC→impl coverage); reuse / DRY vs re-implementation; file paths and package-boundary fit; **phase names/order match the approved architecture's priorities (this is the Conditional-checks phase-order item — it is A's, not B's)**; external-assumption verification (tool-flag semantics, real API/lifecycle-method existence, image/CI schema); English-only (no Japanese) in **production** Go comments, identifiers, and string literals (per step 6); the plan references architecture instead of duplicating it. Out-of-scope (defer to Reviewer B — one-line OUT-OF-LANE FLAGS): test coverage/quality, test-infra isolation, helper-file organization, the Conditional checks **other than the phase-order item**.
     - **Reviewer B — test / test-infra (SDET).** Mandate (go deep): every AC has at least one concrete test/static verification task and an explicit AC-verification section (AC→test coverage); tests cover non-trivial logic, error paths, boundary values; creation tasks have ground-truth verification; no unjustified duplicate tests; test-helper files follow the test-organization guide; **the same English-only rule (no Japanese in comments, identifiers, or string literals) applied to Go test source** — `*_test.go`, `testutil/` helpers, AND package-internal test helpers `test_helpers.go` / `test_helpers_<category>.go` (`//go:build test`), which are Go source too and fall in B's lane not A's; AND **every Conditional check below except the phase-order item** — i.e. the entire Conditional section (rerun isolation, normalization helpers, linter-suppression scoping, CI scripted-logic extraction, env-skip purity, resource cleanup/leak at acquisition, repeated-CLI startup validation, real cleanup/close/logout API existence **for test-code lifecycle calls**, build-tag compilation, …; this list is illustrative, B owns all of them). Out-of-scope (defer to Reviewer A — one-line OUT-OF-LANE FLAGS): production-code task decomposition, file paths, phase ordering, reuse of production code, and real-API/lifecycle existence for **production** code (that is A's).
     - The **seam both reviewers cover** is AC traceability — A checks AC→implementation-task, B checks AC→test-task — so the merged AC→{task,test} matrix catches any dropped AC from both ends; reconcile it in synthesis.
   - **FILES**: the implementation plan document, the architecture document, the requirements document, the requirements process guide, and the test organization guide (paths in `_context.md`), as resolved absolute-path strings. In panel mode, pass the same FILES to both reviewers.
...
- [ ] No phase specifies detail that belongs in the code — per-row test tables, exact assertion text, shell pipelines, message wording. Phases whose task text runs well past ~40 lines are checked for this.
...
- [ ] The plan contains no counterfactual claims about unwritten code ("this row fails if the implementation is reverted to X", "this is the only thing that guarantees Y"). Such obligations appear as implementer tasks instead.
- [ ] Statements about existing behavior (reachability, uniqueness, equivalence of two constructions) cite `file:line` and the commit they were verified against.
