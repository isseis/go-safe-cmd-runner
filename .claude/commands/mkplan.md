...
8. Run the critical-review procedure in `.claude/commands/_lib/review-subagent-pattern.md` with these inputs:
   ...
   - **PERSONA**: default to a **single reviewer** — an experienced senior engineer and senior SRE — for routine plans; the plan checklists below are exhaustive enough that a single reviewer rarely satisfices. Use **panel mode** (per `_lib`, two partitioned reviewers run in parallel) only for **high-risk or large plans** — e.g. a heavy integration-test / CI / external-resource surface, or a security-gate / migration plan (staged rollout, simultaneous behavior raises and lowers, many test updates). When panel mode is warranted, partition by **what to build vs. how to verify it** (the natural seam of a plan), NOT engineer-vs-SRE (those overlap on a plan):
     - **Reviewer A — implementation (SWE).** Mandate (go deep): every AC is addressed by ≥1 implementation task (AC→impl coverage); reuse / DRY vs re-implementation; file paths and package-boundary fit; phase order matches the approved architecture's priorities; external-assumption verification (tool-flag semantics, real API/lifecycle-method existence, image/CI schema); no Japanese in **production** Go source; the plan references architecture instead of duplicating it. Out-of-scope (defer to Reviewer B — one-line OUT-OF-LANE FLAGS): test coverage/quality, test-infra isolation, helper-file organization, the Conditional checks.
     - **Reviewer B — test / test-infra (SDET).** Mandate (go deep): every AC has at least one concrete test/static verification task and an explicit AC-verification section (AC→test coverage); tests cover non-trivial logic, error paths, boundary values; creation tasks have ground-truth verification; no unjustified duplicate tests; test-helper files follow the test-organization guide; **no Japanese in Go test code** (`*_test.go` and `testutil/` helpers are Go source too, and fall in B's lane not A's); AND all **Conditional checks** (rerun isolation, normalization helpers, narrow linter-suppression scoping, CI scripted-logic extraction, env-skip purity, resource cleanup/leak at acquisition, repeated-CLI startup validation). Out-of-scope (defer to Reviewer A — one-line OUT-OF-LANE FLAGS): production-code task decomposition, file paths, phase ordering, reuse of production code.
