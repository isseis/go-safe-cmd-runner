...
8. Run the critical-review procedure in `.claude/commands/_lib/review-subagent-pattern.md` with these inputs:
   ...
   - **CRITERIA**: every item from the Technical correctness checklist, the Conditional checks section, and the Readability and consistency checklist below, copied verbatim. For Conditional checks, evaluate each item that applies to the plan; mark inapplicable items N/A (N/A is not a finding). In panel mode, give both reviewers all of them as the shared floor, but each reports only items within its mandate (deferring the rest as one-line OUT-OF-LANE FLAGS); route the Conditional checks to Reviewer B.

   Extra rule: commit the implementation plan document only after all review passes are complete (in panel mode, after synthesizing the two outputs per `_lib`) and all Critical and Major issues are resolved.
