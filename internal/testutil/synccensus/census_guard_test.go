...
var expectedDeclarations = []expectation{
	...
	{"internal/runner/base/output/capture.go", "mutex", "the executor's output-pump reader goroutines for stdout and stderr share this Capture"},
	{"internal/runner/bootstrap/logger.go", "wg", "this WaitGroup is what makes the Slack flush concurrent"},
	{"internal/logging/log_line_tracker.go", "lineCounter", "incremented from the executor's output-pump reader goroutine"},
	{"internal/redaction/error_collector.go", "mu", "reached from the executor's output-pump reader goroutine through the redacting handler"},
