...
func TestApplyTemplate(t *testing.T) {
	...
	// Check that template environment variables were merged
	expectedEnvLen := 3 // USER=test + PATH + HOME
	if len(cmd.Env) != expectedEnvLen {
		t.Errorf("Command env length = %d, want %d", len(cmd.Env), expectedEnvLen)
	}
...
func TestGenerateTempDir(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			commandName: "backup",
			want:        "/tmp/cmd-runner/backup",
		},
		{
			name:        "command name with path",
			commandName: "backup/db",
			want:        "/tmp/cmd-runner/backup_db",
		},
		{
			name:        "command name with spaces",
			commandName: "backup database",
			want:        "/tmp/cmd-runner/backup_database",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := engine.GenerateTempDir(tt.commandName)
			if err != nil {
				t.Errorf("GenerateTempDir() error = %v", err)
				return
			}
			if got != tt.want {
				t.Errorf("GenerateTempDir() = %v, want %v", got, tt.want)
			}
		})
	}
}
