...
// Use parsePclntabAllFuncs when all address ranges are needed.
...
// parsePclntabAllFuncs returns ALL function entries from .gopclntab as a
// slice, including duplicate entries that share the same name (e.g. ABI0
// wrapper stubs generated by the Go compiler since Go 1.17).
// CGO offset correction is applied before returning.
//
// This is used by loadFromPclntab so that every address range of a wrapper
// function is registered in wrapperRanges, preventing false-positive
// unresolved wrapper calls from ABI stub bodies.
func parsePclntabAllFuncs(elfFile *elf.File) ([]PclntabFunc, error) {
	symTable, err := newPclntabSymTable(elfFile)
	if err != nil {
		return nil, err
	}
	...
	if offset := detectPclntabOffset(elfFile, tmpMap); offset != 0 {
		for i := range allFuncs {
			allFuncs[i].Entry = uint64(int64(allFuncs[i].Entry) + offset) //nolint:gosec // G115: offset is bounded by binary size, no overflow risk
			allFuncs[i].End = uint64(int64(allFuncs[i].End) + offset)     //nolint:gosec // G115: offset is bounded by binary size, no overflow risk
		}
	}
