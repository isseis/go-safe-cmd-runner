...
// including duplicate entries that share the same name (e.g. ABI0 wrapper
// stubs generated by the Go compiler since Go 1.17).
...
func parsePclntabFuncs(elfFile *elf.File) ([]PclntabFunc, error) {
	symTable, err := newPclntabSymTable(elfFile)
	if err != nil {
		return nil, err
	}
...
func detectPclntabOffset(elfFile *elf.File, pclntabFuncs map[string]PclntabFunc) int64 {
	...
	// Callers must pass uncorrected entries; parsePclntabFuncs satisfies
