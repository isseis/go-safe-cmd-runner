...
func TestNewRunner(t *testing.T) {
	config := &runnertypes.ConfigSpec{
		...
		Global: runnertypes.GlobalSpec{
			Timeout: commontesting.Int32Ptr(3600),
		},
	}
...
func TestRunner_CommandTimeoutBehavior(t *testing.T) {
	...
	t.Run("command-specific timeout overrides global timeout", func(t *testing.T) {
		...
		shortTimeoutCmd := runnertypes.CommandSpec{
			...
			Timeout: commontesting.Int32Ptr(1), // But timeout after 1 second
		}

		configWithCmdTimeout := &runnertypes.ConfigSpec{
			Version: "1.0",
			Global: runnertypes.GlobalSpec{
				Timeout: commontesting.Int32Ptr(10), // 10 seconds global timeout
			},
			Groups: []runnertypes.GroupSpec{
				{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			config := &runnertypes.ConfigSpec{
				...
				Global: runnertypes.GlobalSpec{
					Timeout:         commontesting.Int32Ptr(30),
					OutputSizeLimit: commontesting.Int64Ptr(1024 * 1024), // 1MB limit
				},
				Groups: []runnertypes.GroupSpec{
					{
...
// TestCreateNormalResourceManager_FailsWithNilVM verifies that createNormalResourceManager
