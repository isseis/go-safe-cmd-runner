...
// executeWithUserGroup still wraps the whole of the prepare, start and
// supervise phases in PrivMgr.WithPrivileges, so for a run-as command the
// readers do run at euid 0. Narrowing the window to the start phase is what
// makes the sentence above a fact rather than a requirement on the caller.
//
