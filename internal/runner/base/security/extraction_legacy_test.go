...
// legacyScanFlags separates positionals from flags. recognized is false when a token
...
func legacyScanFlags(args []string, valueFlags, boolFlags, recursiveFlags map[string]struct{}) (positionals []string, captured map[string][]string, recursive, recognized bool) {
	recognized = true
...
func legacyExtractCopyMove(args []string, isMove bool) extraction {
	...
	pos, captured, recursive, recognized := legacyScanFlags(args, valueFlags, boolFlags, recursiveFlags)
	ext := extraction{applies: true, recognized: recognized, recursive: recursive}
	ext.preserveMeta = legacyHasAny(args, preserveFlags)
	...
	if tdirs := append(append([]string{}, captured["-t"]...), captured["--target-directory"]...); len(tdirs) > 0 {
		legacyAppendTargetDir(&ext, tdirs)
		for _, s := range pos {
...
func legacyAppendTargetDir(ext *extraction, dirs []string) {
	for _, d := range dirs {
		ext.operands = append(ext.operands, rawOperand{raw: d, role: risktypes.OperandRoleWrite})
	}
}

func legacyExtractRemove(args []string) extraction {
	...
	pos, _, recursive, recognized := legacyScanFlags(args, nil, boolFlags, recursiveFlags)
	ext := extraction{applies: true, recognized: recognized, recursive: recursive}
	for _, p := range pos {
...
func legacyExtractShred(args []string) extraction {
	valueFlags := set("-n", "--iterations", "-s", "--size")
	boolFlags := set("-f", "--force", "-u", "--remove", "-v", "--verbose", "-z", "--zero", "-x", "--exact")
	pos, _, _, recognized := legacyScanFlags(args, valueFlags, boolFlags, nil)
...
func legacyExtractLink(args []string) extraction {
	valueFlags := set("-t", "--target-directory", "-S", "--suffix")
	boolFlags := set("-s", "--symbolic", "-f", "--force", "-n", "--no-dereference", "-r", "--relative",
		"-v", "--verbose", "-i", "--interactive", "-T", "--no-target-directory", "-b", "-L", "-P")
	pos, captured, _, recognized := legacyScanFlags(args, valueFlags, boolFlags, nil)
	ext := extraction{applies: true, recognized: recognized}
	...
	isSymlink := legacyHasAny(args, set("-s", "--symbolic"))
	switch len(pos) {
	case 0:
		ext.recognized = false
	case 1:
...
func legacyExtractTruncate(args []string) extraction {
...
func legacyExtractSed(args []string) extraction {
	...
	pos, captured, _, recognized := legacyScanFlags(rest, valueFlags, boolFlags, nil)
	ext := extraction{applies: true, recognized: recognized}
...
func legacyExtractSimpleWrite(args []string, valueFlags map[string]struct{}) extraction {
...
func legacyExtractTee(args []string) extraction {
...
func legacyExtractInstall(args []string) extraction {
	...
	if legacyHasAny(args, set("-d", "--directory")) {
		for _, p := range pos {
			ext.operands = append(ext.operands, rawOperand{raw: p, role: risktypes.OperandRoleWrite})
		}
		if len(pos) == 0 {
...
func legacyExtractTar(args []string) extraction {
	...
	pos, captured, _, recognized := legacyScanFlags(normalizeTarArgs(args), valueFlags, boolFlags, nil)
	ext := extraction{applies: true, recognized: recognized}
	...
	case 't':
		...
		return extraction{applies: false, recognized: true}
	case 'x':
		dir := legacyFirstNonEmpty(captured["-C"], captured["--directory"], legacyAttachedValue(args, "--one-top-level"))
		if dir == "" {
		...
		return ext
	case 'c':
		archive := legacyFirstNonEmpty(captured["-f"], captured["--file"])
		if archive != "" && archive != "-" {
...
func legacyExtractUnzip(args []string) extraction {
	valueFlags := set("-d", "-x")
	boolFlags := set("-o", "-n", "-q", "-qq", "-v", "-j", "-a", "-u", "-f")
	listing := legacyHasAny(args, set("-l", "-Z"))
	pos, captured, _, recognized := legacyScanFlags(args, valueFlags, boolFlags, nil)
	if listing {
		return extraction{applies: false, recognized: true}
	}
	ext := extraction{applies: true, recognized: recognized}
	dir := legacyFirstNonEmpty(captured["-d"])
	if dir == "" {
		dir = "."
	}
	ext.operands = append(ext.operands, rawOperand{raw: dir, role: risktypes.OperandRoleWrite})
...
func legacyExtractDD(args []string) extraction {
...
func legacyExtractMknod(args []string) extraction {
...
func legacyExtractMount(args []string) extraction {
...
func legacyExtractUmount(args []string) extraction {
	valueFlags := set("-t", "--types", "-O")
	boolFlags := set("-a", "--all", "-r", "--read-only", "-v", "--verbose", "-n", "-l", "--lazy",
		"-f", "--force", "-R", "--recursive", "-d")
	pos, _, _, recognized := legacyScanFlags(args, valueFlags, boolFlags, nil)
	ext := extraction{applies: true, recognized: recognized, umountAll: legacyHasAny(args, set("-a", "--all"))}
	for _, p := range pos {
		ext.operands = append(ext.operands, rawOperand{raw: p, role: risktypes.OperandRoleWrite})
	}
...
func legacyExtractChmod(args []string) extraction {
...
func legacyExtractOwner(args []string) extraction {
...
func legacyExtractSetfacl(args []string) extraction {
...
func legacyExtractChattr(args []string) extraction {
	valueFlags := set("-v", "-p")
...
func legacyExtractFind(args []string) extraction {
	ext := extraction{applies: true, recognized: true}
...
// legacyExtractCurl extracts curl's local write destination (-o FILE, or -O which writes
...
func legacyExtractCurl(args []string) extraction {
	...
	pos, captured, _, recognized := legacyScanFlags(args, valueFlags, boolFlags, nil)
	ext := extraction{applies: true, recognized: recognized}
	if out := legacyFirstNonEmpty(captured["-o"], captured["--output"]); out != "" && out != "-" {
		ext.operands = append(ext.operands, rawOperand{raw: out, role: risktypes.OperandRoleWrite})
	} else if legacyHasAny(args, set("-O", "--remote-name")) {
...
// legacyExtractWget extracts wget's local write destination (-O FILE, -P DIR, or the
// working directory by default).
func legacyExtractWget(args []string) extraction {
	...
	ext := extraction{applies: true, recognized: recognized}
	switch {
	case legacyFirstNonEmpty(captured["-O"], captured["--output-document"]) != "":
		out := legacyFirstNonEmpty(captured["-O"], captured["--output-document"])
		if out != "-" {
			ext.operands = append(ext.operands, rawOperand{raw: out, role: risktypes.OperandRoleWrite})
		}
	case legacyFirstNonEmpty(captured["-P"], captured["--directory-prefix"]) != "":
		ext.operands = append(ext.operands, rawOperand{raw: legacyFirstNonEmpty(captured["-P"], captured["--directory-prefix"]), role: risktypes.OperandRoleWrite})
	default:
...
// legacyExtractScp extracts scp's destination (the final operand). A remote destination
// is an upload (egress); a local destination is zone-classified.
func legacyExtractScp(args []string) extraction {
	...
	return legacyExtractRemoteCopy(args, valueFlags, boolFlags)
}

// legacyExtractRsync extracts rsync's destination (the final operand). A remote
...
func legacyExtractRsync(args []string) extraction {
...
// legacyExtractRemoteCopy is the shared SRC... DEST extractor for scp/rsync.
func legacyExtractRemoteCopy(args []string, valueFlags, boolFlags map[string]struct{}) extraction {
	pos, _, _, recognized := legacyScanFlags(args, valueFlags, boolFlags, nil)
	ext := extraction{applies: true, recognized: recognized}
	if len(pos) < minSpecAndTarget {
		ext.recognized = false
...
// legacyExtractSftp treats sftp as a network egress: its actual writes live in an
...
func legacyExtractSftp(args []string) extraction {
	_ = args
	return extraction{applies: true, recognized: true, remoteEgress: true}
}

func legacyHasAny(args []string, flags map[string]struct{}) bool {
	for _, a := range args {
...
func legacyFirstNonEmpty(lists ...[]string) string {
	for _, l := range lists {
		if len(l) > 0 && l[0] != "" {
			return l[0]
		}
...
// legacyAttachedValue returns the values of an optional-argument flag given in the
// attached --flag=value form (the only form GNU getopt accepts for optional args).
func legacyAttachedValue(args []string, flag string) []string {
	prefix := flag + "="
	var vals []string
	for _, a := range args {
...
// legacyZoningSpecs mirrors zoningSpecs (command -> frozen legacy extraction) so the
// differential test can invoke the pre-refactor extraction for each command.
var legacyZoningSpecs = map[string]func([]string) extraction{
	"cp":       func(a []string) extraction { return legacyExtractCopyMove(a, false) },
	"mv":       func(a []string) extraction { return legacyExtractCopyMove(a, true) },
	"rm":       legacyExtractRemove,
	"rmdir":    legacyExtractRemove,
	"unlink":   legacyExtractRemove,
	"shred":    legacyExtractShred,
	"ln":       legacyExtractLink,
	"truncate": legacyExtractTruncate,
	"sed":      legacyExtractSed,
	"touch": func(a []string) extraction {
		return legacyExtractSimpleWrite(a, set("-r", "--reference", "-d", "--date", "-t"))
	},
	"mkdir":   func(a []string) extraction { return legacyExtractSimpleWrite(a, set("-m", "--mode")) },
	"tee":     legacyExtractTee,
	"sponge":  func(a []string) extraction { return legacyExtractSimpleWrite(a, nil) },
	"install": legacyExtractInstall,
	"tar":     legacyExtractTar,
	"unzip":   legacyExtractUnzip,
	"dd":      legacyExtractDD,
	"mknod":   legacyExtractMknod,
	"mount":   legacyExtractMount,
	"umount":  legacyExtractUmount,
	"chmod":   legacyExtractChmod,
	"chown":   legacyExtractOwner,
	"chgrp":   legacyExtractOwner,
	"setfacl": legacyExtractSetfacl,
	"chattr":  legacyExtractChattr,
	"find":    legacyExtractFind,
	"curl":    legacyExtractCurl,
	"wget":    legacyExtractWget,
	"scp":     legacyExtractScp,
	"sftp":    legacyExtractSftp,
	"rsync":   legacyExtractRsync,
}
