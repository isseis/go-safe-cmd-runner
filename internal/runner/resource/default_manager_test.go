...
func TestDefaultResourceManager_ModeDelegation(t *testing.T) {
	...
	t.Run("Normal Mode", func(t *testing.T) {
		mgr, err := NewDefaultResourceManagerForTest(mocks.exec, mocks.fs, mocks.priv, mocks.pathResolver, slog.Default(), ExecutionModeNormal, &DryRunOptions{}, nil, 0)
	...
	t.Run("Dry Run Mode", func(t *testing.T) {
		mgr, err := NewDefaultResourceManagerForTest(mocks.exec, mocks.fs, mocks.priv, mocks.pathResolver, slog.Default(), ExecutionModeDryRun, &DryRunOptions{DetailLevel: DetailLevelDetailed}, nil, 0)
		require.NoError(t, err)
