...
func TestSubstitutionHashEscape_IsNormalEncoding(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "fallback encoding does not start with tilde",
			encoded:  "AbCdEf123456.json",
			expected: false,
		},
		{
			name:     "empty string",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := encoder.IsNormalEncoding(tt.encoded)
			assert.Equal(t, tt.expected, result)
		})
	}
}
...
func TestSubstitutionHashEscape_IsFallbackEncoding(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := encoder.IsFallbackEncoding(tt.encoded)
			assert.Equal(t, tt.expected, result)
		})
	}
}
