...
// remediation; neither is chosen by inspecting the detail text. Which advice
// a cause maps to differs between builds, so adviseIncompleteness decides it.
func incompleteEnumerationError(groupGID uint32, verdict completenessVerdict) error {
	advice := adviseIncompleteness(verdict.cause)
	...
	return fmt.Errorf("cannot confirm the members of group GID %d: %s (%s); %s: %w",
		groupGID, advice.fact, state, advice.remediation, ErrGroupMemberEnumerationIncomplete)
}
