...
func TestVerify_MachODyldCacheOnly(t *testing.T) {
	...
	verifyErr := m.verifyDynLibDeps(resolved, map[string]string{})
...
func TestVerify_MachOLibraryTampered(t *testing.T) {
	...
	m, err := NewManagerForTest(hashDir)
	require.NoError(t, err)
	require.NoError(t, m.verifyDynLibDeps(cmdPath, map[string]string{}),
	...
	verifyErr := m2.verifyDynLibDeps(cmdPath, map[string]string{})
