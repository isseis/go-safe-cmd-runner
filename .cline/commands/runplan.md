...
> uses the shared procedure in `.cline/commands/_lib/review-self-pattern.md`.
...
- **State invariants before coding.** For any generated value, flag/mode, or side-effecting operation, write its contract in one line as a code comment above the implementation and implement to that — not to the first approach that compiles. Examples that would have prevented real bugs: `--dry-run` → *no command execution or external side effects* (only validate configuration, check file hashes, and perform security checks); a Slack webhook URL → *never logged or exposed in error messages; only sent over TLS; stored in configuration, never in code*; a hash verification → *always verify file hashes against the recorded database before executing any command; fail closed if verification cannot complete*.
...
- For any documentation-only edit (prose, tables, translation pairs, progress-tracking docs like `03_implementation_plan.md` or a process memo), read the actual `git diff` for that file before committing — a misplaced blank line or a paragraph appended after the wrong neighbor can silently break Markdown table rendering or move content out of its section without changing a single word, which a review pass focused on wording alone will not catch (see `docs/tasks/0014_runplan_model_tiering/memo.md` for a case where this recurred after the same lesson was already written down once).
- **Before committing each group, self-check** (catches common defects before the step 7 review):
  ...
  - Reuse existing utilities before writing new ones (e.g. an existing ID-generation helper); consolidate duplicate regexes/constants.
...
6.5. Run programmatic pre-checks on the changed Go files before the review step. These checks are deterministic and cheaper than AI review — catch them here rather than in the review loop.
...
7. Run the critical self-review inline (no subagent — Cline performs this review directly in the current conversation). Follow the procedure in `.cline/commands/_lib/review-self-pattern.md` with these inputs:
   - **ARTIFACT**: this phase group's code changes.
   - **PERSONA**: an experienced senior Go engineer and senior SRE. Surface bugs, missing test coverage, architecture drift, and unclear code.
   - **FILES**: the architecture document and the implementation plan document (paths in `_context.md`), as resolved absolute-path strings; read both in full before evaluating the code. Also list the source files added or modified in this phase group as resolved absolute-path strings (read in full). Provide the specific commit range for this phase group (e.g., `HEAD~N..HEAD`) and run `git diff <range>` to see exactly what changed.
   - **CRITERIA**: every item from the phase-group review checklist below, copied verbatim.

   Extra rule: when fixing Critical and Major issues, run the build checks (defined in `_context.md`) and commit before re-running the review.

Phase-group review checklist (use verbatim as evaluation criteria in the review above):
...
- [ ] Any test helper that assumes uniqueness or completeness of a matched set (e.g. "the one request to X", "the only occurrence of Y") actually asserts that count instead of silently taking the first match.
- [ ] When a fact or behavior is described in more than one documentation location (e.g. a config field's semantics appearing in both a reference table and a package-reference bullet), all locations agree with each other and with the implementation — not just the one edited most recently.
