...
-   認証情報（パスワード、APIキーなど）を別ファイル（例: .env）で管理する機能。
-   多重実行を防止する機能。
-   指定されたファイルのハッシュ値と、コマンド実行直前のファイルのハッシュ値を比較し、ファイルが改ざんされていないか検証する機能。
    -   ファイル改ざんの検証にはinternal/filevalidator にあるパッケージを使用する。
-   検証ルールを定義し、複数のコマンドで再利用可能な形で管理する機能。
-   コマンド定義をテンプレート化し、共通の設定を再利用する機能。
    -   テンプレートの継承とプロパティのオーバーライドをサポート。
    -   テンプレートのデバッグを容易にする機能を提供する。
...
-   グループ単位で定型的な処理を容易に行えるようにする機能：
    -   コマンド実行前に作業用のテンポラリディレクトリを自動作成し、そのディレクトリ内でコマンドを実行する。
    -   コマンド実行後に作業用のテンポラリディレクトリを自動削除する。
    -   ファイルの整合性検証を自動的に行う。
    -   環境変数の自動設定とマッピング。
    -   実行コンテキスト（カレントディレクトリ、環境変数など）の自動管理。
...
-   ログは標準出力に人間が読みやすい形で書き出す。
    -   テンプレートや検証ルールの読み込み状態をログに記録する。
    -   ファイル検証の結果を詳細にログに記録する。
...
- **機能ID:** `F-000`
- **機能名:** 認証情報管理
- **概要:** 認証情報を安全に管理し、コマンド実行時に安全に渡す機能を提供する。
- **詳細:**
  - 環境変数ファイル（.env形式）からの認証情報の読み込み
  - 認証情報ファイルのパーミッションチェック（600のみ許可）
  - 認証情報のメモリ上での保護
  - コマンド実行時の環境変数としての安全な受け渡し
  - 認証情報のログ出力防止
  - プロセスリストへの認証情報の漏洩防止
  - 認証情報の有効期限管理（将来対応）
...
- 認証情報の安全な管理
  - 認証情報は環境変数経由でのみ渡す
  - 認証情報ファイルのパーミッションは600に制限
  - メモリ上での認証情報の保護（スワップ防止、メモリクリア）
  - プロセスリスト（ps）に認証情報が表示されない仕組み
