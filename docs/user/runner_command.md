...
#### Dry-run Exit Codes and Unverified Content
...
- **Policy deny** — the computed effective risk exceeds the command's `risk_level`, or a non-environment Blocking deny (e.g., symlink-resolution failure, indirect-execution form that cannot be bound).
- **Tampering signal** — verification was attempted *and* a `hash_mismatch` was detected. This is the only reason that qualifies as a tampering signal. It applies to unverified config/template files, `verify_files` failures, and any other `FailureReason` with `hash_mismatch`.
- **Environment cause** — verification could not be attempted, or failed for a reason other than `hash_mismatch` (e.g., hash database not configured, hash file not found, file read error, permission denied).

The priority order is:

1. Policy deny → exit `1` (overrides everything else).
2. Tampering signal (`hash_mismatch`) in unverified content or verification failures → exit `1`.
3. Environment cause only (no tampering signal) → exit `3`.
4. Verification-unavailable deny (no unverified content/failures present) → exit `3`.
5. None of the above → exit `0`.
...
| Exit code | Meaning |
|-----------|---------|
| `0` | All commands allowed, all file verification successful. |
| `1` | Policy deny, or a tampering signal (`hash_mismatch`) in unverified config/template content or `verify_files` failures. |
| `3` | Environment cause only (no `hash_mismatch`): unverified content or verification failures due to missing hash database/missing hash file/permission errors/etc., or a verification-unavailable deny. |

**Use Cases**

- **CI verifiability gate**: the dry-run exit code is always non-zero when any verification issue exists, so CI pipelines automatically fail on missing hash records, `hash_mismatch` tampering signals, or `verify_files` failures.
- **Local preview**: check the `UNVERIFIED` and `Failures` sections of the output — these are always shown regardless of detail level — especially any `UNVERIFIED-TAMPER` entry or `security_risk: high` annotation.

**Usage Examples**

```bash
# Dry-run with text output (summary level)
runner -config config.toml -dry-run -dry-run-detail summary

# Dry-run with full detail
runner -config config.toml -dry-run -dry-run-detail full
```
...
All non-zero dry-run exits should be treated as failures. Exit code `3` is diagnostic information for root-cause analysis (distinguishing environment causes from tampering signals), not a signal that the run should be treated as successful.
...
The `Error Message` field of `pre_execution_error` shows the paths of the failed files as a `Files:` section. Each path is wrapped in quotes and listed in a display form that escapes `"`, `\`, control characters and the like. Each path listed in the `Files:` section is subject to redaction, so a keyed secret such as `password=...` or a token-like value is shown as `[REDACTED]` rather than verbatim. When the list fits within the display limit, all entries are shown; when it exceeds the limit, only the entries that fit are shown and the remainder is indicated as an omitted count with ` (+m more)`. When not even one entry fits in full (such as when a single path is very long), the first path is truncated to a length that fits within the limit and shown with `…` appended after the closing quote (e.g., `Files: "/very/long/pa"… (+2 more)`). In this case, ` (+m more)` is the count of the remaining paths other than the first, and it is not appended when there is only one path. The message body and `Component` contain no paths; the group name does not appear in the message body as separate metadata and is shown in Scope (a failed file path may contain a string identical to the group name). The body of a global verification error also changes to the same format, and the failed file paths previously in the body move to the `Files:` section. `error_message` is now deliberately path-free, and the `Files:` section is assembled only by the Slack notification builder, so it never appears in the structured `error_message` value. Update any external scripts that match on `error_message` or structured-log consumers that extract failed paths to read the failed paths from the new `failed_file_paths` attribute.

When collection of verification targets fails (when resolving a command's path fails), the commands that failed resolution are also shown, as written in the configuration (the value of `cmd` after variable expansion), in the same `Files:` section. When only a command name is specified, as in `cmd = "backup-tool"`, it is shown as `"backup-tool"` rather than as a path. In this case the message body becomes `Collection failed: <unresolved target count> of <total target count> targets unresolved, Error: failed to collect verification files`, and the verification breakdown (`Total`/`Verified`/`Failed`) is not shown.
