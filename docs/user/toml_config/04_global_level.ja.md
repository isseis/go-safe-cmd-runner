...
- **TOML展開専用**: `cmd`, `args`, `env_vars`, `verify_files` の値を展開
...
4. **セキュリティ**: 機密情報は vars で管理し、必要な場合のみ env_vars で公開

## 4.3 env_import - システム環境変数の取り込み
...
3. **最小限の公開**: 子プロセスに必要な環境変数のみを env_vars で定義
...
- **Group.env_vars**: グループレベルの環境変数については第5章を参照
- **Command.env_vars**: コマンドレベルの環境変数については第6章を参照
- **変数展開の詳細**: 変数展開の仕組みについては第7章を参照

## 4.5 env_allowed - 環境変数許可リスト
...
**注意（ラッパー経由のインナーコマンド）**: ラッパー（`env`/`timeout`/`nice` 等）が内部で実行する**インナーコマンド**は、自動的なハッシュ検証・identity 束縛の対象外です（自動的にハッシュ検証されるのは外側のラッパー実行ファイルのみ。インナーは監査ログにチェーン成果物として記録されますが、これは実体の固定ではありません）。ラッパー経由のインナーは一律 **High** 扱いとなり、明示的に `risk_level = "high"` を設定しない限り実行されません。インナーの実体を固定したい場合は、当該パスを `verify_files` に明示登録してください。ただし `verify_files` への登録は起動時のハッシュ検査にとどまり、ラッパーが実行時に解決・exec する実体を束縛するものではない点に注意してください（詳細は[リスク評価ガイド](../risk_assessment.ja.md)の §8 移行ノートを参照）。
