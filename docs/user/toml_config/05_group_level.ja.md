...
| **必須/オプション** | 必須 |
| **設定可能な階層** | グループのみ |
| **有効な値** | 英字またはアンダースコアで始まり、英数字とアンダースコアのみで構成される（`[A-Za-z_][A-Za-z0-9_]*`）。ハイフンは使えない |
| **長さ** | 128 バイト以下 |
| **一意性** | 設定ファイル内で一意である必要がある |
| **禁止される語** | 後述の「名前に含められない語」を参照 |
...
グループ名とコマンド名は Slack 通知やログに「どのグループ・コマンドで問題が起きたか」を示す
ために使われます。通知やログは秘匿情報のマスク処理（redaction）を通ってから送られるため、
マスク処理が書き換えてしまう名前は通知の対象を示せなくなります。そのような名前は起動時に
拒否されます。

具体的には、以下の語を大文字・小文字を区別せず部分文字列として含む名前は使えません。

- `password`、`token`、`secret`、`key`
- `bearer`、`basic`、`authorization`
- `google_application_credentials`

`basic_commands`、`test_basic`、`monkey`（`key` を含む）、`tokenizer`（`token` を含む）は
いずれも拒否されます。また、AWS アクセスキー ID や GitHub トークンのような認証情報の形式に
一致する名前も拒否されます。

拒否された場合、エラーメッセージには位置（`groups[0]` など）のみが示され、名前そのものは
表示されません。名前が認証情報である可能性があり、エラーはマスク処理を通らずに標準エラー
出力へ書かれるためです。
