...
コマンドは以下の2つの方法で定義できます：

1. **直接定義**: `cmd` と `args` を直接指定する方法（本章で説明）
2. **テンプレート使用**: 事前定義されたテンプレートを参照する方法（[第7章](07_command_templates.ja.md)で詳細を説明）
...
| **必須/オプション** | 必須 |
| **設定可能な階層** | コマンドのみ |
| **有効な値** | 制御文字・書式制御文字（改行、タブ、双方向制御文字など）を含まず、空白以外の表示可能な文字を 1 文字以上含む文字列。英数字、アンダースコア、ハイフンでの命名を推奨 |
| **長さ** | 128 バイト以下 |
| **一意性** | グループ内で一意である必要がある（別のグループでは同じ名前を使える） |
| **禁止される語** | グループ名と同じ。[5.1.1 name - グループ名](05_group_level.ja.md#511-name---グループ名) の「名前に含められない語」を参照 |
...
| **medium** | 中リスク | ネットワーク通信、ファイル作成・変更 |
| **high** | 高リスク | 動的コード実行、別プロセス起動、破壊的ファイル操作 |

#### リスク評価の仕組み

runner は実行前にコマンドのリスクを自動算出し、`risk_level` を超えると実行を拒否します。リスクはコマンド名・引数の評価と、`record` コマンド実行時のバイナリ静的解析の 2 つの源から算出されます。最終リスクはすべての因子の最大値です。

詳細な算出ルール（何が検出されると medium/high になるか）と確認方法は [リスク評価ガイド](../risk_assessment.ja.md) を参照してください。
...
コマンドは直接定義する代わりに、事前定義されたテンプレートを参照することもできます。テンプレートを使用すると、共通のコマンド定義を一カ所で管理し、複数のグループで再利用できます。

### 6.7.1 template - テンプレート参照

#### 概要

事前定義されたコマンドテンプレートを参照します。
...
|-----|------|
| **型** | 文字列 (string) |
| **必須/オプション** | オプション（`cmd` と排他的） |
| **設定可能な階層** | コマンドのみ |
| **有効な値** | 定義済みのテンプレート名 |

#### 排他的フィールド

`template` を指定した場合、以下のフィールドは指定できません：
...
### 6.7.2 params - テンプレートパラメータ

#### 概要

テンプレートに渡すパラメータを指定します。テンプレート内の `${param}`, `${?param}`, `${@list}` がこれらの値で置換されます。
...
| 項目 | 内容 |
|-----|------|
| **型** | テーブル（文字列または文字列配列の値を持つ） |
| **必須/オプション** | テンプレートの定義による |
| **設定可能な階層** | コマンドのみ（`template` と併用時のみ） |
...
# 結果: args = ["-v", "--no-cache", "backup", "/data/important"]
```

#### 変数との組み合わせ

`params` の値には変数参照（`%{...}`）を含めることができます：
...
# 結果: args = ["backup", "/data/backups/daily"]
```

> **詳細**: テンプレート機能の詳細（パラメータ展開の種類、エスケープ、ベストプラクティスなど）については[第7章: コマンドテンプレート](07_command_templates.ja.md)を参照してください。

## 次のステップ

次章では、コマンドテンプレート機能について詳しく解説します。共通のコマンド定義を一カ所で管理し、複数のグループで再利用する方法を学びます。
