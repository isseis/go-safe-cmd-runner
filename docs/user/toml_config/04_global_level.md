...
**Note (inner commands run through a wrapper)**: An **inner command** that a wrapper (`env`/`timeout`/`nice`, etc.) executes internally is outside automatic hash verification and identity binding (only the outer wrapper executable is automatically hash-verified; the inner command is logged in the audit chain, but that does not pin its identity). An inner command run through a wrapper is treated as a flat **High**, and is not executed unless you explicitly set `risk_level = "high"`. If you need to pin the inner command's identity, register its path explicitly in `verify_files`. Note, however, that registration in `verify_files` only adds a startup-time hash check and does not bind the actual object the wrapper resolves and execs at run time (see §8 Migration Notes in the [Risk Assessment Guide](../risk_assessment.md) for details).
