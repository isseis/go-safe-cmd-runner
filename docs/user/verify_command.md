...
target host lets you detect in advance whether the denial would occur. `verify` only reads and
settles the completeness verdict at startup, so it can be used as-is for a pre-run check before
`runner`.
