...
さらに、作成先（指定したパスのうち実在する最深の祖先）が誰からでも書き込めるディレクトリ（world-writable）である場合は、sticky ビットの有無にかかわらず作成せずにエラー終了します。作成しようとする位置に他者があらかじめハッシュ記録を置ける状態では、`record` が処理していないファイルの記録が混ざりうるためです。この場合は、利用者自身が先にディレクトリを作成し（適切な所有者と権限を与えたうえで）、あらためて `record` を実行してください。
