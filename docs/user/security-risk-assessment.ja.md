...
func (m *UnixPrivilegeManager) WithPrivileges(elevationCtx runnertypes.ElevationContext, fn func() error) (err error) {
    ...
    defer m.handleCleanup(execCtx)                     // クリーンアップフェーズ
    return fn()
}
```
...
- ✅ **危険環境変数検出**: 動的ローダ制御変数（`LD_*`・`DYLD_*` prefix。例: `LD_PRELOAD`、`DYLD_INSERT_LIBRARIES` 等）とインタプリタ起動時コード注入変数（`BASH_ENV`・`PYTHONPATH`・`NODE_OPTIONS` 等）を包括的に検出・拒否
- ✅ **特権コマンド検証**: root権限実行の厳格チェック
- ✅ **設定整合性**: 重複・矛盾検出による安全性確保

#### 破壊的変更: 環境変数 denylist の拡張

本バージョンより、禁止環境変数名の対象リストが拡張されました。新たに `DYLD_*`（macOS 動的ローダ制御変数）、`GLIBC_TUNABLES`、インタプリタ起動時コード注入変数（`BASH_ENV`、`PYTHONPATH`、`NODE_OPTIONS`、`RUBYOPT`、`PERL5LIB` 等）が3層すべて（実行層・config 層・security 層）で拒否されます。あわせて、config 層の `env_vars` KEY に対する denylist チェックが新設されたため、従来の対象変数（`LD_*`、`GCONV_PATH` 等）を `env_vars` に指定していた設定もロードエラーとなります。

**影響**: `env_vars` または `env_import` でこれらの変数を指定していた設定は、改修後に config ロードエラー（`ErrForbiddenEnvVar`）となります。たとえば `ENV=production` のような一般的な変数名が denylist に該当する場合は、変数名の変更（例: `APP_ENV=production`）で対処してください。`ENV` を denylist に含める判断は、POSIX の `$ENV` がおもに対話シェルで解釈され本ランナーの非対話実行では注入強度が相対的に低いこと、非対話スクリプト経路の主ベクタはすでに `BASH_ENV` が塞いでいること、そして誤ブロックが問題化した場合は `APP_ENV` 等の非予約名で回避できることを考慮したものです。
...
**識別子の免除**: group 名とコマンド名は識別子として宣言され、値ベース redaction の対象外（免除）になります。これは通知の Scope や JSON ログの名前に、どの group・どの command で起きたのかを表示するための仕様です。そのため、**設定の名前に機密情報を書くと、その文字列は通知とログにそのまま現れます**。名前に機密情報を書かないでください。名前に `=` や `:` が含まれていても、識別子は書き換えられません。stdout・stderr・展開済みコマンド行・引数・環境変数値・message・error 文字列の redaction は従来どおり適用され、error 文字列に連結された名前は免除されません。

**Slack 通知の配送方式**: Slack 通知は非同期に送信されます。ログの書き込みは Slack の応答を待ちません。プロセス終了時には、未送信の通知が既定 15 秒の期限（`GSCR_SLACK_FLUSH_TIMEOUT` で変更可能）の内で flush されます。Slack が到達不能な場合やプロセスが強制終了された場合（SIGKILL、電源断、OOM kill など）には、送信キューに残った通知が失われる可能性があります。ただし通知の内容自体は、Slack への配送前に JSON ログファイル（0600）へ同期的に書き込まれているため、`run_id` で特定できる完全な記録が残ります。通常の再起動（SIGTERM / SIGINT）では flush が行われます。通知はワーカー 1 本の直列処理で送信されるため、Slack が応答しない状態が続くと、後続の通知の到達が最大で「送信キュー長 × 送信デッドライン（既定 40 秒）」遅延し得ます。送信キューは優先度 2 段で、高優先度キュー（実行前エラー `pre_execution_error`）は容量 32、通常キューは容量 128 です。`GSCR_SLACK_SYNC=1` を設定すると同期送信に戻せますが、これは障害調査用のデバッグ手段であり、通常運用向けではありません。通常キューが飽和すると、通常優先度のコマンド失敗通知（`command_group_summary` と `user_group_command_failure`）が破棄され得ます。これらを高優先度キューへ移すと実行前エラーの通知を押し出し得るため通常優先度のままとし、破棄の個別記録と flush 時の種別別集計を運用上の検知手段としています。
