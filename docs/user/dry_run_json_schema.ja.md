...
**出力ストリーム**: ドライランモードでは、runnerコマンドは出力ストリームを分離します:
- **標準出力 (stdout)**: ドライランJSON出力（本スキーマで定義）
- **標準エラー出力 (stderr)**: 実行ログ（slogメッセージ）

この分離により、ドライラン出力とログを独立してリダイレクトできます。使用例については、[runner コマンドユーザーガイド](runner_command.ja.md)を参照してください。
