...
### 5.7 Write to a Group-Writable File Is Denied (Enumeration Incomplete)

**Symptom**

If a path component of the hash directory, or of an ancestor directory of a file being
recorded, is group-writable, and this host cannot confirm that group member enumeration is
complete, the write-safety check is denied and `record` exits without recording any target
