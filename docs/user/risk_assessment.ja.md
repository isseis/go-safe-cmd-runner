...
| `high` | 破壊的操作・権限昇格・動的/任意コード実行 | ✅ 設定可 |
...
| その他のシステム変更コマンド（`mount`/`crontab`/`mkfs`、`apt install` などのパッケージ導入/削除） | `medium` |
...
最近の coreutils は、1 つの実行ファイルが多数のサブコマンドを実装する単一マルチコールバイナリ（例: BusyBox、または統合された GNU coreutils バイナリ）として配布される場合があります。このようなバイナリでは、リスクを **サブコマンド** から次の 3 区分で分類します。

| coreutils サブコマンドの分類 | 算出リスク |
|------------------------------|-----------|
| 既知の安全な読み取り専用サブコマンド（`echo`、`cat`、`ls` 等） | `low` |
| 副作用はあるが影響範囲が限定的なサブコマンド | `medium` |
| 破壊的サブコマンド、または不明・判別不能なサブコマンド（フェイルセーフ） | `high` |

検証済みの coreutils バイナリについては、安全な読み取り専用サブコマンドではバイナリ解析次元（§3.3）を抑制し、共有マルチコールバイナリがネットワークや `exec` のシンボルをリンクしていても、例えば `echo` は `low` のままになります。ハッシュ検証は依然として必須であり、抑制されるのはバイナリ解析シグナルのみで、同一性の検証は抑制されません。
...
- **パッケージスクリプトランナー**（`npm run`/`npx`/`yarn`/`pnpm run`）: `high`。
...
- **ラッパーコマンド**（`env`/`timeout`/`nice`/`sudo ...`）: ラップされた内部コマンドが評価・ゲートされます。内部コマンドを抽出できないラッパー、またはローダ制御の環境変数（`LD_PRELOAD`、`DYLD_*` 等）を渡すラッパーは拒否されます。
