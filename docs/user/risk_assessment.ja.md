...
既知の不整合が検出された場合は `high` 扱いになります。予期しないエラーが発生した場合はリスクレベルに丸め込まれず、実行がエラーとして失敗します。

| 条件 | 挙動 |
|------|------|
| バイナリのハッシュが未確認（同一性不明） | `high` 扱い |
| 解析記録が存在しない | `high` 扱い |
| ディスク上のバイナリとハッシュが一致しない | `high` 扱い |
| 解析記録のスキーマバージョンが古い | `high` 扱い |
| 解析結果が不確定 | `high` 扱い |
| 予期しない記録読み込みエラー | 実行がエラーで失敗 |
