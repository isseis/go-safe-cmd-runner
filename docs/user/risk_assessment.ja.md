...
- **AI サービスコマンド**（`claude`、`gemini` 等）: `high` になりました。常に外部 API と通信し、データを持ち出す可能性があるためです。
...
- **`risk_level = "unknown"`**: 設定エラーとして拒否されます。`low`/`medium`/`high` を使ってください。
- **バイナリ解析・ファイル検証が無効**: Blocking 拒否になります。同一性を確認できないバイナリは実行されません。
- **ラッパーコマンド**（`env`/`timeout`/`nice` 等）: ラッパー経由で実行されるインナーコマンドは、インナーの内容によらず一律 **High** で評価されるようになりました（無害なインナーであっても、明示的に `risk_level = "high"` を設定しない限り実行されません）。権限昇格トークン（`sudo`/`su`/`doas`）がインナー（ネストしたラッパーの内側を含む）に現れる場合は **Critical** で常に拒否され、一部の形態（ローダ制御環境変数 `LD_*`/`DYLD_*`・作業ディレクトリ変更 `env -C`・解釈不能な `env -S`・`find`/`xargs` の子プロセス実行・動的ローダ直接起動・`rsync -e`/`tar --to-command` 等のヘルパ実行・インナーを抽出できないラッパー・ネスト深さ上限超過・symlink 解決失敗）は依然 **Blocking** で拒否されます（これらは High に緩和されません）。**インナーは自動検証・自動記録されない**ため、インナーの実体を固定したい場合は当該パスを `verify_files` に明示登録してください（ただし下記の残存リスクに留意）。
