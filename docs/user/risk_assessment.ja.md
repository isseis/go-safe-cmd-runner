...
| `systemctl` の変更系サブコマンド（`start`/`stop`/`restart`/`enable`/`disable` 等） | `high` | 読み取り専用サブコマンド（`status`/`show` 等）は low より上の下限（§3.1 の表を参照） |
...
| パッケージマネージャ（`apt`/`yum`/`dnf`/... の install/remove） | `medium` | 個々のツール名は列挙せず汎用的に扱う |
