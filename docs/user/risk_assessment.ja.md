...
`record --debug-info` で確認し、以下を判断します:

| 状況 | 推奨 |
|------|------|
| 実際にネットワークを使うコマンド（wget, curl 等） | `medium` を設定 |
| ネットワーク API を持つが実際には使わないコマンド | `medium` を設定（保守的に） |
| 誤検知と明確に判断できる場合 | 開発チームに報告して調査 |

> **注意**: 誤検知への対処として `low` を設定しないでください。実際にネットワーク API にアクセスした場合に検出できなくなります。
