...
- **ファイル操作コマンド（`rm`/`dd`/`cp`/`mv`/`ln`/`chmod`/`chown` 等）**: 名前だけでは固定せず、対象パスの信頼区分でレベルが決まる（§3.5）。以前は「破壊的ファイル操作は一律 `high`」「`rm`/`dd` は無条件 `high`」としていたが、確定分類では対象パス依存。
...
- 従来は名前だけで一律 `high` としていた `rm`/`dd` 等が、safe-zone・ordinary の通常操作では引き下げられます（移行ノート §8.8）。
- 逆に、従来検出されなかった「ファイル操作の宛先が trust-critical」が `high` に引き上げられます（§8.6）。
