...
> パッケージマネージャと `systemctl`/`service` は、未検証のメンテナンススクリプトや unit/init スクリプト（dpkg の `postinst`、rpm の `%post`、pip の `setup.py`、npm の `postinstall` 等）を特権実行し得るため、サブコマンド・引数を解析せず、解決済みバイナリ名のみで一律 `high` とします。照会系（`apt list`/`dpkg -l`/`systemctl status` 等）も `high` です。
> `rm`/`dd`/`cp`/`mv`/`ln`/`chmod`/`chown` 等の **ファイル操作コマンド** は、名前だけでは固定せず、対象パスの信頼区分でレベルが決まります（§3.5）。これらは以前「破壊的ファイル操作は一律 `high`」「`rm`/`dd` は無条件 `high`」としていましたが、確定分類では対象パス依存です。
...
| 条件 | 算出リスク |
|------|-----------|
| 特権付与（setuid/setgid・world-write の付与、trust-critical への所有権付与） | `high` |
...
> safe-zone への引き下げには安全要件があります。対象が「信頼ディレクトリの許可リスト配下」であり、かつ「実行ユーザーから書込不可（評価後・実行前にすり替えられない＝TOCTOU 安全）」のときのみ `low` になります。満たせない場合や、実 CLI に無いフラグを与えて形が解釈しきれない場合は、安全側に倒して `high` で拒否します（fail-closed）。
>
> この判断軸により、従来は名前だけで一律 `high` としていた `rm`/`dd` 等が、safe-zone・ordinary の通常操作では引き下げられます（移行ノート §8.8 を参照）。逆に、従来検出されなかった「ファイル操作の宛先が trust-critical」が `high` に引き上げられます（§8.6）。
