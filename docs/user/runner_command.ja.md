...
#### ドライランの終了コードと未検証コンテンツ

**概要**

ドライランモードでは、同一性を検証できなかったコマンド（検証不能による拒否）、ハッシュ検証に成功せずに採用された設定／テンプレートファイル、および `verify_files` の検証失敗は、**常にハードな失敗（非ゼロ終了）として扱われます**。これを無効化するフラグはなく、これが唯一の挙動です。

ドライランの終了コードは 3 つの状態を区別します。

- **ポリシー拒否** — 算出された実効リスクがコマンドの `risk_level` を超える場合、または環境起因でない Blocking 拒否（例：シンボリックリンク解決失敗、束縛できない間接実行形態）。
- **改ざん兆候** — 検証を試行し*かつ* `hash_mismatch`（記録ハッシュと実体の不一致）を検出した場合。改ざん兆候に該当する唯一の理由です。未検証の設定／テンプレートファイル、`verify_files` の失敗、その他 `FailureReason` で `hash_mismatch` を持つものすべてに適用されます。
- **環境起因** — 検証を試行できなかった、または `hash_mismatch` 以外の理由で失敗した場合（例：ハッシュ DB 未設定、ハッシュファイル未登録、ファイル読み取りエラー、権限拒否）。

優先順位は以下のとおりです。

1. ポリシー拒否 → 終了コード `1`（他の事象より優先）。
2. 改ざん兆候（`hash_mismatch`）を含む未検証成果物または検証失敗 → 終了コード `1`。
3. 環境起因のみ（改ざん兆候なし）の未検証成果物・検証失敗 → 終了コード `3`。
4. 検証不能による拒否（未検証成果物・検証失敗が存在しない場合）→ 終了コード `3`。
5. 上記のいずれでもない → 終了コード `0`。
...
| 終了コード | 意味 |
|-----------|------|
| `0` | すべてのコマンドが許可され、すべてのファイル検証が成功。 |
| `1` | ポリシー拒否、または未検証の設定／テンプレートコンテンツまたは `verify_files` の失敗に改ざん兆候（`hash_mismatch`）が含まれる。 |
| `3` | 環境起因のみ（`hash_mismatch` なし）：ハッシュ DB 未整備／ハッシュファイル未登録／権限エラー等による未検証コンテンツまたは検証失敗、または検証不能による拒否。 |

**ユースケース**

- **CI の検証可能性ゲート**: 検証上の問題が 1 件でもあれば常に非ゼロ終了するため、ハッシュレコードの欠落、`hash_mismatch` による改ざん兆候、`verify_files` の失敗を CI パイプラインが自動的に検出します。
- **ローカルプレビュー**: 出力の `UNVERIFIED` および `Failures` セクション（詳細レベルによらず常に表示されます）を確認し、特に `UNVERIFIED-TAMPER` エントリや `security_risk: high` 注釈に注意してください。

**使用例**

```bash
# テキスト出力のドライラン（summary レベル）
runner -config config.toml -dry-run -dry-run-detail summary
...
`-dry-run-fail-unverified` フラグは **削除されました**（認識されなくなり、指定すると未定義フラグエラーになります）。従来このフラグでオプトインしていた挙動は **常に有効** になりました。

- 既存の呼び出しで `-dry-run-fail-unverified` を指定していた場合は、フラグを除去するだけで同一の挙動になります。
- フラグを指定して**いなかった**呼び出しでは、ハッシュ DB が未整備または不完全な状態の dry-run が、従来の exit 0 から **非ゼロ**（改ざん兆候がない場合はコード `3`）に変わります。未検証コンテンツがあっても exit 0 に依存していたスクリプトは更新してください。

**運用規則**

dry-run の非ゼロ終了はすべて失敗として扱ってください。終了コード `3` は原因究明のための診断情報（環境起因と改ざん兆候を区別する）であり、実行が成功したことを示すシグナルではありません。

**注意事項**

- ポリシー拒否または改ざん兆候（終了コード `1`）は、環境起因（終了コード `3`）より優先されます。
- この終了コードの挙動は常に有効であり、無効化できません。
...
`<timestamp>` は `YYYYMMDDThhmmssZ` 形式の UTC です（ホストのタイムゾーンには従いません）。
...
`pre_execution_error` の `Error Message` フィールドには、検証に失敗したファイルのパスが `Files:` 節として現れます。各パスは引用符で囲み、`"`・`\`・制御文字などをエスケープした表示形で並びます。`Files:` 節に並ぶ各パスは redaction の対象となり、`password=...` のようなキー付きの機密値やトークン形式の値は、そのままではなく `[REDACTED]` に置き換えて表示されます。一覧が表示上限に収まるときは全件を表示し、超えるときは上限内に収まる範囲だけを表示して、残りを ` (+m more)` で省略件数として示します。1 件も丸ごとは収まらない場合（1 つのパスが非常に長い場合など）は、先頭のパスを上限に収まる長さで切り詰め、閉じ引用符の後に `…` を付けて表示します（例: `Files: "/very/long/pa"… (+2 more)`）。このときの ` (+m more)` は先頭以外の残りのパスの件数で、パスが 1 件だけなら付きません。本文と `Component` にはパスを含めず、group 名は本文へ別個のメタデータとしては現れず Scope に表示されます（失敗ファイルのパスに group 名と同名の文字列が含まれることはあります）。グローバル検証エラーの本文も同じ書式に変わり、以前は本文に含まれていた失敗ファイルのパスは `Files:` 節へ移りました。`error_message` は失敗ファイルのパスを含まない形に意図的に変わり、`Files:` 節は Slack 通知ビルダーだけが組み立てるため構造化ログの `error_message` には現れません。`error_message` を照合する外部スクリプトや、失敗したパスを抽出する構造化ログの消費者は、新しい `failed_file_paths` 属性から失敗したパスを取り出すよう更新してください。

検証対象の収集に失敗した場合（コマンドのパス解決に失敗した場合）も、解決に失敗したコマンドを設定に記述されたとおり（変数展開後の `cmd` の値）に同じ `Files:` 節に表示します。`cmd = "backup-tool"` のようにコマンド名だけを指定した場合は、パスではなく `"backup-tool"` と表示されます。このとき本文は `Collection failed: <解決に失敗した対象数> of <対象総数> targets unresolved, Error: failed to collect verification files` となり、検証の内訳（`Total`／`Verified`／`Failed`）は示しません。
...
#### sudo 経由で起動した場合のファイル読み取り拒否

`sudo runner` で起動した場合、ファイル読み取り可否の判定に用いる UID（基準UID）が呼び出し元ユーザーの UID から `0`（root）へ変わります。これは、`runner` が setuid ビットを用いた一般ユーザーによる起動を想定運用としており、sudo 経由の起動では `SUDO_UID` 環境変数に基づく基準UIDの決定を行わないようにしたためです。

この変化により、グループ書き込み可能なファイルについて、root がそのファイルのグループの構成員でなければ、読み取りが拒否されることがあります。これは権限判定が厳しくなる方向への変化であり、権限の抜け穴ではありません。

想定される運用（`install -m 4755` した `runner` を一般ユーザーが起動する形態）および root の cron から直接起動する形態では、この変化の影響を受けません。

**対処法**: `sudo runner` の代わりに、`install -m 4755` で setuid ビットを設定した `runner` を一般ユーザーとして起動してください。
