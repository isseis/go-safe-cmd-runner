...
- **ローカルプレビュー（既定）**: 本番のハッシュ DB を持たないローカルプレビューではフラグを省略し、検証不能による拒否を失敗させずに注記として表示する。ドライラン出力の `UNVERIFIED` セクション、特に `UNVERIFIED-TAMPER` エントリも確認すること。`-dry-run-fail-unverified` を指定していない場合はこれらは終了コードに影響しないため。
...
- 詳細設計、および本フラグの未検証の設定／テンプレートコンテンツへの拡張については `docs/tasks/0146_security_hardening/02_architecture.md` §3.4 を参照してください。
