...
ドライランモードにおいて、**この環境で検証できなかった**コマンドを、既定の「注記として報告し終了コード 0 で終わる」挙動の代わりに、ハードな失敗（非ゼロ終了）として扱います。

ドライランは、プロセスの終了コードで 2 種類の拒否を区別します。
...
`-dry-run-fail-unverified` を指定すると、検証不能による拒否を失敗扱いにオプトインでき、実際のポリシー拒否と区別できる専用の終了コードを返します。
...
これは `detailed`/`full` の詳細レベルで常に表示されます。`-dry-run-fail-unverified` を指定した場合、この未検証コンテンツもドライランの終了コードに反映されます。真のポリシー拒否より低い優先度で、次のように区分されます。

- *検証失敗*（`verify_failed_<failure_reason>`）はポリシー拒否と同じ終了コード `1` にマップされ、ハッシュ不一致による改ざん兆候（あるいはその他の検証失敗）が環境起因の終了コードに埋没しません。
- *環境起因*（`skipped_no_validator`）は、検証不能による拒否と同じ終了コード `3`（`DryRunExitVerificationUnavailable`）にマップされます。

既定（フラグ未指定）では、未検証コンテンツは注記としてのみ報告され、終了コードには影響しません（終了コード 0）。本番のハッシュ DB を持たないローカルドライランが不必要に失敗しないようにするためです。
...
| 終了コード | 意味 |
|-----------|------|
| `0` | すべてのコマンドが許可される、または既定（注記のみ）挙動での検証不能による拒否／未検証コンテンツ |
| `1` | ポリシー拒否 — 少なくとも 1 つのコマンドの実効リスクが `risk_level` を超える、または環境起因でない Blocking 拒否 — または、`-dry-run-fail-unverified` を指定した場合の、検証に失敗した（`verify_failed_<failure_reason>`、例：`hash_mismatch` による改ざん兆候）未検証の設定／テンプレートコンテンツ |
| `3` | 検証不能による拒否 — `-dry-run-fail-unverified` を指定した場合のみ — または、同フラグを指定した場合の、環境起因（例：`skipped_no_validator`）による未検証の設定／テンプレートコンテンツ |

**ユースケース**

- **CI の検証可能性ゲート**: 本番のハッシュ DB を持つ CI 環境で `-dry-run-fail-unverified` を使い、いずれかのコマンドが検証できない場合にビルドを失敗させ、デプロイ前にハッシュレコードの欠落を検出する。
- **ローカルプレビュー（既定）**: 本番のハッシュ DB を持たないローカルプレビューではフラグを省略し、検証不能による拒否および未検証コンテンツを失敗させずに注記として表示する。ドライラン出力の `UNVERIFIED` セクション、特に `UNVERIFIED-TAMPER` エントリも確認すること。

**使用例**

```bash
# 既定: 検証不能による拒否は注記として報告（終了コード 0）
runner -config config.toml -dry-run

# CI: 検証不能による拒否で失敗させる（終了コード 3）
runner -config config.toml -dry-run -dry-run-fail-unverified
```

**注意事項**

- ポリシー拒否（終了コード `1`）は、検証不能による拒否（終了コード `3`）より優先されます。同様に、未検証コンテンツの検証失敗（`verify_failed_<failure_reason>`、こちらも終了コード `1`）は、環境起因の検証不能による拒否（終了コード `3`）より優先されます。
- このフラグはドライランにのみ影響し、`-dry-run` なしでは効果がありません。
- 未検証の設定／テンプレートコンテンツの終了コードへの反映も含む詳細設計については `docs/tasks/0146_security_hardening/02_architecture.md` §3.4 を参照してください。
