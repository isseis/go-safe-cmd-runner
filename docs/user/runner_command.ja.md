...
ドライランモードにおいて、**この環境で検証できなかった**コマンドを、既定の「注記として報告し終了コード 0 で終わる」挙動の代わりに、ハードな失敗（非ゼロ終了）として扱います。タスク 0146 により、フラグの対象は「未検証コンテンツ（unverified content）」にも拡張されました。これはハッシュ検証に成功せずにドライランが採用した設定／テンプレートファイルです（例：ハッシュ DB が利用できない、あるいは記録されたハッシュが欠落／不一致で、ドライランが `os.ReadFile` にフォールバックした場合）。

ドライランは、プロセスの終了コードで 3 種類の拒否を区別します。

- **ポリシー拒否** — 算出された実効リスクがコマンドの `risk_level` を超える場合、または環境起因でない Blocking 拒否（例：シンボリックリンク解決失敗、束縛できない間接実行形態）。常に非ゼロで終了します。
- **検証不能による拒否** — この環境でバイナリ解析／ファイル検証が利用できず、コマンドの同一性を**ここでは**確認できない場合（例：本番のハッシュ DB が設定されていないローカルや CI のドライラン、またはコマンドの同一性ハッシュが未確立の場合）。既定ではこれは注記として報告するのみで、ドライランを失敗させません（終了コード 0）。プレビュー実行を不必要に壊さないためです。（解析レコードの欠落や不一致のような真のポリシー違反は、検証不能による拒否ではなくポリシー拒否です。）
- **未検証コンテンツ**（タスク 0146）— ドライランがフォールバックして `os.ReadFile` 経由で読み込んだため、ハッシュ検証に成功せずに設定／テンプレートファイルが採用された場合。2 つのサブケースが出力上で区別され、フラグ指定時にそれぞれ異なる終了コードに対応付けられます。
  - *環境起因*（ドライラン用バリデータが未設定、例：ハッシュディレクトリが書き込み不可）。ファイルは出力の **`UNVERIFIED`** セクションに `skipped_no_validator` 理由と `security_risk: low` 注釈とともに表示されます。`-dry-run-fail-unverified` 指定時は、原因が環境（改ざんではない）であるため、終了コード `3`（検証不能）で終了します。
  - *改ざん兆候*（検証を試みたが失敗した、例：記録ハッシュとディスク内容との `hash_mismatch`）。ファイルは `verify_failed_<failure_reason>` 理由で表示され、**`UNVERIFIED-TAMPER`** タグと `security_risk: high` 注釈が付与されます。`-dry-run-fail-unverified` 指定時は、改ざん兆候を環境起因コードに埋没させないため、終了コード `1`（ポリシー拒否）で終了します。

`-dry-run-fail-unverified` を指定すると、検証不能による拒否と未検証コンテンツの両方を失敗扱いにオプトインでき、環境起因と改ざん兆候を区別する終了コードを CI ログ上で維持します。
...
| 終了コード | 意味 |
|-----------|------|
| `0` | すべてのコマンドが許可される、または環境起因の拒否のみが観測された（既定の注記のみ挙動） |
| `1` | ポリシー拒否 — 少なくとも 1 つのコマンドの実効リスクが `risk_level` を超える、環境起因でない Blocking 拒否、または改ざん兆候を伴う未検証コンテンツ（例：`verify_failed_hash_mismatch`） |
| `3` | 検証不能による拒否、または環境起因の未検証コンテンツ（例：`skipped_no_validator`）— `-dry-run-fail-unverified` を指定した場合のみ |

**ユースケース**

- **CI の検証可能性ゲート**: 本番のハッシュ DB を持つ CI 環境で `-dry-run-fail-unverified` を使い、コマンドが検証できない場合や設定ファイルが未検証の場合にビルドを失敗させ、ハッシュレコードの欠落（環境起因）とハッシュ不一致（改ざん兆候）の両方をデプロイ前に検出する。
- **ローカルプレビュー（既定）**: 本番のハッシュ DB を持たないローカルプレビューではフラグを省略し、環境起因の未検証コンテンツを `UNVERIFIED` セクションに失敗させずに注記として表示する。`UNVERIFIED-TAMPER` エントリはフラグの有無に関わらず常に強調されるため、終了コード 0 であっても調査が必要である。

**使用例**

```bash
# 既定: 検証不能による拒否または環境起因の未検証コンテンツは注記として報告
# （終了コード 0）。UNVERIFIED-TAMPER エントリは依然として強調され、
# この終了コードでも調査が必要である。
runner -config config.toml -dry-run

# CI: 検証不能による拒否（終了コード 3）、または改ざん兆候を伴う
# 未検証コンテンツ（終了コード 1）で失敗させる。
runner -config config.toml -dry-run -dry-run-fail-unverified
```

**注意事項**

- ポリシー拒否（終了コード `1`）は、検証不能による拒否（終了コード `3`）より優先されます。
- 既定挙動（フラグなし）は未検証コンテンツが採用されてもドライランを失敗させません。ドライラン出力の `UNVERIFIED` セクションがオペレータが調査すべき指標です。
- ハッシュ不一致（`UNVERIFIED-TAMPER`）はフラグの有無に関わらずドライラン出力で `security_risk: high` として注釈付けされるため、改ざん兆候は既定の終了コード 0 挙動に埋もれません。
- このフラグはドライランにのみ影響し、`-dry-run` なしでは効果がありません。
- 詳細設計と未検証コンテンツ状態機械は `docs/tasks/0146_security_hardening/02_architecture.md` §3.4 を参照してください。
