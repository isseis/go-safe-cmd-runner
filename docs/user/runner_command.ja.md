...
**未検証の設定／テンプレートコンテンツ（表示のみ、本フラグへの結線は未実装）**

上記の終了コード挙動とは別に、ドライラン出力は、ハッシュ検証に成功せずに採用された設定／テンプレートファイル（例：ハッシュ DB が利用できない、あるいは記録されたハッシュが欠落／不一致で、ドライランが `os.ReadFile` にフォールバックした場合）を、本フラグの指定有無にかかわらず、ファイル検証レポート内の独立した **`UNVERIFIED`** セクションに区別表示します。
...
これは `detailed`/`full` の詳細レベルで、`-dry-run-fail-unverified` の指定有無にかかわらず終了コード 0 のまま表示されます。**この未検証コンテンツを `-dry-run-fail-unverified` のハード失敗対象に含める（環境起因と改ざん兆候を区別した専用の終了コードを割り当てる）拡張は設計済みですが未実装です** — `docs/tasks/0146_security_hardening/03_implementation_plan.md` の Step 4-6 を参照してください。現時点では、ハッシュ不一致の設定ファイルは出力上に表示されますが、ドライランの終了コードには影響しません。
...
| 終了コード | 意味 |
|-----------|------|
| `0` | すべてのコマンドが許可される、または既定（注記のみ）挙動での検証不能による拒否 |
| `1` | ポリシー拒否 — 少なくとも 1 つのコマンドの実効リスクが `risk_level` を超える、または環境起因でない Blocking 拒否 |
| `3` | 検証不能による拒否 — `-dry-run-fail-unverified` を指定した場合のみ |

**ユースケース**

- **CI の検証可能性ゲート**: 本番のハッシュ DB を持つ CI 環境で `-dry-run-fail-unverified` を使い、いずれかのコマンドが検証できない場合にビルドを失敗させ、デプロイ前にハッシュレコードの欠落を検出する。
- **ローカルプレビュー（既定）**: 本番のハッシュ DB を持たないローカルプレビューではフラグを省略し、検証不能による拒否を失敗させずに注記として表示する。ドライラン出力の `UNVERIFIED` セクション、特に `UNVERIFIED-TAMPER` エントリも確認すること。これらは現時点では終了コードに影響しないため。
...
- 詳細設計、および本フラグを未検証の設定／テンプレートコンテンツへ拡張する（現時点では未実装の）計画については `docs/tasks/0146_security_hardening/02_architecture.md` §3.4 を参照してください。
