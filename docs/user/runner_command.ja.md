...
`sudo runner` で起動した場合、ファイル読み取り可否の判定に用いる UID（基準UID）が呼び出し元ユーザーから `0`（root）へ変わります。これは、`runner` が setuid ビットを用いた一般ユーザーによる起動を想定運用としており、sudo 経由の起動では `SUDO_UID` 環境変数に基づく基準UIDの推測を行わないようにしたためです。
