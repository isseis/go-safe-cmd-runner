...
- *改ざん兆候*（`verify_failed_<failure_reason>`、例：ハッシュ不一致）はポリシー拒否と同じ終了コード `1` にマップされ、環境起因の終了コードに埋没しません。
...
| `1` | ポリシー拒否 — 少なくとも 1 つのコマンドの実効リスクが `risk_level` を超える、または環境起因でない Blocking 拒否 — または、`-dry-run-fail-unverified` を指定した場合の、改ざん兆候（例：`hash_mismatch`）を伴う未検証の設定／テンプレートコンテンツ |
...
- ポリシー拒否（終了コード `1`）は、検証不能による拒否（終了コード `3`）より優先されます。同様に、未検証コンテンツの改ざん兆候（終了コード `1`）は、環境起因の検証不能による拒否（終了コード `3`）より優先されます。
