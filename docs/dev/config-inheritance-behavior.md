...
| Configuration Item | Global | Group | Command | Inheritance/Merging Behavior | Notes |
|-------------------|--------|-------|---------|------------------------------|-------|
| **env** | - | - | ✓ | **Independent (no cross-level merging)**: Only environment variables defined in Command.Env are used. Independent between multiple Commands | Each Command has its own env<br>Independent behavior, not Union |
...
2. **env**: By having each Command possess its own set of environment variables, independence between Commands is ensured.
...
- **Multi-value items** (env, env_allowlist, verify_files): Union or Override choices are possible, but current implementation adopts **Override or Independent** for all
...
2. **Independent Pattern** (env): Managed independently at each level (no cross-level merging)
...
2. **Command Independence**: By having each Command possess its own environment variables with env, unintended influence between Commands is prevented.
