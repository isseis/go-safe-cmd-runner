...
- `docs/tasks/0077_cgo_binary_network_detection/05_requirements_pclntab_symtab_free.md` - CGO バイナリでの pclntab アドレスずれ問題と `.symtab` 非依存化の要件（CGO バイナリで問題が再発した場合の調査起点）。なお同文書は magic = `0xfffffff1`（Go 1.20+）を対象とするが、これは `debug/gosym` へ解析を委譲するため pcHeader 構造を直接解析しないことによる。本ドキュメントの「Go 1.25+ のみサポート」は pcHeader 構造（ftabOffset の有無）の話であり、レイヤーが異なる。
- `docs/tasks/0077_cgo_binary_network_detection/ac1_verification_result_x86_64.md` - x86_64 CGO バイナリでのアドレスずれ実測データ（−0x100）
