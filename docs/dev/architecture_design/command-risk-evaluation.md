...
- **`output_file` is out of scope**: writing to an arbitrary path via `output_file` is not evaluated by the runtime risk assessment (the output path is only checked for traversal). Protect output targets through the surrounding configuration and filesystem permissions.
