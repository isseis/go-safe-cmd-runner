...
宣言サイトと、宣言面 allowlist・目録 guard・固定アサーションは同じコミットに含まれるため、
revert は 3 つを同時に戻し、CI は緑のままです。同じコミットに別の宣言サイトが混在する場合は、
`git revert -n <commit>` で revert を保留し、戻す hunk を絞ってから `make test` を実行し、
guard の green を確認します。
