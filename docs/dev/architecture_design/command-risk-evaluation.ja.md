...
ファイル操作コマンド（`cp`/`mv`/`rm`/`rmdir`/`unlink`/`shred`/`ln`/`truncate`/`sed`/`touch`/`mkdir`/`sponge`/`tee`/`install`/`tar`/`unzip`/`dd`/`mknod`/`chmod`/`chown`/`rsync`/`mount`）について、引数が指す対象パス（書込先・リンク先・削除対象・デバイス・読み取り元）の **信頼区分** でレベルを判定する（`security.ClassifyDestinationZone`、`destination_zoning.go`）。これは固定 High の破壊系判定を置き換える **唯一の判定基準** である（完全認識時。`evaluateDimensions` が `IsDestructiveFileOperation`・該当する `CheckDangerousArgPatterns`・プロファイル破壊要因・coreutils 破壊/不明 High を抑止する）。
...
- `Level`／`ReasonCodes`／`Operands`：合成レベル・理由コード・オペランド毎の判定（`OperandZone` の配列。監査出力 `operand_zones` の carrier）。
...
ゾーンに加え、対象ゾーンに依らない操作固有の下限が効く（setuid/setgid・world-write・trust-critical 所有権の付与＝High、ブロックデバイス入出力 `dd of=/dev/sdb`＝High、safe-zone 外へ及ぶツリー再帰＝High、機密ファイルの複製＝Medium 下限）。`dd` のデバイス入出力はパスのゾーンではなくデバイスの種別で判定する（`/dev/null` 等の無害なシンクは Low、生デバイスは High）。
...
- `operand_zones`: 判断軸2（宛先パス信頼区分）のオペランド毎の判定。各要素は `index`／`raw`／`resolved`／`zone`／`role`／`matched_critical`／`trusted`／`unresolved_err` を持つ。`raw`／`resolved`／`unresolved_err` は `command_args` と同じ出力境界の redaction を経由してマスクする。判断軸2 が適用されたコマンド（carrier が `len()>0`）のときのみ出力し、空・nil ではキー自体を付けない（`reason_codes`／`risk_factors` と同じ存在条件）。
