...
- **判断軸2: 宛先パス信頼区分**（`security.ClassifyDestinationZone`）。ファイル操作コマンドの宛先パスの信頼区分でレベルを判定する（後述）。**完全に認識**できた（`Applies && Recognized`）ファイル操作コマンドでは、この結果が権威的な分類となり、固定 High の破壊系の各軸（破壊的ファイル操作・該当する危険引数パターン・プロファイルの破壊要因・coreutils の破壊/不明 High）を **抑止** して置き換える。部分認識・非適用のときは抑止せず、解釈しきれない形が過小評価されないようにする（fail-open 回避）。
- **ランク4: coreutils 単一バイナリ分類**（`security.CoreutilsCommandRisk`）。該当する場合は権威的に扱い、バイナリ解析軸（ランク8）を抑止する。ただし他の軸は引き続き寄与する（判断軸2 が完全認識した場合は破壊/不明 High を落とし setuid シグナルのみ保持）。
- **破壊的ファイル操作**（`security.IsDestructiveFileOperation`）→ High（判断軸2 の完全認識時は抑止）。
...
High（`highSystemModificationNames`）は次の 2 系統に大別される。いずれも引数によらず名前だけで High（read-mostly な形 `sysctl -a` も write と同様に扱う fail-safe）。

- 未検証コードを特権実行し得るエントリポイント:
  - パッケージマネージャ（`apt`, `apt-get`, `yum`, `dnf`, `zypper`, `pacman`, `brew`, `pip`, `npm`, `yarn`, `dpkg`, `rpm`）。未検証のメンテナンススクリプト（dpkg `postinst`、rpm `%post`、pip `setup.py`、npm `postinstall` 等）を特権実行し得るため、サブコマンドによらず一律 High（`apt list` のような照会も High）。
  - `systemctl`（全サブコマンド。照会系の `status`/`show`/`list-*` を含む）・`service`（未検証の init スクリプトを実行）。
- 大規模・不可逆な破壊、または永続的なシステム状態変更:
  - ディスク・パーティション破壊系（`wipefs`, `blkdiscard`, `sgdisk`, `gdisk`, `cgdisk`, `sfdisk`, `cfdisk`, `mkswap`, `parted`, `fsck`, `fdisk`, `mkfs`）、直接ファイルシステムユーティリティ（`e2fsck`, `mke2fs`, `tune2fs`, `resize2fs`, `xfs_*`, `btrfs`）、LVM 破壊系（`lvremove`, `vgremove`, `pvremove`, `lvreduce` 等）。
  - カーネルモジュール・パラメータ（`insmod`, `modprobe`, `rmmod`, `kexec`, `sysctl`）。
  - アカウント・認証 DB（`useradd`, `usermod`, `userdel`, `passwd`, `chpasswd`, `visudo` 等）。
  - ブートローダ・カーネルイメージ（`grub-install`, `grub-mkconfig`, `efibootmgr`, `kernel-install`, `update-initramfs`, `dracut` 等）。
  - サービス有効化・電源/runlevel（`chkconfig`, `update-rc.d`, `shutdown`, `reboot`, `halt`, `poweroff`, `telinit`）。
  - ファイアウォール（`iptables`, `ip6tables`, `nft`, `ufw`, `firewall-cmd`, `ebtables`, `arptables`）。
  - 能力付与（`setcap`）・信頼境界の置換（`update-alternatives`, `alternatives`, `dpkg-divert`, `ldconfig`）。
  - ジョブ・遅延・transient 実行スケジューラ（`crontab`, `at`, `batch`, `systemd-run`）。

Medium（`mediumSystemModificationNames`）は、未検証コードの特権実行や大規模・不可逆破壊を伴わない限定スコープのシステム状態変更に限る。

- `mount`, `umount`、LVM の作成・設定系（`lvcreate`, `vgcreate`, `lvextend`, `vgchange` 等）、粗いネットワーク設定（`ip`, `ifconfig`, `route`, `iwconfig`, `iw`）。

該当しない場合は `RiskLevelUnknown` を返し、システム変更軸は寄与しない。`fdisk`/`mkfs` はこの名前集合に加え `CheckDangerousArgPatterns`（および `mkfs.*`/`fsck.*` の接頭辞規則）でも High となり、両者は max で合成される。
...
## 宛先パス信頼区分（判断軸2 / `ClassifyDestinationZone`）

ファイル操作コマンド（`commandFlagSpecs` の全エントリ＝`cp`/`mv`/`rm`/`rmdir`/`unlink`/`shred`/`ln`/`truncate`/`sed`/`touch`/`mkdir`/`sponge`/`tee`/`install`/`mknod`/`tar`/`unzip`/`dd`/`chmod`/`chown`/`chgrp`/`setfacl`/`chattr`/`find`/`rsync`/`scp`/`sftp`/`curl`/`wget`/`mount`/`umount`）について、引数が指す対象パス（書込先・リンク先・削除対象・デバイス・読み取り元）の **信頼区分** でレベルを判定する（`security.ClassifyDestinationZone`、`destination_zoning.go`）。権限変更系（`chmod`/`chown`/`chgrp`/`setfacl`/`chattr`）は `KindPermission`、データ転送系（`curl`/`wget`/`scp`/`sftp`/`rsync`）は `KindDataTransferWrite`、`find` は破壊的アクション時のみ適用される。これは固定 High の破壊系判定を置き換える **唯一の判定基準** である（完全認識時。`evaluateDimensions` が `IsDestructiveFileOperation`・該当する `CheckDangerousArgPatterns`・プロファイル破壊要因・coreutils 破壊/不明 High を抑止する）。

判定結果（`LocationResult`）は次を持つ。

- `Applies`：ファイル操作コマンドで、かつ書込を伴う形のとき true（`sed` の `-i` 無し・`tar -t`・`unzip -l`・破壊的アクションの無い `find` は false）。
- `Recognized`：**完全認識**。argv をすべて解釈でき、かつ全オペランドが解決できたとき true。どちらかが満たせなければ false。
- `Level`／`ReasonCodes`／`Operands`：合成レベル・理由コード・オペランド毎判定（`OperandZone` の配列。監査出力 `operand_zones` の carrier）。

ゾーン判定とオペランド毎下限（floor）:

| 区分 | 例 | レベル |
|------|----|--------|
| trust-critical（`Config.SystemCriticalPaths`。`/usr`・`/etc`・`/boot`・`/dev`・`/var` 等） | 宛先 `/usr/bin` | High |
| ordinary（trust-critical でも safe-zone でもない通常パス） | 宛先 `/srv` | Medium |
| safe-zone（信頼ディレクトリ許可リスト配下、かつ run-as から書込不可＝TOCTOU 安全） | 宛先 `$WORKDIR` | Low |
| 解決不能（`ZoneUnresolved`。変数展開未確定・symlink 解決失敗・identity 未解決 等） | — | 書込=High／読取=Medium（非対称 fail-closed） |

ゾーンに加え、対象ゾーンに依らない操作固有の下限が効く（権限付与＝High〔`chmod` の setuid/world-write、`install -m` の setuid・`-o`/`-g`、`chattr`/`setfacl` 等が `grantsPermission` を立てる。`chown` の所有権変更には専用の下限は無く、trust-critical 宛先はゾーンレベルで High になる〕、ブロックデバイス入出力 `dd of=/dev/sdb`＝High、safe-zone 外へ及ぶツリー再帰＝High、`cp -p`/`-a` の特権メタデータ複製＝High、機密ファイルの複製＝Medium 下限）。`dd` のデバイス入出力はパスのゾーンではなくデバイスの種別で判定する（`/dev/null` 等の無害なシンクは Low、生デバイスは High）。

> **fail-closed の `Recognized` 契約が安全保証を担う**: 完全認識できなかった形（`Recognized=false`）は、固定 High 抑止を行わず、かつ判断軸2 自身が **下限 High** を課す（`res.Level` を High へ引き上げ `ReasonUnresolvedDestination` を付す）。これにより実効リスクは常に High 以上となる（fail-closed）。最終的な実行可否はこの実効リスクと設定の `risk_level` 上限との比較で決まる（`risk_level = "high"` 未満なら拒否）。

### 宣言的フラグ仕様と単一 getopt パーサ（0144／0145）

オペランド抽出は、コマンドごとに独自実装した引数パーサではなく、**宣言的なフラグ仕様**（`flag_spec.go` の `commandFlagSpecs`：コマンド→`CommandFlagSpec`〔`Kind`・`Flags`・`ToExtraction`〕）と、それを駆動する **単一の getopt パーサ**（`getopt.go`）で行う（0144 の集中リファクタ。挙動保存）。各コマンドのフラグ集合の正しさは完全性メタテスト（`flag_spec_test.go`）で担保する。

0145 は各コマンドのフラグ集合を **実 CLI（GNU coreutils と uutils の和集合）に整合** させ、実 CLI に存在しないフラグの受理（過剰認識）を除去した（安全側の挙動変更）。例として `touch` の `-p`/`-i`、`mkdir` の `-a`/`-c`/`-h`/`-f`/`-i`/`-r`、`sponge` の `-r` などは実 CLI に無いため宣言しない。これらを与えた入力は getopt が未知フラグとして扱い `Recognized=false` となり、上記の fail-closed 下限により実効リスクが High へ引き上げられる。`risk_level` が `"high"` 未満の設定では拒否される（`recognized=true→false`）。

旧来あった「実 CLI に無いフラグも recognized のまま通す」過剰認識は解消済みであり、これを旧制約として残さない。安全保証は fail-closed の `Recognized` 契約が担う点は不変である。
...
> **例外（run-as 識別情報の検証失敗）:**
> run-as 識別情報（`run_as_user` / `run_as_group` の解決）の検証失敗は、上記方針の例外として `Impact.SecurityRisk` を `high` へ引き上げる形で表示される。この表示は deny 予告とはならず、dry-run の終了コードにも反映されない。これは検証内容の統合（Task 0158）時点での暫定的な扱いであり、解消は dry-run の deny 判定全体の設計に属する別論点である（経緯は 0158 のアーキテクチャ設計書 §5.7 を参照）。
...
- `operand_zones`: 判断軸2（宛先パス信頼区分）のオペランド毎判定。各要素は `index`／`raw`／`resolved`／`zone`／`role`／`matched_critical`／`trusted`／`unresolved_err` を持つ。`raw`／`resolved`／`unresolved_err` は `command_args` と同じ出力境界の redaction を経由してマスクする。判断軸2 が適用されたコマンド（carrier が `len()>0`）のときのみ出力し、空・nil ではキー自体を付けない（`reason_codes`／`risk_factors` と同じ存在条件）。

`operand_zones` の **3 状態デコード**: キーの有無だけでは「リスク無し」と即断できない。同一エントリの `error_class` と併せて読む。

| エントリの形 | 意味 |
|------|------|
| `operand_zones` 無し・`error_class` 無し | 判断軸2 が **非適用**（ファイル操作コマンドでない）。carrier が空 |
| `operand_zones` 無し・`error_class` あり | 判断軸2 の評価 **到達前に失敗** した deny（identity・パス解決・symlink 解決の失敗等）。carrier 自体が無い |
| `operand_zones` あり（`zone:"unresolved"` を含み得る） | 判断軸2 が **適用**。`unresolved` 要素は解決不能オペランドの fail-closed（write=High／read=Medium） |
