...
> **fail-closed の `Recognized` 契約が安全保証を担う**: 完全認識できなかった形（`Recognized=false`）は、固定 High 抑止を行わず、かつ判断軸2 自身が **下限 High** を課す（`res.Level` を High へ引き上げ `ReasonUnresolvedDestination` を付す）。これにより、解釈しきれない形が `low` を素通りすることはない。
...
0145 は各コマンドのフラグ集合を **実 CLI（GNU coreutils と uutils の和集合）に整合** させ、実 CLI に存在しないフラグの受理（過剰認識）を除去した（安全側の挙動変更）。例として `touch` の `-p`/`-i`、`mkdir` の `-a`/`-c`/`-h`/`-f`/`-i`/`-r`、`sponge` の `-r` などは実 CLI に無いため宣言しない。これらを与えた入力は getopt が未知フラグとして扱い `Recognized=false` となり、上記の fail-closed 下限により High で拒否される（`recognized=true→false`）。
