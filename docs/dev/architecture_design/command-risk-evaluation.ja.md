...
- 大規模・不可逆な破壊、または永続的なシステム状態変更:
  - ディスク・パーティション破壊系（`wipefs`, `blkdiscard`, `sgdisk`, `gdisk`, `sfdisk`, `cfdisk`, `mkswap`, `parted`, `fsck`, `fdisk`, `mkfs`）、直接ファイルシステムユーティリティ（`e2fsck`, `mke2fs`, `tune2fs`, `resize2fs`, `xfs_*`, `btrfs`）、LVM 破壊系（`lvremove`, `vgremove`, `pvremove`, `lvreduce` 等）。
