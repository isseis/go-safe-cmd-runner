...
| `internal/runner/base/security` の誤検知 | `file_validation.go` の `isUserInGroup` が `GroupIds()` を使わず `GetGroupMembers` を直接引くため、SSSD 環境では正当なメンバーが「非メンバー」と判定される | 本タスクでは扱わない。安全側に倒れる誤検知であり方向が逆である。[#1071](https://github.com/isseis/go-safe-cmd-runner/issues/1071) として分離し、残件一覧にも記録する（AC-28） |
...
- **2026-08-29、完全性判定の確定を「遅延＋排他制御」から「起動時に1回」へ改める決定**: 0168 は判定を遅延確定とし、`nsswitchVerdictMu` と `nsswitchVerdictResolved` で最初の1回を選んでいた。本タスクの Phase 1 実装後の指摘により、現在のフローに `nsswitchVerdict()` を並行に呼ぶ経路が無いことを確認した——production コードの `go`／`wg.Go` は Slack 送信の2箇所（`internal/logging/slack_sender.go`・`internal/runner/bootstrap/logger.go`）だけで、いずれも群所属を参照せず、コマンド実行は逐次である。したがって排他制御は何も守っていない。加えて遅延確定は、`getGroupEnumeration` が `cacheMutex` を保持したまま `enumerateGroupMembers` を呼ぶため、警告の `report` が再入不可のロックの下で任意の `slog` ハンドラを呼ぶ経路を生んでいた。**起動時に1回だけ確定させる形に改めると、この経路自体が消える。** 未確定のまま列挙へ到達した場合はゼロ値 `completenessUnstated` が `manager.go` の `switch` の `default` で拒否されるため、fail-secure は追加のコードなしに成り立つ（CLAUDE.md「Enforce invariants with the type, not with convention」）。**実施は Phase 4 とする**: 3つのバイナリすべてが起動時に確定させるようになるのがその時点であり、それ以前に厳密化すると `cmd/runner` が group-writable なパスを一律に拒否するためである。§2.2 のロック順序の注記、§3.2 の3シンボルの提示、§7.1 は Phase 4 の実装と同じコミットで改訂する。
