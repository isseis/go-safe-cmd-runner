...
| AC-12 | test | `incompleteness_advice_cgo.go` `adviseIncompleteness` | `::TestAdviseIncompleteness_CGO`（各原因の `fact`・`remediation` を全文で照合し、組み立て後のメッセージにも同じ文字列が現れること、および `"CGO_ENABLED"` が現れないこと）。**部分文字列では照合しない**——`"chmod g-w"` の有無だけを見ると、`causeNSSSources` の枝が `causeUnsupportedPlatform` の文面を返すようになっても合格してしまう（レビュー指摘） |
