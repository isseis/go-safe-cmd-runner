...
- [x] `commandProfileDefinitions` の `NewProfile("ssh","scp")`（[command_analysis.go:64](../../../internal/runner/base/security/command_analysis.go#L64)）を `NewProfile("ssh","scp","sftp")`
  へ拡張する（AlwaysNetwork・Medium。DRY のため新規 profile を起こさない。[02_architecture.md §3.4](02_architecture.md)）。
- [x] `TestCommandRiskProfiles_NetworkCommands` に `sftp`=NetworkTypeAlways を追加する。
- [x] `TestMigration_RiskLevelConsistency` に `sftp`=Medium を追加する。
- [x] `TestMigration_NetworkTypeConsistency` の `alwaysNetwork` に `sftp` を追加する。
- [x] `command-risk-evaluation.ja.md` に AI 検出の限界（AC-20）を、固有の見出し
...
- [x] グリーンゲート（`_context.md` の "Green gate" 参照）がパスしていることを確認した
- [x] PR を作成した（https://github.com/isseis/go-safe-cmd-runner/pull/779）
- [x] PR がマージされた
- [x] 次のブランチへ切り替えた（次ステップは新しいブランチで作業する）
...
- [x] **PR-1 マージ済み**（対象ステップ: P1）: システム変更名 High/Medium 再配置＋`fsck.*` 規則＋分類テスト（`TestSystemModificationRisk_AllNamesEnumerated`／`_NegativesAndResolution`）
- [x] **PR-2 マージ済み**（対象ステップ: P2）: 特権 profile 拡張＋`IndirectCritical` コメント更新＋privilege/evaluator テスト更新＋AC-17 確認
- [x] **PR-3 マージ済み**（対象ステップ: P3）: chroot/unshare/nsenter/flock/watch 専用ハンドラ＋内側ゲート/no-command High テスト
- [x] **PR-4 マージ済み**（対象ステップ: P4）: `ip netns/vrf exec` 内側ゲート＋非 exec 委譲テスト
- [x] **PR-5 マージ済み**（対象ステップ: P5）: env/timeout redundant-High＋`IndirectFloor` コメント更新＋rsync/ssh ヘルパーオプション抽出＋テスト分割/更新
- [x] **PR-6 マージ済み**（対象ステップ: P6）: sftp Medium 補完＋profile テスト更新＋AC-20 doc 追記
- [x] 各 PR 末で `make fmt`→`make test`→`make lint` 緑（NF-002）
