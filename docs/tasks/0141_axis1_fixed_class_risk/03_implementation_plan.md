...
| `flock` | `-w`/`--timeout`・`-E`/`--conflict-exit-code` | `-s`/`-x`/`-u`/`-n`/`-F`/`-o` | — | `<file>`×1 | argv（`-c` は cmd-string §3.3(d)） | Reject（抽出不能時） |
| `watch` | `-n`/`--interval` | `-t`/`-b`/`-e`/`-g`/`-c`/`-w` | `-d`/`--differences` | — | cmd-string（`-x`/`--exec` は argv） | Reject（抽出不能時） |
...
- [ ] **任意引数オプションを必須分離値オプションと区別する（最重要）**: `nsenter`/`unshare` の名前空間オプション
  （`nsenter -m`/`-u`/`-i`/`-n`/`-p`/`-U`/`-C`/`-T`、`unshare -m`/`-u`/`-i`/`-n`/`-p`/`-U`/`-C`/`-T`）は namespace ファイル
  を**任意引数**として取る。getopt の `optional_argument` 規約では、任意引数は**連結形でしか**束縛されない
  （`-m=FILE`・`-mFILE`）——分離形 `-m FILE` の FILE は値ではなく operand（＝COMMAND）である。したがって
  これらを `-t`/`-S`/`-G`/`-w`（必須分離値オプション。次トークンを消費）と**同じ `valueOpts` に登録してはならない**:
  任意引数オプションは連結値のみ消費し、分離トークンは消費しない（boolean 同様に扱う）。これにより
  `unshare -m sudo id`・`nsenter -m sudo id` の `sudo` がオプション値として食われず、最初の operand として
  正しく内側ゲート（特権トークン → Critical）にかかる。`-mFILE` の連結値は自己完結のため operand 境界に影響しない。
  `TestIndirect_NamespaceWrappersGated` に `unshare -m sudo id`=Critical・`nsenter -m sudo id`=Critical の回帰を加える
  （任意引数オプションが privilege トークンを消費しないことの固定）。
- [ ] `flock`/`watch` 専用ハンドラを追加: **両ツール自身のオプション（値付き含む）を先に読み飛ばしてから**
  内側を抽出する。`flock` は `flock [options] <file> <cmd>` で値オプション `-w`/`--timeout SECS`・`-E`/
  `--conflict-exit-code N`、boolean `-n`/`-s`/`-x`/`-u`/`-F` を持つ。`watch` は `watch [options] command` で値オプション
  `-n`/`--interval SECS`・`-c`/`--color`（一部 boolean）等を持つ。これらを読み飛ばさないと `flock -w 10 /tmp/l sudo id`
  の `10`/ロックパス、`watch -n 1 sudo id` の `1` を内側コマンドと誤認し、本来の `sudo` の Critical ゲートを取りこぼす。
  オプション読み飛ばしには既存の `skipLeadingOptions`＋各ツール用 `optSpec` を用い、unknown-arity は Reject。
  読み飛ばし後、`flock <file> <cmd>` はトークン列として `<file>` の次の first-token をゲート。
  ...
  `TestIndirect_NamespaceWrappersGated` に `flock -w 10 /tmp/l sudo id`=Critical・`watch -n 1 sudo id`=Critical の
  回帰を加える。
...
- [ ] **ショートオプションのバンドル/連結形からの値抽出規約を getopt 準拠で明示する**（検出は既存の
  ...
  取り出した値は §3.3(d) のシェル分割規約に通し、シェルメタ文字を含めば Reject、クリーン分割時のみ first-token を
  内側ゲート。バンドル中間形を「次トークン」と誤読すると `ssh`（=source path）を内側コマンドと取り違える過小抽出に
  なるため、残部優先の規約を厳守する。`TestIndirect_CommandExecOptionsGated` に `-essh`／`-avze ssh`／`-aevz`
...
| AC-22 | test | `indirect_execution_test.go::TestIndirect_NamespaceWrappersGated`（`chroot`/`unshare`/`nsenter`/`flock`/`watch` の内側ゲート High・特権内側 Critical・抽出不能 Reject。オプション読み飛ばし回帰: `nsenter -S 0 sh`=内側 `sh` を正しくゲート、任意引数 `unshare -m sudo id`/`nsenter -m sudo id`=Critical（`sudo` を値として食わない）、`chroot --userspec=0:0 /mnt sudo id`=Critical、`flock -w 10 /tmp/l sudo id`=Critical、`watch -n 1 sudo id`=Critical、`watch -x rm -rf /`=High の argv 形）＋ `TestIndirect_NoCommandImplicitShellHigh`（`chroot /mnt`/`unshare`/`nsenter -t 1 -m`=Level High）＋ `evaluator_test.go`（`env modprobe x` の最終 Level=High 追加ケース。`sudo useradd u`=Critical は既存 `TestStandardEvaluator_EvaluateRisk_PrivilegeEscalation` の sudo 経路で担保） |
