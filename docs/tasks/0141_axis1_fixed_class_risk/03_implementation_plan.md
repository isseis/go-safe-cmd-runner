...
- [x] `mediumSystemModificationNames` を再構成: 上記で High へ移した名（`parted`/`fsck`/`fdisk`/`mkfs`/`crontab`/
  ...
  `lvcreate`/`vgcreate`/`lvextend`/`vgextend`/`vgchange`/`lvchange`（AC-13。`vgextend` は `lvextend` と対の
  VG 拡張で非破壊的なため一貫性のため追加。要件 §4 の確定列挙は実装で確定の方針）と `ip`/`ifconfig`/`route`
  （AC-14）を新規追加。
...
| AC-13 | test | `command_analysis_test.go::TestSystemModificationRisk`（`lvcreate`/`vgcreate`/`lvextend`/`vgextend`/`vgchange`/`lvchange`=Medium） |
