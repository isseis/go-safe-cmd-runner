...
- [ ] **`watch`（`-x` なし）は全オペランドを連結した 1 つの cmd-string として扱う**: procps-ng `watch` は
  ...
  再構成**し、§1.4(b) の拡張拒否集合（`;` `|` `&`・改行ほか）で分割チェックする——区切りを含めば Reject、クリーン時のみ
  first-token を内側ゲート。`TestIndirect_NamespaceWrappersGated` に `watch ls -l ';' sudo id`=Reject・`watch sudo id`=Critical
  の回帰を加える。
- [ ] **`watch -x`/`--exec` の argv 形を別経路として扱う**: `watch -x`（`--exec`）は内側を `sh -c` でなく直接
  ...
  そのままゲートする（シェル分割規約は適用しない）。上記の `-x` 無し連結形とは分岐する。
  境界不確定は Reject。`TestIndirect_NamespaceWrappersGated` に `watch -x rm -rf /`=High の回帰を加える。
