...
- [ ] **任意引数オプションの parser-differential を fail-closed で潰す**: `nsenter`/`unshare` の名前空間オプション
  （`nsenter -m`/`-u`/`-i`/`-n`/`-p`/`-U`/`-C`/`-T`、`unshare -m`/`-u`/`-i`/`-n`/`-p`/`-U`/`-C`/`-T`）は namespace ファイル
  を**任意引数**として取り得る。getopt 規約では任意引数は連結形（`-m=FILE`・`-mFILE`）でしか与えられず、分離形
  （`-m FILE`）の FILE は operand 扱いになるが、ツール側の解釈と静的解析の解釈が食い違う（parser differential）と、
  例えば `nsenter -m /path/to/file sh` の `/path/to/file` を内側コマンドと誤判定し本来の `sh` を素通りさせ得る。
  したがって、任意引数オプションを分離形の値オプションとして扱わず（連結値のみ消費）、分離トークンの解釈が
  曖昧になる形は内側境界不確定として **Reject** に倒す（実装上の安全側。アーキ §3.3(a) の「境界不確定は Reject」を
  この曖昧クラスへ明示適用する）。`TestIndirect_NamespaceWrappersGated` に `nsenter -m /path sh`=Reject の回帰を加える。
...
| AC-22 | test | `indirect_execution_test.go::TestIndirect_NamespaceWrappersGated`（`chroot`/`unshare`/`nsenter`/`flock`/`watch` の内側ゲート High・特権内側 Critical・抽出不能 Reject・`nsenter -S 0 sh` 誤抽出回帰・任意引数曖昧 `nsenter -m /path sh`=Reject・`watch -x rm -rf /`=High の argv 形）＋ `TestIndirect_NoCommandImplicitShellHigh`（`chroot /mnt`/`unshare`/`nsenter -t 1 -m`=Level High）＋ `evaluator_test.go`（`env modprobe x` の最終 Level=High 追加ケース。`sudo useradd u`=Critical は既存 `TestStandardEvaluator_EvaluateRisk_PrivilegeEscalation` の sudo 経路で担保） |
