...
| AC-05 | test | `command_analysis_test.go::TestSystemModificationRisk_AllNamesEnumerated`（`useradd`/`usermod`/`userdel`/`groupadd`/`groupmod`/`groupdel`/`gpasswd`/`passwd`/`chpasswd`/`chage`/`newusers`/`adduser`/`deluser`/`addgroup`/`delgroup`/`vipw`/`vigr`/`visudo`/`chsh`/`chfn`=High。0140 AC-05 継承の account/auth ミューテータを網羅。`/etc/passwd` を変更する SUID `chsh`/`chfn` を含む） |
| AC-06 | test | `command_analysis_test.go::TestSystemModificationRisk_AllNamesEnumerated`（`grub-install`/`grub-mkconfig`/`grub-set-default`/`grub-reboot`/`grub-editenv`/`grub2-install`/`grub2-mkconfig`/`grub2-set-default`/`update-grub`/`efibootmgr`/`kernel-install`/`installkernel`=High。`grub-*`（Debian）と `grub2-*`（RHEL）の両ファミリを各 ≥2 変種検証して展開列挙の漏れを検出） |
...
| AC-13 | test | `command_analysis_test.go::TestSystemModificationRisk_AllNamesEnumerated`（`lvcreate`/`vgcreate`/`lvextend`/`vgextend`/`vgchange`/`lvchange`/`pvchange`=Medium） |
