...
| `unshare` | `-S`/`-G`/`--map-user`/`--map-group`/`--propagation` | `-r`/`-f`/`--kill-child` 等 | `-m`/`-u`/`-i`/`-n`/`-p`/`-U`/`-C`/`-T`/`-w` | — | argv | High（暗黙シェル） |
| `nsenter` | `-t`/`-S`/`-G` | `-F`/`--preserve-credentials` 等 | `-m`/`-u`/`-i`/`-n`/`-p`/`-U`/`-C`/`-T`/`-r`/`-w` | — | argv | High（暗黙シェル） |
| `flock` | `-w`/`--timeout`・`-E`/`--conflict-exit-code` | `-s`/`-x`/`-u`/`-n`/`-F`/`-o` | — | `<file>`×1 | `<file> <cmd>`（argv）／`<file> -c <cmd-string>`（§3.3(d)。`-c` は lock operand の**後**） | fd 専用形 `flock <N>`（数値 fd・コマンド無し）=IndirectNone。コマンド形で抽出不能時のみ Reject |
...
- [ ] `unshare`/`nsenter` 専用ハンドラを追加: 各ツールの**必須分離値オプションを網羅的に** `valueOpts` 登録し
  （`-t`/`-S`/`-G` 等。`nsenter` と `unshare` で集合が異なる。§1.4(b) の各行を正とする）、operand 境界を確定して
  内側 COMMAND をゲート。**`-w`/`--wd` と `-r`/`--root` は必須分離値ではなく optional-attached（`-w[=dir]`/`-r[=dir]`）**
  であり、次の任意引数オプションの項で扱う（`valueOpts` には入れない）。no-command 形（`unshare`/`nsenter -t 1 -m`）は
  暗黙シェル High 下限。境界不確定・`-` 始まり誤抽出は Reject（fail-closed）。
- [ ] **任意引数オプションを必須分離値オプションと区別する（最重要）**: `nsenter`/`unshare` の名前空間オプション
  （`-m`/`-u`/`-i`/`-n`/`-p`/`-U`/`-C`/`-T`）と **`-w`/`--wd`・`-r`/`--root`（dir を任意引数に取る）** は、いずれも
  getopt の `optional_argument`。任意引数は**連結形でしか**束縛されない（`-m=FILE`・`-mFILE`・`-w=DIR`・`-r=DIR`）——
  分離形 `-m FILE`/`-w DIR` の次トークンは値ではなく operand（＝COMMAND）である。したがって
  これらを `-t`/`-S`/`-G`（必須分離値オプション。次トークンを消費）と**同じ `valueOpts` に登録してはならない**:
  任意引数オプションは連結値のみ消費し、分離トークンは消費しない（boolean 同様に扱う）。これにより
  `unshare -m sudo id`・`nsenter -m sudo id`・`nsenter -t 1 -w sudo id` の `sudo` がオプション値として食われず、
  最初の operand として正しく内側ゲート（特権トークン → Critical）にかかる。連結値（`-mFILE`/`-w=DIR`）は自己完結の
  ため operand 境界に影響しない。`TestIndirect_NamespaceWrappersGated` に `unshare -m sudo id`=Critical・
  `nsenter -m sudo id`=Critical・`nsenter -t 1 -w sudo id`=Critical の回帰を加える
  （任意引数オプションが privilege トークンを消費しないことの固定）。
- [ ] `flock`/`watch` 専用ハンドラを追加: **両ツール自身のオプション（値付き含む）を先に読み飛ばしてから**
  内側を抽出する（オプションクラスは §1.4(b) の各行を正とする）。`flock` の値オプションは `-w`/`--timeout`・`-E`/
  `--conflict-exit-code`、boolean は `-s`/`-x`/`-u`/`-n`/`-F`/`-o`。`watch` の値オプションは `-n`/`--interval`・
...
- [ ] **`flock` の operand 後の COMMAND 形を 3 種に分岐する**: 読み飛ばし後の lock operand に続けて、(1) `<file> <cmd>`
  はトークン列として `<cmd>` を first-token ゲート、(2) **`<file> -c <cmd-string>`**（`-c`/`--command` は lock operand の
  **後**に来るシェルコマンド文字列。`flock -c …` の形ではない）は §1.4(b) の cmd-string fail-closed 分割集合で抽出
  （`flock /tmp/l -c 'sudo id'`=Critical）、(3) **fd 専用形 `flock <N>`**（数値 fd・後続コマンド無し。例
  `exec 9>/lock; flock 9`）は helper を実行しないため **IndirectNone**（Reject にしない）。`watch "<cmd-string>"` も
  §1.4(b) の cmd-string 集合で抽出。コマンド形で抽出不能時のみ Reject。
  `TestIndirect_NamespaceWrappersGated` に `flock -w 10 /tmp/l sudo id`=Critical・`flock /tmp/l -c 'sudo id'`=Critical・
  `flock 9`=IndirectNone・`watch -n 1 sudo id`=Critical・`watch -q 1 sudo id`=Critical・`watch -c sudo id`=Critical の
  回帰を加える。
...
| AC-22 | test | `indirect_execution_test.go::TestIndirect_NamespaceWrappersGated`（`chroot`/`unshare`/`nsenter`/`flock`/`watch` の内側ゲート High・特権内側 Critical・抽出不能 Reject。オプション読み飛ばし回帰: `nsenter -S 0 sh`=内側 `sh` を正しくゲート、任意引数 `unshare -m sudo id`/`nsenter -m sudo id`=Critical（`sudo` を値として食わない）、`chroot --userspec=0:0 /mnt sudo id`=Critical、`nsenter -t 1 -w sudo id`=Critical（`-w` は optional-attached）、`flock -w 10 /tmp/l sudo id`=Critical、`flock /tmp/l -c 'sudo id'`=Critical（`-c` は lock operand の後）、`flock 9`=IndirectNone（fd 専用形）、`watch -n 1 sudo id`/`watch -q 1 sudo id`/`watch -c sudo id`=Critical、`watch -x rm -rf /`=High の argv 形）＋ `TestIndirect_NoCommandImplicitShellHigh`（`chroot /mnt`/`unshare`/`nsenter -t 1 -m`=Level High）＋ `evaluator_test.go`（`env modprobe x` の最終 Level=High 追加ケース。`sudo useradd u`=Critical は既存 `TestStandardEvaluator_EvaluateRisk_PrivilegeEscalation` の sudo 経路で担保） |
