...
- **クローズド集合（完全一致マップ）の完全性をテストで強制する（横断方針）**。`SystemModificationRisk` 等の
  ...
  テストは代表サンプルでなく**実装と同じ定数集合を網羅検証**する。具体化として、表駆動の
  `TestSystemModificationRisk`（各名→期待レベルを assert する omission guard。`systemModificationRiskCases`）と、
  それが High/Medium 両集合の**全名を行として含むことをメタ検証**する `TestSystemModificationRisk_RepresentativeTableComplete`
  を併用する。後者により「集合へ名を足したがテスト表へ足し忘れた」ドリフトは**ローカルの `make test` 失敗**になり、
  レビューでの指摘待ちにならない（列挙漏れがサイレントに Low へ落ちる穴を構造的に塞ぐ。§4.1）。
  - **family 完全性の方針と将来コメントの捌き方**: 各 family の tail は実装者判断で各ツールの標準コマンドへ
    ...
    family の標準コマンドかつ同一リスククラスなら**実装と表へ同時に追加**（メタ検証が両者の同期を保証）、そうでなければ
    out-of-scope として replyする。新規 family の sibling 一括追加は §2 P1 を正とし、表は常に集合と同期させる。
...
- [x] `highSystemModificationNames` に F-002 群を追加: kernel/module `insmod`/`modprobe`/`rmmod`/`kexec`/`sysctl`
  ...
  `chage`/`newusers`/`adduser`/`deluser`/`addgroup`/`delgroup`/`vipw`/`vigr`/`visudo`/`chsh`/`chfn`（AC-05。0140 AC-05
  から継承する account/auth ミューテータを網羅。Debian 系 `adduser`/`deluser` 等、`/etc/passwd` を変更する SUID
  `chsh`/`chfn` も含める）、bootloader
  ...
  `installkernel`（AC-06。`grub-*`（Debian）と `grub2-*`（RHEL）の両ファミリを既知の具体名へ**展開列挙**する。
...
- [x] `mediumSystemModificationNames` を再構成: 上記で High へ移した名（`parted`/`fsck`/`fdisk`/`mkfs`/`crontab`/
  ...
  `lvcreate`/`vgcreate`/`lvextend`/`vgextend`/`vgchange`/`lvchange`/`pvchange`（AC-13。`vgextend` は `lvextend` と対の
  VG 拡張、`pvchange` は `vgchange`/`lvchange` と対の PV 設定変更で、いずれも非破壊的なため一貫性のため追加。要件 §4 の
  確定列挙は実装で確定の方針）と `ip`/`ifconfig`/`route`/`iwconfig`/`iw`（AC-14。`iwconfig`/`iw` は `ifconfig`/`ip` の
...
- [x] `TestSystemModificationRisk` を更新: `parted`/`fsck`/`crontab`/`at`/`batch`/`chkconfig`/`update-rc.d` の期待値を
...
- [x] **クローズド集合の完全性を強制する**（§1.2 横断方針）: 表駆動の `TestSystemModificationRisk`
  （`systemModificationRiskCases`）の各行が `SystemModificationRisk` を通して期待レベルを assert し（omission guard）、
  メタテスト `TestSystemModificationRisk_RepresentativeTableComplete` が `highSystemModificationNames`／
  `mediumSystemModificationNames` の**全名が表に High/Medium 行として存在する**ことを検証する。これにより集合へ名を
  足したが表へ足し忘れたドリフトはローカルの `make test` 失敗となり（`grub2-*`・account/auth ファミリ等の展開列挙漏れも
  含む）、サイレントに Low へ落ちる穴をレビュー前に塞ぐ（AC-05/AC-06 の family 完全性も同時に担保）。
...
**レビュー観点**: High/Medium 名集合の再配置の網羅性（クローズド集合完全性テストで列挙漏れを検出） / `parted`/`fsck`/`fdisk`/`mkfs` の High 訂正と既存テストの期待値更新漏れ / `fsck.*` 派生名規則と `mkfs.` 既存規則の max 合成
...
- [ ] **`flock` は非permutation（最初の非オプションで解析停止）として実装する**: 実機 util-linux 2.41.3 で確認——
...
- [ ] **`flock` の COMMAND 形を 3 種に分岐する**: lock operand までの flock 自身のオプションを（非permutation で）読み飛ばし、
...
- [ ] **`watch`（`-x` なし）は全オペランドを連結した 1 つの cmd-string として扱う**: procps-ng `watch` は
...
- [ ] `TestIndirect_CommandExecOptionsGated`（[indirect_execution_test.go:877](../../../internal/runner/base/security/indirect_execution_test.go#L877)）を更新する:
  ...
  - [ ] fail-closed allowlist 規約の検証ケースを追加: §1.4(b) の安全集合外の文字を含む値が Reject になること——区切り
...
- [ ] **PR-1 マージ済み**（対象ステップ: P1）: システム変更名 High/Medium 再配置＋`fsck.*` 規則＋`TestSystemModificationRisk` 更新
...
| AC-05 | test | `command_analysis_test.go::TestSystemModificationRisk`（`useradd`/`usermod`/`userdel`/`groupadd`/`groupmod`/`groupdel`/`gpasswd`/`passwd`/`chpasswd`/`chage`/`newusers`/`adduser`/`deluser`/`addgroup`/`delgroup`/`vipw`/`vigr`/`visudo`/`chsh`/`chfn`=High。0140 AC-05 継承の account/auth ミューテータを網羅。`/etc/passwd` を変更する SUID `chsh`/`chfn` を含む） |
| AC-06 | test | `command_analysis_test.go::TestSystemModificationRisk`（`grub-install`/`grub-mkconfig`/`grub-set-default`/`grub-reboot`/`grub-editenv`/`grub2-install`/`grub2-mkconfig`/`grub2-set-default`/`update-grub`/`efibootmgr`/`kernel-install`/`installkernel`=High。`grub-*`（Debian）と `grub2-*`（RHEL）の両ファミリを各 ≥2 変種検証して展開列挙の漏れを検出） |
...
| AC-09 | test | `command_analysis_test.go::TestSystemModificationRisk`（`iptables`/`ip6tables`/`iptables-restore`/`ip6tables-restore`/`nft`/`ufw`/`firewall-cmd`/`ebtables`/`arptables`=High、`iptables-save`/`ip6tables-save`=Unknown） |
...
| AC-13 | test | `command_analysis_test.go::TestSystemModificationRisk`（`lvcreate`/`vgcreate`/`lvextend`/`vgextend`/`vgchange`/`lvchange`/`pvchange`=Medium） |
| AC-14 | test | `command_analysis_test.go::TestSystemModificationRisk`（`ip`/`ifconfig`/`route`/`iwconfig`/`iw`=Medium）＋ `indirect_execution_test.go::TestIndirect_IpExecGated`（`ip netns exec ns rm -rf /`=High、`ip netns exec ns modprobe x`=High、内側欠落=Reject、`ip netns list`=IndirectNone、グローバルオプション挿入 `ip -json netns exec ns rm -rf /`/`ip -n foo netns exec ns sh`=High でバイパス不可） |
