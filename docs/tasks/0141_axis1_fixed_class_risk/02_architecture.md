...
発火しない場合に**限り**、残りの dimension と判断軸2 を並列に max 合成する。特権昇格ラッパ判定は「Critical を確定して
...
> ゲートは Critical/Reject を確定して**短絡**する（以降を評価しない）。すべて `通過` したときに限り、残りの dimension と
...
名前のみ・引数非依存の固定レベル分類は、既存の `SystemModificationRisk` dimension へ集約する。本関数は
