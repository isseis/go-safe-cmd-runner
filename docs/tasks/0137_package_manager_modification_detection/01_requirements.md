...
- フラグ方式の判定は **fail-safe（取りこぼしより過検出を許容＝安全側）** を基本とする。ただし
  rpm の照会・検証操作については最小権限を優先し、過検出を抑制する（F-002）。加えて、短形式の判定は
  オプションの**先頭文字**（変更モード選択子は常に先頭に来る）で行い、引数連結オプション
  （`rpm -E…`／`dpkg -D…`）や単一ダッシュ長形式（`dpkg -list`）の途中文字による過検出を避ける
  （詳細は 02 §3.1）。これにより非慣用・無効な並び（モード文字が先頭でない例 `rpm -hiv`）は
  取りこぼし得るが、有効な使用には現れず allowlist+ハッシュ固定の backstop で担保される。通常の
  修飾子併用での変更操作（`rpm -e --verbose` 等）の取りこぼしは生じないことを境界テストで担保する
  （AC-05 / NF-001）。
...
**Acceptance Criteria**:
- **AC-09**: pacman の**有効な使用**における判定が保持される。検出される: `pacman -S pkg`／`-R pkg`／
  `-U pkg`／`-Syu`／`-Rns pkg`、長形式 `--sync`／`--remove`／`--upgrade`。検出されない:
  `pacman -Q`／`-Qi pkg`（照会）。`pacman -Ss`／`-Si`（先頭が `S`）は引き続き変更操作として検出される
  （既存の過検出を維持）。本タスクは判定機構を**先頭文字一致**へ統一する（02 §3.1）。pacman の操作
  フラグは常に先頭に来るため有効入力の結果は不変だが、非慣用・無効な並び（例 `-yS`）では従来の文字
  含有方式と結果が異なり得る。この差異は有効な pacman 使用に影響しないため**許容する**（範囲緩和を承認済み）。
...
- **NF-001（大小区別・先頭文字判定の厳密性）**: フラグ方式判定はツールごとに大文字・小文字を厳密に
  区別し、短形式は先頭文字で判定する。境界（dpkg `-i`/`-I`、rpm `-i`/`-qi` および `rpm -e --verbose`、
  pacman `-S`/`-s`、ならびに先頭文字判定の境界 `rpm -E%{…}`・`dpkg -D<level>`・`dpkg -list` が非検出）を
  テストで固定する。
