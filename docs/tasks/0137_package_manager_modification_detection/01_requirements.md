...
状態を変更する rpm 長形式 `--import`（GPG 信頼鍵の追加）／`--initdb`／`--rebuilddb`（RPM データベースの
初期化・再構築）も検出対象に含める。一方、**照会（query, `-q`）・検証（verify, `-V`）モードはシステム
...
- **AC-05**: 次がシステム変更（`medium`）として検出される: `rpm -i pkg.rpm`、`rpm -U pkg.rpm`、
  ...
  また長形式 `rpm --import KEY`、`rpm --initdb`、`rpm --rebuilddb` も検出される。
