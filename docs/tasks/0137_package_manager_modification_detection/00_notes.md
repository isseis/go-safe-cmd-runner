...
タスク 0136（実行時リスク判定の強化, PR #729）のレビューで、`security.IsSystemModification`
（[command_analysis.go](../../../internal/runner/base/security/command_analysis.go) の
`IsSystemModification`）のパッケージマネージャ検出に非対称があることが判明した。
...
- **In**: フラグ方式マネージャ（dpkg, rpm）のシステム変更操作検出。`IsSystemModification` の
  pacman 固有実装を per-tool 化して一般化する。ユーザー文書（`risk_assessment`）への dpkg/rpm 反映。
...
### 論点 1 の補足: 実装構造の凝集（IsSystemModification のハードコード解消）

現状の問題（保守性）: マネージャの**識別情報**と**操作検出規則**が分散している。

- マネージャ名は `packageManagerNames`（集合）。
- 変更系 verb は `packageModifyingVerbs`（全マネージャ共有の集合）。
- フラグ規則は `isPacmanModifyingFlag`（pacman 専用関数）＋ `IsSystemModification` 本体の
  `isPacman := names["pacman"]` という**個別分岐のハードコード**。

→ pacman 1 種でも見通しが悪く、dpkg/rpm を足すたびに本体の分岐・専用関数が増える。

改善方針（たたき台）: **1 マネージャ = 1 定義**に凝集し、`IsSystemModification` からツール固有の
...
- `IsSystemModification` は「解決済み名前集合に一致する `packageManager` を引き、その規則を適用」
...
- `IsSystemModification` は **実行時（`risk.EvaluateRisk` 経由）と dry-run（`AnalyzeCommandSecurity`）の
  双方**から呼ばれる共通関数。ここを一般化すれば両経路が自動的に整合する見込み。
- ただし `AnalyzeCommandSecurity` は early-return（最初に一致したリスクを返す）で、
  `EvaluateRisk` は最大値モデル。新規検出が両経路で同じ結論になるか、0136 の整合性テスト
  （`coreutils_consistency_test.go` 系）相当の検証を要件に含めるか検討。
...
- [command_analysis.go](../../../internal/runner/base/security/command_analysis.go):
  `IsSystemModification`, `packageManagerNames`, `packageModifyingVerbs`, `isPacmanModifyingFlag`
