...
- **passwd 列挙ストリームのプロセス全体の不変条件**: §3.3 のミューテックスは本パッケージ内の
  呼び出しのみをシリアライズする。本パッケージがプロセス内で `setpwent`/`getpwent`/`endpwent` の
  唯一の呼び出し元であり続けることが前提であり、将来この不変条件が破られた場合、列挙結果が
...
- **列挙を有効化した大規模 LDAP 環境**: `getpwent` の各反復が NSS 往復となり、全走査に
  ...
  同一のロック構造であり、本タスクでロック構造は変更しない。大規模ディレクトリ環境での遅延は
  ディレクトリ健全性の問題と相関付けて監視できるよう、運用上の既知事項として記録する。
  ロック外列挙（singleflight 等）への再構成は複雑性の増大であり本タスクでは採らない
  （YAGNI。§9 の拡張余地に記録）。
