...
| detected_symbols（JSON） | `symbol_analysis.detected_symbols` フィールド（JSON）の型は `[]string` であり、シンボル名のみを保存する（schema v19 以降）。`category` フィールドは存在しない。`runner` はシンボル名から `binaryanalyzer.IsNetworkSymbol()` を呼び出してカテゴリを再導出する |
...
> **注**: AC-1〜AC-6 は `checkDynamicSymbols` の **内部動作** を対象とするユニットテスト仕様である。`DetectedSymbols` および `Category` は `binaryanalyzer.AnalysisOutput` の内部フィールドを指す。JSON スキーマの `symbol_analysis.detected_symbols` はシンボル名のみを保存し（schema v19）、`record` コマンドのいずれのオプションを指定しても `category` は JSON に出力されない。
