...
各 `SHN_UNDEF` シンボルについて、以下のようにフィルタを適用する。

- `Library != ""` の場合：Task 0106 の `sym.Library` ベースのフィルタを適用する
- `Library == ""` の場合：FR-1 の名前ベースフィルタを適用する

バイナリ単位での一括判定は行わない。glibc（VERNEED あり）とバージョニングなしのライブラリを混在してリンクするバイナリでは、両方のフィルタが同一バイナリ内で使用される場合がある。

**設計上の考慮**：バイナリ単位の判定は実装が簡単だが、VERNEED ありバイナリでも `Library == ""` のシンボルを無視するため偽陰性が生じる。セキュリティツールとして偽陰性（検出漏れ）の方が偽陽性（誤検知）より危険なため、シンボル単位の判定を採用する。
...
`Library != ""` のシンボルには従来どおり Library ベースフィルタを使用する。名前ベースフィルタは `Library == ""` のシンボルのみに適用されるため、glibc 環境での偽陽性は増加しない。
...
### AC-5: VERNEED ありバイナリでの回帰防止

- `Library != ""` のシンボルには Task 0106 の Library ベースフィルタが適用され、glibc バイナリで Task 0106 と同一の `DetectedSymbols` が得られること
- `Library != ""` のシンボルについて、libc 以外のライブラリのシンボル（`libm` 等）は記録されないこと
- `Library == ""` のシンボルには名前ベースフィルタが適用されること（VERNEED ありバイナリ内に `Library == ""` のシンボルが存在する場合、そのシンボルが `networkSymbols` に一致すれば `DetectedSymbols` に記録される）
