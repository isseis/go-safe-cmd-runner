...
本タスクでは現行の責務分担を維持する。すなわち `record` はバイナリの静的解析結果を runner が利用しやすい特徴量へ正規化して記録し、`runner` は JSON に保存されたシンボル名や依存ライブラリ情報を用いて実行時のリスク判定を行う。`record` が `networkSymbols` を参照するのは保存対象となる解析結果の正規化のためであり、実行可否や `risk_level` の最終判断は `runner` の責務とする。
...
| 正規化済み解析結果 | `record` が静的解析で得た事実を runner 向けに絞り込んだ内部表現。`DetectedSymbols` や JSON `detected_symbols` はこの層に属し、生の全インポートシンボル一覧そのものではない |
| DetectedSymbols（内部） | `binaryanalyzer.AnalysisOutput.DetectedSymbols` の型は `[]binaryanalyzer.DetectedSymbol` であり、各エントリは `Name`（シンボル名）と `Category`（分類文字列: `socket` / `tls` / `dns` / `http` / `syscall_wrapper`）を持つ。`Category` は `Result`（`NetworkDetected` / `NoNetworkSymbols`）の決定と `runner` のリスク判断に使われる内部フィールドであり、JSON ファイルには保存されない（schema v18 以降） |
| detected_symbols（JSON） | `symbol_analysis.detected_symbols` フィールド（JSON）。schema v18 で `category` フィールドが削除され、schema v19 でフォーマットが `[{"name":"..."}]` から `["..."]`（`[]string`）に平坦化された。現行（v19 以降）はシンボル名のみの文字列配列であり、`category` フィールドは存在しない。`runner` はシンボル名から `binaryanalyzer.IsNetworkSymbol()` を呼び出してカテゴリを再導出する |
...
- `record` は `networkSymbols` を使って「runner が後続で使う解析結果をどの粒度で保存するか」を決めるが、これは保存対象の正規化であって実行可否の最終判断ではない。`runner` は保存済みの `detected_symbols` と `known_network_lib_deps` からネットワーク性とリスクを再評価する。
- セキュリティツールとして偽陰性（検出漏れ）の方が偽陽性（誤検知）より危険なため、ステップ 1 はライブラリを問わず適用する。
- VERNEED ありのバイナリでも `Library = ""` のシンボルは発生しうる（バージョン情報を持たない unversioned シンボル）。このケースもステップ 1 が `Library` の値に関わらず適用されるため、VERNEED なしバイナリのシンボルと同じく処理される。実装はバイナリ単位で VERNEED の有無を判定する必要はなく、シンボルごとに FR-2 の 2 ステップを実行するだけでよい。
...
> **注**: AC-1〜AC-6 は `checkDynamicSymbols` の **内部動作** を対象とするユニットテスト仕様である。`DetectedSymbols` および `Category` は `binaryanalyzer.AnalysisOutput` の内部フィールドを指す。JSON スキーマの `symbol_analysis.detected_symbols` は schema v18 以降 `category` フィールドを持たず、schema v19 以降はシンボル名のみの `[]string` 形式で保存される。`record` コマンドのいずれのオプションを指定しても `category` は JSON に出力されない。
...
- VERNEED ありバイナリに `Library == ""` の unversioned シンボルが混在する場合、`networkSymbols` に一致すれば `DetectedSymbols` に記録されること（FR-2 ステップ 1 の適用結果；`isLibcLibrary("")` が偽のため `syscall_wrapper` は付与されない）
