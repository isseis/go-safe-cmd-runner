...
| Reviewer | isseis |
| Comments | 承認後に2点を変更した。(1) AC-24 を追加（`02_architecture.md` §3.4 の再入ガードの採用判断による）。(2) 用語を「特権窓」から「特権の隙」へ変更（用語集の `隙` の項が、攻撃・競合の文脈で「窓」「ウィンドウ」を使わないと定めているため）。 |
...
- `privilege/unix.go` の `mu` については、削除と同時に「特権の隙の直列化と非参加 goroutine の
...
1. 下表「削除候補」の11件の排他制御・アトミック変数の削除。
...
1. **非参加者を守れない。** ロックを取るのは `WithPrivileges` の呼び出し側だけである。特権の隙が
...
**削除の条件**: 削除と同時に、「特権の隙の直列化と、非参加 goroutine の保護は未解決の設計課題で
...
- **AC-11**: `WithPrivileges` の doc コメントに、(a) 特権の隙の直列化が保証されないこと、
  (b) 特権の隙が開いている間はプロセス全体の euid が上がるため、`WithPrivileges` に参加しない
...
- **AC-24**: `mu` が担っていた再入検知が、同期機構を使わない fail-closed なガードに置き換わって
  いる。すなわち `WithPrivileges` の内側から同一マネージャの `WithPrivileges` を呼ぶと、`fn()` が
  実行されないまま `ErrReentrantPrivilegeCall` が返り、特権の隙は開かない。再入しない既存の
  呼び出し経路では、返るエラーの種類も `fn()` の呼び出し回数も本タスクの前後で変わらない。
  `runnertypes.PrivilegeManager` インターフェースの doc コメントから、再入がデッドロックすると
  読める記述が取り除かれている。
