...
**機械的な検出が無いことの明記（実装時に追記）**: §3.4 の再入ガードは素の `bool` であり、同期機構では
ない。したがって (1) `race_test.go` の削除により、`WithPrivileges` に2つ目の goroutine が現れたときに
`-race` が鳴るテストは無くなり、(2) census guard test は同期の**型**を走査するので素の `bool` には
反応しない。単一 goroutine という前提は `executor.go` の唯一の呼び出し側についての事実として保たれて
いるだけで、機械は守っていない。並列実行を導入する者は、何も失敗しないまま前提が壊れることを
承知しておく必要がある。
