...
## L1: template include パスの制約 → 対応なし (Close)

### 判断 (2026-04-12)

本所見はコード変更なしで Close とする。

- **本番実行時**: すべての include ファイルはハッシュ検証の対象であり、パス形式 (`../` や絶対パス) にかかわらず、ハッシュ未登録のファイルは検証エラーで停止する
- **dry-run 時**: ハッシュ未登録ファイルも読めるが、その前提として config ファイルへの書込権限が必要。その権限があれば `cmd` フィールドの書き換えで任意コマンドを実行できるため、パス制約は実質的な防御にならない
- パス制約を追加すると `/opt/company/shared/common.toml` や `../common/` スタイルの正当なレイアウトを阻害する

セキュリティはパス制約ではなくハッシュ検証によって担保されている。
