...
- [x] `internal/verification/manager.go`
  - [x] `GetAnalysisDeps() security.AnalysisDeps` メソッドを追加する
  - [x] `security` の import を追加する
  - [x] `GetNetworkSymbolStore()` を削除する
  - [x] `GetSyscallAnalysisStore()` を削除する
  - [x] `GetDynLibAnalysisStore()` を削除する
  - [x] `GetDynLibDepsStore()` を削除する
  - [x] `GetShebangInterpreterStore()` を削除する
  - [x] `fileanalysis` と `dynamicanalysis` の import は他のメソッドで使用されているため削除しない

### 3-2. verification パッケージのテスト追加

- [x] `internal/verification/manager_test.go`（または既存の verification テストファイル）
  - [x] `TestManager_GetAnalysisDeps` を追加する（`Manager` のゼロ値で `GetAnalysisDeps()` を呼び、全フィールドが nil であることを確認する）

### 3-3. runner.createNormalResourceManager の簡素化

- [x] `internal/runner/runner.go`
  - [x] `createNormalResourceManager` の 5 系統の型アサーション（`networkSymbolStoreProvider` など）を削除する
  - [x] `analysisDepsProvider` インターフェースを定義し、`GetAnalysisDeps()` の 1 系統の型アサーションに変更する
  - [x] `security.NewNetworkAnalyzer(runtime.GOOS, deps)` と `risk.NewStandardEvaluator(networkAnalyzer)` を呼ぶ
  - [x] 組み立てた evaluator を `resource.Config.RiskEvaluator` に設定する
  - [x] 不要になる import（`fileanalysis`、`dynamicanalysis`）を削除する
  - [x] 不要になる var 宣言（`networkStore`、`syscallStore` 等）を削除する
  - [x] `"runtime"` と `risk` の import を追加する（未追加の場合）

### 3-4. runner_test.go の更新

- [x] `internal/runner/runner_test.go`
  - [x] `pathResolverWithStore` 構造体と関連メソッドを削除する
  - [x] `pathResolverWithDeps` 構造体（`ResolvePath` + `GetAnalysisDeps`）を追加する
  - [x] `TestCreateNormalResourceManager_AnalysisStoresInjected` を `GetAnalysisDeps()` が呼ばれることを確認する形に更新する
  - [x] 不要になる import（`fileanalysis`）を削除する
  - [x] `security` の import を追加する（`AnalysisDeps` の使用のため）
...
- [x] `AC-1`: `NewNetworkAnalyzer` と `NewStandardEvaluator` のシグネチャが分析依存 5 個を個別引数で受け取らない
- [x] `AC-2`: `resource.Config` に `NetworkSymbolStore`、`SyscallStore`、`DynLibDepsStore`、`LibAnalysisStore`、`ShebangStore` フィールドが存在しない
- [x] `AC-3`: `runner.createNormalResourceManager` に型アサーションが 1 系統（`analysisDepsProvider`）のみ残り、5 系統の個別 getter 呼び出しが存在しない
- [x] `AC-4`: `security` パッケージに hash dir や store dir の具体構築コードが存在しない
- [x] `AC-5`: `make test` で全テストが通過する
- [x] `AC-6`: `make test`、`make lint`、`make build` がすべて成功する
- [x] コード中に日本語が含まれていない（コメント・文字列ともに）
- [x] 不要 import が残っていない（lint 確認）
