...
- [ ] **ハッシュディレクトリ解決関数実装**
  - [ ] `getHashDirectoryWithValidation()` 関数実装
  - [ ] 優先順位付きディレクトリ取得ロジック
  - [ ] コマンドライン引数 → 環境変数 → デフォルト値の順

- [ ] **TOCTOU脆弱性回避セキュリティ検証機能実装**
  - [ ] `validateHashDirectorySecurely()` 関数実装
  - [ ] 絶対パス検証機能
  - [ ] シンボリックリンク攻撃防御機能（safefileio活用）
  - [ ] **TOCTOU対策ディレクトリ検証機能**：`os.Stat()`の代わりに
    - [ ] `os.Open()`でディレクトリを開く
    - [ ] 取得したファイルディスクリプタに対して`file.Stat()`を実行
    - [ ] チェックと使用の間にファイルシステム変更の余地を排除

- [ ] **エラーハンドリング実装**
  - [ ] `HashDirectoryError` 型定義
  - [ ] 詳細エラー分類機能
  - [ ] エラーメッセージテンプレート定義
...
- [ ] **ハッシュディレクトリ検証テスト**
  - [ ] `TestGetHashDirectoryWithValidation` 実装
  - [ ] 正常系：コマンドライン引数、環境変数、デフォルト値
  - [ ] 異常系：相対パス、存在しないディレクトリ、権限不足

- [ ] **セキュリティ検証テスト**
  - [ ] `TestValidateHashDirectory` 実装
  - [ ] シンボリックリンク攻撃テスト
  - [ ] パーミッション検証テスト
  - [ ] **TOCTOU攻撃対策検証テスト**
    - [ ] ディレクトリ検証中の並行ファイルシステム操作テスト
    - [ ] `os.Open()` + `file.Stat()` 方式の安全性確認テスト
    - [ ] 従来の`os.Stat()`方式との差異検証テスト

- [ ] **テスト実行確認**
  - [ ] 全テストが失敗することを確認（実装前のTDD検証）
  - [ ] テスト修正（必要に応じて）
  - [ ] テスト設計確認とコミット
