...
**`rename` に到達したかどうかは `moveFileAnchored` が申告する（2026-08-21 実装時の決定）。** `moveFileAnchored` は `rename` の後に移動元の削除を行うため、その戻り値のエラーだけでは `rename` の前後どちらで失敗したのかが分からない。そこで Linux 実装は `rename` より後の失敗をパッケージ内部の sentinel `errRenameCommitted` で包み、`moveOpenFileCore` が `errors.Is` でそれを見て公開 sentinel の `ErrDestinationCommitted` に translate する。非 Linux 実装は `renameat` が最後の操作なので申告する失敗を持たない。

当初の実装は代わりに「宛先が自分の inode を指しているか」を `verifySameFileAt` で問い合わせて判定していたが、これは両方向に誤る。`rename` の直後に攻撃者が宛先を奪うと不一致になり、**宛先が壊れているのに「何も起きなかった」と報告する**（§ 5.4 が最重要と定める記録も残らない）。逆に `linkat` の失敗時に宛先がたまたま同じ inode へのハードリンクだと、到達していない `rename` を到達したと報告する。どちらの側の失敗かは `moveFileAnchored` だけが知っているので、推測させずに申告させる（CLAUDE.md「Declare, don't infer」）。sentinel は非公開で、公開 API に現れるのは `ErrDestinationCommitted` だけである。`moveFileAnchored` を直接呼ぶ既存テストは戻り値の形が変わらないため影響を受けない。
