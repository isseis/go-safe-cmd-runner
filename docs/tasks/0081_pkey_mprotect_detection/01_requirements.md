...
#### FR-3.2.2: `EvalMprotectRisk` の拡張

`EvalMprotectRisk`（`elfanalyzer` パッケージ）の評価対象を `pkey_mprotect` エントリにも
拡張すること。
...
`EvalMprotectRisk` の関数名はそのまま維持する（呼び出し元への影響を最小化するため）。
ただし関数コメントに `pkey_mprotect` も評価対象であることを明記すること。
...
`EvalMprotectRisk` のマッピングルールを `pkey_mprotect` エントリにも適用する。
...
### 6.3 `EvalMprotectRisk` の名称

関数名 `EvalMprotectRisk` は `mprotect` のみを想起させるが、本タスク以降は
`pkey_mprotect` も評価する。関数名の変更（例: `EvalProtExecRisk`）はより正確だが、
呼び出し元（`standard_analyzer.go`、複数のテストファイル）の変更が必要になる。

本タスクでは関数名を維持し、コメントの更新で対応する（呼び出し元への影響を最小化）。
将来的にさらに他の syscall が追加される時点で改名を検討する。
