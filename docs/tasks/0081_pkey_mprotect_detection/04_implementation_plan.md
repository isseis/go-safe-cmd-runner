...
### 1.1 `EvalMprotectRisk` 拡張テスト（`mprotect_risk_test.go`）
...
この時点では EvalMprotectRisk は未変更のため、pkey_mprotect テストは失敗する（RED）。
...
- [ ] `analyzeSyscallsInCode` 内の `evaluateMprotectArgs` 呼び出しを `evaluateMprotectFamilyArgs` に更新
  ...
  - [ ] `EvalMprotectRisk` の呼び出しを1エントリずつに変更

### 2.3 `mprotect_risk.go` 更新

- [ ] `EvalMprotectRisk` のフィルター条件を拡張
...
`standard_analyzer.go:354` の `convertSyscallResult` は `EvalMprotectRisk(result.ArgEvalResults)` を
直接呼ぶ。`pkey_mprotect` エントリが `ArgEvalResults` に追加されれば（Phase 2.2 の実装完了後）、
`EvalMprotectRisk` の拡張（Phase 2.3）と組み合わせて自動的に `IsHighRisk` へ反映される。

この経路が正しく動作することを以下のテストで確認すること：

- [ ] `TestSyscallAnalyzer_MprotectAndPkeyMprotect` の各ケースにおいて、`result.Summary.IsHighRisk` が
  `EvalMprotectRisk(result.ArgEvalResults) || result.HasUnknownSyscalls` と一致することを検証する
...
### `EvalMprotectRisk` の呼び出し方

`analyzeSyscallsInCode` からの呼び出しでは、エントリを1件ずつ渡す
（`EvalMprotectRisk([]common.SyscallArgEvalResult{evalResult})`）。
