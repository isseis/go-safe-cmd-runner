...
|-------|---------|------|--------|--------|--------|
| Phase 1: データ構造拡張 | 3 | 0 | 0 | 3 | 0% |
| Phase 2: 変数展開機能 | 5 | 0 | 0 | 5 | 0% |
| Phase 3: セキュリティ検証拡張 | 4 | 0 | 0 | 4 | 0% |
| Phase 4: 統合とテスト | 6 | 0 | 0 | 6 | 0% |
| Phase 5: ドキュメント整備 | 3 | 0 | 0 | 3 | 0% |
| **合計** | **21** | **0** | **0** | **21** | **0%** |
...
|---------|------|----------|----------|------|
| CA-1.1 | `GroupSpec` に `CmdAllowed` フィールド追加 | 30分 | なし | [ ] 未着手 |
| CA-1.2 | `RuntimeGroup` に `ExpandedCmdAllowed` フィールド追加 | 30分 | なし | [ ] 未着手 |
| CA-1.3 | フィールドドキュメントコメント作成 | 30分 | CA-1.1, CA-1.2 | [ ] 未着手 |
...
- [ ] `GroupSpec.CmdAllowed` フィールド追加完了
- [ ] `RuntimeGroup.ExpandedCmdAllowed` フィールド追加完了
- [ ] フィールドドキュメントコメント作成完了
- [ ] 既存テストが全て成功
- [ ] `make lint` がパス
...
| タスクID | 説明 | 所要時間 | 依存関係 | 状態 |
|---------|------|----------|----------|------|
| CA-2.1 | `InvalidPathError` エラー型実装 | 1時間 | Phase 1完了 | [ ] 未着手 |
| CA-2.2 | `expandCmdAllowed()` 関数実装 | 3時間 | CA-2.1 | [ ] 未着手 |
| CA-2.3 | `ExpandGroup()` に展開処理統合 | 2時間 | CA-2.2 | [ ] 未着手 |
| CA-2.4 | 単体テスト: 変数展開 | 2時間 | CA-2.2, CA-2.3 | [ ] 未着手 |
| CA-2.5 | 単体テスト: エラーケース | 1時間 | CA-2.4 | [ ] 未着手 |
...
- [ ] `InvalidPathError` 型実装完了
- [ ] `expandCmdAllowed()` 関数実装完了
- [ ] `ExpandGroup()` への統合完了
- [ ] 単体テスト実装完了（10ケース以上）
- [ ] テストカバレッジ ≥ 90%
- [ ] `make test` がパス
...
| タスクID | 説明 | 所要時間 | 依存関係 | 状態 |
|---------|------|----------|----------|------|
| CA-3.1 | `CommandNotAllowedError` 拡張 | 1時間 | Phase 2完了 | [ ] 未着手 |
| CA-3.2 | `ValidateCommandAllowed()` 拡張 | 3時間 | CA-3.1 | [ ] 未着手 |
| CA-3.3 | 単体テスト: パターンマッチ | 1.5時間 | CA-3.2 | [ ] 未着手 |
| CA-3.4 | 単体テスト: OR条件 | 1.5時間 | CA-3.3 | [ ] 未着手 |
...
- [ ] `CommandNotAllowedError` 拡張完了
- [ ] `ValidateCommandAllowed()` 拡張完了
- [ ] 単体テスト実装完了（9ケース以上）
- [ ] テストカバレッジ ≥ 90%
- [ ] `make test` がパス
- [ ] `make lint` がパス
...
| タスクID | 説明 | 所要時間 | 依存関係 | 状態 |
|---------|------|----------|----------|------|
| CA-4.1 | Group Executor での `ExpandedCmdAllowed` 使用 | 2時間 | Phase 3完了 | [ ] 未着手 |
| CA-4.2 | 統合テスト: 基本機能 | 2時間 | CA-4.1 | [ ] 未着手 |
| CA-4.3 | 統合テスト: 既存動作の維持 | 1時間 | CA-4.2 | [ ] 未着手 |
| CA-4.4 | セキュリティテスト: パストラバーサル | 1.5時間 | CA-4.1 | [ ] 未着手 |
| CA-4.5 | セキュリティテスト: シンボリックリンク | 1.5時間 | CA-4.4 | [ ] 未着手 |
| CA-4.6 | セキュリティテスト: 他チェックとの統合 | 2時間 | CA-4.5 | [ ] 未着手 |
...
| タスクID | 説明 | 所要時間 | 依存関係 | 状態 |
|---------|------|----------|----------|------|
| CA-5.1 | README.md に機能説明追加 | 2時間 | Phase 4完了 | [ ] 未着手 |
| CA-5.2 | サンプル設定ファイル作成 | 1時間 | CA-5.1 | [ ] 未着手 |
| CA-5.3 | CHANGELOG.md 更新 | 1時間 | CA-5.2 | [ ] 未着手 |
