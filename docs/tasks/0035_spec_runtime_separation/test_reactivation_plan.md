...
- [x] Task 0038: テストインフラの最終整備（Phase 1-4 部分完了）
  - ✅ Phase 1.0: environment/integration_test.go 有効化完了
  - ⏸️ Phase 1.1: runner_test.go 大規模移行が必要（保留）
  - ✅ Phase 1.2: test/performance/output_capture_test.go 確認完了（作業不要）
  - ✅ Phase 1.3: test/security/output_security_test.go 確認完了（作業不要）
  - ✅ Phase 3: CI/CD パイプライン確認完了（テスト実行時間: 2.5秒）
  - ✅ Phase 4: カバレッジ確認完了（76.1%、runner_test.goがスキップされているため目標80%に近い）
...
**優先度**: 中
**推定工数**: ほぼ完了（残り: runner_test.go の移行のみ）
...
**完了した作業**:
1. ✅ **Phase 1.0**: environment/integration_test.go 有効化完了（7テストPASS）
2. ✅ **Phase 1.2**: test/performance/output_capture_test.go 確認完了（5テストPASS、作業不要）
3. ✅ **Phase 1.3**: test/security/output_security_test.go 確認完了（8テストPASS、作業不要）
4. ✅ **Phase 3**: CI/CD パイプライン確認完了（テスト実行時間: 2.5秒）
5. ✅ **Phase 4**: カバレッジ確認完了（76.1%）

**残作業**:
1. ⏸️ **Phase 1.1**: runner_test.go の型移行（16-24時間、保留中）
   - 大規模な移行作業が必要（CommandSpecにEffectiveWorkDirがない、GroupSpecにTempDirがないなどの問題）
   - 約48個のコンパイルエラー
   - Task 0036として別途対応予定
2. **Phase 2**: 古い型定義の削除（runner_test.go完了後）
3. **Phase 5**: ドキュメントの最終更新

**成功基準** (ほぼ達成):
- [x] 統合テストの大部分が新しい型システムで動作（runner_test.go除く）
- [ ] `skip_integration_tests` タグが完全に削除（runner_test.goのみ残存）
- [ ] 古い型定義が完全に削除（runner_test.go移行後）
