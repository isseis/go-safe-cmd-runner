# `SyscallSummary.IsHighRisk` 廃止 要件定義書
...
`common.SyscallSummary` から `IsHighRisk` フィールドを削除し、
リスク判定ロジックを `elfanalyzer` パッケージ（runner 側）に完全に閉じ込める。
事実の記録（`HasUnknownSyscalls`・`HighRiskReasons`）とリスク判断を明確に分離する。
...
| 用語 | 定義 |
|------|------|
| 事実の記録（fact recording） | バイナリ解析で観察された事実をそのまま保存すること。例: `HasUnknownSyscalls`、`HighRiskReasons`、`ArgEvalResults` |
...
### 3.2 `elfanalyzer` でのリスク判定の置き換え

#### FR-3.2.1: `convertSyscallResult` の変更
...
`HighRiskReasons` は人間可読の説明文であり判定条件には使わないこと。
...
#### FR-3.2.2: `analyzeSyscallsInCode` での `IsHighRisk` 設定削除とコメント修正
...
その結果を `IsHighRisk` に格納するのではなく、`HighRiskReasons` への追加のみに使用するよう変更する。
...
#### FR-3.3.1: スキーマバージョンの更新

`is_high_risk` フィールドが JSON から削除されることに伴い、`fileanalysis` パッケージの
`CurrentSchemaVersion` を現在の値から 1 インクリメントすること。

#### FR-3.3.2: 旧バージョンの無効化

スキーマバージョン不一致時の既存動作（解析結果を無効化して再解析を要求する
`SchemaVersionMismatchError`）をそのまま維持すること。
旧バージョンの JSON に `is_high_risk` フィールドが含まれていても、
再解析によって正しい結果が得られること。
...
| ファイル | 変更内容 |
|----------|----------|
| `internal/common/syscall_types.go` | `SyscallSummary.IsHighRisk` フィールドを削除 |
...
| `internal/runner/security/elfanalyzer/mprotect_risk.go` | `EvalMprotectRisk` 関数のコメント（L6）を「`IsHighRisk` に設定すべきか」から「mprotect 由来のリスクが存在するか（`HighRiskReasons` への追加判断および `convertSyscallResult` でのリスク判定に使用）」に更新 |
...
| ファイル | 変更内容 |
|----------|----------|
| `internal/common/syscall_types_test.go` | `TestSyscallSummary_JSONRoundTrip`（L87–101）・`TestSyscallAnalysisResultCore_JSONRoundTrip`（L106–135）から `IsHighRisk` フィールドの設定・アサーションを削除 |
| `internal/filevalidator/validator_test.go` | `TestBuildSyscallAnalysisData` 内の `IsHighRisk` アサーション（L1314、L1326）を削除 |
| `internal/fileanalysis/syscall_store_test.go` | 保存・ロードのラウンドトリップテスト（L28–64、L151–193）および ArgEvalResults ラウンドトリップテスト（L328–358）から `IsHighRisk` フィールドの設定・アサーションを削除 |
| `internal/runner/security/elfanalyzer/syscall_analyzer_test.go` | `result.Summary.IsHighRisk` への参照（L163、L194 等）を `EvalMprotectRisk` または `HasUnknownSyscalls` を使った等価な確認に置き換え。`exec_not_set does not overwrite pre-existing IsHighRisk=true` テスト（L871–890）の検証方法を `HasUnknownSyscalls` のみの確認に変更 |
| `internal/runner/security/elfanalyzer/analyzer_test.go` | モックストアが返す `SyscallAnalysisResult` の `Summary.IsHighRisk` 設定（L376、L422、L593）を削除。`convertSyscallResult` がキャッシュ済みデータを読む経路のテストを、`HasUnknownSyscalls` / `ArgEvalResults` から正しくリスク判定されることの確認に更新（詳細は §5.3 参照）|
...
### AC-3: リスク判定の同値性（`elfanalyzer` リアルタイム解析経路）
...
- [ ] `EvalMprotectRisk(ArgEvalResults) = true`（mprotect `exec_confirmed` / `exec_unknown`）のケースで
  `HighRiskReasons` にエントリが追加されていること
  - `TestSyscallAnalyzer_MultipleMprotect` 等が引き続きパスすること
- [ ] `exec_not_set` のみのケースで `HasUnknownSyscalls = false` かつ `HighRiskReasons` が空であること
...
### AC-4: リスク判定の同値性（`elfanalyzer` キャッシュ読み取り経路）
...
- [ ] `CurrentSchemaVersion` が旧バージョンより 1 大きい値になっていること
...
- [ ] 新バージョンの JSON に `is_high_risk` フィールドが含まれないこと
  - `syscall_store_test.go` L28–64（基本ラウンドトリップ）が `IsHighRisk` 設定なしで通過すること
  - `syscall_store_test.go` L151–193（高リスク情報のラウンドトリップ）が `IsHighRisk` アサーションなしで通過すること
  - `syscall_store_test.go` L328–358（ArgEvalResults ラウンドトリップ）が `IsHighRisk` アサーションなしで通過すること
...
ものである。`IsHighRisk` 廃止後も、この関数の戻り値は `HighRiskReasons` へのエントリ追加判断に
引き続き使える。
...
「mprotect 由来のリスクが存在するか（`HighRiskReasons` への追加判断および `convertSyscallResult` でのリスク判定に使用）」
を表すコメントに更新すること。
...
`HighRiskReasons` は現行実装では `EvalMprotectRisk` が `true` になるケースと
連動してエントリが追加されるが、これは偶然の一致であって設計上の保証ではない。
`HighRiskReasons` を判定条件に使うと「説明文の整理 → 判定変化」という壊れやすい設計になるため、
...
`is_high_risk` フィールドが JSON から消えることは後方非互換変更である。
スキーマバージョンを更新し、旧バージョンのキャッシュを強制的に再生成させる必要がある。
ただし `is_high_risk` は `HasUnknownSyscalls` と `ArgEvalResults`（`EvalMprotectRisk` による評価）から再計算可能であるため、
再解析なしに移行できる可能性もあるが、コードの単純性のため既存の「バージョン不一致 → 再解析」
の仕組みをそのまま利用する。
