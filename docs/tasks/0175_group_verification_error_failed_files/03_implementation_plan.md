...
`BenchmarkBuildPreExecutionError_FailedFilePaths`（end-to-end）と `BenchmarkRenderFailedFiles`（ビルダー単体）で 02_architecture.md §7.7 の絶対予算とスケーリングを確認し、数値をコミットメッセージに記す（Phase 4b）。単体テストのしきい値にはしない。実測では end-to-end が予算を満たさず（既存の要素ごとの redaction が大半）、ビルダー単体は満たした（Phase 4b の実測結果を参照）。
...
- **性能**: n = 10,000 の end-to-end 描画が 100 ms 未満で、1 件あたりのコストが n = 1,000 の 3 倍以内（ベンチマークの数値をコミットメッセージに記録）。2026-09-24 の実測では end-to-end が未達（約 375 ms、既存の要素ごとの redaction が大半）、ビルダー単体は達成（約 45 ms、比 約 1.04 倍）。扱いは PR-7 のレビューで決める（Phase 4b の実測結果を参照）。
