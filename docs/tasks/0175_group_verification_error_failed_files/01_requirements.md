...
- group 名は通知コンテキスト（Scope）のみに残し、本文やフィールドで重複させない。
...
group 名は通知コンテキスト（Scope）にだけ表示する。0172 の AC-13・AC-14 が定めた「group 名の唯一の表示場所を Scope にする」を維持し、ファイル一覧を載せる際に本文へ group 名を書き戻さない。
...
Error Message は動的な値であり、0172 の表示安全な補間契約を通る。したがって 1 行であること・制御文字と書式制御文字を含まないこと・長さが上限を超えないことは既存の仕組みで保たれる。件数が多い場合の切り詰めもこの契約に従い、本タスクのための追加の仕組みを設けない（YAGNI）。
...
**Acceptance Criteria**:
- **AC-01**: group 検証エラー通知に、`verification.Error.Details` の各ファイルパスが現れる。
- **AC-02**: 複数のファイルが失敗したとき、すべてのファイルパスが通知に現れる。
- **AC-03**: group 名は通知コンテキスト（Scope）に一度だけ現れ、Error Message には現れない。
...
**Acceptance Criteria**:
- **AC-06**: `Runner.Execute` を通したテストが、失敗ファイルパスが通知に載ることを検証し、`Details` を通知から落とす実装へ戻すと失敗する。
