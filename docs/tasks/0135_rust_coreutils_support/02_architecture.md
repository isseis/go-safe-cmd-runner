...
- **coreutils 判定段:** `EvaluateRisk` / `AnalyzeCommandSecurity` のリスク判定パイプラインに本タスクで追加する 1 段。
- **`CoreutilsCommandRisk`（coreutils 分類関数）:** coreutils 判定段が呼び出す、解決済みパスからリスクレベルを返すパッケージ関数。
...
- **既存リスク評価枠組みの再利用:** 既存の判定段を維持し、その中に coreutils 判定段を 1 つ追加する。新しいリスク評価系を作らない（YAGNI / DRY）。
- **上位シグナル優先の不変条件:** coreutils 判定段は、信頼できる上位シグナル（特権昇格・破壊的パターン・setuid/setgid・ハッシュ不一致）の**後段**に置き、**信頼できない段（バイナリ解析／ディレクトリ既定）のみを置き換える**。coreutils 判定段が上位シグナルの結果を引き下げることはない。
...
実行時の誤判定は `EvaluateRisk` → `NetworkAnalyzer.IsNetworkOperation` → バイナリ解析（`dlsym`/`mprotect`/socket シンボル検出）に起因する。単一バイナリは全サブコマンドでシンボルを共有するため、この経路自体を coreutils に対して迂回する必要がある。よって「`EvaluateRisk` 内に coreutils 判定段を追加する」設計が要件上必須である。

### 1.3. 前提：パス解決後の basename にサブコマンド名が保持されること

本設計は、coreutils コマンドのパス解決後のパスが `/usr/lib/cargo/bin/coreutils/<コマンド名>` となり、basename にサブコマンド名（例 `mkdir`）が保持されることを前提とする。`CoreutilsCommandRisk` はこの basename でコマンドを分類するため、この前提は設計全体の要となる。
...
> **実装着手前の必須確認（リスク）:** もし対象環境で `/usr/bin/<cmd>` が単一バイナリ本体（basename が `coreutils` 等）への**シンボリックリンク**である構成（busybox 型のマルチコールバイナリ）であった場合、`EvalSymlinks` がサブコマンド名を失い本設計は機能しない。実装着手前に Ubuntu 26.04 実機で `readlink -f "$(command -v mkdir)"` 等により basename が保持されることを確認すること。確認できない場合は、解決前のコマンド名で分類する代替設計に切り替える必要がある。
...
flowchart TD
    ...
    DIRCHK -->|"はい"| CLASS["coreutils 判定段<br>(CoreutilsCommandRisk)"]
...
`CoreutilsCommandRisk()` は `security` パッケージに新規追加するパッケージ関数で、実行時経路（`risk.EvaluateRisk`）と dry-run 経路（`security.AnalyzeCommandSecurity` に追加する coreutils 判定段）の双方から**直接**呼び出され共有される。`getDefaultRiskByDirectory` は coreutils へ委譲しない（理由は §3.1 / §5.2 を参照）。`AnalyzeCommandSecurity` は引き続き非 coreutils パスについてのみ `getDefaultRiskByDirectory` を利用する。
...
sequenceDiagram
    ...
    EV->>CU: CoreutilsCommandRisk(cmd.ExpandedCmd)
    alt coreutils ディレクトリ直下
        CU-->>EV: (risk, true)
        EV-->>NM: risk（バイナリ解析を迂回）
    else 直下でない
        CU-->>EV: (RiskLevelUnknown, false)
...
// When the path is not a direct child, it returns (RiskLevelUnknown, false), and
// callers fall back to the normal risk evaluation path.
//
// Classification (by command basename):
...
func CoreutilsCommandRisk(resolvedPath string) (runnertypes.RiskLevel, bool)
```
...
- **High（破壊的）:** `rm`, `rmdir`, `unlink`, `shred`, `dd` 等、データ消失・ディスク破壊を伴うもの（要件 F-004）。
- **Low（既知の安全）:** `mkdir`, `ls`, `cat`, `echo`, `cp`, `mv` 等、読み取り・情報取得・テキスト処理・無害なファイル操作（要件 F-003）。
- **Medium（既定）:** 明示一覧に無い coreutils コマンド。これには (a) `chmod`/`chown`/`chgrp`/`chcon`/`chroot`/`runcon`/`ln`/`install`/`mknod`/`mkfifo`/`truncate` 等の権限・所有者・デバイス操作系、(b) `env`/`nohup`/`nice`/`timeout`/`stdbuf` 等、**任意の別コマンドを起動するラッパー**、(c) 将来追加される未知のサブコマンド、が含まれる。ラッパー系は内部で別コマンドを実行し個別解析を迂回しうるため Low にせず Medium 以上とする（非機能 4.1）。
...
本機能は新たなエラー型を導入しない。`CoreutilsCommandRisk` は純粋な分類関数であり、I/O やエラーを伴わず `(runnertypes.RiskLevel, bool)` のみを返す。

- `EvaluateRisk` の戻り値型 `(runnertypes.RiskLevel, error)` は不変。coreutils 判定段では `error` を返さない（`nil`）。
- リスクが設定された許容レベルを超えた場合のブロックは、既存の `normal_manager.go` のエラー（`runnertypes.ErrCommandSecurityViolation`）をそのまま利用し、型・メッセージともに変更しない。
- パスが coreutils ディレクトリ直下でない場合は `(RiskLevelUnknown, false)` を返し、呼び出し側が従来経路へフォールバックする（例外ではなく通常の制御フロー）。
...
- **適用範囲の限定:** coreutils 判定段はパスが coreutils ディレクトリ直下（`filepath.Dir == CoreutilsDir`）のときのみ発火する。`/usr/bin` 等の非 coreutils バイナリは従来どおりバイナリ解析の対象であり、プロファイル未登録のネットワークツール等の検出能力は低下しない（非機能 4.1）。
- **dry-run 経路は単一機構で処理する:** coreutils 分類は `AnalyzeCommandSecurity` に追加する coreutils 判定段（明示的な早期 return）のみで行い、`getDefaultRiskByDirectory` には coreutils 専用の委譲を加えない。`getDefaultRiskByDirectory` の結果はステップ 9（最終フォールバック）でしか使われず、coreutils 判定段（ステップ 6 の後）が先に確定するため、仮に委譲を加えても coreutils 経路ではデッドコードとなり、かつステップ 8 の `getCommandRiskOverride` と優先順位が衝突する。二重機構を避けるため委譲は行わない。なお `getDefaultRiskByDirectory` の既存 prefix 一致（`HasPrefix(cmdPath, dir+"/")`）と `CoreutilsCommandRisk` の exact 一致（`filepath.Dir == CoreutilsDir`）は判定述語が異なるが、coreutils はフラットに名前付きエントリを配置するため両者の差（ネストしたパス）は実環境で発生しない。発生した場合は coreutils 判定段が非該当となり通常経路へフォールバックする（安全側）。
- **上位シグナルの保全:** coreutils 判定段は信頼できる上位シグナル（特権昇格・破壊的パターン・setuid/setgid・ハッシュ不一致）の後段に置く。これらが検出された場合は coreutils 判定段に到達せず、より厳しい結果が維持される（§1.1 の不変条件）。
- **ファイル整合性検証は不変:** 本機能はリスク「分類」のみを変更し、実行前のハッシュ検証（`internal/verification`、および dry-run の `AnalyzeCommandSecurity` ステップ 4）を迂回・変更しない。
- **特権昇格判定は不変:** `sudo`/`su`/`doas` は coreutils ではなく、coreutils 判定段の前段（特権昇格判定）で Critical となるため影響を受けない（非機能 4.1）。
...
|---|---|---|
| 通常実行 | `EvaluateRisk`（coreutils 判定段を含む） | 許容リスク内であればコマンドを実行（従来どおり） |
| dry-run | `AnalyzeCommandSecurity`（coreutils 判定段を含む） | コマンドは実行せず、判定結果を表示するのみ（従来どおり） |
...
| setuid/setgid | （実行時経路に当該段なし） | ステップ 6 で High | coreutils 単一バイナリは通常 setuid ではない。万一 setuid なら dry-run のみがより厳しく High（**安全側の差**であり、coreutils 判定段は引き下げない） |
| ハッシュ不一致 | 実行前の `internal/verification` が fail-closed で停止 | ステップ 4 で Critical | 両経路ともブロック |
| Low/Medium/High 分類 | coreutils 判定段（`CoreutilsCommandRisk`） | coreutils 判定段（`CoreutilsCommandRisk`、同一入力 basename） | 同一関数・同一入力で一致 |

結論：正常に配置された coreutils バイナリ（通常の regular file、ハッシュ検証済み、非 setuid）に対して、両経路の最終リスクは一致する。setuid・ハッシュ不一致の例外時は dry-run が同等以上に厳しく、coreutils 判定段がリスクを引き下げることはない。
...
flowchart TD
    ...
    CU -->|"はい"| RCU["CoreutilsCommandRisk の結果<br>Low / Medium / High"]
    ...
    class CU,RCU enhanced
    class P,D,NWH,NW,SM,RC,RH,RH2,RM,RM2,RL process
```

**矢印の意味:** 判定の分岐と、確定するリスクレベルへの遷移を表す。`バイナリ解析で高リスク?`→`ネットワーク操作?` は順に評価される逐次判定であり、緑色の段（coreutils 直下判定とその結果）が本タスクの追加部分。
...
coreutils 判定段をネットワーク判定の前に置くことで、誤検出の原因であるバイナリ解析（`IsNetworkOperation` 内）を coreutils コマンドに対して確実に迂回する（要件 F-002）。
...
- **`CoreutilsCommandRisk`（新規 `coreutils_test.go`）**
  - coreutils 直下の安全コマンド（`mkdir`, `ls`, `cat`, `echo` 等）→ `(Low, true)`（F-003）
  - coreutils 直下の権限操作系・ラッパー・未知名（`chmod`, `chown`, `env`, `nohup`, 未知の名前）→ `(Medium, true)`（F-003 / 非機能 4.1）
  - coreutils 直下の破壊的コマンド（`rm`, `dd`, `shred`）→ `(High, true)`（F-004）
  - 非 coreutils パス（`/usr/bin/mkdir`, `/bin/ls`）→ `(RiskLevelUnknown, false)`（適用範囲限定）
...
- 特権昇格（`sudo` 等）が coreutils 判定段の影響を受けず Critical のままであること（非機能 4.1）。

---

## 8. 実装の優先順位 (Implementation Priorities)

1. **フェーズ 0（前提確認）:** 対象環境で coreutils コマンドのパス解決後 basename が保持されることを確認（§1.3 のリスク）。
...
3. **フェーズ 2（実行時経路）:** `risk/evaluator.go` の `EvaluateRisk` に coreutils 判定段を追加（F-002 / F-004）。`evaluator_test.go` にケース追加。
4. **フェーズ 3（dry-run 経路と整合）:** `directory_risk.go` の `DefaultRiskLevels` から coreutils 暫定エントリを削除（委譲は追加しない）。`command_analysis.go` の `AnalyzeCommandSecurity` に coreutils 判定段を所定位置（ステップ 6 の後、ステップ 7 の前）へ挿入し、`CoreutilsCommandRisk` を直接呼び出す（F-005）。関連テスト追加。
...
- **作業ツリーの暫定状態:** 本タスク着手時点の作業ツリーには、暫定の先行編集が未コミットで含まれている。すなわち `internal/common/secure_path.go` の `SecurePathEnv` には既に coreutils ディレクトリが追加され、`internal/runner/base/security/directory_risk.go` の `DefaultRiskLevels` には一律 `coreutils → Medium` が暫定追加されている。本設計のフェーズ 1〜3 は、前者を `CoreutilsDir` 定数化で整理し、後者の一律エントリを削除する。coreutils のコマンド別分類は `AnalyzeCommandSecurity` / `EvaluateRisk` の coreutils 判定段（`CoreutilsCommandRisk`）で行い、`getDefaultRiskByDirectory` には委譲を加えない。
