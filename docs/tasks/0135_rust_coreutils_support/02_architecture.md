...
- **coreutils 判定ステップ:** `EvaluateRisk` / `AnalyzeCommandSecurity` のリスク判定パイプラインに本タスクで追加する 1 ステップ。
...
- **上位シグナル優先の不変条件:** coreutils 判定ステップは、信頼できる上位シグナル（特権昇格・破壊的パターン・ハッシュ不一致）の**後段**に置き、**信頼できないステップ（バイナリ解析／ディレクトリ既定）のみを置き換える**。さらに setuid/setgid は coreutils 判定ステップの内部で最優先に検査して High とする（実行時経路には独立した setuid ステップが無いため）。coreutils 判定ステップが上位シグナルの結果を引き下げることはない。
...
両経路は同一の `CoreutilsCommandRisk` を共有するが、関数の前後に置かれるステップは経路ごとに異なる。F-005 を満たすため、coreutils コマンドに対して両経路の最終リスクが一致することをステップごとに検証する。
...
**矢印の意味:** 判定の分岐と、確定するリスクレベルへの遷移を表す。`バイナリ解析で高リスク?`→`ネットワーク操作?` は順に評価される逐次判定であり、緑色のステップ（coreutils 直下判定・setuid 検査・実効サブコマンド分類）が本タスクの追加部分。
