...
- [ ] A1. `internal/runner/base/privilege/` — 権限昇格・降格管理
      → `findings/A1_privilege.md`
- [ ] A2. `internal/runner/base/executor/` — コマンド実行・環境変数構築・パス検証
      → `findings/A2_executor.md`
- [ ] A3. `internal/runner/base/environment/` — 環境変数処理・フィルタリング
      → `findings/A3_environment.md`
- [ ] A4. `internal/runner/base/security/` — コマンド/環境変数 allowlist 検証フレームワーク
      → `findings/A4_security.md`
- [ ] A5. `internal/runner/base/risk/` — リスクベースのコマンド評価
      → `findings/A5_risk.md`
- [ ] A6. `internal/runner/base/output/` — 出力先パス検証
      → `findings/A6_output.md`
- [ ] A7. `internal/runner/base/audit/` — セキュリティ監査ログ
      → `findings/A7_audit.md`

### Phase B: ファイル整合性・検証

- [ ] B1. `internal/safefileio/` — symlink-safe ファイル I/O（openat2 + フォールバック）
      → `findings/B1_safefileio.md`
- [ ] B2. `internal/filevalidator/`, `internal/filevalidator/pathencoding/` — ハッシュベース整合性検証
      → `findings/B2_filevalidator.md`
- [ ] B3. `internal/verification/` — config/binary 統合検証マネージャ
      → `findings/B3_verification.md`
- [ ] B4. `internal/runner/config/` — TOML 設定ロード・変数展開・テンプレート include
      → `findings/B4_config.md`

### Phase C: バイナリ・依存解析

- [ ] C1. `internal/security/` 配下（`binaryanalyzer/`, `elfanalyzer/`, `machoanalyzer/`）, `internal/arm64util/`
      → `findings/C1_binary_analysis.md`
- [ ] C2. `internal/dynlib/` 配下（`elfdynlib/`, `machodylib/`）, `internal/libccache/`
      → `findings/C2_dynlib.md`
- [ ] C3. `internal/shebang/`, `internal/fileanalysis/`
      → `findings/C3_shebang_fileanalysis.md`

### Phase D: 周辺セキュリティ機能

- [ ] D1. `internal/groupmembership/` — CGO 経由 `getgrgid_r` 呼び出し
      → `findings/D1_groupmembership.md`
- [ ] D2. `internal/logging/`, `internal/redaction/` — 構造化ログ、Slack webhook、機密情報 redaction
      → `findings/D2_logging_redaction.md`

### Phase E: エントリポイント・起動シーケンス

- [ ] E1. `cmd/runner/`, `cmd/record/`, `cmd/verify/`, `internal/runner/bootstrap/`, `internal/runner/cli/`
      → `findings/E1_entrypoints.md`

## 5. 集約フェーズ

- [ ] 全 Phase A〜E のチェックが完了していることを確認
- [ ] `findings/*.md` を通読し、所見一覧表（ID・重大度・領域・概要・ファイルへのリンク）を作成
- [ ] コンポーネント横断で繰り返し出現するパターン（例: 同種のエラー握りつぶしが複数箇所にある等）があれば別枠でまとめる
- [ ] 「観察された良好な防御層」を集約・重複排除して記録
- [ ] 対応優先度（推奨）を重大度と修正コストから暫定順位付け
- [ ] `99_summary.md` として保存
