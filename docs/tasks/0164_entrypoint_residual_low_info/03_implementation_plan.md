...
- [x] 見出し「パス解決の変更により新たに権限違反が検出されることがあります」 — アップグレード前の判定手順として、ハッシュディレクトリと検証対象のパスに `readlink -f` を適用し、その実体の祖先の権限を確認する手順を示す（02_architecture.md §9）。
...
- [x] **地の確認**: CHANGELOG に記した `readlink -f` を用いた判定手順を、シンボリックリンク経由のハッシュディレクトリに対して実行し、実体パスの解決と祖先の権限一覧が記載どおりに出ることを確認した。
