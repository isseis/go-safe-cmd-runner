...
| `record` が world-writable なハッシュディレクトリを拒否する | `record -d` に、(a) 既に存在していて自身が world-writable なパス、または (b) `/tmp` 直下など world-writable なディレクトリの下の、まだ作成されていないパスを指定している場合（本番既定のハッシュディレクトリはいずれにも該当しない）。sticky bit の有無を問わない | `record` が終了コード 1 で停止する | (a) は `chmod go-w` で是正する。(b) は先に利用者自身がディレクトリを作れば通る（§5.2）。恒久的には、いずれも書き込みが制限されたパスへハッシュディレクトリを移す |

CHANGELOG には、0162 と同じく、アップグレード前に影響の有無を判定する手順を載せる（AC-45）。パス解決の変更については、ハッシュディレクトリと検証対象のパスに対し `readlink` で実体を求め、その祖先の権限を確認する手順を示す。**実装時の変更**: 手順には `-f` ではなく `-m` を用いた。`-f` はパスの途中が実在しないと失敗して空文字列を返し、祖先を辿るループが終わらないためである。ハッシュディレクトリが未作成であることは、本タスクが `verify` の終了コード 3 として案内する状態そのものであり、手順が最初に踏む可能性が高い。
