...
`record` では、ハッシュディレクトリの作成位置が引数解析から権限チェックの後ろへ移る。作成後には、作成されたディレクトリ自身に対する再チェックが入る（理由は §5.2）。
...
sequenceDiagram
    ...
    alt 違反あり
        ...
        M->>C: 作成したディレクトリの再チェック（第2ゲート、変更点）
        alt 再チェックで違反あり
            C-->>M: false
            M-->>U: 標準エラー出力にエラー、終了コード 1
            Note over FS: ハッシュ記録は1件も書かない
        else 再チェックも通過
...
| `cmd/record/main.go` | 変更 | ディレクトリ作成を権限チェック後へ移動し作成後に再チェック（F-002）、共有パス解決の利用（F-001）、panic 廃止（F-006）、`cacheDir` の重複計算の解消（F-010） | `TestHashDirPermissions_0o700`、`TestRunUsesDefaultHashDirectoryWhenNotSpecified`、`TestRunTOCTOU_*` |
...
### 5.2 作成順序の入れ替えが生む窓と、その塞ぎ方

作成の前には第1ゲート（`checkDirPermissions`）が走る。ハッシュディレクトリが既に存在する通常の運用では、このゲートが祖先とディレクトリ自身の両方を検査するため、以下の話は当てはまらない。問題になるのは、ハッシュディレクトリがまだ無い初回の実行である。

`record` の作成順序を「作成 → チェック」から「チェック → 作成」へ入れ替えると、この初回の実行に限って**それ自体は新しい窓を開ける**。順を追うと次のようになる。

1. チェック時点でハッシュディレクトリは存在しない。`RunTOCTOUPermissionCheck` は存在しないディレクトリを黙って読み飛ばす（`fs.ErrNotExist` を無視する）ため、検査されるのは祖先だけである。
2. 祖先が sticky ビット付きの world-writable ディレクトリ（`/tmp` 配下など）である場合、既存のチェックはこれを安全とみなして通過させる。
...
したがって本設計では、**作成の直後に、作成されたハッシュディレクトリ自身を対象として権限チェックをもう一度実行する**（§2.3 の第2ゲート）。この再チェックで違反が出た場合、`record` はハッシュ記録を1件も書かずに終了コード 1 で終わる。

第2ゲートは第1ゲートの代わりではなく、「未作成だったために第1ゲートが検査できなかった対象」を作成直後に検査するためのものである。ハッシュディレクトリが既存の場合は `os.MkdirAll` が何もしないため、第2ゲートは第1ゲートと同じ対象を検査して同じ結果になる。

- 第2ゲートで違反が出た時点でディレクトリは既に存在する。AC-08 が保証するのは第1ゲート（利用者が指定したパスの祖先に違反がある場合は作成しない）であり、第2ゲートが発火するのは上記の競合が実際に起きた場合に限られる。この違いは実装計画のトレーサビリティに明記する。
- 第2ゲートの失敗時に、作成された（あるいは差し替えられた）パスの削除は試みない。その時点でパスは攻撃者の管理下にあり得るため、削除はそのパスに対する新たな書き込みになる。
...
- **作成後の再チェックが働くこと**（§5.2）。第1ゲート通過後・作成前にハッシュディレクトリのパスを攻撃者所有ディレクトリへのシンボリックリンクにすり替えた状態を作り、ハッシュ記録が1件も書かれずに非ゼロ終了すること。
...
| 2 | `record` の移行（パス解決・作成順序と再チェック・panic 廃止・重複計算の解消） | F-001・F-002・F-006・F-010 |
...
- **`record` の作成後の再チェックを省く案**: 作成順序の入れ替えが開ける窓を塞げないため採用しなかった（§5.2）。
