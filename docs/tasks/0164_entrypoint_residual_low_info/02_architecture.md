...
この判定により、作成に進むのは「利用者以外が名前を作れない場所」だけになる。チェック通過から `os.MkdirAll` までの間にそのパス名を作れるのは、その時点で既に書き込みを許されている相手だけであり、これは §5.3 が述べる残存する隙と同じ性質に収まる。作成後の再検査は不要になる。
...
パス解決と権限チェックの間、およびチェックと実際の読み書きの間には時間差があり、そこに隙が残る。この隙は本タスクで新しく生まれるものではなく、現行実装にも同じ形で存在する。隙を悪用するには検査対象の祖先ディレクトリへの書き込み権限が必要であり、その権限自体がチェックの違反対象である。本タスクの目標は隙を無くすことではなく、§5.2 のとおり現状より広げないことである。
