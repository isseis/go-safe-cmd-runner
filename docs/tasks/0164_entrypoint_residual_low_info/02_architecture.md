...
そこで、不在の場合のみ記録レベルを `DEBUG` に下げ、それ以外の stat 失敗（＝検査そのものができない）は `ERROR` のまま残す。返すエラーは変えないので、失敗として扱う呼び出し元の挙動は変わらない。記録自体は `DEBUG` に残るため、どのディレクトリが検査対象から外れたかは追える。件数としては `TOCTOUCheckResult.Skipped` が持ち、起動時チェックでは §6.3 の `INFO` に現れる。判定規則そのものは変更していない。
