...
> - **別名・dot import 耐性**: セレクタのローカル識別子を import 宣言から**実 import パスへ解決**して照合するため、別名 import（`import myos "os"`）で迂回できない。禁止パッケージの dot import は参照が無修飾化して選択子検査を迂回するため、それ自体を違反として報告する。
> - **禁止集合（3 カテゴリ）**:
