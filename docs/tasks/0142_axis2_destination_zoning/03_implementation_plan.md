...
>   - *live プロセス状態*: `Getwd`（CWD）・`Getpid`/`Getppid`/`Gettid`（PID）。
>   - これにより AC-22 の「環境・live 状態に依存しない」を静的に担保する。
> - **コントロール**: 「実参照＝検出／同名のコメント・文字列＝無視」を表明し、空振り（fail-open）と誤検出（false positive）を同時に防ぐ。別名・値参照・dot import の各検出ケースを含む。
> - **手動の二次チェック**（任意）: 概略 `rg -n 'os\.Get(euid|uid|gid|egid|groups)|user\.(Current|Lookup)|(syscall|unix)\.Get(euid|uid|gid|egid|groups)|os\.(Getenv|LookupEnv|Environ|ExpandEnv|TempDir|UserHomeDir|UserConfigDir|UserCacheDir|Getwd|Getpid|Getppid)|(syscall|unix)\.(Environ|Getenv|Getwd|Getpid|Getppid|Gettid)' <上記 3 ファイル>`。これは raw テキスト走査ゆえコメント/文字列に誤反応しうる近似であり、権威は AST テストが持つ。
>
