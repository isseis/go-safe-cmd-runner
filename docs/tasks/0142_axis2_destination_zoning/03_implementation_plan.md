...
- [x] grep ガード（AC-21 補助）: 判断軸2 の組み込みコード（`destination_zoning.go`・`operand_path_resolver.go`）が live-identity
      ...
      パターンの空振り（fail-open）を防ぐ。`risk/live_identity_guard_test.go::TestNoLiveIdentityInZoning`（各対象ファイルが実在し
      内容が空でないことも必須とし、ファイルの移動・改名でガードが無言で空振り＝合格扱いになるのを防ぐ）。
...
- [x] グリーンゲート（`_context.md` の "Green gate" 参照）がパスしていることを確認した
- [x] PR を作成した（https://github.com/isseis/go-safe-cmd-runner/pull/788）
- [x] PR がマージされた
- [x] 次のブランチへ切り替えた（本タスクは全 Phase 完了。以降は新ブランチで別タスク）
...
- [x] PR-6 マージ済み（対象ステップ: P6）— config 組み込み＋identity 注入（本番経路）／spec パース・差分テスト・fail-closed 緑
- [x] PR-7 マージ済み（対象ステップ: P7）— 決定性・上限・grep ガード／runtime==dry-run・線形呼出・live 不参照 緑
- [x] 全 PR マージ後: `make fmt`→`make test`→`make lint` 全緑、`./internal/runner/...` コンパイル（NF-002）
...
> リテラルのパイプ文字になる。`\|` を選択のつもりで使うとパターンが空振りし、危険 API が存在してもガードが「0 件＝合格」と
> 誤判定する（fail-open）。よって選択は素の `|` で書く（上記 NF-003 行・下記 AC-21 ガードとも修正済み）。
>
> **ライブ identity ガード（AC-21 static）の構成**: `live_identity_guard_test.go::TestNoLiveIdentityInZoning` が go/ast で実行する。
>
> - **対象**: 分類器・仕様表・リゾルバの 3 ファイル（`destination_zoning.go`・`destination_zoning_spec.go`・`operand_path_resolver.go`）。各ファイルが実在し内容が空でないことを必須とし、ファイルの移動・改名でガードが無言で空振り（＝合格扱い）になるのを防ぐ。
> - **検査方式**: `go/parser` でパースし、すべての `*ast.SelectorExpr`（`pkg.fn` 参照）を走査する。直接呼出だけでなく値としての参照・引数渡し（`fn := os.Geteuid`）も捕捉。コメント/文字列リテラル・整形差・行跨ぎは AST ノードでないため構造的に無視。
> - **別名・dot import 耐性**: セレクタのローカル識別子を import 宣言から実 import パスへ解決して照合するため、別名 import（`import myos "os"`）で迂回できない。禁止パッケージの dot import は参照が無修飾化して選択子検査を迂回するため、それ自体を違反として報告する。
> - **禁止集合（カテゴリ別）**:
>   - identity: `os`/`syscall`/`unix` の uid/euid/gid/egid/groups getter、`os/user` の `Current`／`Lookup*`。
>   - 環境: `os`/`syscall`/`unix` の `Getenv`/`LookupEnv`/`Environ`、`os.ExpandEnv`／`os.TempDir`／`os.User{Home,Config,Cache}Dir`。
>   - live プロセス/システム状態: `Getwd`（CWD）・`Getpid`/`Getppid`/`Gettid`（PID）・`os.Hostname`・`os.Executable`。
>   - 生 syscall・プロセス生成入口: `syscall`/`unix` の `Syscall`/`Syscall6`/`RawSyscall`/`RawSyscall6`（上記を直接呼べてしまうため）・`ForkExec`/`Exec`/`Fork`、`os.StartProcess`。
>   - コマンド実行: `os/exec` の全参照（純粋な分類はコマンドを実行しない。dot import も違反として検出）。
>   - live FS/CWD を触る path 解決: `path/filepath` の `Abs`/`EvalSymlinks`/`Glob`（注入リゾルバを迂回する。純粋ヘルパ `Join`/`Clean`/… は許可）。
>   - これにより AC-22 の「環境・live 状態に依存しない」を静的に担保する。
> - **コントロール**: 「実参照＝検出／同名のコメント・文字列＝無視」を表明し、空振り（fail-open）と誤検出（false positive）を同時に防ぐ。別名・値参照・dot import・`os/exec` 参照の各検出ケースを含む。
> - **手動の二次チェック**（任意）: 上記カテゴリの関数名を `os`/`syscall`/`unix`/`os/user`/`os/exec` で `rg` 走査できるが、これは raw テキスト走査ゆえコメント/文字列に誤反応しうる近似であり、権威は AST テストが持つ。
>
> **静的 grep ガードの位置づけ（再発防止の原則）**: NF-003／AC-21 の grep は禁止 API の非網羅な denylist であり、完全性の
> 保証ではない。read-only／live-identity 不参照の権威ある保証は挙動テスト——`TestDeterminismRuntimeEqualsDryRun`
> （runtime==dry-run）と `TestRunAsIdentDifferential`（注入 identity と実 euid を変えて判定が変わることを表明）——が担う。grep は
> defense-in-depth の二次チェックとして、代表的な書込／identity API を低コストに塞ぐ。
>
> **denylist の確定（このタスクで打ち切り）**: PR-7 のレビューで identity／環境／live 状態／生 syscall・プロセス生成／コマンド実行
> （`os/exec`）／live FS を触る path 解決（`filepath.Abs`/`EvalSymlinks`/`Glob`）まで網羅した時点で、本タスクの denylist は**十分**
> とみなし、これ以上の機械的拡張は行わない。今後の「API X も追加すべき」という同種の指摘は、(1) 既存カテゴリ内の追加 API は
> **挙動テストが本質的に捕捉する**ため denylist へは追加せず本原則を引いてクローズする、(2) 既存カテゴリに収まらない**新種の
> bypass クラス**（例えば判定が新たな ambient 入力に依存しうる経路）を示す指摘のみ、defect として個別に対応する、という方針で
> 一貫トリアージする。denylist の網羅性そのものを合格条件にはしない。
> **自己検証（正のコントロール）を必須にする**: ガードテストは、検査用の正規表現がコンパイルでき、かつ既知の悪例文字列
> （例 `"os.Geteuid()"`・`"os.Create(p)"`）に必ずマッチすることを表明する正のコントロールを含める。これにより将来の
...
- [x] **（PR-6）** `NewStandardEvaluator` の呼び出し箇所を全列挙し、新 `*security.Config` 引数を全て更新: `rg -n "NewStandardEvaluator\(" -g '*.go' internal`（本番＋テストの全呼び出しに引数追加。引数追加はコンパイルで検出されるが、テスト注入箇所の見落とし防止に列挙する）。
- [x] **（PR-1）** reason code 新 7 定数の二重定義・タイポ確認: `rg -n "trust_boundary_write|destination_zone|permission_grant|device_io|recursive_outside_safe_zone|sensitive_source_copy|unresolved_destination" -g '*.go' internal`（定義は `reason_codes.go` の 1 箇所のみ）。
- [x] **（PR-6）** `TrustedDirectories` の用語一貫性（設計・要件・spec・loader・config sample で同一表記。sample config 追従自体は 0143）: `rg -n "trusted_directories|TrustedDirectories" -g '*.go' internal`。
- [x] **（PR-4）** 設計 §3.4 で「不変」とした `security/coreutils_test.go` の `CoreutilsCommandRisk` テストが誤って変更されていないこと（関数挙動は不変、置き換えは評価層のみ）。
- [x] **（PR-5）** AC-16(i) の出所表明で 0141 名前下限由来の理由コードを参照する場合は、その定数が 0141 で確定済みかを事前確認: `rg -n "ReasonCode = \"" internal/runner/base/risktypes/reason_codes.go`（データ送信の名前下限に対応する定数名を特定。未確定なら自己完結の canary 形を採る）。
