...
>
ライブ identity ガード（AC-21 static）は `live_identity_guard_test.go::TestNoLiveIdentityInZoning` が **go/ast による呼出検査**として実行する。対象は分類器・仕様表・リゾルバの 3 ファイル（`destination_zoning.go`・`destination_zoning_spec.go`・`operand_path_resolver.go`）で、各ファイルを `go/parser` でパースし**すべての `*ast.SelectorExpr`（`pkg.fn` 参照）**を検査する（呼出に限らず値としての参照・引数渡し `fn := os.Geteuid` も捕捉。コメント/文字列リテラル・整形の差異・行跨ぎは AST ノードでないため構造的に無視）。セレクタのローカル識別子はファイルの import 宣言から**実 import パスへ解決**してから照合するため、別名 import（`import myos "os"`）でガードを迂回できない。禁止パッケージの dot import はそれ自体を違反として報告する（参照が無修飾になり選択子検査を迂回するため）。禁止集合は uid/euid/gid/egid/groups を `os`/`syscall`/`unix` の各パッケージで網羅し、`user.Current`／`user.Lookup*` の OS ユーザーDB 参照、`os`/`syscall`/`unix` の `Getenv`/`LookupEnv`/`Environ`・`os.ExpandEnv`／`os.TempDir`／`os.User{Home,Config,Cache}Dir` の環境参照、さらに `Getwd`／`Getpid`／`Getppid`／`Gettid` の live なプロセス状態（CWD・PID）参照を塞ぐ（AC-22 の「環境・live 状態に依存しない」を静的に担保）。コントロールは「実呼出＝検出、同名のコメント/文字列＝無視」を 1 ケースで表明し、空振り（fail-open）も誤検出（false positive）も同時に防ぐ。手動の二次チェックとしては概略 `rg -n 'os\.Get(euid|uid|gid|egid|groups)|user\.(Current|Lookup)|(syscall|unix)\.Get(euid|uid|gid|egid|groups)|os\.(Getenv|LookupEnv|Environ|ExpandEnv|UserHomeDir|UserConfigDir|UserCacheDir)|(syscall|unix)\.(Environ|Getenv)' <上記 3 ファイル>` が使えるが、これは raw テキスト走査ゆえコメント/文字列に誤反応しうる近似であり、権威は AST テストが持つ。
>
