...
>
> grep ガード（AC-21 static）の具体コマンド: `rg -n '(os\.Get(euid|uid|gid|egid|groups)|user\.(Current|Lookup\w*)|(syscall|unix)\.Get(euid|uid|gid|egid|groups)|os\.(Getenv|LookupEnv|UserHomeDir|Environ|ExpandEnv|UserConfigDir|UserCacheDir)|(syscall|unix)\.(Environ|Getenv))\s*\(' internal/runner/base/security/destination_zoning.go internal/runner/base/security/destination_zoning_spec.go internal/runner/base/security/operand_path_resolver.go` の期待結果は **マッチ 0 件**（uid/euid/gid/egid/groups を `os`/`syscall`/`unix` の各パッケージで網羅し、`user.Current`／`user.Lookup*` の OS ユーザーDB 参照に加え、`os`/`syscall`/`unix` の `Getenv`/`LookupEnv`/`Environ`・`os.ExpandEnv`／`os.User{Home,Config,Cache}Dir` の環境/$HOME 参照も塞ぐ。AC-22 の「$HOME に依存しない」を静的に担保する。末尾 `\s*\(` で**呼出**のみを検出し、コメント/文字列中の言及は誤検出しない）。`live_identity_guard_test.go` がこの検査をテストとして実行する（対象は分類器・仕様表・リゾルバの 3 ファイル、判定はファイル全体走査で行跨ぎの呼出も検出、正/負のコントロール付き）。
>
