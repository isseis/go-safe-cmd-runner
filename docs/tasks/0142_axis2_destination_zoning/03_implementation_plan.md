...
>
> grep ガード（AC-21 static）の具体コマンド: `rg -n 'os\.Get(euid|uid|gid|egid|groups)|user\.(Current|Lookup)|syscall\.Get(euid|uid|gid|egid|groups)|unix\.Get(euid|uid|gid|egid|groups)|os\.(Getenv|LookupEnv|UserHomeDir)' internal/runner/base/security/destination_zoning.go internal/runner/base/security/operand_path_resolver.go` の期待結果は **マッチ 0 件**（uid/euid/gid/egid/groups を `os`/`syscall`/`unix` の各パッケージで網羅し、`user.Current`／`user.Lookup*` の OS ユーザーDB 参照に加え、`os.Getenv`/`LookupEnv`/`UserHomeDir` の環境/$HOME 参照も塞ぐ。AC-22 の「$HOME に依存しない」を静的に担保する）。`live_identity_guard_test.go` がこの検査をテストとして実行する（判定はファイル全体走査で行い、行跨ぎの呼出も検出する）。
>
