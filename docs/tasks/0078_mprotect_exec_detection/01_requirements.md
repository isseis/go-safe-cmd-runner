...
フィールド名は `MprotectExecStatus`（仮称）とし、値は `"exec_confirmed"` /
`"exec_unknown"` / `"exec_not_set"` のいずれか、または `mprotect` syscall が
検出されなかった場合は空文字列とする。
最終的なフィールド名はアーキテクチャ設計（02）で確定する。
...
- **リスク判定への組み込み方**（第5節の選択肢 A〜D から決定する）
  - **決定タイミング**: アーキテクチャ設計（02）の着手前に確定すること。
    フィールド設計・スキーマ設計の両方に影響するため、未確定のままアーキテクチャ設計を
    進めると設計が揺れるリスクがある。
- Go バイナリで `mprotect(PROT_EXEC)` が検出された場合に、Go ランタイムによる良性の利用（例：goroutine スタック管理）と悪性の利用（例：`dlopen`）をどのように区別するか（偽陽性リスク）
