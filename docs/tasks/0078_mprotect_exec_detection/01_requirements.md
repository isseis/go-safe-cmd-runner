...
| `SyscallArgEvalResult` | 特定の syscall に対する引数値の静的評価結果を表す構造体。syscall 名・評価ステータス・詳細を保持する |
...
現状この定数のコメントは「syscall 番号取得のための後方スキャン上限」と記述されているが、
`prot` 引数スキャンにも同じ上限を適用するため、コメントを「syscall 命令からの後方スキャン上限（syscall 番号・引数いずれにも適用）」に一般化すること。
別定数は導入しない。
...
`SyscallArgEvalResult` は以下のフィールドを持つ（型は JSON 表現）：

| フィールド | JSON 型 | 内容 |
|---|---|---|
| `SyscallName` | `string` | syscall 名（例: `"mprotect"`） |
| `Status` | `string` | 評価ステータス。取りうる値は `"exec_confirmed"` / `"exec_unknown"` / `"exec_not_set"` の3値のみ |
| `Details` | `string` | 補足情報（省略可）。即値確定時は取得した `prot` 値の16進数表記（例: `"prot=0x5"`）、引数不確定時はその理由（例: `"control flow instruction encountered"`、`"scan limit exceeded"`、`"indirect register setting"`）を格納する |

Go 実装では `Status` フィールドを型付き文字列定数（`type SyscallArgEvalStatus string`）として定義し、任意文字列が代入されないようにすること。
...
#### FR-3.2.2: `Summary.IsHighRisk` フィールドへの反映

`ArgEvalResults` の内容をもとに、`SyscallAnalysisResultCore.Summary.IsHighRisk` フィールドを更新すること。
マッピングルールは第5節で定義する。`Summary.HasNetworkSyscalls` フィールドは影響を受けないこと。
...
`mprotect(PROT_EXEC)` の検出結果（`ArgEvalResults`）を `SyscallAnalysisResultCore.Summary.IsHighRisk` フィールドに反映する。
`ArgEvalResults` は解析事実のみを保持し、リスク判定ロジックは解析フェーズ（`analyzeSyscallsInCode`）で完結させる。
判定基準の分散を防ぐため、`ArgEvalResults` から `Summary.IsHighRisk` へのマッピングは
`elfanalyzer` パッケージ内のヘルパー関数（`EvalMprotectRisk`）として一元化すること。

**配置場所の根拠**: `security` パッケージは既に `elfanalyzer` を import しているため、
`security` に置くと `elfanalyzer → security → elfanalyzer` の循環依存が発生する。
`EvalMprotectRisk` の唯一の呼び出し元は `analyzeSyscallsInCode`（`elfanalyzer` パッケージ内）であるため、
パッケージ境界を越える必要がない。
...
**注1:** `exec_unknown` を `Summary.IsHighRisk = true` とする理由は、引数が静的に確定できない場合に動的コードロードの可能性を排除できないためである。ただし Go バイナリ等で false positive が多発することが実バイナリ調査で判明した場合は、`exec_unknown` を `Summary.IsHighRisk` に反映しない方向に変更することを検討する（未解決事項 8 参照）。

既存の `Summary.IsHighRisk = true`（ネットワーク syscall 等の他要因による）は上書きしないこと。
`Summary.IsHighRisk` は一度 `true` になったら `false` に戻さない（OR 条件）。
