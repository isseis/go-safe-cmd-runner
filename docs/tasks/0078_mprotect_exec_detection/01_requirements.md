...
複数の `mprotect` syscall が検出された場合の集約ルール（全件記録するか、最悪ケースに集約するか等）はアーキテクチャ設計（02）で確定する。本要件定義書では集約方針を規定しない。
...
`mprotect(PROT_EXEC)` の検出結果（`ArgEvalResults`）をリスク判定にどう組み込むかは
本タスクのスコープ外とし、将来タスクで決定する。
...
リスク判定値へのマッピングは `security` パッケージ内の共通ヘルパー関数として一元化すること。
...
- [ ] 複数の `mprotect` syscall が検出された場合の `ArgEvalResults` への集約ルールがアーキテクチャ設計（02）で定義され、その仕様に従って動作すること

## 8. 未解決事項

- `mprotect` 未検出のバイナリを low risk とみなして良いか（Go ランタイムが内部で使う可能性）
- 複数の `mprotect` syscall が検出された場合の `ArgEvalResults` への集約ルール → アーキテクチャ設計（02）で確定する（FR-3.2.1 参照）
