...
これらを実行の失敗として監査すると、実際には走っていない実行の `exit_code` を記録することになり、監査の意味が壊れる。配線は子プロセスが開始した失敗だけを対象にし、その区別は値の型・フラグで宣言する。エラー文字列の内容では判定しない（CLAUDE.md「Declare, don't infer」）。

## 目的

- run-as コマンドが開始して非ゼロ終了した場合に、`LogUserGroupExecution` の失敗レコードを監査ログへ書き、`user_group_command_failure` 通知を送る。
...
3. 子プロセスが開始したことの判別を、既存の `ExitCodeUnknown` を含む値の型・フラグで宣言的に行う。文字列の内容による推測を新たに足さない。
...
- **`Result` の構造と `ExitCodeUnknown` の意味の変更。** 既存の表現をそのまま判別に使う。
- **タイムアウト・kill の扱いの変更。** 開始後の失敗は本タスクの監査対象になり得るが、終了ステータスが得られない場合の扱いは既存の `ExitCodeUnknown` の意味に従い、新しい状態を導入しない。
- **新たな通知種別・監査属性の追加。**
...
監査するかどうかを、`Result` の有無と `ExitCode` の意味で決める。`exit_code` が確定している（`ExitCodeUnknown` でない）ことを開始したことの目印にする。少なくとも次を区別する。

- `prepareCommand`・開始失敗など、子プロセスが動いていない失敗は `ExitCodeUnknown` を返す。
- `superviseCommand` を通った失敗は `ProcessState` から確定した `ExitCode` を返す。

区別をエラー文字列の `strings.Contains` などで行わない。判別の根拠が型・値にあり、将来 `Result` に状態が増えても `switch` の `default` が安全側へ倒れる形にする。

### 監査は実行 1 回につき 1 件、失敗と成功で排他

`executeWithUserGroup` の 1 回の実行につき、監査レコードはちょうど 1 件とする。成功（exit 0）は INFO の成功レコード、開始後の非ゼロ終了は ERROR の失敗レコードとし、両方を書かない。`LogUserGroupExecution` が `ExitCode` で分岐する既存の実装をそのまま使う。
...
**Acceptance Criteria**:
- **AC-01**: run-as コマンドが開始して非ゼロで終了したとき、監査ログに `audit_type=user_group_execution` かつ `exit_code` がその終了コードである ERROR レコードが 1 件書かれる。
...
**Acceptance Criteria**:
- **AC-06**: `executeWithUserGroup` を通した失敗のテストが存在し、失敗経路の `LogUserGroupExecution` 呼び出しを取り除くと失敗する。
- **AC-07**: 開始しなかった失敗のテストが存在し、走っていないコマンドまで無条件に監査する実装へ変えると失敗する。
