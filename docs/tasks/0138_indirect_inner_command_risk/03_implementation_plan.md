...
- [x] PR-1 マージ済み（対象ステップ: Phase 1）— コア（`role` 伝播＋`role` 分岐）＋ AC-08 コメント 6 箇所 ＋ `indirect_execution_test.go`（新規 3・アサーション更新 3・コメント修正 2・削除 1）＋ `evaluator_test.go` 削除 1 ＋ 完了ゲート緑（AC-01〜08）（[#743](https://github.com/isseis/go-safe-cmd-runner/pull/743)）
