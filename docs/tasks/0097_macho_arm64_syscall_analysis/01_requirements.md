...
実装し、`svc #0x80` 検出時は `true, true`（ネットワーク検出・高リスク確定）を返す。
...
`NewNetworkAnalyzer()` や `NewNetworkAnalyzerWithStore(nil)` のような互換経路を残す場合、
その nil ストア時の挙動は本タスクの対象外とし、別途後方互換性を維持してよい。
