...
> **[訂正 - タスク 0104]** 上記の前提は誤りであることが判明した。Go ランタイムは macOS arm64 においても syscall ディスパッチスタブ内で `svc #0x80` を直接発行するため、正規の Go バイナリ（本システムの `record` コマンドを含む）の `__TEXT,__text` セクションにも `svc #0x80` が含まれる。タスク 0097 の「`svc #0x80` の存在 = 高リスク確定」方針は Go バイナリ全般に対して偽陽性を生じさせる。タスク 0104 にて X16 レジスタ後方スキャンによる syscall 番号解析と、ネットワーク関連 syscall のみを高リスクとする方針への変更を行う。
...
> **[訂正 - タスク 0104]** 上記の §1.2 全体の前提は誤りであることが判明した。Go ランタイムは macOS arm64 においても `svc #0x80` を `__TEXT,__text` セクション内で直接発行する。正規の Go バイナリの `__TEXT,__text` セクションに `svc #0x80` が含まれることは Go の正規動作であり、異常なシグナルではない。タスク 0104 要件定義書を参照。
...
- `SymbolAnalysis = NetworkDetected` の Mach-O バイナリでも `svc #0x80` 検出時に `isHighRisk` を格上げし、リスクレベルを正確に反映する
...
| 用語 | 定義 |
|------|------|
| `svc #0x80` | arm64 macOS における直接 syscall 命令（エンコード `0xD4001001`）。正規バイナリは `libSystem.dylib` 経由で呼ぶため通常は現れない（**[訂正 - タスク 0104]**: Go ランタイムは正規バイナリにおいても `svc #0x80` を直接発行する）|
...
`record` の責務はバイナリの状態を忠実に記録することであり、`SyscallAnalysis` をどのように利用するかの判断は `runner` が行う（FR-3.3.2 参照）。
...
`runner` の `isNetworkViaBinaryAnalysis` は `SymbolAnalysis` の結果に関わらず、
常に `SyscallAnalysis` キャッシュを参照すること。

- `SymbolAnalysis = NoNetworkSymbols` の場合: `SyscallAnalysis` のシグナルが
  最終的な `isNetwork` および `isHighRisk` の両方を決定する。
- `SymbolAnalysis = NetworkDetected` の場合: `SyscallAnalysis` で `svc #0x80` が
  検出された場合は `isHighRisk` を `true` に格上げする（リスクレベルの上昇）。
  `svc #0x80` が検出されなかった場合は既存の `NetworkDetected` 判定を採用する。

`record` 側でのスキャン有無ではなく、`runner` 側の参照ロジックで制御することで責任境界を明確にする。

### FR-3.3: `runner` 実行時の SyscallAnalysis キャッシュ利用（FR-4.4 / FR-4.5 対応）

#### FR-3.3.1: SyscallAnalysis の一括参照

`runner` の `isNetworkViaBinaryAnalysis` において、`SymbolAnalysis` ロード成功後、
`SymbolAnalysis` の結果に関わらず `SyscallAnalysis` キャッシュを参照すること。

`svc #0x80` シグナルの確認を SymbolAnalysis 結果判定の前段に置くことで、
SymbolAnalysis キャッシュヒット時の検出迂回（§1.1 問題 2）を解消するとともに、
`NetworkDetected` バイナリにおけるリスクレベル格上げも実現する。

#### FR-3.3.2: SyscallAnalysis からの判定ロジック

`SyscallAnalysis` の参照結果により以下の処理を行うこと：

| SyscallAnalysis の状態 | 処理 |
|----------------------|------|
| `svc #0x80` 検出記録あり（`DetectedSyscalls` に `direct_svc_0x80` あり） | `true, true` を返す（`SymbolAnalysis` の結果に関わらず高リスク確定） |
| `SyscallAnalysis` が nil（`ErrNoSyscallAnalysis`：スキャン実施済み・未検出） | `SymbolAnalysis` の結果に基づいて判定を続ける |
| ロード成功かつ svc シグナルなし | `SymbolAnalysis` の結果に基づいて判定を続ける |

`svc #0x80` シグナルなし時の `SymbolAnalysis` 結果による最終判定：

| SymbolAnalysis の状態 | 判定 |
|---------------------|------|
| `NetworkDetected` | `true, false` を返す（`handleAnalysisOutput` に委譲） |
| `NoNetworkSymbols` | `false, false` を返す |
...
- [ ] `SyscallAnalysis` に `svc #0x80` 記録がある場合、`SymbolAnalysis` の結果に関わらず `runner` が `true, true`（ネットワーク検出・高リスク確定）を返すこと
- [ ] `SymbolAnalysis = NetworkDetected` かつ `svc #0x80` 記録がある場合に `true, true` を返すこと（`NetworkDetected` 単独時の `true, false` からのリスク格上げ）
...
- [ ] `SymbolAnalysis = NetworkDetected` かつ `SyscallAnalysis` が `nil`（`ErrNoSyscallAnalysis`）の場合、既存の `NetworkDetected` 判定（`true, false`）を返すこと
...
- [ ] `SymbolAnalysis = NetworkDetected` かつ `svc #0x80` なしの Mach-O バイナリの判定が変わらないこと
...
| SymbolAnalysis=NetworkDetected + SyscallAnalysis に svc #0x80 あり | `true, true`（isHighRisk が true に格上げされること） |
| SymbolAnalysis=NetworkDetected + SyscallAnalysis が nil（ErrNoSyscallAnalysis） | 既存の NetworkDetected 判定（`true, false`）が返されること |
...
| テストケース | 検証内容 |
|-------------|---------|
| `svc #0x80` あり・`SymbolAnalysis=NoNetworkSymbols` → `record` → `runner` | `runner` がキャッシュを利用して `true, true`（isNetwork=true, isHighRisk=true）を返すこと（live 解析なし） |
| `svc #0x80` あり・`SymbolAnalysis=NetworkDetected` → `record` → `runner` | `runner` がキャッシュを利用して `true, true`（isHighRisk 格上げ）を返すこと |
