...
`isNetworkViaBinaryAnalysis` のすべてのエラーケースを直接 return とし、live 解析フォールバックを持たない。
...
`isNetworkViaBinaryAnalysis` 内の live 解析コード（`a.binaryAnalyzer.AnalyzeNetworkSymbols()`）を
完全に削除する。production では store は常に設定済みであり、ファイルは必ず `record` 済みのため、
live 解析コードは dead code となる。すべてのケースで直接 return し、フォールバックパスを持たない。
...
- [ ] `isNetworkViaBinaryAnalysis` 内で live 解析（`AnalyzeNetworkSymbols()`）が呼ばれないこと
