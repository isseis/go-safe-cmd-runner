...
SyscallAnalysis の保存は `SymbolAnalysis = NoNetworkSymbols` の場合にのみ実行すること。
`SymbolAnalysis = NetworkDetected` の場合は svc スキャンをスキップする。
`runner` は `SymbolAnalysis = NoNetworkSymbols` の場合にのみ `SyscallAnalysis` を参照するため、
NetworkDetected バイナリに svc 結果を保存しても実用上のセキュリティ効果がない。
...
#### FR-3.2.2: SymbolAnalysis = NetworkDetected 時のスキップ

`SymbolAnalysis = NetworkDetected` の場合、svc スキャンを実行しない。
`runner` は `SymbolAnalysis = NoNetworkSymbols` の場合にのみ `SyscallAnalysis` キャッシュを参照するため、
NetworkDetected バイナリに svc 結果を保存しても実行フロー上の追加セキュリティ効果はない。
