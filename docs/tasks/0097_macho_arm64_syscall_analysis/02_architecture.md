...
- **Security First**: `svc #0x80` 検出時は番号解析の有無によらず常に `true, true`（ネットワーク検出・高リスク確定）を返す。`ErrHashMismatch` などの解析失敗時は安全側フェイルセーフとして `AnalysisError`（リスク不明・高リスク扱い）を返す
...
flowchart TB
    ...
    subgraph RunnerPhase["runner フェーズ（変更あり）"]
        ...
        BLOCK["true, true を返す<br>（ネットワーク検出・高リスク確定）"]
    end
...
`SyscallAnalysis` からの判定は以下を満たす場合に `true, true`（ネットワーク検出・高リスク確定）を返す。
これは解析が成功した上でネットワーク syscall を検出したケースであり、解析失敗による `AnalysisError`（フェイルセーフ）とは意味合いが異なる：
