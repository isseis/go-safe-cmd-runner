...
- 要件定義書へ対応する AC を追加することを推奨する（§7.3 の AC 追加提案とあわせて扱う）。
...
AC-15 の文言自体も、上記に合わせて要件定義書を修正することを推奨する。
...
**要件定義書への AC 追加を推奨する。** 要件定義書は AC-01〜AC-24 を定義するが、ドキュメント整合の AC を持たない（前例の 0156 における AC-11 に相当するものがない）。`requirements_process.md` は AC を `01_requirements.md` に置き、各 AC に検証手段を対応づけることを求めており、`03_implementation_plan.md` が AC を新設することはできない。したがって次を提案する。

- `01_requirements.md` に AC-25 を追加する（静的検証: 上記の構造体引用が `security-architecture.md` / `.ja.md` に存在しない）。
- あわせて §5.4 の挙動変化に対する AC も追加する。
- 要件定義書の再承認を経てから `03_implementation_plan.md` を作成する。
...
| （AC 未定義） | §7.3 のドキュメント整合、§5.4 の挙動変化。いずれも AC の新設を要件定義書へ提案する |
