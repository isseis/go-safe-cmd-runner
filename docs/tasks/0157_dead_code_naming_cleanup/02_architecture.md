...
- **`runner` の native root 実行サポートの是非。** `runner` の想定運用は root 所有 + setuid ビットのバイナリを一般ユーザーが起動する形であり、sudo 経由の実行も root での直接起動も想定していない。しかし `isPrivilegeExecutionSupported` は native root 実行（実 UID・実効 UID がともに 0）を Case 1 として明示的にサポートしており、`escalatePrivileges` と `restorePrivileges` にも `m.originalUID == 0` の早期リターンがある。想定運用を setuid のみに絞れば、この 3 分岐と `originalUID == 0` の特別扱いが privilege パッケージから消え、`getPermissionCheckUID` の sudo 分岐も `runner` からは構造的に到達しなくなる。ただしこれは cron / systemd から root で起動する構成を切ることを意味し、リファクタではなく運用方針の決定である。先に Issue [#920](https://github.com/isseis/go-safe-cmd-runner/issues/920) の「判定主体の明示指定」を済ませてから検討する（[#921](https://github.com/isseis/go-safe-cmd-runner/issues/921)）。なお Phase 2 で導入する識別情報変更 syscall のガードテスト（§7.2）は、`escalatePrivileges` / `restorePrivileges` を変更する際に許可リストの更新を要する。

- **group-writable なファイルの権限判定に残る passwd 依存。** §5.4 のとおり、`IsUserInGroup` と `isUserOnlyGroupMember` は `user.LookupId` を呼ぶため、passwd エントリを引けない環境では group-writable なファイルの判定が fail-closed で拒否される。この拒否自体は妥当であり（誰がグループに属するか分からないまま許可することになるため）、0151 が意図して導入したものでもある。ただし読み取り経路については、判定対象が自プロセスである場合に限り `os.Getgid()` と `os.Getgroups()` でデータベースを引かずに所属を判定できる余地がある。採用するなら、(a) sudo 分岐は `SUDO_UID`（自プロセスとは別ユーザー）を渡し得るため適用できないこと、(b) カーネルが保持するグループ集合はログイン・exec 時点のスナップショットでありデータベースの現在の設定と食い違い得ること、の 2 点をどう扱うかを決める必要がある。これは `safefileio` 経路の権限モデルに関わる判断であり、要件定義書が `getPermissionCheckUID` の sudo 分岐（D1 M-3）を委ねているのと同じ別タスクで扱うのが適切である（[#920](https://github.com/isseis/go-safe-cmd-runner/issues/920)）。
