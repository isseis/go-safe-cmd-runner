...
- 同一値フラグの綴り違い多重指定に関する正規キー併合の残差（Phase 3 で判明・対処）: §3.1 は別名を 1 つの正規キーに併合する方針
  （綴りごとの多重登録はしない）を採る。現行 `scanFlags` は綴りごとに別キーへ捕捉し、抽出器によっては綴り優先順位
  （例 `firstNonEmpty(captured["-C"], captured["--directory"], …)`）で 1 つを選ぶ。両方の綴りを同一コマンドに与える非現実的入力
  （例 `tar --directory=/d -C /e …`）では、正規キー併合（argv 順）と綴り優先順位が食い違いうる。**影響と対処を 2 分する**:
  - **tar（書込パスが変わる＝fail-open 方向）**: tar は dir/archive を `firstNonEmpty` で 1 つに畳むため、綴りの取り違えや
    取りこぼしが書込先パスを変え、過小分類になりうる。これは許容せず**忠実に修正**した。`-f`/`--file`・`-C`/`--directory` を
    （別名だが）別エントリで宣言して綴り優先順位を保存し、`--one-top-level` は現行同様に生 argv から読む。差分テストに当該入力を追加。
  - **cp/mv/ln/install/curl（順序のみ・パス不変）**: これらは綴り違いの値を**すべて**オペランドに積む（畳まない）ため、
    オペランド集合と最大 Level は不変で、`Operands` のスライス順序（と `Index`）のみが綴り違い多重指定時に変わりうる。これは
    §3.1 が許容する正規キー併合の残差であり、実使用では起こらない（同一フラグを 2 綴りで重複指定しない）。安全性影響はないため
    **そのまま許容**し、本残差を記録する（差分コーパスは cp/curl 等の綴り違い多重指定を生成しない）。
