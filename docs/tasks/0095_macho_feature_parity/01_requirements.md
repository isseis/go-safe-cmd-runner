...
- **対象外**: x86_64 macOS をランタイム（runner 実行環境）とするケース（タスク 0073 のスコープに準拠）。ただし Fat バイナリ内の x86_64 スライスはクロスアーキテクチャ・セキュリティバイパス対策として解析対象に含める
...
- **Fat バイナリ**: 全スライスを解析し、最も深刻な結果を採用する（クロスアーキテクチャ・セキュリティバイパス対策。タスク 0073 の方針に準拠）
...
- **`LC_RPATH` 非対応ケース**: 未サポートのパストークンを検出した場合は `AnalysisWarnings` に警告を追加し、当該ライブラリをハッシュ検証不能（high risk 扱い）として `record` を継続する（FR-4.1 の fail-safe 方針に準拠）
...
- **Fat バイナリ**: 全スライスの解析結果を統合して単一の Record に保存する（スライス単位のキャッシュ構造は不要）
...
- Mach-O 固有の挙動（Load Command 解析、`svc #0x80` 検出等）は macOS 環境でのみ動作するため、各実装サブタスクのテストは macOS 上で実行できる必要がある
- CI 環境への組み込みは各実装サブタスクで対応する（本タスクの検討対象外）
...
| Fat バイナリのキャッシュ粒度 | 全スライスの解析結果を統合して単一の Record に保存する。スライス単位のキャッシュ構造（arch → Result のマップ等）は導入しない |
