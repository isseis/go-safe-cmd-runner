...
| AC-2 | キャッシュミス経路（初回記録）とキャッシュヒット経路（同一インタプリタを使う 2 回目以降の記録）で、書き込まれる `fileanalysis.Record`（`DynLibDeps` / `SymbolAnalysis` / `SyscallAnalysis` / `AnalysisWarnings` / `ShebangChain`）の内容が完全に一致する |
