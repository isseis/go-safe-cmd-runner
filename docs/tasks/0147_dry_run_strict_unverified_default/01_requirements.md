...
| Reviewer | isseis |
| Comments | 2026-07-16: 設計レビューで判明した論点を受け、`verify_files` の検証失敗を終了コードへ反映する要件（F-005 / AC-20〜AC-24）を追加。env ファイル検証は production の呼び出し元が無く AC-21 として防御的に定義。スコープ拡張のため再承認が必要 |
...
本書の AC は AC-01〜AC-24（AC-20〜AC-24 は F-005 で追加）。
