...
- [x] `FailureReason` に `IsTamperingSignal() bool` メソッドを追加する。
      `ReasonHashMismatch` の場合のみ `true` を返す（`02_architecture.md` §3.1.2 のシグネチャ）。
- [x] `UnverifiedFileUsage` に `IsTamperingSignal() bool` メソッドを追加する。
...
- [x] `TestFailureReason_IsTamperingSignal` を追加し、`FailureReason` の 5 値すべてに対し
      戻り値を検証する（`ReasonHashMismatch` のみ `true`、他 4 値は `false`）。
- [x] `TestUnverifiedFileUsage_IsTamperingSignal` を追加し、以下を検証する。
...
- [x] ビルドタグは付与しない（`manager_test.go` の慣習に合わせる。§1.3 参照）。
...
- [x] `hasTamperingSignal`（499-506 行目）の内部判定 `u.Failure != nil` を
...
- [x] `hasFailureTamperingSignal(failures []verification.FileVerificationFailure) bool`
...
- [x] `previewExitCodeLocked`（451-475 行目）を `02_architecture.md` §3.3 の優先順位で
      書き換える。
...
- [x] 判定は `d.fileVerification.UsedUnverifiedContent` フラグを条件に含めない（入口条件が
      `UnverifiedFiles`/`Failures` の長さそのものに変わるため）。
- [x] `d.fileVerification == nil` の場合は 1・4・5 のみで判定する契約を維持する
      （`02_architecture.md` §3.3「ファイル検証サマリが不在の場合の契約」、既存の防御的分岐）。
- [x] `PreviewExitCode`（422-441 行目）と `previewExitCodeLocked`（447-450 行目）の
...
- [x] `failOnVerificationUnavailable bool` フィールド（51 行目）を削除する。
- [x] `NewDryRunResourceManagerWithOutput` 内の代入
      `failOnVerificationUnavailable: opts.FailOnVerificationUnavailable,`（120 行目）を削除する。
- [x] `fileVerification` フィールドの doc comment（79-87 行目）から
...
- [x] `FailOnVerificationUnavailable bool` フィールドと doc comment（91-114 行目）を削除する。
- [x] `DryRunExitVerificationUnavailable` の doc comment（125-137 行目）から
      `FailOnVerificationUnavailable` への言及を除去し、§1.4 の優先順位（環境起因の
      未検証成果物・検証失敗、または検証不能による拒否のいずれかで、かつ改ざん兆候を
      伴わない場合に返る）へ書き換える。
- [x] `DryRunResult.PreviewExitCode` の doc comment（201-214 行目）も同様に書き換える。
...
- [x] `dryRunFailUnverified bool` 変数宣言（47 行目）を削除する。
- [x] `flag.BoolVar(&dryRunFailUnverified, "dry-run-fail-unverified", ...)`（78 行目）を
      削除する。
- [x] `DryRunOptions{...}` 初期化内の `FailOnVerificationUnavailable: dryRunFailUnverified,`
      （413 行目）を削除する。
...
- [x] `TestDryRun_AnalysisUnavailableDenyPreview`（129-148 行目）の期待値を
...
- [x] `TestDryRun_VerificationUnavailableExitCode`（150-186 行目）のテーブルから
...
- [x] `TestHasTamperingSignal`（238-270 行目）のテーブルを是正する。
...
- [x] `TestDryRun_UnverifiedContentExitCode`（272-359 行目、`unverifiedSummaryHashMismatch`
  ...
  - `"tampering signal unverified, flag on"` ケースも同様の理由で削除する。
    `unverifiedSummaryHashMismatch("/etc/app/cfg.toml", "config")` を使うサマリ・
    `RiskLevelLow` の assessment・`DryRunExitPolicyDeny` の期待値のいずれも
    `"tampering signal unverified"`（旧 `flag off`）と完全に一致するため重複になる。
  ...
  - `"mixed unverified, flag on, tampering dominates"` は `opts` から
    ...
    `unverifiedSummaryHashMismatch("/etc/app/tmpl.toml", "template")` を「改ざん」側の
    要素として使うように修正した上で、`FailOnVerificationUnavailable: true` を除去する。
    期待値 `DryRunExitPolicyDeny` は維持する。
- [x] `TestDryRun_SetFileVerificationNilClears`（361-376 行目）の
      `opts := &DryRunOptions{DetailLevel: DetailLevelDetailed, FailOnVerificationUnavailable:
...
- [x] `failuresOnlySummaryFromReason(path, context string, reason verification.FailureReason)
      *verification.FileVerificationSummary` ヘルパーを追加する。`Failures` に単一要素を持ち
      `UnverifiedFiles` は空、`UsedUnverifiedContent: false` のサマリを返す
      （既存の `unverifiedSummaryHashMismatch` と対になるヘルパー）。
- [x] `TestDryRun_UnverifiedContentExitCode` のテーブルへ以下のケースを追加する。
...
**対象ステップ**: 2-1 / 2-2 / 2-3 / 3-1 / 3-2 / 3-3 / 3-4 / 3-5、および weakreview による
グリーンゲート是正のため前倒しした 5-1 / 5-2 / 5-3 / 5-4 / 5-5 / 5-6 / 5-7 / 5-8
（詳細は本 PR 作成ポイントの「グリーンゲート」チェック項目を参照）
...
- [x] グリーンゲート（`_context.md` の "Green gate" 参照）がパスしていることを確認した
  - **weakreview で検出・解消済み**: 当初 `go test -tags test ./cmd/runner/...` が 13 件失敗して
    いた（`TestDryRunE2E_AllSuccess` 等）。`origin/main` では同テストが通ることを確認済みで
    あり、原因は本 PR による終了コード既定値の変更（未検証成果物の常時 hard fail 化）。
    Phase 5（PR-4、Step 5-1〜5-8）を本 PR へ前倒しし、以下を実施して解消した。
    - `newGoRunCmdWithHashDir`（`testutil_ldflags_test.go`）を新設し、既存の `newGoRunCmd` を
      その薄いラッパーへ変更（Step 5-1）。あわせて実装方式を `go run` 経由から「バイナリを
      ビルドして直接実行」へ変更した。`go run` は子プロセスが非ゼロ終了した場合、実際の
      終了コードを `stderr` へ `exit status N` として出力するのみで、呼び出し元プロセスの
      終了コードは常に `1` に丸めてしまうため（Go ツールチェインの既知の挙動）、`exit 3`
      と `exit 1` を区別するテストが `go run` 経由では原理的に成立しないことが判明した
      （E2E テストの設計時に想定されていなかった問題）。
    - `runDryRunCommand` から `require.NoError` を除去（Step 5-2）。
    - `TestDryRunE2E_HashDirectoryNotFound` を削除（Step 5-3、AC-19）。
    - `TestDryRunE2E_HashFilesNotFound` / `TestDryRunE2E_MixedResults` /
      `TestDryRunE2E_NoSideEffects` の期待終了コードを `DryRunExitVerificationUnavailable`
      へ更新（Step 5-4、5-7、5-8）。
    - `TestDryRunE2E_AllSuccess` / `TestDryRunE2E_JSONOutput` は `config.toml` と
      `/bin/echo` の両方のハッシュを事前記録し、`exit 0` を維持（Step 5-5、5-6。要件書
      §1「前提・依存」が言及する対象ファイル数はコマンド実行ファイル自体の検証も含めて
      実測 2 件だった）。
    - 上記に加え、**本計画書の投資調査（§1.3）が捕捉していなかった** 2 ファイルも同じ原因で
      壊れていたため是正した（`dry_run_integration_test.go` の 5 テスト、
      `integration_timeout_test.go` の `TestDryRun_TimeoutResolutionContext`、
      `integration_dryrun_json_test.go` の 2 テスト）。いずれも `testdata` 設定ファイルまたは
      一時設定ファイルのハッシュ（および使用する実行ファイルのハッシュ）を事前記録する形で
      是正した。`integration_timeout_test.go` は元々 `//go:build test` タグを持たず
      `newGoRunCmdWithHashDir` を参照できなかったため、タグを追加した
      （`make test` は常に `-tags test` 付きで実行されるため既存の呼び出し経路には影響しない）。
    - Step 5-9・5-10（AC-01・AC-20 の新規 E2E、PR-5 予定分）は本 PR へ前倒ししていない
      （既存テストの赤字解消がスコープであり、新規カバレッジの追加は対象外のため）。
  - 上記のとおり `go test -tags test ./...` は本 PR の変更に起因する失敗を含まない。
    `test/security` パッケージの `TestPrivilegeEscalationAttack/Tmp_directory_write_(should_succeed)`
    のみ `origin/main` でも再現するサンドボックス環境起因の既存の flaky failure であり
    本タスクと無関係（`git worktree` で `origin/main` を直接検証し確認済み）。
- [x] PR を作成した
- [x] PR がマージされた
- [x] 次のブランチへ切り替えた（次ステップは新しいブランチで作業する）
...
- [x] 関数直上の doc comment（133-139 行目）に「詳細レベルによらず常に表示する」旨を追記する。
...
- [x] `TestTextFormatter_FormatResult_WithFileVerification` のサブテスト
...
- [x] `TestTextFormatter_WriteFileVerification_UnverifiedHiddenAtSummaryLevel`
...
- [x] `TestTextFormatter_WriteFileVerification_SummaryLevelEmptySectionsHidden` を新規追加する。
...
### Phase 5: F-001 / F-004 / F-005 — E2E テストの是正（既実施）

**実施状況（weakreview による前倒し）**: 当初 PR-4 として計画されていた Step 5-1〜5-8 は、
PR-2 の変更がグリーンゲートを壊すこと（`go test -tags test ./cmd/runner/...` が 13 件失敗）が
weakreview で判明したため、PR-2 へ前倒しして実施した。以下の各ステップの `- [x]` は
実施済みの内容を示す。
...
- [x] `newGoRunCmdWithHashDir(t *testing.T, hashDir string, appArgs ...string) *exec.Cmd`
      を新設する。現行の `newGoRunCmd` の本体（`hashDirLDFlags(hashDir)` の組み立てから
      `exec.Command` の生成まで）をこの関数へ移し、`hashDir` を引数として受け取る形にする。
- [x] **計画からの逸脱**: 実装方式を `go run .` 経由から「`go build` でバイナリを一度
      ビルドし、それを直接実行する」方式へ変更した。`go run` は子プロセスが非ゼロ終了した
      場合、実際の終了コードを `stderr` に `exit status N` として出力するのみで、
      呼び出し元プロセス自身の終了コードは常に `1` に丸めてしまう（Go ツールチェインの
      既知の挙動）。本計画は Step 5-4/5-7/5-8 で `exit 3` を検証することを要求しており、
      `go run` 経由ではこの区別が原理的に不可能なため、本ステップの範囲内で実装方式を
      修正した。
- [x] `newGoRunCmd` は `newGoRunCmdWithHashDir(t, tu.SafeTempDir(t), appArgs...)` を呼ぶ
...
- [x] `runDryRunCommand`（30-43 行目）から `require.NoError(t, err, "dry-run should
...
- [x] `TestDryRunE2E_HashDirectoryNotFound`（45-67 行目）を削除する（AC-19。
...
- [x] テスト内で `hashDir := tu.SafeTempDir(t)` を生成する。
- [x] `filevalidator.New(&filevalidator.SHA256{}, hashDir, filevalidator.ValidatorConfig{})`
      ...
      既存の使用パターンを踏襲する）。
- [x] `newGoRunCmdWithHashDir(t, hashDir, "-config", configFile, "-dry-run", "-dry-run-detail",
      "summary", "-dry-run-format", "text")` でコマンドを構築する。
- [x] 出力に `"Verified: 2"` および `"Failed: 0"` が含まれること、
      `cmd.ProcessState.ExitCode()` が `resource.DryRunExitAllow`（= 0）であることを検証する
      （AC-18・AC-05）。実測の結果、対象コマンド（`/bin/echo`）自体の識別も
      `uncertain_unverified_identity` として検証不能 deny の対象になるため、`config.toml` に
      加えて `/bin/echo` のハッシュも事前記録する必要があった（`SaveRecord` を 2 回呼ぶ）。
      これは要件書の「Verified: 2」の想定と一致する。
...
- [x] Step 5-5 と同じ要領で `hashDir` を生成し、`configFile` のハッシュを事前記録した上で
...
- [x] `require.NoError(t, err, "dry-run should succeed even with verification failures")`
      （188 行目）を削除する。
- [x] コメント「Verify exit code is 0 (dry-run never fails)」（200 行目）を削除し、
...
#### Step 5-11（計画外・weakreview で追加）: 本計画書が未捕捉だった追加の壊れたテストを是正する

**対象ファイル**: `cmd/runner/dry_run_integration_test.go`、
`cmd/runner/integration_timeout_test.go`、`cmd/runner/integration_dryrun_json_test.go`

- [x] `dry_run_integration_test.go` の `buildRunnerBinary` に `configPath` を渡し、
      `testdata/dry_run_debug_test.toml` と（PATH 解決した）`echo` バイナリのハッシュを
      事前記録するよう修正（`TestDryRunJSONOutput_WithDebugInfo` 等 5 テスト）。
- [x] `integration_timeout_test.go` の `TestDryRun_TimeoutResolutionContext` を
      `newGoRunCmdWithHashDir` 経由に変更し、一時設定ファイルと `/bin/sleep` のハッシュを
      事前記録。同ファイルは元々 `//go:build test` タグを持たず `newGoRunCmdWithHashDir`
      を参照できなかったため、タグを追加した（`make test` は常に `-tags test` 付きで
      実行されるため既存の呼び出し経路への影響はない）。
- [x] `integration_dryrun_json_test.go` の `TestDryRun_JSON_OutputStructure` /
      `TestDryRun_JSON_TimeoutResolutionContext` を同様に是正。

**背景**: 本計画書 §1.3 の既存コード調査はこれら 3 ファイルを Phase 5 是正対象として
捕捉していなかった（`newGoRunCmd`/`newGoRunCmdWithHashDir` を参照する全ファイルの
横断調査が §1.3 時点で不足していた）。weakreview の `make test` 実行により発覚し、
本 PR 内で是正した。

**成功基準**: `go test -tags test ./cmd/runner/...` が全件通る。

### PR-4 作成ポイント: E2E test helper and existing-test realignment

**実施状況**: 本 PR-4 の対象ステップ（5-1〜5-8、および計画外だった 5-11）は、PR-2 の
グリーンゲートを満たすため PR-2 へ前倒しして実施済み。本 PR-4 は独立した作業としては
不要になったため、以降のフェーズ（Phase 6）は PR-4 のマージを待たずに着手してよい。
§6「実装チェックリスト」の PR-4 行はこの前倒しを反映して更新する。

**対象ステップ**: 5-1 / 5-2 / 5-3 / 5-4 / 5-5 / 5-6 / 5-7 / 5-8 / 5-11（すべて PR-2 で実施済み）
...
- [x] グリーンゲート（`_context.md` の "Green gate" 参照）がパスしていることを確認した（PR-2 側で確認済み）
- [x] PR を作成した（PR-2 に統合済み。独立した PR は作成しない）
- [x] PR がマージされた（PR-2 のマージをもって完了とする）
- [x] 次のブランチへ切り替えた（次ステップは新しいブランチで作業する）
...
- [x] `TestDryRunE2E_RemovedFlagRejected` を新規追加する。
...
- [x] `TestDryRunE2E_VerifyFilesHashMismatch` を新規追加する。
...
### PR-7 作成ポイント（PR-5/6 統合）: E2E coverage and documentation updates

**対象ステップ**: 5-9 / 5-10 / 6-1 / 6-2 / 6-3

**推奨タイトル**: `test,docs(0147): E2E coverage and documentation for hard-fail default`

**作成済み**: GitHub PR #847

**レビュー観点（PR-5相当）**: AC-01 の新規 E2E が実際に「未定義フラグ」エラーメッセージ・非ゼロ終了を検証しているか / AC-20 の新規 E2E が `config.toml` 自体は正しく検証させたうえで `verify_files` のみの改ざんを再現できているか / PR-4 で導入した `newGoRunCmdWithHashDir` を正しく再利用しているか

**レビュー観点（PR-6相当）**: 終了コード表（0/1/3）の説明が §1.4 の優先順位と 1 行ずつ一致しているか / 破壊的変更の移行方法がフラグ指定あり・なし双方のケースを網羅しているか / 日英版の章構成・記述内容が対応しているか（AC-17） / 用語集の新規用語（改ざん兆候・未検証成果物）が正しく追加されているか

**実装モデル要件**: frontier-required + standard

**判定理由**: E2E テスト追加（frontier-required）とドキュメント更新（standard）を統合。元々独立可能な 2 つの PR（PR-5・PR-6）を 1 つに統合したもの。

- [x] グリーンゲート（`_context.md` の "Green gate" 参照）がパスしていることを確認した
  - **weakreview で検出・解消済み**: `TestDryRunE2E_VerifyFilesHashMismatch`
    （Step 5-10）が `echo` バイナリのハッシュ記録先として、テスト作成時のサンドボックス
    環境固有のパス `/usr/lib/cargo/bin/coreutils/echo` をハードコードしており、
    このリポジトリの本来の実行環境（macOS・通常の Linux・本プロジェクトの CI）では
    `lstat /usr/lib/cargo: no such file or directory` で確実に失敗することが
    weakreview で判明した。同ファイル内の他の 4 箇所（`TestDryRunE2E_AllSuccess` /
    `TestDryRunE2E_JSONOutput`）が用いる `/bin/echo` の慣習に合わせて修正した。
  - **CI で再発・再修正**: 上記修正後も CI（本プロジェクトのセルフホストランナー）で
    `TestDryRunE2E_VerifyFilesHashMismatch` が同じ `lstat /usr/lib/cargo: no such file
    or directory` で失敗した。原因は `/bin/echo` 自体がそのランナー上で
    `/usr/lib/cargo/bin/coreutils/echo`（Rust 製 coreutils の実体）への symlink であり、
    その実体が存在しなかったこと。`/bin/echo` という慣習自体がこの環境では
    ポータブルではなかった。`/bin/true`（このランナーでは `/bin` 配下のローカルな
    実バイナリ `gnutrue` に解決される。標準的な環境では通常の GNU coreutils の実体）に
    差し替え、`/usr/lib/cargo` のような外部パスへ依存しないようにした。
- [x] PR を作成した（#847）
- [x] PR がマージされた
- [x] 次のブランチへ切り替えた（次ステップは新しいブランチで作業する）
...
- [x] `#### \`-dry-run-fail-unverified\``（650 行目）節を削除し、同じ位置（§3.2
...
- [x] 節内のコマンド例（旧 680, 703 行目付近）から `-dry-run-fail-unverified` を除去する。
- [x] 追記した終了コード表の `0`/`1`/`3` の説明を、`02_architecture.md` §1.4 の分類表・
...
- [x] Step 6-1 と同一の変更を日本語版へ適用する（`#### \`-dry-run-fail-unverified\``
      節、650 行目、を同じ位置の新節へ置き換える）。章構成・記述内容が英語版と対応するよう
      維持する（AC-17）。
- [x] Step 6-1 と同様に、追記した終了コード表の説明を `02_architecture.md` §1.4 と
...
※ PR-6 作成ポイントは PR-7（PR-5/6統合）に統合されました。
...
| M5 | AC-01・AC-20 の新規 E2E カバレッジと、ユーザードキュメント・用語集が新しい挙動と整合する | PR-7（PR-5/6統合） |
...
| PR-7（PR-5/6統合） | 5-9 / 5-10 / 6-1 / 6-2 / 6-3 | AC-01・AC-20 の新規 E2E テスト、およびユーザードキュメント（日英）・用語集の更新 | frontier-required + standard |
...
- [x] PR-2 マージ済み（対象ステップ: 2-1 / 2-2 / 2-3 / 3-1 / 3-2 / 3-3 / 3-4 / 3-5、および
      weakreview によるグリーンゲート是正のため前倒しした 5-1 〜 5-8 / 5-11。作成・グリーン
      ゲート確認済み、マージ済み）
- [x] PR-3 マージ済み（対象ステップ: 4-1 / 4-2）
- [x] PR-4（対象ステップ: 5-1 〜 5-8 / 5-11）は PR-2 へ前倒し済みのため、独立した PR としては
      作成しない
- [x] PR-7（PR-5/6統合） 作成済み #847（対象ステップ: 5-9 / 5-10 / 6-1 / 6-2 / 6-3。レビュー・マージ待ち）
- [x] 全 PR で `make fmt && make test && make lint` がグリーン
- [x] 本書「7. 受け入れ基準の検証」の全 AC（AC-01〜AC-28、AC-21 を除く）が検証済み
...
- [x] `docs/tasks/0146_security_hardening/02_architecture.md` (3.4.3) への参照のうち、
...
- [x] `hasTamperingSignal` の doc comment に残る「Hash mismatch, hash file not found,
