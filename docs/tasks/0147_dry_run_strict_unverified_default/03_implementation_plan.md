...
**推奨タイトル**: `feat(0147): add IsTamperingSignal predicate and delegate display logic`

**レビュー観点**: 述語の判定基準が要件書 F-002 の分類表（`hash_mismatch` のみ改ざん兆候）と
一致しているか / 表示側の出力が既存テストどおり回帰していないか（AC-14）
...
- [ ] `TestDryRun_UnverifiedContentExitCode`（272-359 行目、`unverifiedSummaryFromFailureReason`
  ...
  - `"environment cause unverified, flag on"` / `"policy deny dominates unverified
    tampering"` の 2 ケースは `opts` から `FailOnVerificationUnavailable: true` を除去する
    （期待値はフラグ有無に依存しなくなったため変更不要）。
  - `"tampering signal unverified, flag on"` も同様に `FailOnVerificationUnavailable: true`
    を除去する（期待値は `DryRunExitPolicyDeny` のまま変更不要）。
...
### PR-2 作成ポイント: exit-code priority rewrite and flag removal

**対象ステップ**: 2-1 / 2-2 / 2-3 / 3-1 / 3-2 / 3-3 / 3-4 / 3-5

**推奨タイトル**: `feat(0147): remove --dry-run-fail-unverified and hard-fail on verification failures by default`

**レビュー観点**: `previewExitCodeLocked` の優先順位が要件書 §1.4 と一致しているか / フラグの
痕跡（変数・フィールド・フラグ定義・doc comment）が残っていないか / F-005 の `Failures` 判定が
発生源を問わず一般則として適用されているか

- [ ] グリーンゲートがパスしていることを確認した
- [ ] PR を作成した
- [ ] PR がマージされた
- [ ] 次のブランチへ切り替えた
...
### PR-3 作成ポイント: detail-level-independent evidence display

**対象ステップ**: 4-1 / 4-2

**推奨タイトル**: `feat(0147): show Failures/UNVERIFIED sections regardless of detail level`

**レビュー観点**: `summary` レベルで根拠が表示されること（AC-25/AC-26） / 正常系の `summary`
出力が引き続き簡潔であること（AC-27） / `detailed`/`full` の既存出力が回帰していないこと
（AC-14/AC-24）
...
### PR-4 作成ポイント: E2E test corrections and new coverage

**対象ステップ**: 5-1 〜 5-10

**推奨タイトル**: `test(0147): correct dry-run E2E exit-code expectations and add flag/verify_files coverage`

**レビュー観点**: 各 E2E テストの期待終了コードが分類根拠（環境起因 vs 改ざん兆候）と一致
しているか / `TestDryRunE2E_AllSuccess`・`TestDryRunE2E_JSONOutput` が実際にハッシュ検証
成功を再現しているか（テスト名を偽っていないか） / 新規テストが AC-01・AC-20 を過不足なく
検証しているか
...
### PR-5 作成ポイント: user documentation and glossary updates

**対象ステップ**: 6-1 / 6-2 / 6-3

**推奨タイトル**: `docs(0147): document flag removal, verify_files exit codes, and summary-level evidence`

**レビュー観点**: 破壊的変更と移行方法が明確に記載されているか（AC-16） / 日英の章構成・
記述内容が対応しているか（AC-17） / 用語集の新規用語が本文の記述と整合しているか
...
| M4 | E2E テストが新しい挙動を正しく検証し、AC-01・AC-20 のカバレッジが揃う | PR-4 |
| M5 | ユーザードキュメント・用語集が新しい挙動と整合する | PR-5 |

Phase 1（M1）を先に完了させることで、表示側の出力が不変であることを確認したうえで
終了コード側の判定を寄せられる（`02_architecture.md` §8）。Phase 2/3（M2）は同一 PR 内で
完結させる（フラグ削除と判定書き換えを分離すると一時的な不整合が生じるため）。Phase 4〜6
（M3〜M5）は M2 の完了後に着手する（E2E テストの期待値是正は終了コード判定の変更を前提と
するため）。
...
- 本タスクは 5 PR（PR-1〜PR-5）に分割されており、各 PR は独立してグリーンゲートを
...
- [ ] PR-4 マージ済み（対象ステップ: 5-1 〜 5-10）
- [ ] PR-5 マージ済み（対象ステップ: 6-1 / 6-2 / 6-3）
