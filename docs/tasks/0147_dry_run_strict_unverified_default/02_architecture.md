...
| Review date | - |
| Reviewer | - |
| Comments | 設計レビューを受け、F-005（`verify_files` の検証失敗を終了コードへ反映。§5.5）と F-006（非ゼロ終了の根拠を詳細レベルによらず出力。§4.1 / §6.3）を要件へ追加し、本設計へ反映した。要件書は再承認待ち（`draft`）。残る未解決の論点はない（§10 参照） |
...
設計レビューで判明した 2 件の論点は、いずれも利用者の判断により本タスクのスコープへ
取り込んだ。これに伴い要件書はスコープ拡張のため再承認が必要な状態（`draft`）に戻っている。

- **`verify_files` の検証失敗の終了コード反映** → **F-005**（要件書 AC-20〜AC-24、本書 §5.5）。
  env ファイル検証は production の呼び出し元が無いため、AC-21 として防御的に分類を定義するに
  留めた。
