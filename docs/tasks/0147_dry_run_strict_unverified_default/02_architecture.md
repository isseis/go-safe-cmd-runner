...
ファイル検証の実行は一切変更せず、レポートに新しいフィールドやデータを追加することもない。
ただし例外として、F-006（§4.1 / §6.3）により既存の `Failures` / `UNVERIFIED` セクションの
**表示条件**（詳細レベルによる出し分け）を変更し、`summary` レベルでも表示されるようにする。
これは終了コードの根拠を追跡可能にするための表示条件の変更であり、レポートの内容（フィール
ドやデータ）そのものを変更するものではない。したがって設計は、
`DryRunResourceManager.previewExitCodeLocked` という 1 つの判定関数と、そこへ入力を渡す
経路の整理、および F-006 に伴うフォーマッタの表示条件変更に集約される。
...
2. 改ざん兆候（`hash_mismatch`）を含む未検証成果物・検証失敗 → `1`（AC-10、AC-13、AC-20、AC-23）
...
| `internal/runner/resource/formatter.go` | F-002, F-006 | `formatUnverifiedMarker` / `securityRiskForFailureReason` の判定を述語へ委譲（出力内容は不変）。`writeFileVerification` の `Failures` / `UNVERIFIED` セクションから `DetailLevel >= DetailLevelDetailed` の出力条件を除去し、スライスが空でなければ常に出力（§6.3） | `formatter_test.go`（F-002 は期待値不変。F-006 は下記のとおり是正） |
| `internal/runner/resource/formatter_test.go` | F-006 | `TestTextFormatter_WriteFileVerification` のサブテスト `"Summary level hides failures"` は旧挙動（summary で `Failures` を隠す）を前提とするため、summary でも表示される期待へ是正。正常系で空セクションが出ない期待（AC-27）を追加 | 本ファイル自体 |
...
### 4.1 終了コードの根拠の追跡可能性（F-006）

非ゼロ終了する場合、その根拠は `Failures` セクション（`verify_files` などの検証失敗）と
`UNVERIFIED` / `UNVERIFIED-TAMPER` セクション（未検証成果物）に、理由（`Reason:` 行）と
ファイルパスとともに示される。**F-006 により、これらのセクションは詳細レベル
（`-dry-run-detail`）によらず、要素が存在する限り常に出力する。** したがって
`-dry-run-detail summary` を指定しても、非ゼロ終了の根拠を標準出力から追跡できる。

これは常時 hard fail 化（F-001 / F-005）の帰結である。`Failures` または `UnverifiedFiles` に
要素が 1 件でもあれば dry-run は必ず非ゼロ終了するため、これらのセクションに載る情報は
すべて終了コードの根拠であり、詳細レベルで省略してよい補助情報ではない。この可視性ルールの
設計は §6.3 で詳述する。

リスクゲートによるポリシー拒否（exit 1）については、`summary` 出力の Summary セクションに
Security Risks の件数が従来から表示される。本タスクはこの表示を変更しない。F-006 が対象と
するのはファイル検証由来（`Failures` / 未検証成果物）の根拠表示である。
...
`Failures` 経路を使う設計だったが、production の呼び出し元が存在しない死んだコードであり
（テストのみが呼び出す）、本タスクの事前クリーンアップ（F-007、§5.8）として削除する。削除後は
`Failures` に env コンテキストのエントリを生成する production 経路がそもそも存在しないため、
「env コンテキストの `hash_mismatch` を個別に分類する」という AC は定義しない（要件書
「AC 番号について」のとおり AC-21 は意図的な欠番）。
...
`security_risk: high` 注釈をレポート本文で確認する必要がある。F-006 により、これらは
詳細レベルによらず出力されるため、`summary` 指定時でも確認できる（§4.1、§6.3）。
終了コード体系の再設計は要件書が明示的にスコープ外としているため、本タスクではこの多義性を
解消しない。
...
### 5.8 事前クリーンアップ：`VerifyEnvironmentFile` の削除（F-007）

**設計判断は「削除のみ」**: 本関数は `internal/verification/manager.go:92-124` に定義され、
production の呼び出し元を持たない（`-env-file` フラグ廃止後の残骸。要件書 F-007 参照）。
代替実装は設計しない。将来 env ファイル検証を復活させる場合は、`VerifyAndReadConfigFile` /
`VerifyAndReadTemplateFile` と同様に検証と読み込みを同一 I/O で行う新規関数として設計し直す
必要があり、本関数のシグネチャ（検証のみ・内容を返さない）を復元・再利用する対象ではない。

**変更内容**:
- `internal/verification/manager.go` から `VerifyEnvironmentFile`（92-124行目）を削除する。
- `internal/verification/manager_test.go` から `TestVerifyEnvironmentFile` を削除する。
- 削除に伴い不要となる import・ヘルパーがあれば併せて削除する（`make build` / `make lint` で
  検出）。

**本タスクの他の設計への影響**: §5.5（F-005）の分類ロジック（`hasFailureTamperingSignal` /
`previewExitCodeLocked`）はコンテキストラベルを分岐条件に使わない設計であるため、本削除は
その設計を変更しない。唯一の影響は、要件書で防御的に検討されていた AC-21（env コンテキストの
`hash_mismatch` を個別分類する AC）を、production から到達し得ないケースとして定義しない
判断に至ったことである（要件書「AC 番号について」、AC-21 は意図的な欠番）。
...
| AC-20 | §3.3（`hasFailureTamperingSignal`）、§5.5、§6.1（`Failures` の `hash_mismatch` → 1） |
| AC-22 | §1.4、§3.3（`Failures` の `hash_mismatch` 以外 → 3） |
| AC-23 | §1.4 優先順位 2、§3.3（和集合に対する存在判定） |
| AC-24 | §3.1.2（表示側は基準不変） |
| AC-25、AC-26 | §6.3（`Failures` / `UNVERIFIED` セクションの詳細レベル非依存表示） |
| AC-27 | §6.3（両セクションが空なら非表示。正常系 summary は不変） |
| AC-28 | §5.8（`VerifyEnvironmentFile` の削除）、§7.2 |

### 6.3 非ゼロ終了の根拠表示（F-006）

`TextFormatter.writeFileVerification` は現在、`Failures` と `UNVERIFIED` /
`UNVERIFIED-TAMPER` の各セクションを `DetailLevel >= DetailLevelDetailed` の場合にのみ
出力する。F-006 では、この詳細レベルによる出力条件を取り除き、**各セクションを対応する
スライスが空でない限り常に出力する**。
...
flowchart TD
    ...
    Start(["writeFileVerification"]) --> Stat["サマリ統計を出力<br>(Total / Verified / Failed)"]
    Stat --> HasF{"Failures が空でない?"}
    HasF -->|"Yes"| ShowF["Failures セクションを出力する"]
    HasF -->|"No"| SkipF["Failures セクションを出力しない"]
    ShowF --> HasU{"UnverifiedFiles が空でない?"}
    SkipF --> HasU
    HasU -->|"Yes"| ShowU["UNVERIFIED セクションを出力する"]
    HasU -->|"No"| SkipU["UNVERIFIED セクションを出力しない"]

    class Stat process
    class HasF,HasU,ShowF,ShowU enhanced

    subgraph Legend["凡例"]
        direction LR
        LP["既存（変更なし）の処理"]:::process
        LE["本タスクで変更する判定・処理"]:::enhanced
    end
```

矢印 A → B は「A の次に B を評価する（処理の流れ）」ことを表す。菱形は分岐条件を表す。

この設計は §4.1 のとおり、常時 hard fail 化の帰結として妥当である。空でないスライスは
必ず非ゼロ終了の根拠であるため、詳細レベルによる抑制は不要になった。正常系（両スライスとも
空）では従来どおり両セクションは現れず、`summary` の簡潔さは保たれる（AC-27）。各行の項目・
`security_risk` 注釈・マーカーは変更しないため、`detailed` / `full` の出力は回帰しない。

**対象は `TextFormatter` のみ**: `JSONFormatter` は詳細レベルによらず
`FileVerificationSummary` 全体を出力しており、変更不要である。
...
- **検証失敗由来の終了コード**（`internal/runner/resource/security_test.go`、追加、F-005）:
  ...
  env コンテキストの `hash_mismatch` を個別に検証する AC は定義しない（F-007 で
  `VerifyEnvironmentFile` を削除するため production から到達し得ない。AC-21 は意図的な
  欠番、§5.8 参照）。
- **事前クリーンアップの回帰確認**（F-007）: `VerifyEnvironmentFile` および
  `TestVerifyEnvironmentFile` の削除後、`make build` と `make test` が通ることを確認する
  （AC-28）。
- **表示の非回帰**（`internal/runner/resource/formatter_test.go`、既存）: `detailed` / `full`
  の期待値を変更せずに通ることをもって AC-14 を担保する。
- **詳細レベルによらない根拠表示**（`internal/runner/resource/formatter_test.go`、既存を更新、
  F-006）: `TestTextFormatter_WriteFileVerification` の `"Summary level hides failures"` を
  是正し、`summary` でも `Failures` の一覧が出力されることを検証する（AC-25）。同様に
  `UnverifiedFiles` を持つサマリを `summary` で整形し、`UNVERIFIED` セクションが出力される
  ことを検証する（AC-26）。両スライスが空のサマリを `summary` で整形し、どちらのセクションも
  現れないことを検証する（AC-27）。
...
| 4 | `writeFileVerification` の出力条件から詳細レベルの制約を除去し、`Failures` / `UNVERIFIED` セクションをスライスが空でなければ常に出力（§6.3）。formatter テストを是正 | F-006 |
| 5 | E2E テストの是正（ヘルパー修正、AC-18、AC-19、AC-01 の E2E 追加、AC-20 の `verify_files` 改ざん E2E 追加） | F-001, F-004, F-005 |
| 6 | ユーザードキュメントと用語集の更新（§5.4 の運用規則、`verify_files` の終了コード反映、`summary` でも根拠が出力される旨を含む） | F-003, F-005, F-006 |
...
設計レビューで判明した 3 件の論点は、いずれも利用者の判断により本タスクのスコープへ
取り込んだ。これに伴い要件書はスコープ拡張のため再承認が必要な状態（`draft`）に戻っている。

- **`verify_files` の検証失敗の終了コード反映** → **F-005**（要件書 AC-20、AC-22〜AC-24、
  本書 §5.5）。
- **`VerifyEnvironmentFile` の事前クリーンアップ** → **F-007**（要件書 AC-28、本書 §5.8）。
  F-005 の検討過程で、env ファイル検証（`VerifyEnvironmentFile`）が `-env-file` フラグ廃止後
  production の呼び出し元を持たない死んだコードであることが判明したため削除する。削除後は
  env コンテキストの `Failures` を生成する production 経路が存在しなくなるため、当初防御的に
  検討していた AC-21（env コンテキストの `hash_mismatch` の個別分類）は「production から
  到達し得ないケースを検証する AC」となり不要と判断し、定義しないこととした（AC-21 は
  意図的な欠番）。
- **`-dry-run-detail summary` での非ゼロ終了の根拠表示** → **F-006**（要件書 AC-25〜AC-27、
  本書 §4.1 / §6.3）。`Failures` / `UNVERIFIED` セクションを詳細レベルによらず出力する
  （表示内容は不変、可視性のみ変更）。
