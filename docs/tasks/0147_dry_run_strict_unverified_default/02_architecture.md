...
| Review date | - |
| Reviewer | - |
| Comments | 設計レビューで 2 点の要件レベルの論点が判明した（§10 参照）。承認前に判断が必要 |
...
| 検証失敗 | ファイル検証が失敗として記録された事象。`FileVerificationSummary.Failures` に記録される。未検証成果物とは記録先が異なり、両者は一致しない（§5.5 参照） |
...
3. **未検証成果物に由来する終了コードを表示と一致させる**。未検証成果物が理由で exit 1 と
   なるのは `UNVERIFIED-TAMPER` と表示される場合だけであり、その逆、すなわち
   `UNVERIFIED-TAMPER` と表示されれば必ず exit 1 となることも成り立つ。ただし
   exit 1 は未検証成果物以外の理由（リスクゲートによるポリシー拒否、および §5.6 に挙げる
   一般的な実行時エラー）でも返るため、「exit 1 ならば `UNVERIFIED-TAMPER` 表示がある」とは
   限らない。
...
下すコマンド単位の判定、もう 1 つはファイル検証が記録する未検証成果物である。本タスクは
後者の分類規則を変更し、両系統を終了コードへ対応づける写像から、フラグによる分岐を取り除く。
...
flowchart TD
    ...
    UF["未検証成果物<br>(UnverifiedFileUsage)"]
...
> 図の読み方: `改ざん兆候` と `環境起因` の分類規則、および 4 つの入力から
> `プレビュー終了コード` への写像が本タスクの変更対象である。リスクゲート自体の
> 判定ロジックは変更しない（要件書「スコープ外」）。`FileVerificationSummary` のうち
> 終了コードの判定材料になるのは `UnverifiedFiles` のみであり、`Failures` は含まれない。
> この非対称性は本タスクの重要な限界であり、§5.5 で詳述する。
...
複数の事象が同時に発生した場合の優先順位は以下のとおり（高い順）。

1. リスクゲートによるポリシー拒否 → `1`（AC-12。他の事象に影響されない）
2. 改ざん兆候（`hash_mismatch`）を含む未検証成果物 → `1`（AC-10、AC-13）
3. 環境起因のみの未検証成果物 → `3`（AC-07、AC-08、AC-09、AC-11）
4. リスクゲートによる検証不能による拒否 → `3`（AC-04）
5. 上記のいずれでもない → `0`（AC-05）

順位 1 と 2 はいずれも `1` を返すため両者の相対順序は結果に影響しないが、順位 2 が順位 3・4 より
上位であることが AC-13（改ざん兆候が環境起因のコードに埋没しない）を担保する。
...
sequenceDiagram
    ...
    V-->>R: FileVerificationSummary<br>(UnverifiedFiles を含む)
    R->>D: FinalizeDryRunResults(summary)
    D->>D: previewExitCodeLocked()
    Note over D: 改ざん兆候の有無を<br>IsTamperingSignal で判定
...
重複し、§3.1.1 で述べた乖離のリスクが半分残ってしまう。

述語の参照先は以下のとおり。

| 参照元 | 用途 | 使用する述語 |
|---|---|---|
| `resource/dryrun_manager.go` の `hasTamperingSignal` | 終了コード | `UnverifiedFileUsage.IsTamperingSignal` |
...
表示側の基準は変わらないため **出力は回帰しない**（AC-14）。参照先を述語へ差し替えるのみで
ある。
...
そのものは §3.1 の述語へ委譲し、本関数は `[]verification.UnverifiedFileUsage` を走査する
責務のみを持つ。

**ファイル検証サマリが不在（`nil`）の場合の契約**: `previewExitCodeLocked` は未検証成果物の
判定を飛ばし、リスクゲート由来の判定のみで終了コードを決める。これは現状の挙動であり、
本タスクでも維持する。dry-run の本番経路ではサマリが必ず存在する。
`createDryRunResourceManager` は `verificationManager` が `nil` なら初期化エラーを返し、
dry-run では `resultCollector` が必ず構成されるためである。したがって `nil` は防御的な
分岐であり、到達するのはテストからの直接呼び出しに限られる。この分岐が実運用で
フェイルオープンを生むことはない。
...
|---|---|---|---|
| `internal/verification/types.go` | F-002 | `FailureReason.IsTamperingSignal` と `UnverifiedFileUsage.IsTamperingSignal` を追加（§3.1.2） | なし（新規テストを追加） |
...
| `internal/runner/resource/dryrun_manager.go` | F-001, F-002 | `failOnVerificationUnavailable` フィールドと関連分岐を削除。`hasTamperingSignal` の基準を述語へ委譲。`PreviewExitCode` / `previewExitCodeLocked` の doc comment を更新 | `security_test.go` |
...
| `internal/runner/resource/security_test.go` | F-001, F-002 | 旧既定（exit 0）を期待する全ケースの期待値を更新（フラグ設定の有無を問わない）。`TestHasTamperingSignal` の分類期待値を是正（§7.1、§5.4） | 本ファイル自体 |
| `cmd/runner/integration_dryrun_verification_test.go` | F-001, F-004 | 共通ヘルパーを修正。`TestDryRunE2E_AllSuccess` を修正、`TestDryRunE2E_HashDirectoryNotFound` を削除、フラグ拒否の E2E を追加（§7.2、§7.3） | 本ファイル自体 |
...
**設定ファイルおよびテンプレートファイルの `hash_mismatch`（改ざん兆候）を含む dry-run が
CI を緑にすることがなくなる** 点にある。

この効果の範囲には重要な限界が 2 つある。いずれも §5.5・§5.6 で詳述する。

1. 終了コードに反映されるのは **未検証成果物（設定／テンプレート）だけ** であり、
   `verify_files` や env ファイルの検証失敗は反映されない（§5.5）。
2. exit 1 は改ざん兆候以外の理由でも返るため、終了コードだけでは改ざんを特定できない（§5.6）。
...
この図は、内容が採用される設定ファイル／テンプレートファイルの経路を示す。`verify_files` の
経路には当てはまらない（§5.5）。
...
### 5.5 終了コードに反映されない検証失敗（既知の限界）

**`verify_files`（`global.verify_files` / `groups[].verify_files`）および env ファイルの
検証失敗は、`hash_mismatch` であっても dry-run の終了コードに反映されず、exit 0 のままと
なる。** 本タスクはこの状態を変えない。

理由は記録経路の違いである。未検証成果物（`UnverifiedFiles`）として記録されるのは、
検証に失敗してもなお **内容を読んで採用する** 経路（設定ファイル・テンプレートファイル）
だけである。`VerifyGlobalFiles` / `VerifyGroupFiles` / `VerifyEnvironmentFile` は内容を
採用しないため、失敗を `Failures` にのみ記録し、dry-run ではエラーを握り潰す。
`previewExitCodeLocked` は `UsedUnverifiedContent` を入口条件とするため、`Failures` は
終了コードに影響しない。
...
flowchart LR
    ...
    class CFG,VF data
    class P1,P2,FL process
    class UF,EX enhanced
    class NO problem
    ...
    subgraph Legend["凡例"]
        ...
        LP["既存（変更なし）の構成要素"]:::process
        LE["本タスクで変更する構成要素"]:::enhanced
        LX["本タスクで解消しない限界"]:::problem
    end
```

矢印 A → B は「A が B へ流れる（データの経路）」ことを表す。

この限界があるため、本タスクは目的（`hash_mismatch` を CI で検知する）を部分的にしか
達成できない。`verify_files` は任意の成果物をハッシュで固定するための機構であり、改ざん検知の
主要な対象だからである。ただし要件書の F-002 は分類対象を `UnverifiedReason` に限定しており、
`Failures` を終了コードの判定材料へ加えることは要件レベルの変更であり、新たな受け入れ条件を
要する。本設計は承認済み要件の範囲を超えないため、**限界として記録し、後続タスクへ送る**。
§10 に論点として挙げる。

なお、この限界は本タスクによる後退ではない。現状も同じく exit 0 である。
...
flowchart TD
    ...
    class PD,UC,VU process
    class TS,E1,E3,E0 enhanced
...
現状との差分は 2 点のみである。第一に、`フラグ指定あり?` の分岐がすべて消える。第二に、
`改ざん兆候あり?` の基準が `Failure != nil` から `hash_mismatch` のみへ狭まる。

`未検証成果物を採用?` は、ファイル検証サマリが存在し、かつ `UsedUnverifiedContent` が真である
ことを指す。サマリが `nil` の場合は「No」の枝へ進む（§3.3）。
...
| 2 | 終了コード判定の書き換え（`hasTamperingSignal` の委譲、フラグ分岐の削除）とユニットテスト更新 | F-001, F-002 |
| 3 | `DryRunOptions` フィールド・`cmd/runner` フラグ・関連 doc comment の削除 | F-001 |
| 4 | E2E テストの是正（ヘルパー修正、AC-18、AC-19、AC-01 の E2E 追加） | F-001, F-004 |
| 5 | ユーザードキュメントと用語集の更新（§5.4 の運用規則を含む） | F-003 |
...
設計レビューで、承認済み要件の範囲を超えるため本設計では解消できない論点が 2 件判明した。
いずれも本タスクによる後退ではなく、既存の限界が常時有効化によって顕在化するものである。

- **論点 1: `verify_files` / env ファイルの `hash_mismatch` が終了コードに反映されない**
  （§5.5）。本タスクの目的である「改ざん兆候を CI で検知する」を部分的にしか満たさない。
  解消には `Failures` を終了コードの判定材料へ加える新たな受け入れ条件が必要である。
  本タスクへ追加するか、後続タスクとするかの判断を要する。
- **論点 2: `-dry-run-detail summary` では非ゼロ終了の根拠が出力されない**（§4.1）。
  解消には出力内容の変更が必要で、要件書がスコープ外としている。
