...
- [x] `internal/libccache/macos_syscall_table.go` を編集
...
- [x] macOS 環境で `make generate-macos-syscall-table` を実行（Linux ヘッダー不要、macOS 専用ターゲット）
...
- [x] `internal/libccache/macos_syscall_table_test.go` を編集
