...
- **実行全体の中断時に集めた失敗を報告すること。** SIGINT・SIGTERM で実行全体が中断されたときは、現状どおり中断のエラーをすぐに返し（決定事項「意図した挙動の変化」の 8 のとおり、常に `context.Canceled` を含む）、それまでに集めた失敗は報告しない。中断は利用者の操作であり、残りの結果が無いことを利用者が知っているためである。
...
- **先頭だけを保持する理由。** redaction の検出は、値の手前にある目印（`key=`、`Bearer `、`ghp_`、`BEGIN ... PRIVATE KEY` など）から値を見つける。残るテキストが出力の先頭から続く完全な行であれば、残った値の目印は必ず一緒に残り、行ごとの一致は変更前と変わらない。先頭と末尾を保持すると、省略の境目が値とその目印の間に入りうる。例えば目印が省略された側にあると、末尾に残った値の断片は目印を失い、検出に一致しない。また、空白や改行をまたいで一致する検出（`Bearer` と値の間、`ghp_` と値の間）は、境目で行を切り詰めても、目印を先頭に、値を末尾に分けて残しうる。先頭だけの保持では、このような境目は生じない。
...
8. **実行全体の中断の報告。** 中断された実行は、常に `errors.Is(err, context.Canceled)` が成り立つエラーを返し、終了コード 1 で終わり、`RUN_SUMMARY` 行は失敗を報告する（AC-21、AC-35、AC-36）。`Details:` には `context canceled` の行が加わる。変更前は、中断と重なった失敗の形によって次のようになっていた（`internal/runner/runner.go:405-465`）。
...
- **AC-36**: 最後の group のコマンドの子プロセスが中断のシグナルで先に終わり、`ExecuteGroup` のエラーが `context.Canceled` を含まないとき（例: 終了コード 130）、`executeGroups` の戻り値で `errors.Is(err, context.Canceled)` が成り立ち、`ExecuteGroup` のエラーにも `errors.Is`・`errors.AsType` で届く。
...
- 省略の境目で秘密鍵のブロックの前側だけが保持されても、その本文がログと通知に出ない。上限を超えた出力で、保持した値がその目印と切り離されない。
