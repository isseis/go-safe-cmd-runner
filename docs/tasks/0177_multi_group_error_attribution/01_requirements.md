...
| Review date | - |
| Reviewer | - |
| Comments | 2026-09-26: PR #1185 のレビューを受けて改訂した。編集上の修正は 1 件で、AC-28 が出力ファイルに全体が書かれると過大に述べていたのを、メモリ上の上限が出力ファイルに書かれる内容を減らさない、という主張に改めた（スコープ 11、決定事項「コマンドの出力はメモリ上で常に上限付きにする」、AC-31 もこれに合わせた。この修正は決定を変えない）。決定の変更は次の 3 件で、このため状態を `draft` に戻した。(1) コマンドのタイムアウトは `Error()`・`Details:` の文言に `failed to execute group <g>: ` が付くようになるので、AC-09・AC-11 と Success Criteria の「文言は変わらない」から除外した。(2) タイムアウト後に実行される group の通知を加え、AC-12 を実行エラーのレコードに限定し、AC-32 を追加した。(3) 複数行の原因の続きの行を group の行より深く字下げすることにし、AC-09 を 1 行の原因に限定し、AC-33 を追加した。 同日、コードレビューを受けて次を改訂した（決定の変更）。AC-22 を「タイムアウトが唯一の group の失敗のとき」に限り、先に別の group が失敗していれば外側の context は空になることを明記した。対象 9 と AC-26 に、負の値が設定の読み込みで拒否されることの記載を加えた。 2026-09-26: PR #1185 の再レビューを受けて次を改訂した（決定の変更を含む）。本タスクによる意図した挙動の変化を決定事項「意図した挙動の変化」の 1 つの一覧にまとめ、Success Criteria、対象外の「Slack 通知」、AC-09・AC-11・AC-12 はこの一覧を参照する形にした。AC-09 の「同じ文言」は、同じ原因の文言に変更前の組み立て方を適用したものとの比較であることを明記した。AC-29 を、省略の印と、保持した先頭・末尾の範囲の中の完全な行だけが残る（改行の無い範囲からは何も残らない）という規則に改めた。決定の変更として、一部だけ保持された秘密鍵の PEM ブロックを redaction で隠すことにし、対象 12、決定事項「一部だけ残った秘密鍵のブロックも隠す」、AC-34 を追加した。 2026-09-26: PR #1185 の 3 回目のレビューを受けて次を改訂した（決定の変更を含む）。(1) 決定の変更として、上限を超えた出力の保持を先頭と末尾から先頭だけ（完全な行と省略の印）に改め、対象 11、決定事項「コマンドの出力はメモリ上で常に上限付きにする」（先頭だけにする理由と、採らなかった案を追記）、AC-28、AC-29、意図した挙動の変化の 4 を改訂した。これに合わせて対象 12、決定事項「一部だけ残った秘密鍵のブロックも隠す」、AC-34 を `BEGIN` の側だけの規則に改め、`END` の側だけの規則を削除した。末尾の保持を対象外に加え、[#1186](https://github.com/isseis/go-safe-cmd-runner/issues/1186) で扱うことにした。(2) 決定の変更として、実行全体の中断では常に `context.Canceled` を含むエラーを返し終了コード 1 で終わることを、意図した挙動の変化の 8 として宣言し、AC-35・AC-36 を追加した。AC-12、Success Criteria、決定事項「実行全体の中断は context の状態で判定する」をこれに合わせた。(3) 編集上の修正として、AC-25 を、上限値が 0 以下のサイズ超過のエラーはパッケージの外から構築できない、という表現に改めた（決定は変えない）。(4) 編集上の修正として、意図した挙動の変化の 4 に、デバッグログと Slack の欄が変わらない条件（残った完全な行が切り詰めの位置以上の長さであること、redaction の後で比べること）を明記した。 2026-09-26: PR #1185 の 4 回目のレビューを受けて、次の記述を明確にした（決定は変えず、仕組みも加えない）。(1) 決定事項「コマンドの出力はメモリ上で常に上限付きにする」の「先頭だけを保持する理由」で、目印が値と一緒に残るという主張を、値の手前に目印を持つ検出に限った。値の後ろの区切り `@` を必要とする `urlCred` について、1 行の URL は全体が残るか全体が残らないこと、複数行にわたる不正な形の URL がパスワードと `@` の間で切れると隠されないことを既知の制限として記し、設計書 §3.7・§4.4 を参照するようにした。Success Criteria の対応する文も同じく限った。(2) 決定事項「実行全体の中断は context の状態で判定する」に「実行全体が中断された」の定義（`ExecuteGroup` がエラーを返した時点で実行全体の context が取り消されている、または取り消し済みのため group を開始しなかった）を加え、最後の group が nil を返した後の取り消しは成功のままであること、その理由（nil を返した group はすべてのコマンドを実行し終えている）を記した。意図した挙動の変化の 8、対象外「実行全体の中断時に集めた失敗を報告すること」、AC-21・AC-36 の文言をこの定義に合わせた（AC の意味は変えない）。 |
...
11. すべてのコマンドについて、メモリ上に保持する stdout・stderr を、出力ファイルの有無と `output_size_limit` によらず上限付き（先頭だけを保持し、それ以降を省略する）にする。出力ファイルが無いときの stderr の現状の保持（先頭と末尾を保持する）も、同じ先頭だけの保持に変える。メモリ上の上限は、出力ファイルに書かれる内容を減らさない（出力ファイルには、変更前と同じバイトが書かれる）。
12. redaction（`internal/redaction` の値の形の検出）で、秘密鍵の PEM ブロックの `BEGIN` の側だけが残ったテキストも隠す。対応する `END` の行が無い `BEGIN ... PRIVATE KEY` の行は、その行からテキストの末尾までを隠す。対象 11 の省略でブロックの後ろが失われる場合に効く。
...
- **実行全体の中断時に集めた失敗を報告すること。** SIGINT・SIGTERM で実行全体が中断されたとき（中断の定義は決定事項「実行全体の中断は context の状態で判定する」のとおり）は、現状どおり中断のエラーをすぐに返し（決定事項「意図した挙動の変化」の 8 のとおり、常に `context.Canceled` を含む）、それまでに集めた失敗は報告しない。中断は利用者の操作であり、残りの結果が無いことを利用者が知っているためである。
...
- **上限を超えた出力の末尾を保持すること。** 本タスクでは、上限を超えた出力は先頭だけを保持する（決定事項「コマンドの出力はメモリ上で常に上限付きにする」）。末尾も保持する仕組み（リングバッファなど）は、省略の境目をまたぐ redaction の扱いと合わせて [#1186](https://github.com/isseis/go-safe-cmd-runner/issues/1186) で扱う。
...
- 実行全体の context が取り消されていれば、中断のエラー（`context.Canceled`）を含むエラーをすぐに返し、残りの group を実行しない。現状は、中断と重なった失敗の形によっては中断のエラーを含まないエラーや nil が返っていた（決定事項「意図した挙動の変化」の 8）。
...
本書で「実行全体が中断された」とは、次のどちらかを指す。

- `ExecuteGroup` がエラーを返し、その時点で実行全体の context が取り消されている。
- 実行全体の context がすでに取り消されていたため、group を開始しなかった（`executeGroups` が各 group の前に行う判定: `internal/runner/runner.go:411-417`）。

最後の group の `ExecuteGroup` が nil を返した後に取り消しが届いた場合は、中断に当たらず、実行は成功（戻り値は nil、終了コード 0）のままである。group の各コマンドは順に実行され、最初のエラーで group の実行が止まる（`executeAllCommands`: `internal/runner/group_executor.go:253-293`）。コマンドの間に取り消しが届くと、次のコマンドは開始前の判定で失敗する（`internal/runner/base/executor/command_lifecycle.go:457-459`）。したがって、nil を返した group はすべてのコマンドを実行し終えており、中断で実行されなかった処理は無い。このため、`executeGroups` はすべての group を実行した後に取り消しを判定しない。
...
すべてのコマンドについて、メモリ上の stdout・stderr の保持を上限付きにする。上限を超えたときは先頭だけを保持し、その後ろに省略した量を示す印を置く。末尾は保持しない。出力ファイルが無いときの stderr の現状の保持（先頭と末尾を保持する）も、この形に変える。上限は出力ファイルの有無と `output_size_limit` によらない一定の値（stdout・stderr それぞれ先頭の 64 KiB）とする。これは、現状の出力ファイルが無いときの stderr の保持（先頭と末尾の 32 KiB ずつ）と同じメモリの量である。これにより、runner のメモリ使用量はコマンドの出力の大きさに比例しない。

保持した先頭は、完全な行だけに切り詰める。残るテキストは、元の出力の先頭から続く完全な行の並びになる。

- **先頭だけを保持する理由。** redaction の検出の多くは、値の手前にある目印（`key=`、`Bearer `、`ghp_`、`BEGIN ... PRIVATE KEY` など）から値を見つける。残るテキストが出力の先頭から続く完全な行であれば、値の手前に目印を持つ検出では、残った値の目印は必ず一緒に残り、行ごとの一致は変更前と変わらない。URL に埋め込んだ認証情報の検出（`urlCred`）は例外で、一致には値の後ろにある区切り（`@`）も必要である。現実の URL の認証情報は 1 行に収まるので、完全な行だけを残す規則により、`@` を含めて URL 全体が残るか、全体が残らないかのどちらかになる。複数行にわたる不正な形の URL で、パスワードと `@` の間で切れた場合は隠されない。これは既知の制限として受け入れる（設計書 §3.7、§4.4）。先頭と末尾を保持すると、省略の境目が値とその目印の間に入りうる。例えば目印が省略された側にあると、末尾に残った値の断片は目印を失い、検出に一致しない。また、空白や改行をまたいで一致する検出（`Bearer` と値の間、`ghp_` と値の間）は、境目で行を切り詰めても、目印を先頭に、値を末尾に分けて残しうる。先頭だけの保持では、このような境目は生じない。
- **採らなかった案。** 次の案は、境目をまたぐ redaction を完全には防げないか、上限の目的と両立しないので採らない。
  - 検出ごとに境目の規則を足す案（例: PEM の `END` の側だけが残るときも隠す）: 検出の形を 1 つずつ追う必要があり、漏れを型や構造で防げない。
  - 省略の印を空白だけにして、空白をまたぐ検出を印の上で一致させる案: 省略された範囲の内容によっては誤って一致し、目印が省略された値の断片は依然として隠れない。
  - 出力を流れのまま redaction してから保持する案: 行をまたぐ検出のために状態を持つ redaction が必要になり、変更の範囲が redaction の仕組み全体に及ぶ。
...
メモリ上の出力を上限付きにすると（対象 11）、秘密鍵の PEM ブロックが省略の境目をまたいだとき、`BEGIN` の行と本文の前半だけが保持され、`END` の行は失われうる。保持するのは先頭だけなので、`END` の行だけが残ることは無い。現状の値の形の検出は `BEGIN` の行と `END` の行の両方がそろったブロックだけを隠すので、`END` の行が失われると、残った本文の行（秘密鍵そのもの）が redaction を通り抜ける。このため、redaction の値の形の検出で、`BEGIN` の側だけが残ったブロックも隠す（対象 12、AC-34）。対応する `END` の行が無い `BEGIN` の行は、その行からテキストの末尾までを隠す。この範囲には省略の印も含まれるが、秘密鍵を出すより安全な側に倒す。
...
出力ファイルが無いときの stderr の既存の上限（先頭と末尾の保持）では、`END` の側だけが残る抜けも現状ある。本タスクでこの保持も先頭だけに変える（対象 11）ので、その抜けは生じなくなる。
...
目的に当たる変化（報告の各行への帰属の表示、原因の差し替えの廃止、`output_size_limit = 0` の修正、負の `output_size_limit` の拒否）のほかに、本タスクで利用者から観察できる挙動は次のとおり変わる。これ以外の観察できる挙動（`Error()` の文言、Slack 通知、終了コード、`RUN_SUMMARY` 行）は変えない。終了コードと `RUN_SUMMARY` 行が変わるのは、実行全体の中断（下の 8）だけである。設計上の細部を含む変更の一覧は、設計書の「変更前との互換」に置く。
...
4. **上限を超えた出力の保持。** メモリ上の stdout・stderr は上限付きになり、上限を超えたときは、保持した先頭の範囲の中の完全な行と、その後ろの省略の印だけが残る（AC-28、AC-29）。末尾は残らない。このため、上限を超えた出力について、デバッグログ、構造化ログ（`command_group_summary` の `output`、`Command failed` の `stderr`）、監査ログ、Slack の `command_group_summary` と `user_group_command_failure` の出力の欄の内容が変わる。
   - 失敗したコマンドの stderr の最後の行（多くの場合、失敗の理由）も残らない。出力ファイルがあっても、コマンドが失敗するとその一時ファイルは削除される（`internal/runner/resource/normal_manager.go:270-275`）ので、これらの行はどこにも記録されない。
   - デバッグログと Slack の出力の欄が変わらないのは、残った完全な行の長さが、その欄の切り詰めの位置以上あるときだけである。それより短いときは、欄の中に省略の印が現れる。保持した先頭の範囲に改行が無い出力は、完全な行が 1 つも残らないのでその一例であり、欄は省略の印から始まる。この比較は redaction の後のテキストに対して行う（AC-34 により、`BEGIN ... PRIVATE KEY` の行以降が隠れることがある）。
   - 出力ファイルが無いコマンドの stdout で上限を超えた部分は、どこにも残らない。
5. **タイムアウト後の group の通知。** コマンドのタイムアウトの後に実行される group が、それぞれの通知（実行前段の失敗の通知、`command_group_summary`）を行うので、Slack 通知が増える（AC-32）。
6. **一部だけ残った秘密鍵のブロックの隠し方。** 対応する `END` の行が無い `BEGIN ... PRIVATE KEY` の行を含むテキストは、redaction でその行からテキストの末尾までが隠される（AC-34）。`RedactText`・`SanitizeOutputForLogging` を通るすべての出力先（構造化ログ、監査ログ、Slack 通知）に及ぶ。
7. **失敗 1 件の外側の context。** 失敗が 1 group だけで原因が `*CommandExecutionError` でないとき、外側の context が付く（AC-15）。
8. **実行全体の中断の報告。** 中断された実行（決定事項「実行全体の中断は context の状態で判定する」の定義のとおり。最後の group が nil を返した後に届いた取り消しは中断に当たらず、成功のまま報告される）は、常に `errors.Is(err, context.Canceled)` が成り立つエラーを返し、終了コード 1 で終わり、`RUN_SUMMARY` 行は失敗を報告する（AC-21、AC-35、AC-36）。`Details:` には `context canceled` の行が加わる。変更前は、中断と重なった失敗の形によって次のようになっていた（`internal/runner/runner.go:405-465`）。
   - `ExecuteGroup` のエラーが `context.Canceled` を含むときは、そのエラーを返していた。
   - 子プロセスがシグナルで先に終わり、エラーが `context.Canceled` を含まないときは、失敗として集めて次の group の前の判定で `context canceled` だけを返していた。最後の group では、集めた失敗（`context.Canceled` を含まない）を返していた。
   - group ファイル検証の失敗は、通知して集めずに次へ進み、次の group の前の判定で `context canceled` を返していた。最後の group では、それまでに集めた失敗か nil を返していたので、中断された実行が終了コード 0 と成功の `RUN_SUMMARY` 行で報告されることがあった。本タスクの後は、この場合も終了コード 1 で失敗として報告される。
...
- **AC-12**: 実行エラーの構造化ログレコードは `slack_notify=false` のままであり、このレコードが Slack へ送られないことは変わらない。プロセスの終了コードと `RUN_SUMMARY` 行も、実行全体の中断（決定事項「意図した挙動の変化」の 8、AC-35、AC-36）を除いて変わらない。Slack 通知の件数と内容の変化は、決定事項「意図した挙動の変化」に挙げたもの（4〜6、AC-29・AC-32・AC-34）に限る。
...
- **AC-21**: 実行全体が中断されたとき（決定事項「実行全体の中断は context の状態で判定する」の定義のとおり。例: コマンドの実行中に SIGINT・SIGTERM を受けた）、`executeGroups` は残りの group を実行せずに返す（現状どおり）。この判定は実行全体の context の状態で行い、本番コードに、エラーが `context.Canceled`・`context.DeadlineExceeded` を含むかどうかで中断を判定する分岐がない。
...
- **AC-35**: 最後の group の group ファイル検証の失敗と実行全体の中断が重なったとき（検証の失敗が通知され、`ExecuteGroup` から戻った時点で実行全体の context が取り消されているとき）、`executeGroups` は nil でないエラーを返し、`errors.Is(err, context.Canceled)` が成り立つ。プロセスは終了コード 1 で終わる。検証の失敗は現状どおり通知され、集める対象には入らない。
- **AC-36**: 最後の group のコマンドの子プロセスが中断のシグナルで先に終わり、`ExecuteGroup` のエラーが `context.Canceled` を含まないとき（例: 終了コード 130。`ExecuteGroup` から戻った時点で実行全体の context は取り消されている）、`executeGroups` の戻り値で `errors.Is(err, context.Canceled)` が成り立ち、`ExecuteGroup` のエラーにも `errors.Is`・`errors.AsType` で届く。
...
- **AC-25**: 上限値が 0 以下のサイズ超過のエラーは、パッケージの外からは構築できない。サイズ超過のエラーを作る唯一の手段はパッケージの構築関数であり、構築関数は上限値が 0 以下の入力を拒否する。
...
- **AC-28**: すべてのコマンドについて、メモリ上に保持される stdout・stderr の大きさは、出力ファイルの有無と `output_size_limit`（0 を含む）によらず一定の上限（それぞれ先頭の 64 KiB と省略の印）を超えない。出力ファイルを指定したコマンドでは、メモリ上の上限は出力ファイルに書かれる内容を減らさない。出力ファイルには変更前と同じバイトが書かれる。
- **AC-29**: AC-28 の上限を超えたとき、保持される stdout・stderr は、保持した先頭の範囲の中の完全な行（出力の先頭からその範囲の最後の改行まで）と、その後ろに置く、省略があったことを示す印だけからなる。末尾は保持しない。先頭の範囲に改行が無いときは、省略の印だけが残る。行が途中までだけ保持されることはない。省略の印が示すバイト数は、保持しなかったすべてのバイト（先頭の範囲の最後の改行より後ろのバイトを含む）を含む。
...
- **AC-34**: 秘密鍵の PEM ブロックの前側だけが保持されたとき（`BEGIN ... PRIVATE KEY` の行が残り、対応する `END ... PRIVATE KEY` の行が残らないとき）、redaction を通った出力は、そのブロックの本文の行を含まない。対応する `END` の行が無い `BEGIN` の行は、その行からテキストの末尾までが隠される。この規則は `RedactText`・`SanitizeOutputForLogging` が適用されるすべての箇所に及ぶ。
...
- `Error()` の文言（同じ原因の文言に変更前の組み立て方を適用したものとの比較）、実行エラーのレコードを Slack へ送らないこと、終了コード（実行全体の中断を除く）は変わらない。観察できる挙動の変化は、決定事項「意図した挙動の変化」に挙げたものに限る。
- コマンドのタイムアウトで、先に失敗した group の報告が失われず、後続の group が実行される。実行全体の中断では、現状どおり残りの group を実行せず、中断された実行は常に失敗として報告される。
...
- 省略の境目で秘密鍵のブロックの前側だけが保持されても、その本文がログと通知に出ない。上限を超えた出力で、保持した値が、値の手前にあるその目印と切り離されない。
