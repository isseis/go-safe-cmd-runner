...
> **カーネル制約：ソース差し替えは `linkat` 自体を失敗させる（実装時判明）**: Linux カーネルは、`O_TMPFILE` で開かれたのではない通常のファイルが完全に unlink 済み（nlink が 0）になった後、`/proc/self/fd/<n>` 経由で新たな名前を与えること（再リンク）を許可しない（`may_linkat` の制約）。改ざんがない通常経路では、`linkat` はソースパスがまだ元の名前を保持している間（nlink ≥ 1）に実行されるため成功する。しかし、攻撃者が fd 取得後にソースパスを別 inode へ差し替える（unlink して新規作成する、または別ファイルを rename で上書きする）と、検証済み inode の最後の参照が失われ nlink が 0 になるため、その後の `linkat` は `ENOENT` で失敗する。これは実装上のバグではなくカーネルの意図的な制約であり、結果として「ソース差し替え」攻撃は `linkat` 自身によって検出され fail-closed（rename 未実行・エラー返却）となる。すなわち、この技法は「差し替え後も検証済み inode への到達に成功する」ことを保証するのではなく、「差し替えられた場合は必ず fail-closed で中断する」ことを保証する。AC-05 はこの帰結を踏まえて記述されている（[01_requirements.md](01_requirements.md) AC-05 参照）。
...
- **ソース差し替えによる `linkat` の `ENOENT`（カーネル制約）**: 上記コールアウトのとおり、ソースパスが fd 取得後に差し替えられ検証済み inode の nlink が 0 になった場合、`linkat` は `ENOENT` を返す。これは他の `linkat` 失敗（`EPERM`/`EEXIST` 等）と同じ fail-closed 経路（rename 未実行・エラー返却）で扱い、特別扱いしない。
...
- **F-002**: (a) 改ざんなしの移動が成功し内容が保たれ、移動先の inode が検証済み fd の inode と一致すること（AC-05, AC-06 正常系）。(b) 検証後にソースパスを別 inode へ差し替えると、検証済み inode の nlink が 0 になり `linkat` が `ENOENT` で失敗するため、rename が行われずエラーが返ること（fail-closed。カーネル制約により「差し替え後も検証済み inode への到達に成功する」ことは実現できない。AC-06、[3.2 節](#32-f-002-atomicmovefile-のソース同一性保証)参照）。(c) `linkat` 後の rename 失敗で一時リンクがリークしないこと（クリーンアップ）。(d) 一時名の `EEXIST` がエラーになること。
