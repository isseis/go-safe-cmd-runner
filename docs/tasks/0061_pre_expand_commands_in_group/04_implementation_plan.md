...
**受け入れ基準**:
- [x] `preExpandCommands` が適切なタイミングで呼ばれる
- [x] エラー時にグループ実行が中断される
- [x] エラーメッセージが適切
...
- [x] 展開済みコマンドが使用される
- [x] 重複した展開処理が削除される
- [x] コマンドレベル変数が検証時に利用可能
- [x] パス解決エラーのハンドリングが維持される
...
**受け入れ基準**:
- [x] コマンドレベル変数が検証時に使用される
- [x] 展開エラーが実行前に検出される
- [x] Dry-runモードが正常に動作する
- [x] Workdir解決エラーが事前に検出される
...
**テスト修正が必要な可能性のあるケース**:
- [x] コマンドパス展開エラーを期待するテスト (修正不要)
- [x] ログ出力内容を検証するテスト (修正不要)
- [x] エラーメッセージの文字列マッチングを使用するテスト (修正不要)
...
**受け入れ基準**:
- [x] `preExpandCommands` に適切なGoDocコメント
...
- [x] `preExpandCommands` 関数実装
- [x] `ExecuteGroup` 修正
- [x] `executeAllCommands` 修正
- [x] 基本単体テスト作成
- [x] 既存テスト実行・確認
