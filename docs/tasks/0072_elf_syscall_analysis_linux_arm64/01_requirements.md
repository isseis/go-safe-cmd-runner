...
|------|--------|-------------|
| 命令 | `syscall` (オペコード `0F 05`) | `svc #0` (エンコード `0x00000001`) |
...
```asm
mov  w8, #41       // MOVZ: 16ビット範囲(0-65535)の即値設定（多くの syscall 番号で使用）
svc  #0
```

```asm
mov  w8, #288      // accept4(288) なども MOVZ で設定可能
svc  #0
```
...
```asm
mov  x0, #41       // syscall 番号 (socket=41) を x0 に設定
bl   syscall.Syscall
```
...
| 用語 | 定義 |
|------|------|
| `svc #0` 命令 | arm64 Linux でカーネルに syscall を発行する命令（エンコード `0x00000001`） |
...
```asm
mov  w8, #41       // MOVZ: syscall 番号 41 (socket) を設定
svc  #0
```

`arm64asm` パッケージでは `mov w8, #41` は `MOVZ` 命令としてデコードされる。
...
- [ ] `movz w8, #41` のような即値設定命令を正しく検出できること
...
4. **`BLR` 命令**: `blr xN`（レジスタ間接呼び出し）は静的解析では追跡不可能なため、high risk の原因とはならず無視する（x86_64 の間接 `CALL` と同様の扱い）。
