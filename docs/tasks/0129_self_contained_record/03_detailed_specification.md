...
```go
// DepEntry represents a single resolved dynamic library dependency.
// path is the primary dedup key; same path with different hash is a fatal error.
...
// ShebangBinaryInfo represents one binary in a shebang chain.
...
1. `calculateHash` でコンテンツハッシュを算出
2. ELF dynlib analyzer で `DynLibDeps` を収集（`elfDynlibAnalyzer.Analyze(filePath)`）
...
注: `analyzeELFSyscalls` はリブックへの参照を受け取り libc エントリを探すため（`findLibcEntry(record.DynLibDeps)`）、手順 2 で `DynLibDeps` を設定してから呼ぶ必要がある。

返却型: `[]ShebangBinaryInfo`

**フィールドマッピング:**
| `shebang.Info` フィールド | `ShebangBinaryInfo` フィールド |
|---|---|
| `RawInterpreterPath` | 先頭エントリの `RawPath` |
| `InterpreterPath`（env 形式の env バイナリ） | 先頭エントリの `Path` |
| `CommandName` | 先頭エントリの `CommandName` |
| `ResolvedPath`（env 形式の実コマンド） | 2番目エントリの `Path` |

直接形式: `ShebangChain` に 1 エントリ。`RawPath` = `shebang.Info.RawInterpreterPath`。

env 形式: `ShebangChain` に 2 エントリ。1番目は env バイナリ（`RawPath`、`CommandName` を保持）、2番目は解決済みバイナリ。
...
```
if isKnownVDSO(dep.SOName) || binaryanalyzer.IsSyscallWrapperLibrary(dep.SOName):
...
analyzeBinarySignals(cmdPath, contentHash):
  ...
  7. followShebangChain(record.ShebangChain) → isNetwork, isHighRisk (OR-merge)
  8. return isNetwork, isHighRisk, nil
```
...
checkDepsSignals(deps []DepEntry) (isNetwork, isHighRisk bool, err error):
  for each dep in deps:
    if isVDSOEntry(dep.SOName) || IsSyscallWrapperLibrary(dep.SOName):
        continue
    if dep.SyscallAnalysis == nil && dep.SymbolAnalysis == nil:
        // analysis not embedded: fail-closed
        return false, false, fmt.Errorf("dep analysis not embedded for %s", dep.Path)
...
- `analyzeDepSignals(result *dynamicanalysis.Result)` → リファクタリング: `*dynamicanalysis.Result` から `(*SyscallAnalysisData, *SymbolAnalysisData)` を受け取るよう変更（または新関数 `extractDepSignals` を追加）

#### 3.1.4 `followShebangChain` の変更（新仕様）

```
followShebangChain(chain []ShebangBinaryInfo) (isNetwork, isHighRisk bool):
  for each entry in chain:
    checkSyscallSignals(entry.SyscallAnalysis) → OR-merge
    checkSymbolSignals(entry.SymbolAnalysis) → OR-merge
  return isNetwork, isHighRisk
```

shebang チェーンのインタープリターについては dep の解析（`entry` 自身の deps への参照）は `Record.Deps` から行う。インタープリターの解析結果は `ShebangBinaryInfo` に埋め込まれており、追加のファイル読み込みは不要。

`ErrAnalysisNotFound` の処理および `ShebangStore.LoadInterpreterAnalysisPath` への依存を除去する。
...
`Manager` から `dynlibAnalysisStore` フィールドと `dynLibDepsStore` フィールドを削除する。関連する `dynamicanalysis.New` 呼び出しも除去する。
...
| テスト | 対応 AC |
|---|---|
| `DepEntry` JSON round-trip (syscall_analysis null / 非 null) | F-001 AC1 |
| `DepEntry` JSON round-trip (warnings あり / なし) | F-001 AC4 |
| `ShebangBinaryInfo` JSON round-trip (direct form) | F-002 AC1, AC2 |
...
| テスト | 対応 AC |
|---|---|
| `Record.Deps` から解析結果を読む（network signal） | F-003 AC2 |
| `Record.ShebangChain` から interpreter signals を読む | F-003 AC5 |
| dep の `SyscallAnalysis == nil` かつ非 wrapper → `ErrDepAnalysisNotEmbedded` | F-003 AC6 |
...
- `validator_shebang_test.go`: `saveInterpreterRecord` の呼び出しが消えるため、インタープリターの別 Record ファイル生成を前提とするテストは削除/書き換え
- `validator_library_analysis_test.go`: `AnalysisWarnings` の期待値を `DepEntry.Warnings` に変更
- `network_analyzer_test.go`: `AnalysisDeps` から `LibAnalysisStore`、`DynLibDepsStore`、`ShebangStore` を使うテストを削除し、`RecordStore` を使う形に書き換え
