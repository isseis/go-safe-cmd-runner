...
- [x] `run_as` を使用しない経路（run_as 未指定のコマンド）でリゾルバが一切呼ばれないことを検証するテストを追加した（`TestExecuteWithUserGroup_NoRunAs_ResolverNotInvoked`、AC-04）。dry-run 経路（`OperationUserGroupDryRun` で `changeUserGroupInternal` が識別変更を行わないこと）は Step 1-4 以前からの既存挙動であり回帰していないことを `make test` のグリーンで確認した（新規テスト追加は不要と判断）。
...
- [ ] PR-3 マージ済み（対象ステップ: 1-4）（レビュー中: GitHub PR #823）
