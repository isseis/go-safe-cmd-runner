...
- [x] `ResultCollector`（23-30行目）または `FileVerificationFailure`（`types.go` 127-133行目）に、未検証内容を採用したかを表すフラグと理由区分（`skipped_no_validator` / `verify_failed_<reason>`）を追加する。既存 `FailureReason` 列挙は変更しない（意味が異なるため）。
- [x] 新しいフラグ・理由区分を記録する `RecordUnverifiedContent`（仮称）等のメソッドを追加する。
...
- [x] `readAndVerifyFileWithReadFallback`（422-450行目）の423-427行目（`fileValidator == nil` 経路）に、Step 4-1 のフラグを `skipped_no_validator` として記録する呼び出しを追加する（現状この経路は `ResultCollector` に一切記録しない）。
...
- [x] `readAndVerifyFileWithReadFallback` の432-447行目（dry-run 検証失敗経路）で、既存の `RecordFailure` 呼び出しに加え、Step 4-1 のフラグを `verify_failed_<reason>`（`reason` は既存 `FailureReason`）として記録する。
- [x] `TestReadAndVerifyFileWithReadFallback_DryRunLogging`（991行目）を拡張し、`UsedUnverifiedContent` とその理由区分が正しく設定されることを検証する（既存の `assert.NoError` によるコンテンツ返却の検証は維持する）。
...
- [x] `VerifyEnvironmentFile`（`manager.go:92-124`）以外で env ファイルの実内容を読み込む箇所（`config` ローダ）が、同種のフォールバック（検証なしで読み込む経路）を持つかを確認する。
- [x] 持つ場合は、Step 4-1〜4-3 と同じ `UsedUnverifiedContent` 記録を適用する。持たない場合は、その旨を本ステップの完了メモとして記録する（`02_architecture.md` §3.4.1 の実装時確認事項）。

**成功基準**: env 経路の扱いが確定し、必要な記録処理が実装されているか、不要である理由が明確であること。

**完了メモ（2026-07-15 追記、weakreview 指摘により修正）**: `internal/runner/config/loader.go:127` の `safefileio.SafeReadFile` 呼び出しは env ファイル経路ではなく、`loadTemplate`（108行目）内の `l.verificationMgr == nil` 時のテスト専用フォールバック分岐（`NewLoaderForTest()` 経由でのみ到達、本番コードでは `verificationMgr` は常に non-nil）である。env ファイルの実内容を読み込む経路はリポジトリ内を検索した限り存在しない: `VerifyEnvironmentFile`（`manager.go:92-124`）は検証のみを行い内容を返さず（dry-run 時は106-108行目で `nil` を返す）、かつ本番コードからの呼び出し元はゼロである（`rg VerifyEnvironmentFile` はテストコードのみヒット）。したがって env ファイルの実内容読み込み自体が本番経路に存在せず、本フェーズの `UsedUnverifiedContent` 記録対象は該当なし（対象拡大の必要なし）。
...
- [x] グリーンゲート（`_context.md` の "Green gate" 参照）がパスしていることを確認した
- [x] PR を作成した（[#830](https://github.com/isseis/go-safe-cmd-runner/pull/830)）
- [x] PR がマージされた
- [x] 次のブランチへ切り替えた（次ステップは新しいブランチで作業する）
...
- [x] text 形式で、UNVERIFIED として採用されたファイルを検証済みと区別して表示し、理由区分（検証スキップ／検証失敗）を併記する。
- [x] json 形式でも同様の情報を出力する。
- [x] ハッシュ不一致（`ReasonHashMismatch`）は `security_risk: high` として強調表示する（既存 `getSecurityRisk`、`result_collector.go:145-156` の分類を利用）。

**成功基準**: text/json 双方の出力に UNVERIFIED マーカーが含まれることを検証するテストがパスする（AC-13）。

**weakreview 指摘・修正記録（2026-07-16 追記）**: 本ステップ完了時のドキュメント更新コミット（`docs(0146): document UNVERIFIED dry-run content and split exit codes`）が、`docs/user/runner_command.md`/`.ja.md` と `internal/runner/resource/types.go` の `FailOnVerificationUnavailable` doc comment に、Step 4-6（本ステップの次）で実装予定の終了コード分離（環境起因→exit 3、改ざん兆候→exit 1）を、あたかも実装済みであるかのように記述していた。実際には `internal/runner/resource/dryrun_manager.go`（`previewExitCodeLocked`/`recordPreviewDecision`）はコマンドのリスク評価による deny のみを終了コードに反映しており、本ステップで追加した `verification.FileVerificationSummary.UsedUnverifiedContent`/`UnverifiedFiles` を一切参照していなかった（Step 4-6 のチェックボックスも未実装のまま `[ ]` で正直に残っていたが、コミットメッセージおよび同時にコミットされたドキュメントは実装済みであるかのような記述になっていた）。加えて `skipped_no_validator` に対する `security_risk: low` 注釈も、`formatter.go` には存在しない架空の記述だった。weakreview で検出し、上記3ファイルの記述を「UNVERIFIED 表示は本ステップで実装済みだが、`-dry-run-fail-unverified` への結線は Step 4-6 で未実装」と正確に区別する記述に修正した。

**weakreview 解消記録（2026-07-16 追記、2026-07-16 訂正）**: 本弱評価記録は Step 4-6 の実装で解消された、と当初記載していたが、この記述自体が不正確だった。`internal/runner/resource/dryrun_manager.go` に `fileVerification` フィールドと `SetFileVerification` メソッドを追加し、`previewExitCodeLocked` を `verification.FileVerificationSummary.UsedUnverifiedContent` と `UnverifiedFiles` の各エントリの `Failure` フィールド（`verify_failed_<reason>` vs `skipped_no_validator`）を参照して、Step 4-6 設計どおり「改ざん兆候 → policy-deny (= 1)」「環境起因 → verification-unavailable (= 3)」（いずれも `FailOnVerificationUnavailable` セット時）の 2 値結線を実装した点は事実だが、この時点では `SetFileVerification` を呼び出す本番コード経路が存在せず、`docs/user/runner_command.md`/`.ja.md` が引き続き正しく「未実装」と記載していたのは実装漏れの結果であって意図的な区別ではなかった。`types.go` の doc comment のみが実装済みであるかのような記述になっており、3ファイル間で状態の記述が食い違っていた。この配線漏れは Step 4-6 の完了基準の直後（本節、2026-07-16 追記の weakreview 指摘・修正記録）で検出・修正され、`runner.Runner.GetDryRunResults` の配線修正により `docs/user/runner_command.md`/`.ja.md` の「未実装」の記述が実態と一致する状態になった（AC-14 のドキュメント更新は Step 4-7 で別途行う）。
...
- [x] `previewExitCodeLocked()`（421-439行目）に、未検証内容（経路1/経路2）の採用有無を判定する分岐を追加する。
- [x] 環境起因（検証不能・スキップ）は既存 `DryRunExitVerificationUnavailable`（= 3）を再利用する。
- [x] 改ざん兆候（検証失敗、ハッシュ不一致等）は既存の policy-deny 終了コード（`DryRunExitPolicyDeny`）を再利用し、`DryRunExitVerificationUnavailable` に埋没させない（`02_architecture.md` §3.4.3 の対応表）。
- [x] `FailOnVerificationUnavailable`（`types.go:97`）の doc コメントを、対象拡張後の意味に更新する。
- [x] 既定（フラグなし）では dry-run が継続し終了コード0であることを回帰確認する（AC-15）。
- [x] `02_architecture.md` §3.4.3 は回帰対象として「`cmd/runner` の該当フラグ挙動テスト」を挙げているが、実装時点で `cmd/runner/*_test.go` に `--dry-run-fail-unverified`/`FailOnVerificationUnavailable`/`DryRunExitVerificationUnavailable` を参照するテストは存在しない（`rg` で確認済み）。該当テストが存在しないことを実装時に再確認し、存在しなければ本ステップの対象外とする（存在する場合のみ追加で更新する）。

**成功基準**: `security_test.go` に未検証成果物全般（経路1/経路2）を対象にした終了コード分岐を検証する新規テスト（`TestDryRun_UnverifiedContentExitCode`、272行目以降）を追加する。既存の `TestDryRun_VerificationUnavailableExitCode`・`TestDryRun_AnalysisUnavailableDenyPreview`・`TestDryRun_DenyVsHardError` が回帰しないことを確認する。

**⚠️ weakreview 指摘・修正記録（2026-07-16 追記）**: 本ステップ完了時点で `previewExitCodeLocked()` は `DryRunResourceManager.fileVerification` フィールドを正しく参照していたが、そのフィールドを設定する `SetFileVerification` の呼び出しが **本番コード経路のどこにも存在しなかった**（`grep -rn SetFileVerification` は `security_test.go` のみにヒット）。`runner.Runner.GetDryRunResults`（`internal/runner/runner.go:500-509`）は検証サマリを表示用の `result.FileVerification` フィールドへ代入するのみで、`resourceManager.SetFileVerification(...)` を一切呼んでいなかった。加えて `PreviewExitCode` は `resourceManager.GetDryRunResults()` の内部（`dryrun_manager.go:813`）で計算されるため、たとえ代入順序を直しても代入自体が欠落していれば手遅れだった。結果として、本ステップが実装した「改ざん兆候 → exit 1」「環境起因 → exit 3」の分岐は、ユニットテスト（`mgr.SetFileVerification(...)` を直接呼ぶ）でのみ到達可能で、実際の `cmd/runner` 実行では `fileVerification` が常に `nil` のままとなり、`--dry-run-fail-unverified` を指定してもハッシュ不一致の設定ファイルが dry-run の終了コードに一切影響しない状態だった（AC-13/AC-14 が意図するセキュリティ保証が本番では機能していなかった）。weakreview で検出し、`runner.Runner.GetDryRunResults` が `resourceManager.GetDryRunResults()` を呼ぶ**前**に `fileVerificationSetter` インタフェース（既存の `executionErrorSetter` と同方式の型アサーション）経由で `SetFileVerification` を呼ぶよう修正した。回帰防止のため `internal/runner/runner_test.go` に `Runner.GetDryRunResults` を経由した統合テストを追加し、改ざん兆候ケースで `PreviewExitCode() == DryRunExitPolicyDeny` になることを検証する（Step 4-6 の対象ファイルに `internal/runner/runner.go` を含めていなかったことが本欠陥の直接原因であるため、今後同種のステップでは「値を計算する構造体」だけでなく「その値を実際の実行経路に配線する呼び出し元」まで対象ファイルに含めること）。

#### Step 4-7: ドキュメントを更新する（AC-14）

**対象ファイル**: `docs/user/runner_command.md`, `docs/user/runner_command.ja.md`

- [x] `--dry-run-fail-unverified` の既定挙動（未指定時は継続・終了コード0）と、CI 用途での推奨運用（未検証を hard fail にする用途での使用）をドキュメント化する。
- [x] 環境起因（検証不能）と改ざん兆候（検証失敗）で終了コードが異なることを明記する。
...
- [x] グリーンゲート（`_context.md` の "Green gate" 参照）がパスしていることを確認した
- [x] PR を作成した（[#831](https://github.com/isseis/go-safe-cmd-runner/pull/831)）
- [x] PR がマージされた
- [x] 次のブランチへ切り替えた（次ステップは新しいブランチで作業する）
...
- [x] `safefileio.MaxFileSize`（128MB）とその根拠（メモリ枯渇対策）、大容量バイナリを検証・解析できない可用性上の制約を明記する（AC-20）。
- [x] `filevalidator` の内部 `maxFileSize`（1GB、バイナリ解析専用）が `safefileio.MaxFileSize` とは別の制限であることを明確に区別して記述する（既存コード調査結果参照。両者を混同しない）。
- [x] 閾値の設定可能化／ハッシュ・解析の上限分離について、本タスクでは実装しないと結論した理由（`02_architecture.md` §3.6 の(1)〜(3)）を記録する（AC-21。設計文書への記録は `02_architecture.md` に既に存在するため、ユーザードキュメントには結論の要約を記載する）。
...
- [x] 本番ターゲットが Linux + openat2 前提であることを明記する（既存の openat2 記述セクションを拡張する）。
- [x] non-Linux（openat2 非対応）環境には `safeOpenFileFallback` の二段階チェックでも原理的に残る極短の TOCTOU 競合ウィンドウが存在することを明記する。
- [x] macOS 等を開発・限定用途に限る運用ガイドを明記する。
...
- [x] 「補助グループ」→「supplementary group」を追加する。
- [x] 「saved-set-uid」（英語表記のまま、または訳語）のエントリを追加する。
...
| `TestDryRun_VerificationUnavailableExitCode` | `internal/runner/resource/security_test.go:151-176` | 既存の検証不能ケースは無変更のまま回帰確認（未検証成果物全般を対象にした新規分岐は `TestDryRun_UnverifiedContentExitCode`（同ファイル 272行目以降）として別関数で追加。weakreview 指摘、2026-07-16 追記: 本表は当初 `TestDryRun_VerificationUnavailableExitCode` が「拡張された」としていたが、実際に拡張されたのは同ファイル内の別関数であり同テストは無変更だった） |
...
- 種別: `test` + `static`
- テスト: `internal/runner/resource/security_test.go::TestDryRun_UnverifiedContentExitCode`（Step 4-6）+ `internal/runner/runner_test.go`（`Runner.GetDryRunResults` を経由した配線の統合テスト、weakreview 指摘・修正、2026-07-16 追記）
