...
- [x] `identity_linux.go` に `readSavedIDs() (suid, sgid int)` を実装し、`golang.org/x/sys/unix.Getresuid`/`Getresgid` を呼ぶ（`!windows` 単一ファイルへの簡略化はしない。darwin に同関数が存在しないことを確認済み、既存コード調査結果参照）。
- [x] `identity_other.go` に同シグネチャの no-op 実装（`suid=0, sgid=0` を返す。呼び出し側は「非 Linux では検証を省略する」前提でこの戻り値を扱う）を用意する。
- [x] `identity_other_test.go` を追加し、no-op 実装が `readSavedIDs()` で常に `(0, 0)` を返すことを検証する（本プロジェクトの開発機は darwin であり、この no-op 経路は実際に到達可能なコードパスであるため）。
...
- [x] `prepareExecution` に `readSavedIDs()` 呼び出しを追加し `originalSUID`/`originalSGID` を捕捉する。非 Linux では no-op が (0,0) を返す（呼び出し側は非 Linux では検証をスキップする前提）。
