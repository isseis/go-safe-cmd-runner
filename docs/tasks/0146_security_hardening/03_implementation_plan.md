...
**weakreview 解消記録（2026-07-16 追記）**: 本弱評価記録は Step 4-6 の実装で解消された。`internal/runner/resource/dryrun_manager.go` に `fileVerification` フィールドと `SetFileVerification` メソッドを追加し、`previewExitCodeLocked` を `verification.FileVerificationSummary.UsedUnverifiedContent` と `UnverifiedFiles` の各エントリの `Failure` フィールド（`verify_failed_<reason>` vs `skipped_no_validator`）を参照して、Step 4-6 設計どおり「改ざん兆候 → policy-deny (= 1)」「環境起因 → verification-unavailable (= 3)」（いずれも `FailOnVerificationUnavailable` セット時）の 2 値結線を実装した。デフォルト（フラグなし）では exit 0 のまま（AC-15 回帰なし）。これに合わせて `docs/user/runner_command.md`/`.ja.md` と `types.go` の `FailOnVerificationUnavailable` doc comment の記述を「実装済み」と「未実装」の区別を明瞭化した。
