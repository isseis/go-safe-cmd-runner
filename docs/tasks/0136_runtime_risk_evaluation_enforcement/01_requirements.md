...
- 一部 AC は実現機構の選定をアーキテクチャ設計に委ねている（AC-64 の TOCTOU 結合方式、AC-68 の coreutils 判別不能時の既定リスク値、F-013 のラッパー網羅範囲、F-005 解析無効時の挙動）。これらは `02_architecture.md` で確定値・方式に落とし、`03_implementation_plan.md` のトレーサビリティで検証可能な形にすること。
...
3. **判定対象外の副作用がある**: `output_file` / `cmd.Output()` による任意パスへの書き込みは、書き込み先のリスク寄与を `EvaluateRisk` が評価しない（`ValidateOutputPath` は traversal 検査のみ）。本タスクのスコープ外だが、限界として明記する。
4. **root 実行向け判定系（`IsDangerousRootCommand` / `HasDangerousRootArgs`）は別系統**: 部分一致方式で `EvaluateRisk`（exact match）と方針が異なる。本タスクでは統合しないが、両系統の適用パスと優先順位を文書化する（F-008 / AC-67）。
...
さらに、`EvaluateRisk` 全体の不変条件として、**全判定ステップ（特権昇格・破壊操作・coreutils・ネットワーク・システム変更・引数パターン・バイナリ解析・プロファイル）の結果の最大値を返す**こととし、現行の「最初に該当したステップで早期 return する」方式による取りこぼし（後段ステップがより高いリスクを返す組み合わせの見逃し）を排除する。Critical（特権昇格・不確実）は最大値として常に優先される。

なお、プロファイルに **引数条件付きの要因**（F-011 の `SystemModRisk` 等）が含まれる場合、`BaseRiskLevel()` は **条件評価後の要因値** で計算する（静的宣言値ではない）。詳細は F-011 を参照。
...
- **AC-05**: コマンド名はシンボリックリンクを解決して照合する（既存の `extractAllCommandNames` と一貫した方式）。リンク連鎖上の複数の名前が異なるプロファイルに一致した場合は、一致した全プロファイルの `BaseRiskLevel()` の **最大値** を採用する。ただしリンク走査中に解決失敗（stat/readlink 失敗・循環・深度超過・解決不能）が混在する場合は、部分一致の最大値で続行せず F-012 の安全側中止を **優先** する。
...
- **AC-63**: `EvaluateRisk` は全判定ステップの **最大値** を返す（早期 return による取りこぼしがない）。複数要因に該当するコマンドは、各要因が単独で返す値の最大値が実効リスクになる（順序非依存）ことをテストで検証する。
...
- **AC-62**: `find` の `-exec` 対象コマンドおよび `rsync --delete` 等の判定が、**絶対パス・coreutils 配下・basename** のいずれの形でも機能する。例: `find . -exec /usr/bin/rm {} \;` および coreutils 配下 rm を exec 対象とするケースを破壊的と判定する（`-exec` 引数にも basename 正規化／`extractAllCommandNames` を適用）。
...
- **AC-69**: バイナリ解析の各シグナル・各分岐（`handleAnalysisOutput` の NetworkDetected / NoNetworkSymbols / NotSupportedBinary / StaticBinary / AnalysisError / default、および svc / exec / mprotect / network 各シグナル）に固有の reason code が割り当てられ、テストで全列挙される（「なぜそのリスクか」が分岐ごとに区別できる）。
- **AC-14**: リスク超過で実行拒否されたコマンドについても、リスク判定結果が監査可能である（拒否ログまたはリスクプロファイルログのいずれかで追跡できる）。
- **AC-56**: 監査ログのリスク判定エントリは、相関に必要な以下のフィールドを **空でない実値で** 含む: `resolved_path`、`content_hash`、解析レコードの識別情報（レコードの hash または schema version。レコード非使用時はその旨を示す明示値）、`max_allowed_risk`、`decision`（allow/deny）、`reason_codes`。リスク超過・不確実・特権昇格などで **拒否（deny）された場合にも必ず出力される**（早期 return で監査が抜けない）。
...
- **AC-51**: `RecordStore` が nil（バイナリ解析無効）の構成では、(a) 既定でコマンド実行が拒否される（安全側既定）、(b) 実行を許可するには `risk_level` とは独立した明示オプトイン設定が必要、(c) その設定の既定値が「拒否」側である、(d) 解析無効で実行/拒否した旨がログ・監査で明示される。High 許可（`risk_level = "high"`）だけでは解析無効状態の許可にはならない。オプトインは可能な限り狭いスコープ（コマンド単位）を既定とし、広域（グローバル一括）許可を採用する場合は警告・監査を必須とする（リスク補償の抑止）。
...
- **AC-29**: 複数機構に定義が重複するコマンドの優先順位（どの定義がどのパスで効くか）が、設計文書および開発者向けドキュメントに明記される。root 実行向け判定系（`IsDangerousRootCommand` / `HasDangerousRootArgs`、部分一致方式）と `EvaluateRisk` 系（exact match 方式）の **適用パスと優先順位の関係** も文書化する（統合は本タスクスコープ外だが、関係の明示は行う）。
- **AC-68**: coreutils マルチコール（`coreutils <subcmd>`）のサブコマンド判別が、破壊的サブコマンド（`rm`/`dd`/`shred` 等）を確実に検出する。サブコマンド判別不能時の既定が、破壊的バイナリを含み得る以上、安全側（Medium ではなく High を採用するか否かを設計で決定し、根拠を明記）であることを保証する。現状 `findFirstSubcommand` は git 用オプションスキップ表を流用しており、coreutils への適用妥当性を検証する。
...
**F-001（最大値方式）との合成順序（重要）**: 条件付き評価の結果（`status`→Medium、`restart`→High 等）が **`SystemModRisk` の実効値** として `CommandRiskProfile.BaseRiskLevel()` の計算に入力される。すなわち F-001 が採る「全要因の最大値」は、この **引数条件評価後の `SystemModRisk` 値** を用いる。プロファイルが静的に宣言する `SystemModRisk=High` を無条件に最大値へ持ち込むと `systemctl status` まで High に固定され F-011 の目的（最小権限）と AC-49 が破綻するため、**条件評価が BaseRiskLevel 計算より前段で適用される**ことを必須とする。
...
`EvaluateRisk` は `cmd.ExpandedCmd`（argv[0]）と引数だけを見るため、実体が危険なコマンドを無害なラッパー／シェルでくるむと、リスク判定（および特権昇格の Critical ゲート）を回避できる。
...
- シェル例: `cmd=/bin/bash, args=[-c, "rm -rf / ; systemctl stop firewalld"]` → profile の Medium 止まり（`-c` の中身は 1 個の文字列引数で危険パターン非該当）。
...
- **特権昇格の独立ルール（抽出可否に依らず適用）**: 間接実行（ラッパー／シェル）の引数列に `sudo` / `su` / `doas` が出現する場合は、実コマンド抽出の成否に依らず **Critical** とする。これにより `env sudo …` / `xargs sudo` 等が抽出失敗時の High 下限へ落ちて Critical を回避することを防ぐ（AC-59 の保証根拠）。
- **シェル／インタプリタのインラインコード実行**（`sh -c` / `bash -c` / `python -c` / `perl -e` 等）: インライン文字列は静的に安全解析できないため、実効リスクの **High 下限** とする（Medium 止まりにしない）。
...
- **AC-61**: シェル／インタプリタのインラインコード実行（`bash -c "…"` / `python -c "…"` 等）の実効リスクが Medium 止まりにならず **High 以上** になる。
...
リスク判定は、ハッシュ検証で確定した実体・パスに対して行わなければならない。現状は検証（`verifyGroupFiles` でハッシュ確定）後に実行直前で再度 `ResolvePath` し、さらに `EvaluateRisk` 内の `extractAllCommandNames` / `CoreutilsCommandRisk`（setuid 判定）が独立に `os.Lstat` / `os.Readlink` / `os.Stat` を行うため、検証〜判定〜実行の間に **TOCTOU 窓** が開く。また content hash は現状バイナリ解析経路でのみ照合され、coreutils 判定・破壊判定には伝播していない。

- リスク判定が参照する実体・パスを、ハッシュ検証で確定したものに結合し、検証〜判定間の **独立した再解決・再 stat を最小化** する。やむを得ず再 stat する場合は、確定済み content hash と再突合する。
- content hash（`ExpandedCmdContentHash`）が空のコマンドが、coreutils 判定・破壊判定など **バイナリ解析以外の経路だけで Low/通過しない** ようにする（F-005 の不確実扱いを、参照実体が検証済みハッシュを持たないあらゆる経路へ一貫適用する）。
...
- **AC-65**: `ExpandedCmdContentHash` が空のコマンドが、coreutils 判定・破壊判定のみで Low/通過とならず、F-005 の不確実扱い（実行中止）に一貫して帰着することをテストで検証する（解析無効構成 AC-51 と整合）。

## 3. 非機能要件

### NF-001: 後方互換性

- 既存の TOML 設定の解釈を破壊しない。ただし F-001/F-002/F-005/F-007/F-013 により、一部設定の挙動が変わる:
  ...
  - 解析無効構成（F-005 解析無効時）: 既定で拒否となり、明示オプトインが必要になる（従来は黙って通過）。
  - `risk_level = "unknown"` は設定エラーになる（従来は全コマンド拒否として黙って動作）。
- これらの変更点を移行ノートとしてドキュメントに記載する。

**Acceptance Criteria**:
- **AC-19**: 実効リスク上昇により拒否され得る代表的ケース（`claude` を `risk_level=medium` で実行、絶対パス `rm` を `risk_level=low` で実行、`systemctl` を `risk_level=low` または `medium` で実行、ラッパー/シェル経由実行、解析無効構成）、不確実ケースの設定によらない拒否（解消手段をケース別に記載）、および `"unknown"` の設定エラー化が、移行ノートまたは変更点としてドキュメントに記載される。
