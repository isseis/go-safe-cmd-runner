...
- **ラッパーコマンド**（`env` / `nice` / `ionice` / `timeout` / `nohup` / `stdbuf` / `setsid` / `xargs` / `time` / `chrt` / `taskset` 等）が argv[0] の場合: (a) 実引数から実コマンドを抽出して再帰的にリスク評価する、または (b) 抽出が不能・未対応なら安全側（最低 High、特権昇格を内包し得る場合は Critical）に倒す。いずれの方式でも、**ラッパー経由で `sudo`/`su`/`doas` が Critical を回避できないこと**を必須とする。
