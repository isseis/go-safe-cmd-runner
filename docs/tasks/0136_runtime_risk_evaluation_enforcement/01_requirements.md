...
| Review date | - |
| Reviewer | - |
| Comments | 2026-06-13: 作成中の事前フィードバック（正式レビュー前）により E の対応方針を「ドキュメント修正のみ」から「不確実ケースの実行中止（内部 Critical 化）＋ドキュメント修正」へ変更（F-005 改訂、AC-40〜AC-43 追加）。同日: アーキテクト観点のサブエージェントレビュー指摘（Critical 1・Major 8・Minor 7）を反映（F-005 根拠事実の訂正、F-011 追加、AC-44〜AC-50 追加ほか） |
...
- **AC-05**: コマンド名はシンボリックリンクを解決して照合する（既存の `extractAllCommandNames` と一貫した方式）。リンク連鎖上の複数の名前が異なるプロファイルに一致した場合は、一致した全プロファイルの `BaseRiskLevel()` の **最大値** を採用する。
- **AC-22**: `systemctl` / `service`（`SystemModRisk=High` を宣言）の **変更系サブコマンド**（F-011 不採用時は全サブコマンド）の実行時実効リスクが **High** になる（従来は実行時（絶対パス解決後）Low、basename 入力時 Medium）。この振る舞い変更は意図的なものとして移行ノート（AC-19）に記載される。
...
実行時にコマンドのリスク判定を行った際、その結果（リスクレベル・判定根拠・ネットワーク種別）が監査ログ（`LogRiskProfile` 相当の `command_risk_profile` エントリ）として出力されなければならない。

**実現上の前提（アーキテクチャ設計への入力）**:
- 現行の `Evaluator.EvaluateRisk` は `(RiskLevel, error)` のみを返し、判定根拠（どのステップ・どの要因で決まったか）を返さない。プロファイル由来でないコマンド（破壊的操作・バイナリ解析・coreutils 分類等で決まったもの）について判定根拠を監査に残すには、評価器の戻り値の拡張が必要である。
...
- **AC-12**: 出力エントリには、評価されたリスクレベルと判定根拠が含まれる。判定根拠は、プロファイル由来のコマンドではリスク要因の理由（`risk_factors`）、それ以外のコマンドでは判定を決めたステップの種別（例: destructive file operation / binary analysis: dynamic load / coreutils classification）である。
- **AC-13**: ログレベルがリスクレベルに対応する（Critical→Error, High→Warn, Medium→Info, それ以外→Debug）。
- **AC-48**: プロファイルを持たないコマンド（例: バイナリ解析で Medium となった未知コマンド）についても、AC-12 の判定根拠が出力される。
...
- 現状は黙ってフェイルオープン（解析スキップ = Low 側）となる。これは「解析機能を構成していない」という運用上の選択であり、不確実ケースとは区別する。ただし黙って素通りさせず、解析無効で実行している旨をログで明示する（AC-51）。実行可否の方針（フェイルオープン維持か、設定による明示オプトインか）はアーキテクチャ設計で決定し、根拠とともに記録する。
...
- **AC-51**: `RecordStore` が nil（バイナリ解析無効）の構成でコマンドのリスク判定が行われた場合、解析無効のまま実行している旨のログが出力される（黙ったフェイルオープンを許さない）。
...
- **AC-47**: `dangerousCommandPatterns` にのみ定義があるコマンド・引数の組（例: `mkfs` / `fdisk` → High、`chmod 777` / `chown root` → Medium）について、実行時とdry-runの実効リスクが分裂しないよう定義の整合が取られ、テストで検証される（現状は dry-run のみパターンが効き、実行時は `IsSystemModification` の Medium またはバイナリ解析に落ちる）。
...
- **AC-46**: ハッシュディレクトリ利用不能などで dry-run が `contentHash` を取得できない場合、不確実ケース（Critical/拒否予告）と **誤断定せず**、「判定不能（ハッシュ未検証のため実行時に再評価）」に相当する第四の分類で表示する。
...
これを避けるため、`SystemModRisk` に **引数（サブコマンド）条件付き評価** を導入する。既存の `NetworkSubcommands`（`ConditionalNetwork`）と対称な機構とする。

- 変更系サブコマンド（`start` / `stop` / `restart` / `enable` / `disable` / `mask` 等）→ High
- 読み取り専用サブコマンド（`status` / `show` / `is-active` / `list-units` 等）→ プロファイルによる昇格なし（他ステップの判定に委ねる）
- **未知のサブコマンド・サブコマンド判別不能 → High（安全側既定）**

> 📝 **レビュー判断事項**: 本要件は実装スコープを広げる（プロファイル機構の拡張）。よりシンプルな代替案は「無条件 High を受け入れ、ユーザー向け文書の設定例を `risk_level = "high"` に改訂する」ことである。レビュー時にいずれかを選択すること。本ドキュメントは条件付き評価の導入を推奨する。**採否確定後は、本注記および AC-22 / AC-49 の条件文を除去して一本化すること**（これをレビュー完了の条件とし、確定までは AC-22 / AC-37 / AC-49 / NF-001 の検証条件が浮動状態であることに留意）。

**Acceptance Criteria**:
- **AC-49**: `systemctl status <unit>` の実行時実効リスクが High にならず、`systemctl restart <unit>` および未知サブコマンドは High になる（F-011 を採用した場合）。不採用（無条件 High）の場合は本 AC を欠番とし、AC-50 の設定例改訂で対応する。
...
- 既存の TOML 設定の解釈を破壊しない。ただし F-001/F-002/F-005/F-007 により、一部設定の挙動が変わる:
  - `claude` 等（実効リスク Medium→High）、`systemctl`/`service`（実行時 Low→High。F-011 採用時は変更系サブコマンドのみ）、絶対パス指定の破壊的コマンド（Low→High）は、`risk_level` 設定によっては従来実行できていたものが拒否される。これは **意図的な安全性向上** である。
...
| フェイルセーフ（High 化） | 不確実・危険検出時に安全側（より高いリスク）へ倒すが、`risk_level = "high"` で許可可能な挙動 |
| フェイルクローズド | 上記「エラー中止」「無条件拒否」の総称（確実に実行させない挙動） |
