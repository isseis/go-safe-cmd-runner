...
> **注（後続タスク 0138 による更新）**: 上表「ラッパー」行のインナーコマンドの扱い（実コマンドを抽出し再帰的に細粒度評価＋ゲートし、将来 fd 束縛する方針）は、後続タスク 0138 により変更された。ラッパー経由インナーは一律 High（特権昇格は Critical／禁止形態は Reject 優先）として扱い、インナーの fd 束縛・ラッパー再実装は取り下げられた。詳細は [タスク 0138 のアーキテクチャ設計書](../0138_indirect_inner_command_risk/02_architecture.md) を参照。
...
- **許可ラッパーのインナーコマンド fd 束縛（実装上の残存制約）**: `env`/`timeout` 等の許可ラッパーは runner が **ラッパー自身**を fd 束縛で exec する一方、インナーコマンドはラッパープロセスが PATH で解決・exec するため、インナーの実体は fd 束縛されない（インナーは名前ベースのゲートを PR-3 で受ける）。これは `find`/`xargs` と同列の残存制約であり、現行 main と同一の TOCTOU 特性（退行ではない）。インナーを runner が直接 exec し直す実行モデルでの fd 束縛は将来対応とする。
  > **注（後続タスク 0138 による更新）**: 「インナーを runner が直接 exec し直して fd 束縛する」将来方針は後続タスク 0138 により取り下げられた。ラッパー経由インナーは一律 High（特権昇格は Critical／禁止形態は Reject）として扱い、インナーの fd 束縛・自動ハッシュ検証は導入しない。詳細は [タスク 0138 のアーキテクチャ設計書](../0138_indirect_inner_command_risk/02_architecture.md) を参照。
