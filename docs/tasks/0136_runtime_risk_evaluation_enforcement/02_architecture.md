...
type ErrorClass string         // エラー起因 deny の分類
...
type RiskAuditEntry struct {
    ...
    ErrorClass     ErrorClass            // エラー起因の deny 時の分類（例: ErrorSymlinkResolution）。空=エラー起因でない
