...
評価結果を構造化し、reason code・判定根拠・**評価ステータス**を運ぶ（F-001/F-003/F-005、H-4/M-7 対応）。dry-run の `unknown` と normal の拒否を **型で区別** する。

```go
// AssessmentStatus は判定の最終区分（新規。dry-run unknown / error を型で表現）。
type AssessmentStatus int

const (
    StatusAllow   AssessmentStatus = iota // 許可（risk_level 以下、非 Blocking）
    StatusDeny                            // 拒否（risk_level 超過、または Blocking）
    StatusUnknown                         // dry-run でハッシュ/レコード未取得（実行時に再評価。AC-46）
    StatusError                           // エラー中止（解決失敗等。実行も preview も行わない）
)

// EvaluationContext は評価の文脈（モードと検証状態）を評価器へ渡す（新規）。
// 同じコマンドでも dry-run でハッシュ未取得なら StatusUnknown、normal なら Blocking と分岐する。
type EvaluationContext struct {
    Mode              ExecutionMode     // normal / dry-run
    VerificationState VerificationState // verified / hash-unavailable / analysis-disabled
}
...
type RiskAssessment struct {
    ...
    Status        AssessmentStatus      // allow/deny/unknown/error（比較ゲート確定後に設定）
...
type Evaluator interface {
    EvaluateRisk(cmd *runnertypes.RuntimeCommand, ctx EvaluationContext) (VerifiedCommandPlan, error)
}
```

> 現行の `EvaluateRisk(cmd) (runnertypes.RiskLevel, error)` を上記へ変更する。`error`（→ `StatusError`）は「エラー中止」（解決失敗・予期しないレコード読込エラー等）専用。設定によらず実行を中止する不確実ケースは `RiskAssessment.Blocking=true`＋`BlockingReason`（内部 Critical 相当）で表現する。**特権昇格は `Level=Critical`**（設定不可のため常に拒否。監査では `effective_risk=critical, decision=deny, blocking_reason=privilege_escalation` で一貫させる。M-8 対応）。`Level=Critical` と `Blocking=true` はいずれも「必ず拒否」だが、前者はリスクレベル（特権昇格）、後者は不確実/identity 失敗という意味の異なる経路である。
...
classDiagram
    class Evaluator {
        <<interface>>
        +EvaluateRisk(cmd, ctx) (VerifiedCommandPlan, error)
    }
...
```go
// Decision は監査上の最終判定（enum。allow/deny/unknown/error を表現。M-7 対応）。
type Decision int

const (
    DecisionAllow   Decision = iota
    DecisionDeny
    DecisionUnknown // dry-run でハッシュ/レコード未取得（AC-46）
    DecisionError   // エラー中止
)
...
type RiskAuditEntry struct {
    ...
    Decision       Decision              // allow/deny/unknown/error
...
サブコマンド抽出の argv 解析規則（M-9 対応、判別不能のみ High）:
- **グローバルオプションを読み飛ばす**（`-H`/`--host`、`-M`/`--machine`、`--user`、`--system`、`--no-pager` 等）。
- **値を取るオプションは値も consume** する（`--host VALUE` のように分離した値、`--host=VALUE` の結合形の双方）。
- **`--` 以降は最初をサブコマンドとして扱う**（オプション終端）。
- **サブコマンド省略時**（`systemctl` 単体・グローバルオプションのみ）は情報表示用途とみなし読み取り専用相当（Medium 下限）。
- 上記規則で **一意に判別できない場合のみ High**（安全側）。既存の `findFirstSubcommand`（git 用オプション表流用）はこの規則に置き換える。
...
flowchart TD
    ...
    S["EvaluateRisk(cmd, ctx)"] --> IDG{"identity ゲート:<br>検証済みか?<br>(content_hash 有 / 検証・解析有効 / identity 束縛可)"}
...
1. **フェーズ 1 — 評価コア＋拒否ゲート**: `RiskAssessment`/`VerifiedCommandPlan`/`AssessmentStatus` 導入、`EvaluateRisk` の最大値化、identity ゲート、`NetworkAnalyzer` 分類化、不確実→Blocking、coreutils 優先、`ParseRiskLevel("unknown")`。**normal の拒否（deny）が機能し監査される所まで**を 1 段に含める。F-001/F-002/F-005/F-007/F-008/F-011。
...
3. **フェーズ 3 — dry-run preview＋監査拡張**: `LogRiskProfile` 拡張・`NormalResourceManager`/`dryrun_manager` 配線・deny 重大度・unknown 区分・dry-run の allow/deny/unknown preview。F-003/F-006/F-009。
