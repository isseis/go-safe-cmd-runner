...
- [ ] 全 AC が §7 の検証表で `test` または `static` により充足（現ブランチで全 static rg・`make test` が pass 済み。AC-15/17/18/29/66/67 の command-risk-evaluation 部分は PR-7 #736 で記述済み・PR-7 マージで main 確定）
