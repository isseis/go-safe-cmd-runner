...
- [ ] PR-7 マージ済み（対象ステップ: 4-3）— command-risk-evaluation 整合（**PR #724 マージ後**。PR #724 は 2026-06-17 時点で Draft/OPEN のため未着手）
- [ ] 全 AC が §7 の検証表で `test` または `static` により充足（AC-15/18・AC-29/17 の command-risk-evaluation 部分は PR-7 待ち）
- [ ] §8 クロス検索チェックリスト完了
