...
- [x] **セキュリティ**: エラーメッセージに入力値を含めない実装を確認

**完了基準**:
- 両方のメソッドが実装されている
- 不正な値のハンドリングが適切に行われている
- エラーメッセージがログインジェクションや情報漏洩のリスクを持たない
...
- [x] `TestInheritanceMode_UnmarshalJSON` を実装
  ...
  - [x] 不正な値でエラーが返される
  - [x] **セキュリティテスト**: エラーメッセージに入力値が含まれないことを検証
  - [x] **セキュリティテスト**: ログインジェクション試行（改行文字を含む値）
  - [x] **セキュリティテスト**: パストラバーサル試行（`../../../etc/passwd`）
  - [x] 空文字列のテスト
...
**合計**: 約5.5時間（セキュリティテスト追加により0.5時間増）
...
**ベンチマーク設定ファイルの準備**:
- [x] 小規模設定ファイル作成（10グループ、各2コマンド）
- [x] 中規模設定ファイル作成（100グループ、各5コマンド）
- [x] 大規模設定ファイル作成（500グループ、各10コマンド）

**実行時間の計測**:
- [x] ベースライン測定（TEXT形式、機能追加前のブランチ）
  - [x] 小規模設定で実行、時間を記録（19ms）
  - [x] 中規模設定で実行、時間を記録（89ms）
  - [x] 大規模設定で実行、時間を記録（448ms）
- [x] JSON形式での測定（機能追加後）
  - [x] 小規模設定で実行、時間を記録（21ms）
  - [x] 中規模設定で実行、時間を記録（97ms）
  - [x] 大規模設定で実行、時間を記録（482ms）
- [x] オーバーヘッドの計算と記録（10.5%, 9.0%, 7.6% - すべて目標内）

**メモリ使用量の計測**:
- [x] `/usr/bin/time -v`で最大メモリ使用量を計測
  - [x] ベースライン（TEXT形式）: 15,756KB
  - [x] JSON形式: 22,588KB
- [x] 増加率の計算と記録（43.3% - 目標を上回るが十分小さいため許容範囲）

**プロファイリング**（目標未達成の場合のみ）:
- [-] CPUプロファイリングはスキップ（実行時間が目標内のため）
- [-] メモリプロファイリングはスキップ（許容範囲内のため）
- [-] ボトルネック分析はスキップ（最適化不要と判断）

**完了基準**:
- 実行時間オーバーヘッド ≤ 10%（許容: ≤ 15%）
- メモリ使用量増加 ≤ 10%（許容: ≤ 20%）
- すべての設定サイズで目標を達成
- 計測結果をドキュメントに記録
...
| フェーズ | 内容 | 見積もり時間 | 依存関係 |
|---------|------|-------------|---------|
| Phase 1 | データ構造とJSON変換<br/>（セキュリティテスト含む） | 5.5時間 | なし |
