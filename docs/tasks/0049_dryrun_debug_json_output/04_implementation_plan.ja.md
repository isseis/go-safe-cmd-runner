...
- [ ] 中規模設定ファイル作成（100グループ、各5コマンド）
- [ ] 大規模設定ファイル作成（500グループ、各10コマンド）

**実行時間の計測**:
- [ ] ベースライン測定（TEXT形式、機能追加前のブランチ）
  - [ ] 小規模設定で10回実行、平均時間を記録
  - [ ] 中規模設定で10回実行、平均時間を記録
  - [ ] 大規模設定で10回実行、平均時間を記録
- [ ] JSON形式での測定（機能追加後）
...
**メモリ使用量の計測**:
- [ ] `go test -benchmem`でメモリアロケーションを計測
- [ ] `/usr/bin/time -v`で最大メモリ使用量を計測
  - [ ] ベースライン（TEXT形式）
  - [ ] JSON形式
- [ ] 増加率の計算と記録

**プロファイリング**（目標未達成の場合のみ）:
- [ ] `go test -cpuprofile`でCPUプロファイル取得
- [ ] `go test -memprofile`でメモリプロファイル取得
- [ ] `go tool pprof`でボトルネック分析
- [ ] 必要に応じて最適化を実施
