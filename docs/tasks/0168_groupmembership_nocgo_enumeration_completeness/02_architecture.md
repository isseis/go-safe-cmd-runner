...
ただし「常に完全」という申告には、要件書が対象外とした残存リスクが2つある。いずれも SSSD 側の設定であり、`/etc/nsswitch.conf` からは見極められない。

1. SSSD は既定で `enumerate = false` であり、その構成では `getpwent` がディレクトリ管理ユーザーを返さない。すなわち CGO 版のプライマリメンバー収集も NSS バックエンドの設定次第で不完全になりうる。
2. SSSD の性能チューニングとして使われる `ignore_group_members = True` を設定すると、`getgrgid_r` が返す `gr_mem`（明示メンバー）が常に空になる。この設定は `enumerate = false` とは独立に効くため、1 の条件を満たさない構成（`enumerate` が既定の `true` のまま）でも明示メンバー側が欠落しうる。

この見極めは `/etc/nsswitch.conf` からは行えず、バックエンドごとの設定調査を要するため本タスクでは扱わない（§5.4、AC-29 の Issue 分離）。
...
| CGO 版の SSSD 設定依存の列挙不完全性 | SSSD の既定 `enumerate = false` では、CGO 版のプライマリメンバー収集もディレクトリ管理ユーザーを含まない。`ignore_group_members = True` を設定した場合は、`enumerate` の値によらず明示メンバー（`gr_mem`）が常に空になる。それでも本設計はいずれの構成でも「完全」と申告する | 本タスクでは扱わない。新規 Issue として分離する（AC-29） |
