...
**要件書への差し戻し候補**: この変更は既存の利用者に対する挙動の変更であるため、リリースノートまたは変更履歴への記載が要る。要件書の AC-23〜AC-25 は利用者向けの説明文書のみを対象としており、リリースノートに対する AC が無い。AC の追加が必要かどうかはレビューで確認を求める。
...
利用者向け文書の日本語版を更新してコミットし、`/mktrans` で英語版へ反映する。0149 の残件一覧と findings を更新する。対象外とした2件を Issue として登録し、その番号を要件書と残件一覧へ書き戻す。§5.5 のとおり、リリースノートへの記載の要否をあわせて確認する。
