...
- **完全性は列挙結果の型に持たせる**。`getGroupMembers` が返す値に、集合が網羅的かどうかを表すフィールドを設ける。CLAUDE.md「Declare, don't infer」に従い、真偽値ではなく列挙型とし、ゼロ値が「不完全」になるようにする。新しい列挙元を追加した実装者が完全性の申告を忘れた場合、既定で fail-closed になるためである。
...
- **AC-01**: `getGroupMembers` は、メンバー名の集合に加えて列挙の完全性を返す。完全性を表す型のゼロ値は「不完全」であり、「完全」を意味する値ではない。
- **AC-02**: CGO 版 `getGroupMembers` は、成功時に常に「完全」を返す。
- **AC-03**: 完全性を表す型が想定外の値であった場合、それを解釈する側は「完全」として扱わない。`switch` の `default` は拒否側に倒れる。
