...
| Reviewer | isseis |
| Comments | 2026-08-26: `02_architecture.md` の設計レビューで、既存の配布バイナリに対する挙動変更を変更履歴へ記載する受け入れ基準が欠けていることが判明したため、AC-30・AC-31 を追記した（承認済みの範囲への追加であり、既存 AC の変更・番号の振り直しは行っていない）。<br>2026-08-26: `03_implementation_plan.md` の作成時の調査で、`CHANGELOG.ja.md` の同じ「未リリース」ブロックに、本タスクが塞ぐ経路を既知の制限として述べた項目が既に存在し、AC-30 の項目と同一リリース内で矛盾することが判明したため、AC-32・AC-33 を追記した（同上、既存 AC の変更・番号の振り直しは行っていない）。 |
...
- **AC-32**: [CHANGELOG.ja.md](../../../CHANGELOG.ja.md) の「未リリース」→「セキュリティ」にある既存項目「既知の制限: 公式バイナリ（`CGO_ENABLED=0`）はグループメンバーシップで NSS を参照しない」が解消されている。同項目は、本タスクが塞ぐ経路を「group-writable なファイルの書き込み安全性判定（`CanUserSafelyWriteFile`）が実際より緩く評価されることがあります」と述べ、回復手段として `CGO_ENABLED=1` でのセルフビルドの検討を促すものであり、**AC-30 で追加する破壊的変更の項目と同一リリースの中で矛盾する**。解消の形は、当該項目を削除して AC-30 の項目へ統合するか、本タスク後の挙動（拒否される）へ書き換えるかのいずれでもよい。未リリースの記述であるため、履歴として残す必要はない。
- **AC-33**: AC-32 の英語版（`CHANGELOG.md` の "Known limitation: official binaries (`CGO_ENABLED=0`) do not consult NSS for group membership"）が `/mktrans` により反映されている。日本語版を先に作成・コミットする。
