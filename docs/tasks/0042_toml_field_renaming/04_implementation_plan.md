...
- [x] **フォーマット確認**: `make fmt`
- [x] **Lint チェック**: `make lint`
  - エラーがあれば修正
- [x] **すべてのテスト実行**: `make test`
  - すべてのテストがパスすることを確認
...
- [x] **サンプル実行テスト**:
  ...
  ```
- [x] **実行結果確認**: すべてのサンプルが正常動作することを確認

### 6.3 ドキュメント最終確認

- [x] **リンク切れチェック**: すべてのドキュメント内リンクが有効か確認
- [x] **コード例の一貫性**: すべてのコード例が新フィールド名を使用しているか確認
- [x] **翻訳の一貫性**:
...
- [x] **コミット履歴確認**: `git log --oneline`
  - 各 Phase のコミットが適切に分かれているか
  - コミットメッセージが明確か
- [x] **変更内容確認**: `git diff main`
  - 意図しない変更が含まれていないか
