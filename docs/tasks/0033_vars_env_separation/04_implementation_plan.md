...
- [ ] `internal/runner/config/expansion.go`に追加
  - [ ] `ProcessVars`メソッドを実装:
    ...
    ```
    - [ ] ベース変数をコピー
    - [ ] 各変数定義をパース（`var_name=value`）
    - [ ] 変数名のバリデーション
    - [ ] 予約プレフィックスチェック
    - [ ] `ExpandString`で値を展開（現在の結果マップを使用）
    - [ ] 結果マップに格納
...
- [ ] `internal/runner/debug/inheritance.go`を作成
  - [ ] `PrintFromEnvInheritance`関数を実装:
    ...
    - [ ] Group.from_env の継承/上書き/空配列の状態を表示
...
#### 2.13.5 Phase 13の完了確認
- [ ] dry-run で詳細な変数展開情報が表示されることを確認
- [ ] コミット: "Implement debug trace for variable expansion"
