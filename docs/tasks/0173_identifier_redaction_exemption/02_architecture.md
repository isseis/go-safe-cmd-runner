...
`identifier.Identifier` を構築しない限り、同じ内容の文字列でも 3 へ進む。これが免除が宣言型だけに掛かることの意味である。`identifier.Identifier` の構築は leaf パッケージ `internal/identifier` の `NewIdentifier` だけが行える。定義パッケージ外はコンパイラが、`internal/identifier` 内は `identifier_guard_test.go` の `go/types` 検査が構築形を `NewIdentifier` に限定する（§3.1、§7.3）。
...
leaf パッケージ `internal/identifier` に次の型を置く。フィールドは非公開であり、型の定義パッケージ外は `Identifier` の非公開フィールドに触れられない。したがって、名前を持つ `Identifier` を組み立てられるのは `NewIdentifier` だけである。`internal/common`（`NotificationContext.LogValue`・`CommandResult(s).LogValue`）も `internal/redaction` も同じく定義パッケージ外なので、`identifier.Identifier{name: …}` の要素付き複合リテラルと `id.name = …` のフィールド代入はコンパイラが拒否する（ゼロ値 `identifier.Identifier{}` は構築できるが空名のままで、名前は設定できない）。ただし、コンパイラが守るのは定義パッケージ外の呼び出し元だけである。`internal/identifier` に production ファイルが追加されると、`func FromString(s string) Identifier { return Identifier{name: s} }` のような別のコンストラクタ、`func FromString(s string) Identifier { return NewIdentifier(s) }` のような `NewIdentifier` へ転送するラッパー、`id.name = …` の代入がパッケージ内ではコンパイルでき、呼び出しサイトの目録に `NewIdentifier` の呼び出しが現れないまま免除が掛かる経路ができてしまう。そこで `identifier_guard_test.go`（§7.3）が `internal/identifier` の production ファイルを `go/types` で型検査し、`Identifier` に由来する型（別名・定義型を含む）の複合リテラルと型変換、`name` フィールドへの書き込みが `NewIdentifier` の本体内だけに現れること、および `NewIdentifier` 以外で宣言された関数・メソッドが `Identifier` に由来する型（値・ポインタ）を結果に持たないことを要求する。ファイル選択は `build.ImportDir`（ホスト GOOS／GOARCH のビルド制約で除外されるファイルを落とす）ではなく、`identitymutationguard.ProductionGoFiles(t, dir)` による全 production ファイルの明示列挙とし、platform-tagged な別コンストラクタ（例: `identifier_windows.go`）も検査対象にする。単一集合の型検査が成立しないプラットフォーム別宣言はその時点で失敗するため fail-closed である。判定は構文の綴りではなく、型検査器が解決したシンボルと型（`types.Unalias`・`types.Identical`・フィールドオブジェクトの同一性）で行うため、キーの有無や別名の綴りを列挙する必要がない。パッケージ外の guard は「どの値が宣言されたか」の目録、結果の使用の固定、エイリアス拒否を守る（CLAUDE.md「Enforce invariants with the type, not with convention」）。
...
型を `internal/common` に置くと、同じパッケージの production コード（`notification_context.go`・`logschema.go`）は非公開フィールドを直接書けるため、guard が構築形（複合リテラル・フィールド代入）を 1 つずつ列挙して塞ぐ必要があった。leaf パッケージへ移すと、検査は定義パッケージ内の `NewIdentifier` 本体 1 箇所に限定した `go/types` による型検査で足りる。定義パッケージ外の構築はコンパイラが拒否し、guard は目録と結果の使用の固定、エイリアス拒否に加えて、この限定された構築・書き込み検査と `NewIdentifier` 以外の結果型の拒否を担う（付録 B）。
...
- 新しい設定由来の識別子をログへ載せるときは、そのサイトで `identifier.NewIdentifier` を使い、`identifier_guard_test.go` の目録へファイル・関数・囲むログ呼び出し／文・属性キー・引数式・結果の使用の組と出現数を追加する。戻り値はそのまま属性値・ヘルパー引数として使う。`NewIdentifier(…).Name()` のように戻り値を加工した式は「結果の使用」が拒否し、`internal/identifier` に `NewIdentifier` 以外の関数・メソッドが `Identifier` に由来する型を結果に持つ宣言（転送ラッパー）は `go/types` 検査が拒否する。宣言を削除するときは目録からも取り除く（取り除かなければ、目録にある宣言が走査に見つからず guard が失敗する）。`identifier.NewIdentifier` を変数へ束縛して間接に呼び出すこと（エイリアス）は guard が拒否するため、宣言は常に呼び出しサイトで直接行う。`internal/redaction` にサイト固有の分岐を足す必要はない。
...
- `Identifier` の構築は、定義パッケージ外ではコンパイラが、`internal/identifier` 内では `identifier_guard_test.go` の `go/types` 検査が `NewIdentifier` に限定している。型検査は `build.ImportDir` のホスト GOOS／GOARCH 選別ではなく `ProductionGoFiles` による全 production ファイルの明示列挙で行い、platform-tagged な別コンストラクタも対象にする。`NewIdentifier` 以外の関数・メソッドが `Identifier` に由来する型を結果に持つ宣言（転送ラッパーを含む）は拒否する。消費者側に識別子構築の都合（別の生成規則など）が生じた場合は、消費者へ型を戻すのではなく、leaf パッケージにコンストラクタを追加し、そのコンストラクタを許可するよう guard の型検査と目録を同じコミットで更新する。
...
- **`Identifier` を leaf パッケージ `internal/identifier` に置く理由。** 旧設計は `internal/common` に置き、`internal/redaction` の guard が複合リテラル・フィールド代入を列挙して構築経路を塞いでいた。この方式はレビューで抜け道（件数を保つ移設・複合リテラル・`Identifier.name` への代入）が順次見つかり、全パッケージを対象にした列挙が網羅できないことが確認された。型を定義パッケージ外から触れない leaf パッケージへ移すと、パッケージ外の名前を設定する構築はコンパイラが拒否するため、パッケージ外の構築形検査と対応する AC-19 mutation は削除できる。コンパイラが守らない定義パッケージ内に限り、`go/types` で解決した型とフィールドオブジェクトに基づく検査を guard に残す（§3.1、§7.3。CLAUDE.md「Enforce invariants with the type, not with convention」）。`internal/common` の 2 エンコーダも定義パッケージ外になり、修飾形 `identifier.NewIdentifier` を使う。
- **定義パッケージ内の構築検査を `go/types` にした理由。** 当初は `name` を設定する複合リテラルとフィールド代入を AST で列挙する計画だったが、レビューでキーなし位置指定リテラル・型エイリアス・定義型・型変換の抜け道が順次見つかり、構文の綴りを列挙する方式は収束しないことが確認された。解決済みの型（`types.Unalias`・`types.Identical`）とフィールドオブジェクトの同一性で判定すれば、綴りや形の変種を列挙せずに同じ不変条件を表せる。型検査は leaf パッケージ 1 つに限定でき、`privileged_window_guard_test.go` が同じ方式を既に採っている（§3.1、§7.3）。
...
- **定義パッケージ内の生産者と結果の使用の検査を強化した理由。** レビューで、`NewIdentifier` 以外の関数が `Identifier` を返す転送ラッパー（`func FromString(s string) Identifier { return NewIdentifier(s) }`）と、`NewIdentifier` の戻り値に `.Name()` を適用してログ属性へ渡す式（`identifier.NewIdentifier(entry.CommandName).Name()`）が、それぞれ複合リテラル・型変換の検査と (ファイル, 関数, 引数式) の目録をすり抜けることが確認された。前者は `info.Defs[funcDecl.Name].(*types.Func)` の `Signature().Results()` が `Identifier` 由来型（値・ポインタ）である宣言の拒否で塞ぎ、後者は目録の組に「結果の使用」を足して戻り値が属性値・引数そのものであることの要求で塞いだ。あわせて型検査のファイル選択を `build.ImportDir` から `identitymutationguard.ProductionGoFiles` へ変え、ホスト GOOS／GOARCH のビルド制約で隠れる platform-tagged な別コンストラクタ（例: `identifier_windows.go`）も全 production ファイルの明示列挙で検査する（§3.1、§7.3）。
