...
| ライブラリキャッシュ | ライブラリファイルのパスとハッシュ値をキーとして、システムコールラッパー関数の一覧（関数名 → 実測 syscall 番号）を保存したファイル |
...
`syscall_wrappers` は `number` 昇順でソートして保存する。決定論的な出力とすることで diff が読みやすくなる。
...
複数バージョンの libc が共存する環境でも衝突しないよう、`internal/filevalidator/encoding` パッケージが提供する既存のファイル名エンコーディング方式（[hash-file-naming-adr.ja.md](../../dev/hash-file-naming-adr.ja.md) 参照）を使用してこのパスをエンコードする。
...
キャッシュファイルが存在し、かつキャッシュ内の `lib_hash` が現在のライブラリファイルのハッシュ値と一致する場合、キャッシュを有効とみなす。ハッシュ値が異なる場合（ライブラリ更新時）は再解析を行い、キャッシュを上書きする。
...
1つの関数が複数の `syscall` 命令を含む場合（例: スレッドキャンセル対応のパス分岐）、最初に検出された syscall 番号を採用する。複数の異なる syscall 番号が検出された場合は、その関数をキャッシュに含めない（単純ラッパーとみなせないため）。
...
2. 対応するキャッシュファイルが存在し有効（ハッシュ一致）であれば、キャッシュを読み込む
3. キャッシュが存在しない、または無効（ハッシュ不一致）な場合、libc を解析してキャッシュを生成する
...
本タスクが要求する保存順序（キャッシュ → 記録ファイル）を実現するため、処理フローを以下に変更する：

```
libc キャッシュ参照・生成 → SaveRecord（記録ファイル保存）→ SaveSyscallAnalysis（記録ファイル Update）
```

この変更に伴い以下の実装変更が必要となる：

- `processFiles`（`main.go` の処理ループ）において、`SaveRecord` 呼び出し前に libc キャッシュの参照・生成を行う処理を挿入する
- `analyzeFile` と libc キャッシュ生成を統合または連携させる。`analyzeFile` は `dyn_lib_deps` の取得後に呼び出される必要があるため、`SaveRecord` が返す情報（`contentHash`、`dyn_lib_deps`）を受け取れるよう引数・インターフェースを拡張する
- 既存の「`SaveRecord` 失敗 → `continue`」ロジックの前段に libc キャッシュ処理が来るため、libc キャッシュ失敗時も同様に `continue` せずエラー終了する分岐を追加する

**失敗時の挙動**（すべてのケースで `record` をエラーで終了し、記録ファイルを保存しない）:
...
集約キーを `(Number, Source, Name)` とする。同一の `(Number, Source, Name)` の組み合わせが複数生成される場合は 1 件に集約する。

`open` と `open64` のように異なる Name が同一の syscall 番号に解決された場合は、それぞれ別エントリとして `DetectedSyscalls` に残す。これにより「どのシンボルが照合されたか」が記録から追跡可能となり、監査性と決定性を保証する。

`Source` が異なるエントリ（`syscall` 命令由来と libc import 由来）も別エントリとして残す。

これらの結果として `TotalDetectedEvents` は「何種類の syscall を呼ぶか」ではなく「何通りの根拠で検出されたか」を表す。この意味の変化は設計上の意図であり、セキュリティ判定（`HasNetworkSyscalls`, `IsHighRisk`）は影響を受けない。
...
- [ ] キャッシュファイルに `lib_path`, `lib_hash`, `analyzed_at`, `syscall_wrappers` が含まれること
...
- [ ] キャッシュファイルが存在し `lib_hash` が一致する場合、libc の再解析が行われないこと
...
- [ ] `/usr/bin/mkdir` を `record` した際、`SyscallAnalysis.DetectedSyscalls` に `mkdir`（syscall 番号 83）が含まれること
- [ ] 照合によって検出された `SyscallInfo` の `Source` が `"libc_symbol_import"` であること
- [ ] 照合によって検出された `SyscallInfo` の `Location` が `0` であること
- [ ] 同一 `(Number, Source, Name)` の組み合わせが `DetectedSyscalls` に重複して含まれないこと
- [ ] 異なる Name が同一 syscall 番号に解決された場合（`open` と `open64` 等）、それぞれ別エントリとして共存すること
- [ ] `syscall` 命令由来（`Source` 空）と libc import 由来（`Source: "libc_symbol_import"`）の同一番号は別エントリとして共存すること
...
| `syscall_wrappers` が `number` 昇順でソートされていること | 決定論的出力 |
