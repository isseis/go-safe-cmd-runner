...
- **AC-19**: 絵文字・STATUS・添付の色が「統一書式」の表のとおりログレベルだけで決まり、
  種別によって変わらない。
- **AC-20**: 共通エンベロープの静的な部分、すなわち Text 行の骨格、添付フィールドの見出し
  （Scope、Hostname、Run ID）、および Hostname と Run ID の値に `###` が含まれない。Scope の
  値や要約に入る group 名・コマンド名などの動的な値は本基準の対象外とし、§3.5 の表示安全な
  補間契約（1 行化・実体参照化・長さ上限）で扱う。動的な値は補間契約を通しても `###` を
  含みうるため、メッセージ全体を対象にすると満たしようのない基準になる。
