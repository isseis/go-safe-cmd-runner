...
| 特権監査 | `privilege.logElevationOutcome` が native root と `seteuid` の結果を記録し続けることを、新規テストで検証する。既存の `TestWithPrivileges_WritesNoRecordWhileElevated` は native root の記録が存在することまでしか assert せず、記録の属性を固定するテストは無いため、Phase 3 で追加する。呼び出し元から `logElevationOutcome` の呼び出しを取り除くと失敗する形にし、関数本体だけの検証にしない | AC-05 |
