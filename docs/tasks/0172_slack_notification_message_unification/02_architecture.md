...
ゼロ値が `ScopeGlobal` であることには意味がある。スコープ属性を持たないレコードは、最も仮定の少ない解釈であるグローバルとして扱われる。

**構築の入口をコンストラクタに限る理由**。フィールドが公開されていると、グループ名を入れ忘れた `ScopeGroup` の値をパッケージ外で作れてしまう。そうした値は「グループで起きたと主張しているのに、どのグループか言えない」通知になる。コンストラクタだけを入口にすれば、この形の値はパッケージ外からは作れない。
...
| 制約 | 実現方法 | 理由 |
|---|---|---|
| 改行と制御文字を含めない | 該当文字を別の文字へ置き換える | 改行を通すと、本物の見出し行の直下に任意の行を作れる（5.1 の脅威2） |
...
| 長さに上限を設け、超えた分は切り詰める | 置き換えの後に上限で切り詰める | Text 行はプッシュ通知に出る 1 行であり、長大な本文が入ると読めなくなる |
...
置き換えの順序は、制御文字の除去、`&`・`<`・`>` の置き換え、mrkdwn 記号の置き換え、切り詰めの順に固定する。`&` の置き換えを先に行わないと、後段が入れた `&amp;` の `&` が二重に置き換えられる。mrkdwn 記号の置き換えは `&amp;` のような実体参照の中に該当文字を作らないため、この順序で干渉は起きない。
...
| スコープの描画 | 6.2 の表の全行（余分な発生箇所の情報を伴うスコープが `(global)` ではなく `(scope: invalid)` になることを含む） | AC-12, AC-15 |
...
| Text 行の偽装（改行） | 改行と製品名の接頭辞を含むコマンド名・レコード本文を与えても、Text 行が 1 行のままであり、偽の見出し行が作られないこと | 脅威2 |
...
**スコープの矛盾をコンストラクタで拒否する案**。`GroupScope` がエラーを返す形も検討した。採らなかったのは、読み側が受け取るのは型ではなくレコードの属性であり、コンストラクタで拒否しても読み側が矛盾した組み合わせを受け取る可能性は残るためである。拒否の位置は描画の直前に置き、通知の上で表に出す。
