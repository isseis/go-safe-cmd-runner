...
| 制約 | 理由 |
|---|---|
| 改行と制御文字を含めない（別の文字へ置き換える） | 改行を通すと、本物の見出し行の直下に任意の行を作れる（5.1 の脅威2） |
| mrkdwn の記号（`*`、`` ` ``、`_`、`~`）をそのまま強調として解釈させない | コマンド名や本文の記号で行の書式が崩れるのを防ぐ |
| 長さに上限を設け、超えた分は切り詰める | Text 行はプッシュ通知に出る 1 行であり、長大な本文が入ると読めなくなる |
...
| Text 行の偽装 | 改行と製品名の接頭辞を含むコマンド名・レコード本文を与えても、Text 行が 1 行のままであり、偽の見出し行が作られないこと | 脅威2 |
