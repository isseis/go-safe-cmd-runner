...
| 後始末の隙 | staging フォールバックでのみ、子プロセスの終了後に開く特権の隙。中で行うのは staged copy の削除だけ |
...
後始末の隙は staging フォールバックのときにだけ開く（§3.4、§5.2）。
...
3. **外から見える挙動は、意図して変える所だけを変える。** 終了コード、標準出力、`OutputWriter` への
   ...
   対応表（§4.3）で1件ずつ突き合わせ、意図して変える2点（タイムアウトのエラー、
   `OutputWriter` が `nil` のときの標準エラー出力）はその表と §5.6 に明記する。
...
キャンセルが起きたときの kill の隙と、staging フォールバックの後始末の隙は、監督フェーズの中で
開く（§3.3、§3.4）。
...
graph TB
    ...
    class E1,P1 enhanced
    class E2,E3 newpkg
    class E4,E5,O1 process
```

矢印 A → B は「A が B を使う」を表す。緑は本タスクで実装を変える既存の部品、紫は本タスクで
追加する型、橙は実装を変えない部品である。`outputWrapper` と `Capture` は型も挙動も変わらず、
doc コメントだけを更新する（§3.6）ので橙に置いている。`executor.go` の節点が2つあるのは、同じ
ファイルの中で役割の異なる2つの型を示すためである。
...
`seteuid(0)` から `seteuid(originalUID)` までが起動の隙である。書き込み端を閉じるのが隙の内側に
あるのは、`Start()` の直後に閉じるという不変条件（§3.2）を、`Start()` が失敗した場合も含めて
1箇所で守るためである。
...
// completes execCmd.Path and execCmd.Args[0] inside the privilege window; for
...
| フェーズ | 関数 | 行うこと | 特権 |
|---|---|---|---|
| 準備 | `prepareCommand` | 引数検査、`exec.Command` の組み立て、`os.DevNull` の open、環境変数の組み立て、出力ポンプの生成、fd-bound 実行のための記述子複製、`binding`／`kill` の宣言、`ctx.Err()` の確認 | 不要 |
| 起動 | `startPrepared` | staging フォールバック時の複製作成と `execCmd.Path`／`Args[0]` の確定（§3.4）、`execCmd.Start()`、親側の書き込み端の解放 | 起動の隙の内側（run-as のときのみ隙を開く） |
...
// releaseChildEnds closes the parent's copies of the write ends. The start
// phase calls it immediately after Start returns -- success or failure -- or
// the read ends never reach EOF and wait blocks forever.
...
func (p *outputPump) wait(deadline time.Duration) (stdout, stderr []byte, writeErr error)
...
1. **`outputWrapper` を再利用する。** 収集（`bytes.Buffer`）、`OutputWriter` への転送、
   最初の書き込みエラーの保持という責務は現在の
   [`outputWrapper`](../../../internal/runner/base/executor/executor.go#L643) がすでに持つ。
   ポンプはその呼び出し元が `os/exec` から自分へ替わるだけで、型そのものは変えない。
...
3. **書き込み端は起動フェーズが `Start()` の直後に閉じる。** 閉じ忘れると子プロセスが終了しても
   読み取り端が EOF に達せず、`wait` が戻らない。所有者と時期を1箇所（`startPrepared`）に定め、
   `Start()` が失敗した経路も同じ呼び出しを通す。閉じられなかった場合は記述子が残り、以後の
   `wait` が deadline まで戻らなくなるため、`releaseChildEnds` のエラーは握り潰さずに
   起動フェーズのエラーとして返す。
4. **両端は `O_CLOEXEC` でなければならない。** `os.Pipe` はこれを満たす。満たさない作り方をすると、
   読み取り端が別 UID の子プロセスへ漏れ、その子が生きている限り EOF が起きなくなる。
...
6. **`OutputWriter` が `nil` の経路には上限を持たせる。** この経路は現在
   `execCmd.Output()` を使っており、標準エラー出力は `os/exec` の内部バッファで 32 KiB に
   抑えられている。ポンプの `bytes.Buffer` には上限が無いため、そのまま置き換えると
   大量の標準エラー出力でメモリを使い切る。`newOutputPump` の `stderrLimit` はこの上限であり、
   `OutputWriter` が `nil` のときだけ有効にする（`OutputWriter` がある経路の上限は
   `Capture.MaxSize` が担う）。詳細と現在との差は §4.3 と §5.6 に示す。
...
- **kill の後の回収。** kill の後は待機結果を待つが、`killGraceDelay` を上限とする。子プロセスが
  パイプの書き込み端を持ったまま離れた孫プロセスを残した場合、あるいは kill 自体が失敗した場合に、
  ここで無限に止まらないためである。上限を越えたときはポンプの読み取り端を閉じ、
...
classDiagram
    class DefaultExecutor {
        ...
        -startPrepared(pc *preparedCommand) error
        -superviseCommand(ctx context.Context, pc *preparedCommand) (*Result, error)
    ...
    class outputPump {
        ...
        -start()
        -wait(deadline time.Duration) ([]byte, []byte, error)
        -release() error
    }
...
`Validate`／`stageFromFD` の署名、`OutputWriter` の署名、`outputWrapper` のフィールドとメソッドは
現在の実装と同じである。`PrivilegeManager` は
...
`superviseCommand`／`preparedCommand`／`stagingRequest`／`outputPump`／`pumpStream` が
本タスクで追加される。
...
| `internal/runner/base/executor/output_pump.go` | 新規 | `outputPump`、`pumpStream`、パイプの生成・解放・読み取り | - |
| `internal/runner/base/executor/executor.go` | 変更 | `executeWithUserGroup`／`executeNormal` を3フェーズ構成へ組み替え、`executeCommandWithPath` を廃止、`prepareExecCommand` を準備フェーズと起動フェーズへ分割、昇格時間の計測を `startPrepared` の前後だけへ狭め、kill の隙の記録方法を決め、`outputWrapper` の doc コメントを更新 | `executor_test.go`、`executor_fdexec_test.go`、`executor_privilege_check_test.go`、`executor_usergroup_test.go` |
| `internal/runner/base/executor/stagefromfd_test.go` | 変更 | 位置づけの更新（§3.4） | 同左 |
| `internal/runner/base/executor/privileged_test_condition_test.go` | 変更 | `canRunPrivilegedIntegrationTest` に「実 UID が 0 でないこと」を追加（§7.3） | `TestCanRunPrivilegedIntegrationTest` |
| `internal/runner/base/privilege/unix.go` | 変更 | `WithPrivileges` の doc コメントから出力コピー goroutine の記述を除き、残る未解決課題（`Start()` 中の露出、kill の隙・後始末の隙、別プロセス化の是非）を明示（AC-20） | `unix_privilege_test.go`（挙動は変わらないため確認のみ） |
...
| 4 | `ErrKillAfterCancel`／`ErrChildNotReaped` | kill できなかった、または回収できなかった場合。`ctx.Err()` を含めて返す |
...
| `OutputWriter` が `nil` のとき、標準エラー出力は 32 KiB（先頭と末尾を残し中間を省略）に抑えられる | `Cmd.Output()` | 上限は同じ大きさを保つが、**中間ではなく末尾を落とす**（§3.2 要点6）。`os/exec` の内部型を再実装しないためであり、失う情報は上限を超えた場合の末尾に限られる。§5.6 に差分として記す |
...
| 後始末の隙 | staging フォールバックのときだけ | staged copy の削除（`os.RemoveAll`） | ディレクトリ1つの削除 | 読み取り goroutine（終了済みの場合が多い）、Slack 送信ワーカー |
...
原則3のとおり、変えるのは次の2点だけである。どちらも実装計画書でテストと結び付ける。
...
| `OutputWriter` が `nil` のときの標準エラー出力の切り詰め方 | 32 KiB を超えると中間を省略（先頭と末尾を残す） | 32 KiB を超えると末尾を落とす | `os/exec` の内部型を再実装しないため（§4.3） |
...
| `killGraceDelay` の間に子が終わらない（孫プロセスがパイプを保持しているなど） | ポンプの読み取り端を閉じ、`ErrChildNotReaped` に PID を添えて返す。`Error` で記録する |
| 隙の中で復帰に失敗した | 既存の `emergencyShutdown` が即座にプロセスを終える。子プロセスは残るため、その旨を doc コメントに記す |
...
| 資源 | 所有者 | 解放する場所 |
|---|---|---|
| パイプの書き込み端（親側の複製） | `outputPump` | 起動フェーズ、`Start()` の直後（成功・失敗の別を問わない） |
| パイプの読み取り端 | `outputPump` | 読み取り goroutine の終了時。`start` へ到達しなかった経路では `preparedCommand.release()` |
| 複製した検証済み記述子（`ExtraFiles`） | `preparedCommand` | 起動フェーズの終わり。`Start()` の成否を問わず、`Wait()` を待たずに閉じる |
| `os.DevNull` | `preparedCommand` | 監督フェーズの最後、または `release()` |
| staged copy とその親ディレクトリ | `preparedCommand` | 子プロセスの終了後、後始末の隙の中。`Start()` が失敗した場合は起動の隙の中 |
...
| 互換性 | 終了コード・標準出力・標準エラー出力・エラー種別が現在と一致すること。`OutputWriter` が `nil` の経路で、正常終了時に標準エラー出力が載らないことと、32 KiB を超える標準エラー出力が上限で切れること（AC-16、§5.6） |
...
`runtime.Stack(all=true)` を採り、executor パッケージのフレームを持つ goroutine が呼び出し元
以外に無いことを確かめる形で検査する。特権を要さないので通常のユニットテストとして走る。
...
| 隙 | 許可する呼び出し |
|---|---|
| 起動の隙 | `execCmd.Start`、`outputPump.releaseChildEnds`、`stageFromFD`、および `stageFromFD` の内側で必要な `os.MkdirTemp`／`syscall.Dup`／`os.NewFile`／`os.OpenFile`／`io.Copy`／`os.Chmod`／`os.Chown`／`os.RemoveAll`／`(*os.File).Stat`／`(*os.File).Close` |
...
**スキップ判定の強化。** 現在の `canRunPrivilegedIntegrationTest` は実効 UID が 0 であることしか
...
何も検証しないまま緑になる。そこで判定に **実 UID が 0 でないこと** を加える。実行には、
root 所有の setuid ビットを立てたテストバイナリ、または実 UID を非 root に落としてから走らせる
仕組みが要る。その手順は実装計画書に記し、条件が揃わない環境では理由を示してスキップする。
...
`elevation_count` と経過時間を読む。あわせて、計測の範囲を `startPrepared` の前後だけに狭める
変更（§3.6 の `executor.go` の行）が要る。現在の計測は `WithPrivileges` 全体を囲んでおり、
そのままではコマンドの実行時間を含み続ける。
...
| AC-13、AC-14 | 書き込みエラー時の読み取り端の close を落とす |
...
| Phase | 内容 | 完了の目安 |
|---|---|---|
| 1 | 出力ポンプの追加と、`Stdout`／`Stderr` を `*os.File` へ切り替え（F-001） | 既存テストが緑。AC-01、AC-15、AC-16 のテストが通る |
| 2 | 準備・起動・監督の3フェーズへの分解（`WithPrivileges` の範囲はまだ変えない） | 外から見える挙動が変わらないこと |
| 3 | `exec.CommandContext` の置き換えとキャンセル・kill の実装（F-003） | AC-07、AC-10〜AC-12 のテストが通る。通常実行のタイムアウトが働く |
| 4 | `WithPrivileges` の範囲を `startPrepared` へ縮小、staging の位置づけ変更（F-002、§3.4） | AC-02、AC-04、AC-06 のテストと静的検査が通る |
...
| AC-16 | 標準エラー出力の内容が現在と一致する | `OutputWriter` が `nil` のとき、32 KiB 超過分は中間ではなく末尾を落とす | `os/exec` の内部型を再実装しないため（§4.3、§5.6） |
