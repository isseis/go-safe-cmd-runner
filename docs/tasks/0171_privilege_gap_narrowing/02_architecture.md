...
**特権管理ロジック自身が隙の中で出す記録（残存リスク）。** 本タスクは executor 側の隙の中から
ログを取り除き、`WithPrivileges` 自身が出していた `Debug` 2件も隙の外へ移した。残るのは
`escalatePrivileges` の `Info`（「Privileges elevated」）1件で、これは `seteuid(0)` の直後、
すなわち隙の中で出る。移さないのは、この記録の意味が「いつ昇格したか」であり、復帰の後へ
動かすと監査の時系列が実際と食い違うためである。**したがって「隙の中では何もログしない」は
executor の隙の本体についての性質であって、プロセス全体の不変条件ではない。** §7.2 の静的検査が
固定するのも executor の隙の本体だけである。露出は1回のハンドラ呼び出しに限られ、
§5.3 の他の残存リスク（読み取り goroutine の slog 呼び出し）と同種・同程度である。
