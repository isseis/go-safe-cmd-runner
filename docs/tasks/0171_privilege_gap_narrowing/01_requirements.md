...
現時点で被害が出ていないのは、これらの goroutine が既に開いている記述子への書き込みと slog 呼び出ししか行わないためである。これは `Capture` とハンドラ群の現在の実装がたまたまそうであるという事実にすぎず、設計上の不変条件ではない。ハンドラの1つがファイルを開くようになれば、その `open` はサイレントに root 権限で行われる。
