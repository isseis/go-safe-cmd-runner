...
> **注記**: AC-03／AC-04／AC-06 については、設計上の意図的な差分が
> [02_architecture.md](02_architecture.md) の §3.4「staging フォールバックの位置づけ（要件との差分）」で
> 決定されている。staging フォールバックでは複製の作成も特権の隙の内側に置き、その後始末のために
> もう1組の昇格と復帰を使う。差分の理由と影響範囲は同節を参照のこと。
...
- **AC-05**: 特権の隙の長さがコマンドの実行時間に依存しない。実行時間の異なる2つのコマンドについて、記録される昇格時間（`PrivilegeMetrics` の値）に有意な差が出ない。
...
| 読み取り側の閉じ忘れによるデッドロック | `Wait()` が戻らず、コマンドがハングする | 親側の書き込み側を `Start()` の直後に閉じることを不変条件とし、実行時間の長いコマンドと大量出力のコマンドの両方でテストする |
