...
> **[訂正 - タスク 0104]** 上記「正規の macOS バイナリは `svc #0x80` を含まない」という前提は誤りであることが判明した。Go ランタイムは macOS arm64 においても syscall ディスパッチスタブ内で `svc #0x80` を直接発行するため、正規の Go バイナリの `__TEXT,__text` セクションにも `svc #0x80` が含まれる。`svc #0x80` の存在のみに基づく high risk 判定は Go バイナリ全般に対して偽陽性を生じさせる。詳細および対処方針はタスク 0104 要件定義書を参照。
...
- **対象**: macOS（Darwin）上の Mach-O バイナリ（arm64 のみ。x86_64 Mach-O は対象外）
- **対象**: Fat バイナリ（複数アーキテクチャを含む Mach-O）の全スライス（クロスアーキテクチャ・セキュリティバイパス対策として全スライスを解析）
...
#### FR-3.1.2: Fat バイナリのスライス解析

Fat バイナリを受け取った場合、**全スライスを解析**し、いずれかのスライスにネットワーク関連シンボルまたは `svc #0x80` が検出された場合に脅威とみなすこと。

`debug/macho` パッケージの `FatFile` 型を使用する。

**セキュリティ上の理由**: arm64 スライスのみを解析する場合、攻撃者が「benign な arm64 スライス + malicious な x86_64 スライス」を組み合わせたFatバイナリを作成することで、セキュリティチェックを回避できる（クロスアーキテクチャ・セキュリティバイパス）。これを防ぐため、全スライスを解析し最も深刻な結果を返す。

**優先度順（高→低）**: `NetworkDetected` > `AnalysisError`（直接syscall）> `NoNetworkSymbols`。解析可能なスライスが存在しない場合のみ `NotSupportedBinary` を返す。
...
シンボル解析（FR-3.1.3/3.1.4）でネットワーク関連シンボルが検出されなかった場合、Mach-O バイナリの実行可能セクション（`__TEXT,__text`）を走査し、`svc #0x80` 命令（エンコード `0xD4001001`）の有無を確認すること。
...
**実装上の注記**: arm64 の命令は固定長 4 バイトであるため、`svc #0x80` の検出はリトルエンディアンの 4 バイト列 `01 10 00 D4` のバイトパターン検索で実現できる（設計書参照）。x86_64 は対象外のため `syscall`（`0F 05`）の検索は行わない。
...
- [x] Mach-O バイナリ（arm64）に対して解析が実行されること
  - `TestStandardMachOAnalyzer_NetworkSymbols_Detected` でパス確認
- [x] Fat バイナリに対して全スライスが解析されること
  - `TestStandardMachOAnalyzer_FatBinary_Arm64Selected` でパス確認（arm64+x86_64両スライスにネットワークシンボルあり）
- [x] Fat バイナリでクロスアーキテクチャ・セキュリティバイパスが防止されること
  - `TestStandardMachOAnalyzer_FatBinary_AllSlicesAnalyzed` でパス確認（benign arm64 + malicious x86_64 → `NetworkDetected`）
- [x] 解析可能なスライスが存在しない Fat バイナリは `NotSupportedBinary` として扱われること
  - `analyzeAllFatSlices` の実装で `analyzedAny == false` 時に `NotSupportedBinary` を返す
- [x] 非 Mach-O ファイル（ELF、スクリプト等）に対して解析がスキップされ、エラーにならないこと
  - `TestStandardMachOAnalyzer_NonMachO_Script` でパス確認

### AC-2: ネットワークシンボルの検出

- [x] `socket`, `connect` 等のネットワーク関連シンボルをインポートする Mach-O バイナリが、ネットワーク操作ありと判定されること
  - `TestStandardMachOAnalyzer_NetworkSymbols_Detected` でパス確認
- [x] ネットワーク関連シンボルをインポートしない Mach-O バイナリが、ネットワーク操作なしと判定されること
  - `TestStandardMachOAnalyzer_NoNetworkSymbols` でパス確認

### AC-3: Go バイナリの検出

- [x] `net` パッケージを使用する Go バイナリ（`net.Dial` 等）が、ネットワーク操作ありと判定されること
  - `TestStandardMachOAnalyzer_GoNetwork_Detected` でパス確認
- [x] ネットワーク操作を行わない Go バイナリが、ネットワーク操作なしと判定されること
  - `TestStandardMachOAnalyzer_GoNoNetwork_NoSymbols` でパス確認

### AC-4: フォールバック動作

- [x] `commandProfileDefinitions` に含まれるコマンドは Mach-O 解析を行わず従来のリストベース判定が適用されること
  - `IsNetworkOperation` のプロファイル優先ロジック（`network_analyzer.go`）で実装済み。プロファイル一致時は `isNetworkViaBinaryAnalysis` を呼ばない
- [x] `commandProfileDefinitions` に含まれないコマンドに対して Mach-O 解析が実行されること
  - `TestNetworkAnalyzer_Integration_MachO` で `/usr/bin/curl`（プロファイル外）に対して `NetworkDetected` を確認

### AC-5: 解析失敗時の安全性

- [x] Mach-O 解析が失敗した場合、コマンド実行がブロックされないこと
  - `AnalysisError` 時に `isNetworkViaBinaryAnalysis` が `true` を返してブロックせず通過する実装で担保
- [x] 不正な Mach-O ファイルに対してパニックが発生しないこと
  - `TestStandardMachOAnalyzer_InvalidMachO_NoPanic` でパス確認（破損 Mach-O が `AnalysisError` を返すことを確認）

### AC-6: `svc #0x80` 直接 syscall の high risk 検出

- [x] シンボル解析でネットワーク関連シンボルが検出されなかった Mach-O バイナリに `svc #0x80` 命令が含まれる場合、high risk と判定されること
  - `TestStandardMachOAnalyzer_SVCOnly_HighRisk` でパス確認（`AnalysisError` + `ErrDirectSyscall` を確認）
- [x] シンボル解析でネットワーク関連シンボルが検出された場合、`svc #0x80` の有無に関わらず `NetworkDetected` と判定されること（シンボル解析が優先）
  - `TestStandardMachOAnalyzer_NetworkSymbols_SVCIgnored` でパス確認
- [x] `svc #0x80` を含まない正規の Mach-O バイナリは high risk と判定されないこと
  - `TestStandardMachOAnalyzer_NoNetworkSymbols` で `no_network_macho_arm64` が `NoNetworkSymbols` を返すことを確認

### AC-7: 既存機能への非影響

- [x] Linux 環境での ELF 解析（タスク 0069/0070/0072）が引き続き正常に動作すること
  - `make test`（全 38 パッケージ）で既存テストすべてパス確認
- [x] 既存のテストがすべてパスすること
  - `make test` で全パッケージ PASS 確認
...
1. **`svc #0x80` 検出は存在確認のみ**: macOS では正規バイナリが `libSystem.dylib` 経由で syscall を発行するため、`svc #0x80` の存在自体を異常とみなして high risk 判定する。syscall 番号（`x16` レジスタへの設定値）の解析は行わない。なお、シンボル解析でネットワーク関連シンボルが検出された場合は `svc #0x80` の有無に関わらず `NetworkDetected` を優先する。
   > **[訂正 - タスク 0104]** 正規の Go バイナリも `svc #0x80` を含むため、この前提は正しくない。タスク 0104 要件定義書を参照。
