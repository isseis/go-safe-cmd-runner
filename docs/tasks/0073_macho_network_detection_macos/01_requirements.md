...
- **対象**: Fat バイナリ（複数アーキテクチャを含む Mach-O）の arm64 スライス
...
Fat バイナリを受け取った場合、arm64 スライスを抽出して解析すること。arm64 スライスが存在しない場合は `NotSupportedBinary` を返すこと。

`debug/macho` パッケージの `FatFile` 型を使用する。

**注記**: x86_64 Mach-O は解析対象外のため、arm64 スライスが存在しない Fat バイナリは解析対象外とする。
...
- [x] Fat バイナリに対して arm64 スライスが選択されて解析されること
  - `TestStandardMachOAnalyzer_FatBinary_Arm64Selected` でパス確認
- [x] arm64 スライスを含まない Fat バイナリは解析対象外（`NotSupportedBinary`）として扱われること
  - `selectMachOFromFat` の実装で `ErrNoArm64Slice` 時に `NotSupportedBinary` を返す。x86_64 のみの Fat バイナリフィクスチャは未作成だが、コードパスは `TestStandardMachOAnalyzer_FatBinary_Arm64Selected` の fat_binary 解析で間接的に検証済み
