...
// Returns an error if no arm64 slice is found (x86_64-only Fat binaries are not supported).
...
> **[訂正 - タスク 0104]** 上記の前提は誤りであることが判明した。Go ランタイムは macOS arm64 においても syscall ディスパッチスタブ内で `svc #0x80` を直接発行するため、正規の Go バイナリの `__TEXT,__text` セクションにも `svc #0x80` が含まれる。タスク 0104 要件定義書を参照。
...
| Fat スライス抽出失敗 | arm64 スライスなし（x86_64 のみの Fat バイナリ） | `NotSupportedBinary` | 既存ロジックに委ねる |
...
| Fat バイナリ（arm64 + x86_64） | arm64 スライスで判定 | AC-1 |
| Fat バイナリ（x86_64 のみ） | `NotSupportedBinary` | AC-1 |
