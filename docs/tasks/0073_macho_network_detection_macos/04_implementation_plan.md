...
- [ ] `elfanalyzer/analyzer.go` の定数を更新する
...
- [ ] `elfanalyzer/standard_analyzer.go` の参照を更新する
  - `NotELFBinary` → `NotSupportedBinary` に変更（2 箇所）
  - 仕様: 詳細仕様書 §2.4

### 1.4 既存テストの更新

- [ ] `elfanalyzer/analyzer_test.go` を更新する
...
- [ ] `machoanalyzer/symbol_normalizer.go` を新規作成する
...
- [ ] `machoanalyzer/svc_scanner.go` を新規作成する
...
- [ ] `machoanalyzer/analyzer.go` を新規作成する
...
- [ ] `machoanalyzer/standard_analyzer.go` に定数とヘルパーを実装する
...
- [ ] `machoanalyzer/standard_analyzer.go` に実装する
  - `parseFatBinaryArm64Slice(file safefileio.File) (*macho.File, error)` を実装
    - `macho.NewFatFile(file)` でパース
    - `arch.Cpu == macho.CpuArm64` のスライスを返す
    - arm64 スライスが存在しない場合はエラーを返す
    - **注意**: `fat.Close()` を呼ばず、`arch.File` のライフサイクムは呼び出し側で管理
...
- [ ] `machoanalyzer/standard_analyzer.go` に実装する
  - `parseMachO(file safefileio.File, magic []byte) (*macho.File, *elfanalyzer.AnalysisOutput)` を実装
    - Fat バイナリ: `parseFatBinaryArm64Slice` を呼び出す。arm64 スライスなしは `NotSupportedBinary`
    - 単一 Mach-O: `macho.NewFile(file)` でパース
  - 仕様: 詳細仕様書 §3.3.2, §3.6（parseMachO ヘルパー節）

### 2.7 AnalyzeNetworkSymbols の実装

- [ ] `machoanalyzer/standard_analyzer.go` に `AnalyzeNetworkSymbols` を実装する
...
- [ ] `machoanalyzer/analyzer_test.go` を新規作成する（`//go:build test`）
...
- [ ] `security/network_analyzer.go` のメソッドをリネーム・更新する
...
- [ ] `security/network_analyzer_test_helpers.go` を更新する
...
- [ ] `security/command_analysis_test.go` を更新する
...
- [ ] `svc_only_arm64` を生成する（`svc #0x80` のみを含む最小バイナリ）
  - macOS arm64 でアセンブルしてリンク
  - `__TEXT,__text` に `svc #0x80`（`0x01 0x10 0x00 0xD4`）を1命令含む最小構成
...
- [ ] `machoanalyzer/analyzer_test.go` に macOS 統合テストを追加する
