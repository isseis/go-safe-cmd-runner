...
- [x] `internal/runner/security/elfanalyzer/analyzer.go` を更新する
...
- [x] `internal/runner/security/elfanalyzer/analyzer.go` の定数を更新する
...
- [x] `internal/runner/security/elfanalyzer/standard_analyzer.go` の参照を更新する
  - `NotELFBinary` → `NotSupportedBinary` に変更（2 箇所）
  - 仕様: 詳細仕様書 §2.4

### 1.4 既存テストの更新

- [x] `internal/runner/security/elfanalyzer/analyzer_test.go` を更新する
...
- [x] `internal/runner/security/machoanalyzer/symbol_normalizer.go` を新規作成する
...
- [x] `internal/runner/security/machoanalyzer/svc_scanner.go` を新規作成する
...
- [x] `internal/runner/security/machoanalyzer/analyzer.go` を新規作成する
...
- [x] `internal/runner/security/machoanalyzer/standard_analyzer.go` に定数とヘルパーを実装する
...
- [x] `internal/runner/security/machoanalyzer/standard_analyzer.go` に実装する
...
- [x] `internal/runner/security/machoanalyzer/standard_analyzer.go` に `AnalyzeNetworkSymbols` を実装する
...
- [x] `internal/runner/security/machoanalyzer/analyzer_test.go` を新規作成する（`//go:build test`）
...
- [x] `internal/runner/security/network_analyzer.go` のフィールドをリネームする
  - `elfAnalyzer elfanalyzer.ELFAnalyzer` → `binaryAnalyzer elfanalyzer.BinaryAnalyzer`
  - 仕様: 詳細仕様書 §4.1

### 3.2 NewNetworkAnalyzer の macOS 対応

- [x] `internal/runner/security/network_analyzer.go` の `NewNetworkAnalyzer` を更新する
...
- [x] `internal/runner/security/network_analyzer.go` のメソッドをリネーム・更新する
...
- [x] `internal/runner/security/network_analyzer_test_helpers.go` を更新する
...
- [x] `internal/runner/security/command_analysis_test.go` を更新する
...
- [x] `network_macho_arm64` を生成する（socket をインポートする arm64 C バイナリ）
  ```bash
  ...
  ```
- [x] `no_network_macho_arm64` を生成する（ネットワークシンボルなしの arm64 C バイナリ）
  ```bash
  ...
  ```
- [x] `svc_only_arm64` を生成する（`svc #0x80` のみを含む最小バイナリ）
  ```bash
  ...
  ld -o internal/runner/security/machoanalyzer/testdata/svc_only_arm64 \
      ...
      -arch arm64
  ```
- [x] `fat_binary` を生成する（arm64 + x86_64 Fat バイナリ）
  ```bash
  ...
  ```
- [x] `network_go_macho_arm64` を生成する（net パッケージを使用する Go バイナリ）
  ```bash
  ...
  ```
- [x] `no_network_go_arm64` を生成する（ネットワーク操作なし Go バイナリ）
  ```bash
  GOOS=darwin GOARCH=arm64 go build \
  ...
  ```
- [x] `script.sh` を生成する（非 Mach-O ファイル）
  ```bash
  echo '#!/bin/sh' > \
      internal/runner/security/machoanalyzer/testdata/script.sh
  ```
- [x] `testdata/README.md` に各フィクスチャの説明と生成コマンドを記録する
- [x] フィクスチャをリポジトリにコミットする
...
- [x] `internal/runner/security/machoanalyzer/analyzer_test.go` に macOS 統合テストを追加する
...
- [x] `make build` を実行し全バイナリがビルドできることを確認
- [x] Linux 環境でのクロスビルドが壊れていないことを確認
  ```bash
