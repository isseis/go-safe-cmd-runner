...
| レベル | 内容 | セキュリティ考慮事項 |
|--------|------|---------------------|
| Debug | 変数展開の詳細（変数名、型、展開状態） | **値は出力しない**（機密情報漏洩防止） |
| Info | 設定ファイル読み込み成功 | - |
| Warn | 制限値に近づいた場合（例: 変数数800超） | - |
| Error | 展開エラー、型エラー、制限超過 | エラーメッセージに値を含めない |

**セキュリティ要件**:
- 変数の**値**は絶対にログに出力しない（パスワード、トークン、APIキーなどの機密情報漏洩を防止）
- ログに含めて良い情報:
  - 変数名
  - 変数の型（string, array）
  - 展開状態（expanded, pending, failed）
  - エラーの種類（circular reference, undefined variable, type mismatch）
- エラーメッセージでの制約:
  - `ErrUndefinedVariableDetail` などのエラー型では変数名のみを含め、値は含めない
  - コンテキスト情報（`Context` フィールド）も値を含めない
