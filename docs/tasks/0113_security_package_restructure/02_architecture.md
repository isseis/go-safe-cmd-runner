...
flowchart TD
    ...
    class S1,S2,S3,S4,S5,S6 newpkg;
    class R1,R2,R3,R4 runner;
```
...
flowchart TD
    ...
    subgraph gen_sec ["internal/security/ (新設・汎用)"]
        GS["security.*<br>(DirectoryPermChecker,<br>TOCTOU, NewBinaryAnalyzer)"]
        BA["binaryanalyzer/"]
        EA["elfanalyzer/"]
        MA["machoanalyzer/"]
    end
...
graph TB
    ...
    subgraph internal ["internal/"]
        subgraph security_new ["security/ (新設)"]
            ...
            BAN["binary_analyzer.go<br>NewBinaryAnalyzer()"]
    ...
    class DPC,TC,BAN,ERR,BA2,EA2,MA2 newpkg;
    class VAL,CA,ENV,NET,LOG,TOCTOU_KEEP runner;
```
...
#### 3.1.2 `NewDirectoryPermChecker()` ファクトリ
...
#### 3.1.4 `NewBinaryAnalyzer()` ファクトリ

`internal/runner/security/binary_analyzer.go` から `internal/security/binary_analyzer.go` へ移動。
ロジックは変更しない。

### 3.2 `internal/runner/security` の変更点
...
| `binary_analyzer.go` 削除 | `internal/security` へ移動 |
| `binaryanalyzer/`・`elfanalyzer/`・`machoanalyzer/` 削除 | `internal/security/` へ移動 |
...
sequenceDiagram
    ...
    R->>GS: NewBinaryAnalyzer(runtime.GOOS)
    GS-->>R: binaryAnalyzer
...
- `internal/security/NewBinaryAnalyzer`: 既存の `binary_analyzer_test.go` を移植

### 7.2 統合テスト

- `make test` での全テスト合格を確認
- `go list -deps ./cmd/record/` で `internal/runner/security` が含まれないことを確認
...
7. `internal/security/binary_analyzer.go` 作成
...
15. `internal/runner/security/elfanalyzer/` 削除
16. `internal/runner/security/machoanalyzer/` 削除
17. `internal/runner/security/binary_analyzer.go` 削除（不要な関数等の整理）
