...
flowchart TD
    ...
    subgraph record["record コマンド（解析・保存）"]
        ...
        C --> J[("SyscallAnalysisData\nv16 保存")]
    end

    subgraph verify["verify 時（ネットワーク判定）"]
        K[("SyscallAnalysisData\nv16 読み込み")] --> L["isNetworkViaBinaryAnalysis()"]
        L --> M{"DetectedSyscalls に\nIsNetwork=true あり?"}
        M -->|"Yes"| N["true, true"]
        M -->|"No"| O["SymbolAnalysis 判定へ委譲"]
    end
...
graph TB
    ...
    subgraph "変更ファイル"
        subgraph "internal/fileanalysis"
            SA["schema.go\nCurrentSchemaVersion 15→16"]
        end

        subgraph "internal/libccache"
            LA["macho_analyzer.go\nbackwardScanX16 → BackwardScanX16 (公開)"]
        end

        subgraph "internal/filevalidator"
            VA["validator.go\nanalyzeMachoSyscalls() 置き換え"]
        end

        subgraph "internal/runner/security"
            NA["network_analyzer.go\nsyscallAnalysisHasSVCSignal 廃止\nIsNetwork ベース判定に変更"]
        end
    end

    subgraph "新規ファイル（internal/runner/security/machoanalyzer/）"
        AN["syscall_number_analyzer.go\nMachoSyscallNumberAnalyzer\n（Pass1 + Pass2 統合）"]
        P1["pass1_scanner.go\nscanSVCWithX16()\nGo スタブ除外 + X16 解析"]
        P2["pass2_scanner.go\nMachoWrapperResolver\nスタック ABI 呼び出しサイト解析"]
        PC["pclntab_macho.go\nParseMachoPclntab()\nMach-O 用 pclntab 読み込み"]
    end
...
sequenceDiagram
    ...
    AN-->>V: SyscallAnalysisResult（Pass1 + Pass2 マージ済み）
...
ELF の `SyscallAnalyzer` と同等の役割。戻り値は `fileanalysis.SyscallAnalysisResult`（`common.SyscallAnalysisResultCore` 埋め込み）を使用して既存の保存・読み込みパスを変更なしに再利用する。
...
`libccache.BackwardScanX16` を呼ぶには `backwardScanX16` を公開名（`BackwardScanX16`）に変更する。`BackwardScanX16` は `libccache` パッケージの安定公開 API となるため、将来のシグネチャ変更時は互換性に注意する。
...
ELF 版との違い: ELF arm64 は第1引数が X0 レジスタ（レジスタ ABI）だが、macOS arm64 では旧スタック ABI により第1引数は `[SP, #8]`。この `[SP, #8]` オフセットは Go 1.20+ の `NOSPLIT` フレーム規約に基づく値であり、Go 1.20 未満でのサポートは対象外。
...
| `SyscallAnalysis == nil`（`nil, nil` 返却） | `false, false` | `false, false`（変更なし） |
...
graph LR
    ...
    ELF["elfanalyzer\n（変更なし）"] --- COMMON["common/syscall_types.go\n（変更なし）"]
    COMMON --- FSA["fileanalysis/schema.go\nv15→v16"]
    FSA --- STORE["fileanalysis/syscall_store.go\n（変更なし）"]
    STORE --- NA["network_analyzer.go\n判定ロジック変更"]
    LC["libccache/macho_analyzer.go\nBackwardScanX16 公開"] --- P1["machoanalyzer/pass1_scanner.go\n（新規）"]
    LC2["libccache/MacOSSyscallTable\n（変更なし）"] --- P1
    LC2 --- P2["machoanalyzer/pass2_scanner.go\n（新規）"]
    P1 --- ANALY["machoanalyzer/\nsyscall_number_analyzer.go\n（新規）"]
    P2 --- ANALY
    PT["machoanalyzer/pclntab_macho.go\n（新規）"] --- ANALY
    ANALY --- FVAL["filevalidator/validator.go\nanalyzeMachoSyscalls 変更"]
