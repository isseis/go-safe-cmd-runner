...
flowchart TD
    ...
    subgraph record["record コマンド（解析・保存）"]
        ...
        C --> J[("SyscallAnalysisData<br>v16 保存")]
    end

    subgraph verify["verify 時（ネットワーク判定）"]
        K[("SyscallAnalysisData<br>v16 読み込み")] --> L["isNetworkViaBinaryAnalysis()"]
...
graph TB
    ...
    subgraph "変更ファイル"
        ...
        subgraph "internal/filevalidator"
            VA["validator.go<br>analyzeMachoSyscalls() 置き換え"]
        end
...
graph LR
    ...
    ANALY --- FVAL["filevalidator/validator.go<br>analyzeMachoSyscalls 変更"]
