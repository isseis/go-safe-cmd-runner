...
| `SyscallAnalysis` が nil（`LoadSyscallAnalysis` が `nil, nil` を返す）| `false, false` | `SymbolAnalysis` 判定へ委譲 |
...
| `SyscallAnalysis` が nil（`LoadSyscallAnalysis` が `nil, nil` を返す）| `runner` が `SymbolAnalysis` 判定へ委譲すること |
