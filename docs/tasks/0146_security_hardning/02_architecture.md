...
本タスクは、信頼境界を跨ぐ 3 つの面（**下方委譲**・**外部流出**・**信頼の起点**）を堅牢化する。
...
flowchart TB
    ...
    subgraph Down["下方委譲面（特権分離）"]
        P1["privilege<br>UnixPrivilegeManager"]
        E1["executor<br>DefaultExecutor"]
    end

    subgraph Out["外部流出面（機微情報）"]
        R1["redaction<br>ValueDetector"]
        S1["logging<br>SlackHandler"]
    end

    subgraph Trust["信頼の起点／プレビュー"]
        REC["cmd/record"]
        VER["verification<br>dry-run fallback"]
    end

    ROOT[("起動元 identity<br>root / setuid-root")] --> P1
    P1 --> E1
    E1 --> TARGET[("run_as 対象ユーザー<br>identity")]
    R1 --> S1
    REC --> HDB[("ハッシュDB<br>信頼の起点")]
    HDB --> VER

    class P1,E1,R1,REC,VER enhanced
    class S1 process
    class ROOT,TARGET,HDB data
...
> 矢印 A → B は「A が生成・制御・供給する対象が B である」という制御・データの流れを表す。
...
| `cmd/record/main.go` | F-005 | `RunTOCTOUPermissionCheck` の戻り値を評価し、違反時は新設の明示フラグ `--allow-insecure-perms` 未指定なら ERROR ログ出力のうえ非ゼロ終了（ハッシュ生成せず）。`hashDirPermissions` を `0o750` → `0o700`。 | `main_test.go` |
...
flowchart TD
    ...
    Has -->|"Yes"| Force{"--allow-insecure-perms<br>指定?"}
    Force -->|"No"| Fail["ERROR ログ (違反パス/理由)<br>→ 非ゼロ終了・ハッシュ生成せず"]
    Force -->|"Yes"| Warn["ERROR ログのうえ続行"]
    Warn --> Gen
    Gen --> End(["完了"])
    Fail --> EndF(["異常終了"])

    class Chk,Has,Force,Fail enhanced
...
- `RunTOCTOUPermissionCheck` の戻り値（`[]TOCTOUViolation`）を評価し、違反が 1 件以上あり
  新設フラグ `--allow-insecure-perms` 未指定なら、**違反ディレクトリと具体的な権限問題を ERROR ログに出力**の
  うえハッシュレコードを生成・保存せず非ゼロ終了する（AC-16）。
  - **権限違反バイパスの指定は既存 `--force` と分離する**。`--force` は「既存ハッシュファイルの上書き」という日常的で
    無害な指定であり、これに権限違反バイパスを兼ねさせると、再記録のため常用する運用者が信頼の起点の
    fail-closed を**気付かず無効化**してしまう。そのため権限違反の意図的無視は独立した明示フラグ
    `--allow-insecure-perms` とし、指定時もどの違反をバイパスしたかを ERROR で残す（AC-18）。
...
- **F-005**: `mkdirAll` と TOCTOU チェッカを注入し、違反あり・`--allow-insecure-perms` 無で ERROR ログ＋
  非ゼロ終了かつ未生成、違反あり・`--allow-insecure-perms` 有で続行、`--force`（上書き）が権限バイパスに
  作用しないこと、新規作成時の権限 `0o700` を検証。
...
- **Phase 3 — F-005 / F-004**: record の fail-closed 化・`0o700`・`--allow-insecure-perms`、
...
- record の権限違反バイパスは既存 `--force`（ハッシュ上書き）と分離した明示フラグ `--allow-insecure-perms`
  とする（`--force` 常用による fail-closed の無自覚な無効化を避けるため。§3.5）。
