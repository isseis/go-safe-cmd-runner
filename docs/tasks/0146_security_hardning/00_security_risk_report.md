...
| F-5 | **Medium** | `record` コマンドの TOCTOU/ディレクトリ権限チェックが警告のみで、信頼の起点であるハッシュDB生成を止めない | `cmd/record/main.go` |
...
### F-5 [Medium] record のディレクトリ権限チェックが警告のみ
...
**重要度判定の根拠**: 本件は「堅牢化の余地」ではなく、信頼の起点の fail-open である。
`RunTOCTOUPermissionCheck` は違反を検出できているにもかかわらず、戻り値を破棄して続行する
設計であり、これはシステム全体の検証を無効化し得る。悪用には攻撃者が既にハッシュディレクトリ
またはその祖先への書き込み権限を持つ必要があるため High ではないが、Low としていた F-4
（dry-run の未検証取り込み、dry-run は実行しないため直接被害が限定的）と比較して影響度が
明らかに大きく、Medium が妥当と判断する。後続の要件・アーキテクチャ文書で「バイパスフラグ
設けない」「`--force` に権限バイパスを兼ねさせない」等の厳格な対応を規定していることとも
整合する。
...
3. **F-5（信頼の起点の fail-open 是正）** — ハッシュDB は本システムの信頼の起点であり、その fail-open はシステム全体の検証を無効化し得るため F-4 より優先。**F-4** — dry-run の安全側デフォルトを固める（F-5 より後に実施）。
