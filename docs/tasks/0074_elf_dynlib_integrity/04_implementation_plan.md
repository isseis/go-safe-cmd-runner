...
- `CurrentSchemaVersion` を 1 → 3 に上げ、旧記録を一律拒否する
...
**目標**: `fileanalysis.Record` を拡張し、`DynLibDeps` と `HasDynamicLoad` を格納できるようにする。スキーマバージョンを 3 に上げる。
...
```go
const (
    CurrentSchemaVersion = 3  // 1 → 3 に変更
)
```
...
if errors.As(err, &schemaErr) {
    ...
    // Older schema (e.g. v1 or v2 record, current binary is v3):
...
- [x] `CurrentSchemaVersion` が 3 に変更されていること
...
- [x] `schema_version: 1` または `schema_version: 2` の記録で `SchemaVersionMismatchError` が返ること
...
**リスク**: バージョン 1 または 2 から 3 への変更後、全管理対象バイナリの `record --force` 再実行が必要。失念すると `runner` が全コマンドをブロックする。
...
| 3 | `TestResolve_LDLibraryPath` | `dynlibanalysis` | LD_LIBRARY_PATH の enabled/disabled 差異 |
| 4 | `TestResolve_LDCache` | `dynlibanalysis` | ld.so.cache 経由の解決 |
| 5 | `TestResolve_DefaultPaths` | `dynlibanalysis` | アーキテクチャ別デフォルトパス |
| 6 | `TestResolve_Failure` | `dynlibanalysis` | 解決失敗エラーの内容検証 |
| 7 | `TestParseLDCache_NewFormat` | `dynlibanalysis` | 新形式キャッシュのパース |
| 8 | `TestParseLDCache_NotFound` | `dynlibanalysis` | キャッシュ不在時の動作 |
| 9 | `TestParseLDCache_UnsupportedFormat` | `dynlibanalysis` | 非対応フォーマット |
| 10 | `TestParseLDCache_Truncated` | `dynlibanalysis` | データ切れ |
| 11 | `TestLDCache_Lookup` | `dynlibanalysis` | soname → パス検索 |
| 12 | `TestNewRootContext` | `dynlibanalysis` | ルートコンテキスト初期化 |
| 13 | `TestNewChildContext` | `dynlibanalysis` | 子コンテキスト初期化（RUNPATH は継承されない） |
| 14 | `TestAnalyze_DynamicELF` | `dynlibanalysis` | 動的 ELF の解析 |
...
| 27 | `TestVerify_SchemaVersion` | `verification` | schema_version 1 or 2 → SchemaVersionMismatchError |
...
### Phase 1: スキーマ拡張
- [x] `fileanalysis/schema.go`: `CurrentSchemaVersion = 3`（1 → 3 に変更）
...
- [x] `dynlibanalysis/resolver_context.go`: `ResolveContext`（`OwnRUNPATH` のみ）, `NewRootContext`, `NewChildContext`（継承なし）, `expandOrigin`
- [x] `dynlibanalysis/resolver.go`: `LibraryResolver`, `Resolve`（4 段階）, `tryResolve`
...
- [x] `dynlibanalysis/analyzer.go`: `DynLibAnalyzer`, `Analyze`（DT_RPATH 検出→エラー, `traversalKey` は resolvedPath のみ）, ヘルパー関数
