...
**解析精度の前提と限界**:
本手順はシンボル情報（`.symtab` または `.dynsym` の `STT_FUNC` エントリ）が利用可能な場合に機能する。配布済み共有ライブラリの多くは `strip` 済みで `.symtab` を持たず、`.dynsym` には exported シンボルのみが含まれる。このため、内部関数経由のネットワーク呼び出しは追跡できず、**false negative が高い確率で生じる**。方策 C は精度が保証できないことを前提として設計すること（FR-3.5.5 の間接呼び出し制約と同様に許容する）。
...
> **stripped ライブラリでの制約（重要）**: 配布済み共有ライブラリの多くは `.symtab` が存在しない（`strip` で除去済み）。この場合、`.dynsym` の exported シンボルのみが境界情報を持ち、内部関数の境界は不明となる。その結果、内部関数が PLT 経由でネットワーク API を呼び出していても、ステップ 3 の伝播でその内部関数をノードとして扱えず、exported 関数から内部関数への呼び出しチェーンが追跡できない。この限界により、stripped ライブラリでは **false negative（見逃し）が高い確率で生じる**。方策 C は stripped な実ライブラリを主な解析対象とする場合、精度保証ができない点に注意すること（FR-3.5.5 の間接呼び出し制約と同様に許容する）。

**ステップ 3: 内部呼び出し伝播**

`.text` 内の関数呼び出しグラフを構築し、ネットワーク呼び出し関数を呼び出す（直接・間接）全関数を求める。

> **stripped ライブラリでの制約**: ステップ 2 の制約に起因し、内部関数の境界が不明な場合は関数呼び出しグラフに内部関数ノードを追加できない。伝播はシンボル境界が判明している関数（`.dynsym` の exported 関数）の範囲内でのみ機能する。
