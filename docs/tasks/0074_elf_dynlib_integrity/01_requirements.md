...
3. **間接依存ライブラリの解決**: 本タスクでは間接依存（依存ライブラリが依存するライブラリ）も再帰的に解決し、完全な依存ツリーを検証対象とする（FR-3.1.5 参照）。これにより、直接依存する `.so` の背後にある `.so`（例: `libssl.so.3` が依存する `libcrypto.so.3`）の差し替えも検出できる。
