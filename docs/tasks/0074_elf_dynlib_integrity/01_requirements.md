...
- **方策 A**: `record` 時に ELF バイナリの `DT_NEEDED` から依存ライブラリを解決し、各ライブラリのフルパスとハッシュを記録する。`runner` 実行時に現在のライブラリのハッシュと照合し、差し替えを検出する。（ケース 2 への**部分的な**対策：「`record` 時と異なるライブラリへの差し替え」は検出できるが、「依存ライブラリが実際にネットワーク API を呼ぶかどうか」は判定しない）
...
FR-3.1.2 の「デフォルトパス」は、解析対象バイナリの ELF ヘッダ（`e_machine` フィールド）から判定したアーキテクチャに応じて以下のディレクトリを順に試みる：
...
**vDSO（仮想共有オブジェクト）の扱い**:
`linux-vdso.so.1`（x86_64）や `linux-gate.so.1`（32bit x86）などの仮想共有オブジェクトは、カーネルが実行時にプロセスのアドレス空間に直接マップするため、ファイルシステム上に実体が存在しない。通常これらは `DT_NEEDED` に現れないが（`ldd` の出力には `ld.so` が付加して表示するだけで ELF の `DT_NEEDED` セクションには含まれない）、実装上は以下の方針とする：

- `DT_NEEDED` エントリの中に既知の vDSO 名（`linux-vdso.so.1`、`linux-gate.so.1`、`linux-vdso64.so.1` 等）が含まれていた場合、そのエントリをスキップして解決の失敗扱いにしない
- スキップリストは実装定数として管理し、将来の vDSO 名追加に対応できるようにする
- スキップしたエントリは `DynLibDeps` に記録しない（ファイルシステム上のハッシュ検証が不可能なため）
...
| バイナリの種類 | 代表例 | `dlopen` を使う理由 |
|--------------|--------|-------------------|
| スクリプト言語インタプリタ | `python3`, `ruby`, `perl`, `php` | 拡張モジュール（`.so`）の実行時ロード |
| JVM / 言語ランタイム | `java`, `node` | ネイティブ拡張（JNI / Node.js addon）のロード |
| シェル | `bash`, `zsh` | プラグイン・補完モジュールのロード |
| データベースクライアント | `psql`, `mysql` | 認証プラグイン・ドライバのロード |
| 汎用 CLI ツール | `git`, `gpg` | 暗号ライブラリ・プロトコル実装のロード |

これらのバイナリを管理対象コマンドとして登録すると `NetworkDetected` と判定される。運用担当者はこれを異常ではなく仕様と理解した上で、当該コマンドのネットワーク使用をシステム管理者が承認済みであることを確認すること。
...
4. **`/etc/ld.so.cache` フォーマットの差異**: 異なる Linux ディストリビューションや `glibc` バージョンでフォーマットが異なる可能性がある。古いフォーマット（`CACHEMAGIC` のみ）は対象外とし、解析失敗時は FR-3.1.2a のデフォルトパス（multiarch ディレクトリを含む）のみで解決を試みる。デフォルトパスでも解決できない場合は FR-3.1.7 に従い `record` をエラーで終了する（FR-3.1.5 参照）。

5. **依存ライブラリ経由のネットワーク到達可能性は判定しない（ケース 2 の未解決部分）**: 方策 A は「依存ライブラリが `record` 時から差し替えられていないこと」を保証するが、「そのライブラリ（または依存ツリー上のいずれかの `.so`）が実際にネットワーク API を呼ぶかどうか」は判定しない。たとえばバイナリが `libfoo.so` を `DT_NEEDED` で参照し、`libfoo.so` が内部的に `connect()` を呼ぶ場合、バイナリの `.dynsym` には `connect` が現れないため、タスク 0069 の手法でも本タスクの方策 A・B でも `NetworkDetected` とならない可能性がある（`libfoo.so` 自身が `dlopen` を使わない限り）。この判定を行うには逆アセンブルによる呼び出しグラフ解析（方策 C 相当）が必要だが、stripped ライブラリでの精度上の問題から本タスクでは採用しない（設計判断については設計記録を参照）。

6. **`dlopen` 検出の false positive**: 方策 B はバイナリ自身の `.dynsym` に `dlopen` 等が含まれる場合のみ検出する。依存ライブラリ（`.so`）が `dlopen` を内部実装として使用する場合は検出しない。これは、`libc.so.6` 等の基本ライブラリが内部実装として `dlopen` 相当の関数（`__libc_dlopen_mode` 等）を持つため、全 `.so` を対象にするとほぼ全バイナリが high-risk となり実用性を失うためである。なお、`python3`・`java`・`bash`・`git` 等のバイナリ自身が `dlopen` を直接使用するケースは広範に存在し、これらが `NetworkDetected` と判定されることは仕様通りである（FR-3.4.1 参照）。
...
| 解析手法 | `.dynsym` シンボル解析 | 機械語 syscall 解析 | Mach-O インポートシンボル解析 | `DT_NEEDED` 解決 + ライブラリハッシュ検証 + `dlopen` シンボル検出 |
...
| `elfanalyzer` の再利用 | なし | 新規実装 | なし | なし |
