...
- 判定関数の形（違反時に各違反を ERROR で記録し、標準エラー出力に理由を書いて `false` を返す）は `cmd/record/main.go` の `checkDirPermissions`（[main.go:102](../../../cmd/record/main.go#L102)）に揃える。`record` が既に fail-closed であり（Task 0146、コミット `be92e759`）、両コマンドが同じ信頼の起点を守る以上、挙動と診断メッセージの形が揃っているほうが運用者にとって予測可能である。
  - **例外2（実装時に判明）**: `security.ErrInvalidDirPermissions` で分岐して `chmod` を促す点も `record` に倣わない。同エラーは6つの異なる原因で返る（world-writable〔[dir_permissions_unix.go:155](../../../internal/security/dir_permissions_unix.go#L155)〕、所有者UIDの不一致〔:167〕、グループ書き込み可でグループ所属を検証できない〔:190〕、ディレクトリでない〔:89〕、および一般的な検証失敗2種〔:195・:198〕）。是正方法は原因ごとに `chmod go-w`・`chown`・`chmod g-w` と異なるため、`chmod go-w` を名指しすると6原因中5つで誤った指示になる。原因の判別はエラー文字列のマッチングでしか行えず、これは CLAUDE.md が禁じている（`internal/security` が文言を変えると `cmd/verify` の是正指示が黙って誤りに戻る）。したがって分岐を置かず、6原因すべてに対して真である1つの文言（「violation が名指しするディレクトリの権限と所有者を直す、またはハッシュディレクトリを適切な権限のパスへ移す」）とする。どの原因かは `violation` 属性の全文が述べているため、運用者が判断できる情報は失われない。原因ごとのセンチネルエラーを `internal/security` に追加する案は、共有部品（呼び出し元4箇所）の変更であり要件のスコープ外であるため、`record` の修正とあわせて別途起票する。
  - **例外1（実装時に判明）**: `remediation` 属性に `chmod go-w <v.Path>` の形でパスを埋め込む点も `record` に倣わない。`ValidateDirectoryPermissions` は root から対象ディレクトリまでの全構成要素を検証するため、`v.Path` は「検証を要求したディレクトリ」であって「違反しているディレクトリ」ではない。ハッシュディレクトリ集合は祖先を展開した列であるから、祖先1つが world-writable なら列の全要素が違反として報告され、`chmod go-w <v.Path>` は最後の1件を除きすべて権限の正しいディレクトリを指す。fail-open だった頃は捨てられる WARN だったため実害がなかったが、fail-closed 化後はこれが停止時の唯一の是正指示になる（AC-21）。違反しているディレクトリを名指ししているのは `v.Err`（`violation` 属性）であるため、`remediation` はそこを参照させる文言とする。祖先展開そのものは維持する（ハッシュディレクトリが存在しない場合、`RunTOCTOUPermissionCheck` はそのパスを読み飛ばすため、祖先を個別に渡さないと祖先側の違反を取りこぼす）。同じ誤りが `cmd/record/main.go:145` にも存在するが、`record` の修正は要件のスコープ外であるため別途起票する。
