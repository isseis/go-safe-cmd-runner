...
- [x] Test location: `internal/runner/security/elfanalyzer/analyzer_test.go::TestAnalyzeNetworkSymbols_DetectsLibcSocketAndSyscallWrapper`
- [x] Implementation: `internal/runner/security/elfanalyzer/standard_analyzer.go` の `checkDynamicSymbols`
- [x] Verification method: ELF fixture 解析結果の `DetectedSymbols` に `socket` と `read` が含まれ、カテゴリがそれぞれ `socket` と `syscall_wrapper` であることを確認する

#### AC-2: ELF バイナリの非 libc シンボル非記録

- [x] Test location: `internal/runner/security/elfanalyzer/analyzer_test.go::TestAnalyzeNetworkSymbols_IgnoresNonLibcSymbols`
- [x] Implementation: `internal/runner/security/elfanalyzer/standard_analyzer.go` の libc 判定分岐
- [x] Verification method: libc を持たない ELF fixture を解析し、`DetectedSymbols` が空であることを確認する

#### AC-3: Mach-O バイナリの libSystem シンボル全記録

- [x] Test location: `internal/runner/security/machoanalyzer/analyzer_test.go::TestAnalyzeSlice_DetectsLibSystemSocketAndSyscallWrapper`
- [x] Implementation: `internal/runner/security/machoanalyzer/standard_analyzer.go` の `analyzeSlice` と `analyzeSliceFallback`
- [x] Verification method: library ordinal またはフォールバック経路で `socket` と `read` が検出されることを確認する

#### AC-4: `runner` のネットワーク判定正確性

- [x] Test location: `internal/runner/security/network_analyzer_test.go::TestIsNetworkViaBinaryAnalysis_NetworkCategorySymbol`
- [x] Test location: `internal/runner/security/network_analyzer_test.go::TestIsNetworkViaBinaryAnalysis_SyscallWrapperOnly`
- [x] Implementation: `internal/runner/security/network_analyzer.go` の `isNetworkViaBinaryAnalysis`
- [x] Verification method: 旧レコード互換ケースを含め、ネットワーク系カテゴリのみが `NetworkDetected` を引き起こすことを確認する

#### AC-5: 回帰テスト全通過

- [x] Test location: `make test`
- [x] Test location: `make lint`
- [x] Implementation: 変更一式
- [x] Verification method: リポジトリ全体のテストと lint を成功させる
