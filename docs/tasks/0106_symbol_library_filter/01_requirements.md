...
| 条件 | Category |
|------|----------|
| ネットワーク関連シンボル名（既存の `networkSymbols` マップに一致） | `"network"` |
| その他の libc シンボル | `"syscall_wrapper"` またはより詳細なカテゴリ（アーキテクチャ設計で決定） |
...
if isLibcSymbol(sym) {
    cat := categorize(sym.Name, a.networkSymbols)
    detected = append(...)
}
```
...
FR-1.2 と同様に `"network"` / `"syscall_wrapper"` 等のカテゴリを付与する。
...
for _, sym := range result.DetectedSymbols {
    if sym.Category == "network" {
        return true
    }
}
```

`AnalysisResult` の `NetworkDetected` / `NoNetworkSymbols` の判定も同様に更新する。
...
旧レコードを持つバイナリでは `Category == "network"` のシンボルのみが記録されており、`runner` は旧レコードに対しても正しく動作する（ネットワーク判定に支障なし）。
...
- `socket()` のカテゴリが `"network"` であること
- `read()` のカテゴリが `"network"` 以外（例: `"syscall_wrapper"`）であること
...
- `socket` のカテゴリが `"network"` であること
- `read` のカテゴリが `"network"` 以外であること

### AC-4: `runner` のネットワーク判定正確性（シンボル経由）

- `Category == "network"` のシンボルを含むレコードで `runner` を実行すると、ネットワーク操作あり（`isNetwork = true`）と判定されること
- `Category != "network"` のシンボルのみを含むレコードで `runner` を実行すると、シンボル由来のネットワーク操作なし（`isNetwork = false`）と判定されること
