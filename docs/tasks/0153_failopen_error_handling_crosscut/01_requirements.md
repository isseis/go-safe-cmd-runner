...
Issue #860 は「解析・検証に失敗した場合、安全側ではなく『対象なし』『問題なし』に倒れる」という同型の fail-open 欠陥がセキュリティクリティカル部に横断的に分布していると指摘している。このうち D1 (groupmembership) は既に #858 で個別対応済み（[[0150]]/[[0151]]）。本タスクは #860 に挙げられている残りの該当箇所（C1, C2, B3, A5 の各所見）をまとめて是正する。
