...
4. 上記に対応するテストの追加・更新。
5. 開発者向け文書（`docs/dev/architecture_design/security-architecture.ja.md` の基準UID解決規則の記述）および利用者向け文書（`record` / `verify` の文書）の更新。
...
- **実在確認結果の再利用**: 解決処理は読み取り安全性チェックのたびに呼ばれるため、`record` で多数のファイルを処理すると `LookupId` が同じ値に対して繰り返し実行される。プロセス内での再利用（キャッシュ）の要否と、再利用する場合の有効期間・並行安全性を決定する。既存のグループメンバーシップキャッシュ（30 秒 TTL）との整合も併せて検討する。
...
- **AC-18**: `record` / `verify` の利用者向け文書に、`SUDO_UID` が実在しないユーザーを指す場合は実行が失敗すること、および `SUDO_UID` による基準UIDの変更が警告として記録されることが記載されている。日本語版を先に更新し、英語版は `/mktrans` で反映する。
