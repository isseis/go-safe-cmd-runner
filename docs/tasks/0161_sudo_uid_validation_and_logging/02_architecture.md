...
生の文字列は、長さが 16 文字を超える場合に切り詰め、切り詰めた旨を示す語を付す。`strconv.Atoi` は先頭の `0` を長さの制限なく受け付けるため、範囲内の UID を指したまま任意の長さになりうるからである。両エラーは処理対象ファイルごとに生成されるため、切り詰めがないと1件あたりのメッセージ長を環境変数が決めることになる。妥当な値は最大 10 桁であり、それを超える部分は診断上の情報を持たない。
