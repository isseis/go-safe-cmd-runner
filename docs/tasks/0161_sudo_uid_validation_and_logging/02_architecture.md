...
| Reviewer | isseis |
| Comments | 2026-08-01: 承認後の修正2件（実装との整合、レビュー指摘による）。§4.3 のエラーメッセージの構成要素から「対象 UID」を外し、生の文字列のみを含める旨へ改めた（実装は当初からこの形であり、本書の記述だけが残っていた）。§6.2 に状態遷移と出力の順序（compare-and-swap が先）を明記し、§5.5 の「記録の欠落」へ出力失敗による恒久的な欠落を追記した |
