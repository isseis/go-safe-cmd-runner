...
| Reviewer | isseis |
| Comments | 2026-08-01: 承認後の修正3件（実装との整合、レビュー指摘による）。§4.3 のエラーメッセージの構成要素から「対象 UID」を外し、生の文字列のみを含める旨へ改めた（実装は当初からこの形であり、本書の記述だけが残っていた）。§4.3 に生の文字列の長さの上限（16 文字で切り詰め）を追加した。§6.2 に状態遷移と出力の順序（compare-and-swap が先）を明記し、§5.5 の「記録の欠落」へ出力失敗による恒久的な欠落を追記した |
...
再利用が必要な理由は、読み取り安全性チェックが処理対象ファイルごとに繰り返し実行され、その回数を利用者が指定するファイル数で決められることである。`verify` は対象ファイル1件につき、ファイル本体の読み出し（`validator.go:1452` の `SafeReadFile`）と記録ファイルの読み出し（`file_analysis_store.go:109`）の双方で読み取り安全性チェックを行う。数百ファイルをまとめて検証すれば、照会回数も同じ桁になる。

なお、`record` の対象ファイル読み出しはこの経路に入らない。`SafeOpenFile` は読み取り安全性チェックを含まないためである（§3.7.7）。`validator.go:798` などの共有ライブラリの読み出しも同様であり、当初見積もった「1ファイルあたり数十回」は成立しない。それでもメモを置くのは、`verify` について上記の比例関係が成立すること、および §3.7.7 の起動時解決を含めて「照会はインスタンスごとに1回」という性質を保つためである。
...
失敗を記録しないため、ユーザーデータベースが停止している環境では照会が繰り返されうる。ただし §3.7.7 の起動時解決が最初の失敗で実行を終わらせるため、`record` / `verify` では実際に繰り返されることはない。この点は §5.5 に整理する。
...
- 期待される出現頻度は、`sudo record` / `sudo verify` の実行1回あたり1件である。記録の有無だけを見て事故を判別することはできない。この1件は §3.7.7 の起動時解決によって出力されるため、実行が対象ファイルを読む前に、また読み取り安全性チェックが1回も走らない実行でも出力される。
...
#### 3.7.7 `record` / `verify` は起動時に基準UIDを1回解決する

§3.1〜§3.6 は解決処理の中身を定めるが、解決処理が呼ばれること自体は前提であって設計ではない。この前提を調べたところ、`record` については成立していなかった。要件書 F-006 が述べる事実を、設計上の根拠として改めて示す。

- `internal/safefileio` の `SafeOpenFile`（`safe_file.go:86`）が行うのは `openat2(RESOLVE_NO_SYMLINKS)` によるシンボリックリンク対策のみであり、読み取り安全性チェックを含まない。基準UIDを参照する `canSafelyReadFromFile` を呼ぶのは `readFileContent`（`safe_file.go:369`）、すなわち `SafeReadFile` 系だけである。
- `record` は対象ファイルを `SafeOpenFile` で直接開いて読む（`validator.go:387`、`validator.go:1409`、`validator.go:798`）。実行中に読み取り安全性チェックへ至る経路は、`fileanalysis` の記録ファイルの読み出し（`file_analysis_store.go:109`）1つだけであり、これは既存の記録がある場合にしか到達しない。新規記録だけの実行では解決処理が一度も呼ばれない。
- 書き込み側（`CanCurrentUserSafelyWriteFile`）は実 UID を直接使うため、この経路には入らない。

結果として、実在確認によるフェイルクローズドも採用事実の記録も、`record` の主要な用途である新規記録では成立していなかった。§3.7.4 が述べる事故シナリオ（root の cron に `SUDO_UID` が残留した状態での実行）の検出も、その cron が `record` を回している場合には信号が出ない。

これを、読み取り経路に依存しない形で解消する。`SudoUIDAware` を宣言する `cmd/record` / `cmd/verify` が、対象ファイルの処理を始める前に基準UIDを1回解決する。入口は `GroupMembership` のメソッド1つとする。

```go
// EnsurePermissionCheckUID resolves the permission check UID and reports only
// whether the resolution succeeded, without checking any file.
func (gm *GroupMembership) EnsurePermissionCheckUID() error
```

中身は既存の `getPermissionCheckUID` への委譲のみである。新しい解決規則も新しい差し替え口も設けない。呼び出し位置は、引数解析の直後・ディレクトリのパーミッションチェック（TOCTOU チェック）より前とする。解決に失敗した場合は失敗の内容を標準エラー出力へ示して非ゼロで終了する。

この案の性質を3点挙げる。

- **判定規則を変えない**: 対象ファイルの読み出しに読み取り安全性チェックを追加する案も検討したが採らない。それは `record` に対して「group/world 書き込み可能なファイルは記録できない」という新しい拒否条件を持ち込むものであり、要件書「対象外」が除外した範囲を超える挙動変更になる。起動時の解決は、基準UIDが決まるかどうかだけを問い、ファイルのパーミッションには触れない（AC-23）。
- **記録の回数は変わらない**: 採用事実の記録の1回制限はパッケージレベルのレポータ実体が持つ（§3.3、§3.7.3）。起動時に解決しても、その後の読み取り安全性チェックが改めて解決しても、記録は1件のままである（AC-09、AC-22）。
- **照会が1回増える**: 起動時の解決には専用の `GroupMembership` インスタンスを使うため、メモ（§3.4）はこのインスタンスの分だけ別に持たれる。したがってユーザーデータベースへの照会が1回増える。これは §3.4 が定めた「インスタンスごとに1回」という性質のとおりの増え方であり、ファイル数にも共有ライブラリ数にも比例しない。

`runner` には入れない。`RealUIDOnly` では解決が失敗しえず、起動時に解決しても得るものがないためである（要件書 F-003）。

### 3.8 コンポーネント責務表

| ファイル | 区分 | 責務 | 更新が必要な既存テスト・既存記述 |
|---|---|---|---|
| `internal/groupmembership/manager.go` | 変更 | `permissionCheckUIDDeps` / `sudoUIDAdoptionReporter` / `sudoUIDExistenceMemo` の3型、パッケージレベルのレポータ実体、`lookupUserByUID`、`resolvePermissionCheckUID` への実在確認と記録の追加、`getPermissionCheckUID` での本番依存の組み立て、§4.1 の2つのエラー変数、`GroupMembership` への `sudoUIDExistence` フィールド追加、`New` での `sudoUIDExistence.confirmed` の初期化（§3.4）、起動時解決の入口 `EnsurePermissionCheckUID`（§3.7.7） | `resolvePermissionCheckUID`（465-484行目）のドキュメントコメント: 引数の記述と、返しうるエラーの記述が変わる。`getPermissionCheckUID`（446-456行目）のドキュメントコメント: 返しうるエラーの記述が変わる |
...
| `cmd/record/main.go` | 変更 | 起動時の基準UID解決（§3.7.7）。`deps` への差し替え口の追加と、引数解析直後・TOCTOU チェック前での呼び出し | - |
| `cmd/verify/main.go` | 変更 | 同上（`verify` はパッケージレベル変数による差し替え方式） | - |
| `cmd/record/main_test.go` | 変更 | 新しいラッパーのシグネチャへの移行、起動時解決の失敗で1件も処理せず終了することの検証（AC-20、AC-21） | `TestRecordDeclaresSudoUIDAwarePolicy`（420行目） |
...
`internal/safefileio` と `cmd/runner` の本番コードは変更しない。`cmd/record` / `cmd/verify` の変更は §3.7.7 の起動時解決の呼び出しに限られ、ファイルの判定規則には触れない。3つの `main.go` の `init()` に付されたコメント（`cmd/record/main.go:29`、`cmd/verify/main.go:32`、`cmd/runner/main.go:84`）と `docs/user/runner_command.ja.md:1856` の記述は、いずれも `runner` が `RealUIDOnly` である点と `record` / `verify` が `SudoUIDAware` である点だけを述べており、本タスクによって偽にならないことを確認した。
...
エラーメッセージは既存の書式に揃え、環境変数から読んだ `SUDO_UID` の生の文字列と、参照したユーザーデータベース種別（§3.7.5）を含める。種別を含めるのは、非 CGO ビルドでユーザーが見えていないだけの場合と本当に存在しない場合を運用者が判別できるようにするためである。

生の文字列は、長さが 16 文字を超える場合に切り詰め、切り詰めた旨を示す語を付す。`strconv.Atoi` は先頭の `0` を長さの制限なく受け付けるため、範囲内の UID を指したまま任意の長さになりうるからである。切り詰めがないと、メッセージ長を環境変数が決めることになる。妥当な値は最大 10 桁であり、それを超える部分は診断上の情報を持たない。

数値へ変換したあとの UID は含めない。運用者が環境や設定と突き合わせる対象は環境変数に設定されている文字列そのものであり、変換後の値を併記しても突き合わせの助けにならないためである。両者は同じ値を指すが、表記は必ずしも一致しない（例: `SUDO_UID="01000"` は UID 1000 を指す）。メッセージに現れるのが生の文字列である点は、この差が出る値を入力に使うテストで固定する。
...
その結果、拒否は構造化されたログとしては観測できない。拒否が運用者に届く経路は2つあり、いずれも標準エラー出力である。

1つ目は §3.7.7 の起動時解決である。`cmd/record` / `cmd/verify` は解決処理のエラーをそのまま標準エラー出力へ1行表示し、非ゼロで終了する。`record` / `verify` において実在確認の失敗が現れるのは実際にはこの経路であり、ファイル数によらず1行である。

2つ目は読み取り安全性チェックである。`internal/safefileio` の `canSafelyAccessFile` / `canSafelyReadFromFile` がエラーを `ErrInvalidFilePermissions` で包み（`safe_file.go:447`、`safe_file.go:494`。いずれも `%w` を2つ使うため元のエラーは保持される）、`cmd/record/main.go` および `cmd/verify/main.go` が対象ファイルごとに標準エラー出力へ表示する。起動時解決が先に実行を終わらせるため、`record` / `verify` ではこの経路に実在確認の失敗が現れることはない。この経路が意味を持つのは、解決処理を起動時に呼ばない利用者（`internal/groupmembership` を使う他のコード）に対してである。

いずれの経路でも、拒否は標準エラー出力のテキストとしてのみ現れ、`log/slog` を集約する監視からは見えない。この非対称（正常系は構造化ログ、失敗系は標準エラー出力のみ）は AC-15 の制約に従った結果であり、拒否も構造化ログへ出す案は §9 に将来の課題として残す。
...
正常系の他の事実（実在確認の成功、`RealUIDOnly` の選択、メモの命中）は記録しない。読み取り安全性チェックは処理対象ファイルごとに実行されるため、これらを記録すると多数のファイルを指定した実行で大量の出力が生じる。
...
失敗は §3.7.7 の起動時解決で起きる。`record` / `verify` は対象ファイルを1件も処理せず、標準エラー出力へ1行を表示して非ゼロで終了する。指定したファイルの一部だけが記録される、といった中途半端な結果は生じない。
...
| 副作用 | 本設計による変化 |
|---|---|
| ユーザーデータベースへの読み取り照会 | `SudoUIDAware` かつ実 UID が 0 かつ `SUDO_UID` が範囲内の数値である場合に限り増える。回数は `GroupMembership` インスタンスごとに1回であり、処理対象ファイル数にも共有ライブラリ数にも比例しない（§3.4）。起動時解決用のインスタンスの分として1回を含む（§3.7.7） |
| `log/slog` への書き出し | 上記の条件が成立し、かつ採用によって基準UIDが実 UID と異なった場合に限り、プロセス全体で1件の警告が増える。出力は起動時解決の時点である（§3.7.7） |
| 標準エラー出力への書き出し | 実在確認に失敗した場合、`record` / `verify` が起動時に1行のエラーを出力して終了する（§3.7.7、§4.3） |
| 実行時間 | 照会が成功する環境では、インスタンスごとに1回の照会分のみ増える。照会が失敗する環境では、起動時解決の1回の照会がタイムアウトするまでの時間が加算され、そこで実行が終わる（§5.5） |
| ファイルの書き込み・削除 | 変化しない |
...
- **照会のタイムアウトを制御できない**: `os/user` の `LookupId` は文脈もタイムアウトも受け取らないため、応答の遅いユーザーデータベースに対する照会を打ち切る手段がない。照会が成功する環境ではインスタンスごとに1回で済み（§3.4）、失敗する環境では §3.7.7 の起動時解決が最初の失敗で実行を終わらせるため、タイムアウトが積み上がることはない。ただし応答が返らない照会に対して `record` が起動直後に長時間停止して見える点は避けられない。
- **実在確認から実際の利用までの時間差**: 実在確認の成功後、その UID が判定に使われるまでの間にユーザーが削除される余地がある。§3.7.7 の起動時解決により、この窓は「起動時から最後のファイルの判定まで」に広がる。ただし窓の長さは結論を変えない。この窓が判定を緩める方向へ働くことはないためである。分岐ごとの理由は次のとおりである。
...
- **記録の欠落**: 本タスクの記録は `log/slog` の既定ロガー、すなわち標準エラー出力へ書かれる。記録は本設計が持つ唯一の構造化された信号であり、失われると §3.7.4 の検出手段が成立しない。
    ...
    - 出力が失敗した場合も失われる。§6.2 のとおり状態遷移は出力より先に行うため、`Handler.Handle` が失敗しても状態は「記録済み」になり、そのプロセスで再度出力されることはない。既定ロガーの出力先である標準エラー出力への書き込みが失敗する状況（ディスクの枯渇、閉じられた記述子）は限られるが、成立した場合の欠落は恒久的である。`slog.Logger` が `Handle` の戻り値を破棄するため、順序を入れ替えても救えない（§6.2）。
...
上表の「警告を1件出力する」と状態遷移の順序は、**遷移が先、出力が後**である。compare-and-swap に成功した呼び出しだけが `logger.Warn` へ進む。したがって出力に失敗した場合でも状態は「記録済み」であり、そのプロセスで記録が再度試みられることはない。この順序を採る理由は次の2点である。

- 出力を先にすると、同時に `report` を呼んだ複数の goroutine が出力へ進みうる。1回制限は排他区間を設けなければ保てず、`log/slog` の出力を含む区間をロックで囲うことになる。
- 出力を後にしても、失敗を検出して記録を再試行することはできない。`slog.Logger` の `Warn` は `Handler.Handle` の戻り値を破棄するため、呼び出し側は失敗を知る手段を持たない。すなわち出力先行への変更は、1回制限の実装を重くする代わりに失われる記録を救えるわけではない。

出力の失敗によって記録が恒久的に失われる点は残存リスクとして §5.5 に記載する。
...
- **記録に事故と正常を判別する情報を持たせる案は採らない**: §3.7.4 のとおり記録単独では事故を判別できないが、本タスクの目的は攻撃者の検知ではなく誤設定・残留の発見である（要件書「本タスクで解消されないもの」）。運用者は「この実行経路は `sudo` を経由しないはずだ」という知識を既に持っており、そこに記録が現れること自体が十分な信号になる（§3.7.4）。したがって判別力を追加する拡張は不要とし、将来の課題としない。
