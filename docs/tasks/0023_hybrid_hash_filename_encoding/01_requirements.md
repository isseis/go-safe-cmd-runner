...
1. **ハッシュ衝突の可能性**: 72ビット名前空間では約687億ファイルで50%の衝突確率
2. **可読性の欠如**: ハッシュファイル名から元ファイルを特定できない
...
- **機能ID:** `F-006`
- **機能名:** 既存システムとの移行サポート
- **概要:** 新方式と旧方式のハッシュファイルを段階的に移行する
- **入力:** 元ファイルパス（string）
- **出力:** 有効なハッシュファイルパス（string）
- **処理:**
  1. 新方式のハッシュファイル存在確認
  2. 存在しない場合、旧方式のハッシュファイル確認
  3. どちらも存在しない場合、新方式で作成
...
- **フォールバック効率**: フォールバック時でも43文字以内に収める
...
- **衝突回避**: 異なるパスが同じハッシュファイル名を生成しないことを保証する
...
- **段階的移行**: 既存システムから新システムへの段階的移行をサポートする
