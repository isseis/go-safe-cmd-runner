...
#### [x] 3.2. record, verify, runner コマンドでの統合
- **record コマンド**:
  - ✅ `HybridHashFilePathGetter` の使用に切り替え (`cmdcommon.CreateValidator()` 修正)
  - ✅ 既存ハッシュファイルとの競合検出機能（既存の機能を継承）
  - ✅ 新方式ハッシュファイル生成の確認（テスト通過で確認）
- **verify コマンド**:
  - ✅ 新旧両方式のハッシュファイル対応（ハイブリッド方式で自動対応）
  - ✅ フォールバック検証機能（HybridHashFilePathGetter内蔵）
  - ✅ エラーメッセージの改善（既存メッセージを維持）
- **runner コマンド**:
  - ✅ `verification.Manager` での新方式使用（newManagerInternal修正）
  - ✅ 設定ファイル検証での新方式対応（自動適用）
  - ✅ パフォーマンス測定とログ出力（既存機能を継承）
