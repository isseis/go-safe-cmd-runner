...
|---|---|---|---|
| **0141** `axis1_fixed_class_risk` | 軸1＝コマンド名分類の引き上げ・Critical 尖鋭化・実行ラッパ/特権昇格拡張・env/timeout High。**argv の宛先解析を伴わない**名前/ラッパ判定に限定。 | 根本原因4（evaluator/wrapper 結線をフェーズ内に明示） | なし |
| **0142** `axis2_destination_zoning` | 軸2＝ロケーション定義コマンドの宛先ゾーン分類（trust-critical/ordinary/safe-zone/unresolved）。path 解決・safe-zone・オペランド毎のTrusted・argv オペランド抽出。 | **根本原因1・2・4**（下記 §3） | 0141（**共有境界**: evaluator dispatch / 名前集合。直列） |
