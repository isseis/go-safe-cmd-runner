...
ファイル操作コマンド（rm/cp/dd 等の個々の実行コマンド）については、判断軸2 のパス信頼区分の結果を唯一の判定基準とし、これらを High に分類している
既存の判定（5 系統）を引き下げるのに、各系統を個別に無力化して最終 max を下げる（＝「選択的 max 抑止」。
最終リスクは 5 系統の max なので、1 つでも外し漏れると High が残る）のではなく、評価器のディスパッチで
5 系統まとめて判断軸2 に置き換える。すなわち:
