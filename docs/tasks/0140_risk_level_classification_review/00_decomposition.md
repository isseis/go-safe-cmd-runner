...
| **0142** `axis2_destination_zoning` | 判断軸2＝ファイル操作コマンドの宛先パス信頼区分の判定（trust-critical/ordinary/safe-zone/unresolved）。path 解決・safe-zone・オペランド毎のTrusted・argv オペランド抽出。 | 根本原因1・2・4（下記 §3） | 0141（共有境界: evaluator dispatch / 名前集合。0141 完了後） |
...
- **当初提案（不採用）**: 5 系統を個別に無力化して最終 max を下げる（＝「選択的 max 抑止」）。1 つでも外し漏れると
  High が残るため脆い。
- **訂正方針（採用）**: 判断軸2 がコマンドを完全に認識できた場合に限り、リスク評価ロジックのディスパッチで 5 系統をまとめて
