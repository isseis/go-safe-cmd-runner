...
|---|---|---|---|
| **0141** `axis1_fixed_class_risk` | 判断軸1＝コマンド名分類の引き上げ・Critical 限定・実行ラッパ/特権昇格拡張・env/timeout High。argv の宛先解析を伴わない名前/ラッパ判定に限定。 | 根本原因4（evaluator/wrapper 結線をフェーズ内に明示） | なし |
...
- 各タスクの 02 にデータフロー結線章を設け、TOML→loader→runner→evaluator→（判断軸2 では）パス信頼区分の判定→
...
- **identity 注入（非決定性）**: run-as 名→UID/GID/補助 group の解決はパス信頼区分判定の外（評価器結線層）で行い、
