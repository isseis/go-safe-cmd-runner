...
- **根本原因2（max 抑止の脆さ＝アーキ §3.8 起因）**: 最終リスクは全判定の **max** で決まるため、D7 の「引き下げ」を
  ...
  `DestructionRisk`／`dangerousCommandPatterns` rank6／setuid floor）あり、経路ごとに抑止/非抑止の規則が異なる
  （whack-a-mole）。
...
| **0142** `axis2_destination_zoning` | 軸2＝ロケーション定義コマンドの宛先ゾーン分類（trust-critical/ordinary/safe-zone/unresolved）。path 解決・safe-zone・per-operand Trusted・argv オペランド抽出。 | **根本原因1・2・4**（下記 §3） | 0141（**共有境界**: evaluator dispatch / 名前集合。直列） |
| **0143** `risk_audit_and_docs` | per-operand 監査フィールド・理由コード網羅・変更ノート（changelog）・文書整合・sample config・ガイド最終化。**段階ロールアウト/フラグは設けない（後方互換不要）**。 | 監査・文書（横断成果物の集約） | 0141・0142 |

**実装順序**: 0141（軸1）→ 0142（軸2）→ 0143（監査・文書）。
- **共有境界**: `risk/evaluator.go` の dimension ディスパッチと `security/command_analysis.go` の名前集合は 0141 が
...
### 3.1 根本原因2 の訂正（0142/02）: 「既存判定の選択的 max 抑止」をやめ「軸2 が唯一の判定者として置き換える」
...
ロケーション定義コマンドについては、**軸2 のゾーン結果を唯一の判定者**とし、これらを High に分類している
...
- **置き換え対象＝当該コマンドを High にしている既存判定を「全数」列挙する**（5 系統。§1 と一致）:
  ...
  ⑤coreutils の **setuid/setgid lstat floor**。ロケーション定義コマンドと判定されたら、`evaluateDimensions` はこれら
  **全 5 系統を評価対象から外し**、軸2 の `LocationResult` を**唯一の寄与**とする。
- **置き換えは「完全認識（positive recognition）」に限定する（fail-open 回避）**: `applies=true` だけでなく、軸2 が
  **非 `Unknown` の確定ゾーンを返し、かつ Kind 別オペランド抽出表が当該危険オペランドを実際に消費した**ときに限る。
  コマンド名の一致だけで①〜⑤を外すと、軸2 が認識できない危険な形が benign ゾーン→net Low の fail-open になる。
...
これにより「判定経路が 5 つあって 1 つ漏れる」構造を解消し、**1 経路 = 1 箇所**にする。ロケーション定義でない
コマンド（`find -exec` 等の引数破壊、軸2 が扱わない未知のコマンドの fail-safe）は従来どおり既存判定/間接実行が担う。
...
> - 0142: 「fail-closed パース既定」「`LocationKind` 別オペランド抽出仕様表＋網羅テスト」「軸2 が既存の High 判定
>   5 系統を**完全認識のときのみ**置き換える（単一権威）」「per-operand DTO は `risktypes` 配置」「identity 注入の
>   純関数性」を AC 化。
> - 0143: 「per-operand 監査フィールドの logger 出力」「変更ノート（changelog）/文書/ガイド」を AC 化
>   （shadow/フラグは無し）。
