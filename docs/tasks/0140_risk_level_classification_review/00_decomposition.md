...
- **根本原因1（argv 解析サーフェスの発散）**: 判断軸2 は本質的に多数 CLI ツールのセキュリティ重要 argv パーサ。
...
|---|---|---|---|
| **0141** `axis1_fixed_class_risk` | 判断軸1＝コマンド名分類の引き上げ・Critical 尖鋭化・実行ラッパ/特権昇格拡張・env/timeout High。**argv の宛先解析を伴わない**名前/ラッパ判定に限定。 | 根本原因4（evaluator/wrapper 結線をフェーズ内に明示） | なし |
...
### 3.1 根本原因2 の訂正（0142/02）: 「既存判定の選択的 max 抑止」をやめ「判断軸2 が唯一の判定者として置き換える」
...
ファイル操作コマンドについては、**判断軸2 のゾーン結果を唯一の判定者**とし、これらを High に分類している
...
- **置き換えは「完全認識（positive recognition）」に限定する（fail-open 回避）**: `applies=true` だけでなく、判断軸2 が
  **非 `Unknown` の確定ゾーンを返し、かつ Kind 別オペランド抽出表が当該危険オペランドを実際に消費した**ときに限る。
  コマンド名の一致だけで①〜⑤を外すと、判断軸2 が認識できない危険な形が benign ゾーン→net Low の fail-open になる。
- 例外（判断軸2 がカバーしない破壊）は判断軸2 自身が floor として内包する: **setuid/setgid 付与（判断軸 A）**・
...
- 各タスクの 02 に**データフロー結線章**を設け、TOML→loader→runner→evaluator→（判断軸2 では）zoning→
