...
| **0142** `axis2_destination_zoning` | 判断軸2＝ファイル操作コマンドの宛先ゾーン分類（trust-critical/ordinary/safe-zone/unresolved）。path 解決・safe-zone・オペランド毎のTrusted・argv オペランド抽出。 | 根本原因1・2・4（下記 §3） | 0141（共有境界: evaluator dispatch / 名前集合。0141 完了後） |
...
ファイル操作コマンドについては、判断軸2 のゾーン結果を唯一の判定基準とし、これらを High に分類している
...
- **置き換えは「完全認識（positive recognition）」に限定する（fail-open 回避）**: `applies=true` だけでなく、判断軸2 が
  非 `Unknown` の確定ゾーンを返し、かつ Kind 別オペランド抽出表が当該危険オペランドを実際に消費したときに限る。
...
- 各タスクの 02 にデータフロー結線章を設け、TOML→loader→runner→evaluator→（判断軸2 では）ゾーン分類→
...
- **identity 注入（非決定性）**: run-as 名→UID/GID/補助 group の解決はゾーン分類の外（評価器結線層）で行い、
  precomputed identity を注入する。ゾーン分類は live identity を読まない。これをgrep ガードでなく、注入入力の
