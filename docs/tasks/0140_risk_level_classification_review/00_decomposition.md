...
| **0142** `axis2_destination_zoning` | 軸2＝ロケーション定義コマンドの宛先ゾーン分類（trust-critical/ordinary/safe-zone/unresolved）。path 解決・safe-zone・per-operand Trusted・argv オペランド抽出。 | **根因1・2・4**（下記 §3） | 0141（**共有シーム**: rollout seam / evaluator dispatch / 名前集合。直列） |
...
### 3.2 根因3 の訂正: shadow を横断要件化し、フラグの「機構」と「デプロイ面」を分離する
