...
| **0142** `axis2_destination_zoning` | 軸2＝ロケーション定義コマンドの宛先ゾーン分類（trust-critical/ordinary/safe-zone/unresolved）。path 解決・safe-zone・per-operand Trusted・argv オペランド抽出。 | **根本原因1・2・4**（下記 §3） | 0141（**共有境界**: ロールアウト切替点 / evaluator dispatch / 名前集合。直列） |
...
**実装順序**: 0141（ロールアウト切替点＋軸1）→ 0142（軸2、切替点経由）→ 0143（デプロイ可能設定・観測・移行・切替）。
...
> 成果物を消費する循環になる。よって**切替点と デプロイ面/観測 を分離**する。

- **(1) フラグの切替点は基盤として 0141 が用意する（依存なしで先行可能）**: `EvaluateRisk` の enforce 挙動を
  **モード値**で選択する最小の切替点（`RolloutMode ∈ {off, shadow, enforce}`、既定 `off`）を 0141 で導入する。
  配置は `risktypes`（または評価器結線層）とし、constructor 注入で評価器へ渡す（`risktypes↔security` 循環を
  避ける。§3.4）。0141/0142 の新挙動はこの切替点経由でのみ enforce へ反映する（旧経路は保持、in-place 破壊
  置換をしない）。
- **(2) デプロイ面と観測は 0143 が所有**: 切替点を **TOML 設定**（運用者が `off→shadow→enforce` を切替える。
...
- **(3) 単一モード値が「本一連の全 enforce 変更」を覆う**（0141 引き上げ・0142 ゾーン・env/timeout・特権拡張）。
  各タスクの 01/02 は「新挙動は切替点経由・テストは `off`/`enforce` 両モード」を**横断制約**として参照する
  （`shadow` の差分計算は 0143 が観測する）。
- **依存順の帰結**: 切替点(0141) → 軸1/軸2 を切替点経由で導入(0141/0142) → デプロイ面・観測・切替(0143)。
  0141 は依存なしで切替点＋自タスクの enforce 変更を完結できる。
...
> - 0141: 「**ロールアウト切替点（`RolloutMode{off,shadow,enforce}`・既定 off・constructor 注入）**の導入」「自タスクの
>   全 enforce 引き上げを切替点経由で反映・`off`/`enforce` 両モードテスト」を AC 化（§3.2(1)）。
...
>   AC 化。
> - 0143: 「shadow/audit-only モードの差分ログ」「**切替点を TOML へ結線したデプロイ可能なロールアウト設定**（off→
