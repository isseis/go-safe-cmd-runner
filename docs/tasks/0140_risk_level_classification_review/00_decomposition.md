...
| **0142** `axis2_destination_zoning` | 軸2＝ファイル操作コマンドの宛先ゾーン分類（trust-critical/ordinary/safe-zone/unresolved）。path 解決・safe-zone・オペランド毎のTrusted・argv オペランド抽出。 | **根本原因1・2・4**（下記 §3） | 0141（**共有境界**: evaluator dispatch / 名前集合。直列） |
...
ファイル操作コマンドについては、**軸2 のゾーン結果を唯一の判定者**とし、これらを High に分類している
...
- **置き換え対象＝当該コマンドを High にしている既存判定を「全数」列挙する**（5 系統。§1 と一致）:
  ...
  ⑤coreutils の **setuid/setgid lstat floor**。ファイル操作コマンドと判定されたら、`evaluateDimensions` はこれら
...
| AC-14〜AC-22e | ファイル操作コマンドの 3 ゾーン判定（軸2 全体） | **0142** |
