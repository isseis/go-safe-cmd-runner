...
- **identity 注入（非決定性）**: run-as 名→UID/GID/補助 group の解決はパス信頼区分判定の外（評価器の組み込み層）で行い、
...
| AC-25（名前下限） | データ送信コマンドの名前→Medium 下限（名前集合エントリのみ）＋ヘルパー実行（ssh ProxyCommand/rsync -e）＝間接 | **0141** |
