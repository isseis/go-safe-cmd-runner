...
- **根本原因1（argv 解析の対象が膨大で列挙しきれない）**: 判断軸2 は本質的に、多数の CLI ツールの**コマンドライン
  引数（argv）を解析する**セキュリティ重要パーサである。解析すべき「コマンド×フラグ×形式」の組み合わせは膨大で
...
- **共有境界**: `risk/evaluator.go` の `evaluateDimensions`（判断軸のディスパッチ）と `security/command_analysis.go` の名前集合は 0141 が
  再編し、**0142 はその完了後に、その上へ構築する**（同一ファイルで衝突するため並行はできない）。01/02 の**文書
...
>   5 系統を**完全認識のときのみ**置き換える（唯一の判定基準）」「オペランド毎のDTO は `risktypes` 配置」「identity 注入の
>   純関数性」を AC 化。
