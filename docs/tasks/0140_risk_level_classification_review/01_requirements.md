...
ファイルを書込/上書/削除/リンク/展開するもの——`cp`・`mv`・`rm`・`rmdir`・`unlink`・`shred`・`ln`・
`mkdir`・`install`・`tee`・`sponge`・`truncate`（FILE 切詰め/上書き）・`sed -i` 等の**インプレース
編集**・`tar`/`unzip` 等の**アーカイブ展開先**（`tar -C`/`unzip -d`、上書き展開）、および
...
- **AC-22e**（`find` の破壊/実行/書込アクション）: `find` の `-delete`（ツリー削除）は AC-22 のツリー
  破壊と同等、`-exec`/`-execdir`（内側コマンド実行）は③任意コード実行（内側ゲート）、`-fprint`/
  `-fprint0`/`-fprintf`/`-fls`（FILE への書込）は `tee` 同様の FILE オペランド zoning（trust-critical
  宛先なら High）として扱い、**Low に素通りさせない**（探索起点が trust-critical なら High）。
  アクション検出の具体は 02 で確定。
...
- **AC-25**: データ egress 系（`curl`・`wget`・`scp`・`rsync`・`ssh`・`nc`）は egress 軸で **Medium**
  ...
  さらに、**ローカルでヘルパーを実行させるオプションは egress Medium に留めず間接実行として扱う**:
  `ssh -o ProxyCommand=…`・`-o PermitLocalCommand=yes -o LocalCommand=…`（ユーザシェルで任意ローカル
  コマンドを実行）は内側ゲート/拒否の対象とする。オプション検出の具体は 02。
...
- **AC-29**: ラッパー/間接実行経由の判定が維持される。例: `env modprobe x` は High 以上、
  ...
  **COMMAND を省略した形（`chroot /mnt`）は暗黙の対話シェル（`$SHELL -i`/`/bin/sh -i`）を起動する**ため、
  内側コマンド未指定でも素通りさせず High 以上として扱う。これらラッパの解析対象と確定列挙は 02。
