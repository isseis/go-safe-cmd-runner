...
> 根本原因への対処。詳細は [00_decomposition.md](00_decomposition.md)）。確定要件は各後続タスクの
...
- **AC-36**（リスクレベル分類ガイドの最終化）: アーキテクト/SRE 向け概念ガイド
  ...
  を最終化する。**作業順序は厳守**する:
  1. **コード実装の完了が先**（本要件 F-001〜F-008 の実装）。
  2. 実装完了後、**日本語ドラフトを実装の確定挙動に合わせて改訂・確定**する（軸1/軸2・max 合成・
     Critical 尖鋭化・安全要件等が実装と齟齬しないこと）。現時点の `.ja.md` は **draft**。
  3. 日本語版の確定後に**最後に英語版 `risk-level-classification-guide.md` を `/mktrans` で作成**する
     （CLAUDE.md のバイリンガル方針・翻訳ガイドライン・用語集に従う）。
  **英語版は実装・日本語確定前には作成しない**。
