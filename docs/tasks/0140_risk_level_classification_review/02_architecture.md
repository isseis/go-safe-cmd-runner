...
|---|---|---|
| KindWriteCopy（**cp**） | 宛先 FILE のみ（**source は read を mutate しないため zoning 対象外**。ただし機微 source 複製は Medium 下限、`-p`/`-a` 特権メタデータ複製は High。AC-22b） | setuid 保持コピーは軸 A で High |
| KindWriteMove（**mv**/install/tee/sponge/truncate/sed -i） | **宛先と source の双方**（mv は source を除去するため source も zoning。trust-critical source→High。AC-22b）。`tee`/`sponge` は全 FILE 引数（AC-22d）、`-t/--target-directory` は宛先 | `install` の setuid/setgid・`-o`/`-g` は軸 A で High（AC-22a） |
...
| KindFlagWriter（**フラグ選択の書込/削除先**） | フラグで選ぶローカル書込/削除先: `iptables-save -f <file>`・`curl -o <file>`/`--output`・`wget -O <file>`/`-P <dir>`・`rsync … <DEST>`（特に `--delete`）。当該 file/DEST を zoning（trust-critical→High。AC-08/AC-25） | egress 軸の Medium とは別に、ローカル書込先は④信頼境界として max 合成 |
...
が trust-critical→High、`find $WORKDIR -delete` は safe-zone なら Low（AC-22e）。`-fprint*`（FILE 書込）は
その FILE を zoning、`-delete` はツリー破壊（AC-22）、`-exec`/`-execdir`/`-ok`/`-okdir` は間接実行 Reject
