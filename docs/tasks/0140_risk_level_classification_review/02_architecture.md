...
> **【Superseded / 履歴】** 本書は 0141/0142/0143 へ**分割・継承**された（PR #765 レビューの root-cause
> 対処）。特に **§3.8 の「max 抑止」方式は 0142 で「単一権威ゾーン経路による置換」へ訂正**、**§6.4 の
> shadow は 0143 で中心成果物・横断要件へ訂正**する（[00_decomposition.md](00_decomposition.md) §3）。
> 確定アーキは各後続タスクの `02_architecture.md` を正とする。本書は経緯保存のため改変せず残す。
