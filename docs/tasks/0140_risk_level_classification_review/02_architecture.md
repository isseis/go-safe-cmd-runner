...
> **【Superseded / 履歴】** 本書は 0141/0142/0143 へ**分割・継承**された（PR #765 レビューの根本原因への
> 対処）。特に **§3.8 の「選択的 max 抑止」方式は 0142 で「判断軸2 を唯一の判定基準とする既存判定の置換」へ
> 訂正**、**§6.4 の shadow/フラグは廃止**し新分類を直接適用する（後方互換不要。[00_decomposition.md](00_decomposition.md)
> §3.1/§3.2）。確定アーキは各後続タスクの `02_architecture.md` を正とする。本書は経緯保存のため改変せず残す。
...
sequenceDiagram
    ...
    EV->>SZ: cmd, cfg
    SZ-->>EV: SafeZone{Roots, TrustedRoots}
    EV->>LZ: names, args, ZoningInput
    alt ロケーション定義ファミリでない
...
- 各ファミリは別 `map[string]struct{}` として定義し、`anyNameInSet` で照合する。**監査の family 別理由
  コード（§3.11）のため、軸1 分類器は level だけでなく一致した family も返す**: 既存 `SystemModificationRisk`
  はシグネチャ互換のため残しつつ、family を返す兄弟 `SystemModificationFamily(names) (level, family, ok)`
  を設け、evaluator はこの family を family 別 `ReasonCode` に対応づける（`useradd`/FW/boot/scheduler 等を
  監査ストリームで区別）。
...
const (
    ...
    KindWriteCopy     // cp（宛先のみ。source は read で zoning 対象外）
    KindWriteMove     // mv/tee/sponge/truncate/sed -i（mv は source も zoning）
    KindWriteInstall  // install（宛先のみ＋setuid/owner フラグ。source は read で対象外）
    KindFlagWriter    // フラグ選択の書込/削除先（curl -o/-O, wget -O/-P/default, iptables-save -f, rsync DEST/--delete）
    KindCreate        // mkdir/touch（新規作成。対象パスを zoning）
    KindDeleteTarget  // rm/rmdir/unlink/shred
    KindLink          // ln（source/リンク先も対象。-s は link 親基点で解決）
    KindDevice        // dd（of= は書込先、if= は read source）
    KindMount         // mount/umount
    KindPermission    // chmod/chown/chgrp/setfacl/chattr（軸 A・⑤）
    KindArchive       // tar/unzip（展開先。-C/-d 無しは workdir）
...
|---|---|---|
| KindWriteCopy（**cp**） | 宛先 FILE のみ（**source は read を mutate しないため zoning 対象外**。機微 source 複製は Medium 下限、`-p`/`-a` 特権メタデータ複製は High。AC-22b） | setuid 保持コピーは軸 A で High |
| KindWriteMove（**mv**/tee/sponge/truncate/sed -i） | mv は**宛先と source の双方**（source を除去するため source も zoning。trust-critical source→High。AC-22b）。`tee`/`sponge` は全 FILE 引数（AC-22d）、`-t/--target-directory` は宛先 | — |
| KindWriteInstall（**install**） | **宛先のみ**（install は source を*複製*し除去しない＝read。source は zoning 対象外。ordinary source で Medium、trust-critical source で High に**しない**） | setuid/setgid・`-o`/`-g` は軸 A で High（AC-22a） |
...
| KindFlagWriter（**フラグ選択の書込/削除先**） | `iptables-save -f <file>`・`curl -o <file>`/`--output`・**`curl -O`/`--remote-name`（URL 由来名を cwd に書く）**・`wget -O <file>`/`-P <dir>`・**`wget` 既定（URL 由来名を cwd に書く）**・`rsync … <DEST>`（特に `--delete`）。明示先が無いものは **`EffectiveWorkDir` 配下の URL 由来名**を zoning（解析不能なら fail-closed High） | egress 軸の Medium とは別に、ローカル書込先は④信頼境界として max 合成（AC-08/AC-25） |
| KindMknod（mknod） | （対象に依らず） | 無条件 **High**（safe-zone へのデバイスノード生成。AC-16 注記） |

`find` の破壊/書込アクションは **search root（探索起点）を zoning** する: `find /etc -delete` は起点 `/etc`
が trust-critical→High、`find $WORKDIR -delete` は safe-zone なら Low（AC-22e）。**探索起点を省略した形
（`find -delete`）は `EffectiveWorkDir` を起点とみなす**（`/etc` workdir の `find -delete` は High）。
`-fprint*`（FILE 書込）はその FILE を zoning、`-delete` はツリー破壊（AC-22）、`-exec`/`-execdir`/`-ok`/`-okdir` は間接実行 Reject
（§3.6）。読取専用検索（破壊/書込/実行アクションを伴わない）は昇格しない。
...
```go
// ZoningInput は軸2 が必要とする評価コンテキスト（純粋・read-only）。
type ZoningInput struct {
    WorkDir       string      // RuntimeCommand.EffectiveWorkDir。相対オペランドの解決基点（cp x → <WorkDir>/x）
    SafeZone      SafeZone    // 信頼ディレクトリ許可リスト＋ポリシー（Trusted は per-operand に判定）
    CriticalPaths []string    // Config.SystemCriticalPaths（deployment 拡張を反映）
    RunAs         RunAsIdent  // config 由来の run-as UID/GID/補助 group（§3.4 の書込不可判定の参照 identity）
}

// RunAsIdent は委譲先（run-as）の解決済み identity。live euid ではなく config 値を用いる（AC-28）。
type RunAsIdent struct {
    UID, GID int
    Groups   []int
}

// OperandZone は 1 オペランドの判定記録（監査・理由コードへ運ぶ。§3.11）。
...
// LocationResult は軸2 の結果。最終 Level と per-operand 記録を持つ。
type LocationResult struct {
    Level    runnertypes.RiskLevel
    Reasons  []risktypes.ReasonCode
    Operands []OperandZone
}

// LocationDefinedRisk は軸2を評価する。applies=false のとき本 dimension は寄与しない。
// WorkDir で相対オペランドを解決し、CriticalPaths で trust-critical を判定し、SafeZone ポリシーで
// per-operand に Trusted を判定する（評価コンテキストはすべて引数で受け、グローバル状態に依らない）。
func LocationDefinedRisk(names map[string]struct{}, args []string, in ZoningInput) (res LocationResult, applies bool)
```

### 3.4 safe-zone の導出（新規 `safezone.go`）

```go
// SafeZone は Low 降格の候補となる起点ディレクトリ集合と信頼ポリシーを保持する。
// Trusted 判定は zone 全体の単一 bool ではなく per-operand（解決後パスごと）に行う（下記）。
type SafeZone struct {
    Roots          []string // 正規化済み絶対パス（run の EffectiveWorkDir / 専用 temp）
    TrustedRoots   []string // config の信頼ディレクトリ許可リスト（既定は空）
}
...
**信頼判定（AC-17(d) TOCTOU、本書で確定。per-operand・正直な限界）**: 軸2 はオペランドを**パス文字列**として
外部コマンド（`cp`/`mv`/`rm`）に渡すだけで、**バイナリ実行のような fd バインドは行わない**。したがって
評価時の解決と外部コマンドの `open()` の間に**真の TOCTOU 窓が残る**。Low 降格は『緩和』なので、この窓を
最大限狭める保守的条件を課し、**満たせなければ降格しない（fail-closed）**。信頼は **zone 全体の単一 bool では
なく、解決後の各オペランドパスごと（per-operand）に判定**する（同一 TrustedRoot 配下に safe な子と
group-writable な子が混在し得るため。Trusted を前段で 1 回だけ計算すると誤判定する）:
- ある解決後オペランドパスが `Trusted` となる条件 = 次をすべて満たすこと:
  (1) そのパスが **config の信頼ディレクトリ許可リスト（`TrustedRoots`、既定は空）配下**にある、かつ
  (2) そのパスの**親までの各経路要素が、委譲先（run-as）= 本ツールの脅威モデル上の*非信頼*主体から
      書込不可**であること。すなわち**経路要素は run-as 以外（root/管理者）所有で、run-as が属する
      group/other からも書込不可**。`run-as 所有で 0755` のような「run-as 自身が書ける」起点は **Trusted に
      しない**（run-as が評価後に子を symlink にすり替えられるため）。参照 identity は live euid でなく
      config の run-as 値（dry-run/runtime 一致。AC-28）。
...
- **未存在 leaf の扱い（重要・正規化契約）**: 書込/作成系（`cp x /newdir/y`）では宛先 leaf が未存在なのが通常
  ...
  **leaf-symlink の追従は操作別（重要）**: 宛先を dereference して*書き込む*操作（cp/mv/install/tee/`>` 等の
  write-open）では、**leaf-symlink のターゲットを解決してターゲットで zoning**（`symlink→/etc/passwd`→
  ZoneCritical→High。解決不可なら `ZoneUnresolved`→High）。一方 **`rm`/`unlink`/`rmdir` 系の削除は symlink
  *実体*を消す**（referent を消さない）ため**追従しない**: `rm $WORKDIR/link`（→`/etc/passwd`）は「safe-zone の
  link を削除」と分類する（末尾 `/` 付きディレクトリ指定は別途ディレクトリ解決）。これにより write 経路の
  すり替え（leaf を Medium に見せて critical を書く）を塞ぎつつ、delete を過大分類しない。
...
- **`ZoneUnresolved` の floor は Kind 依存**: 書込/削除（KindWriteCopy/KindWriteMove/KindWriteInstall/KindFlagWriter/KindDeleteTarget/KindDevice の of= 等）の
...
// 相対パスは workDir（EffectiveWorkDir）基点で解決する（`cp x` は <workDir>/x）。
// 解決不能時は型付き err を返す（呼び出し側は Kind に応じ High/Medium 下限へ倒す）。
func ResolveOperandPath(operand, workDir string) (resolved string, err error)
```

> 相対オペランドの解決は必須（AC-14）。`EffectiveWorkDir` が `/etc` の run で `cp x passwd` は宛先
> `/etc/passwd`＝trust-critical→High になる。workDir を渡さないと相対宛先を critical 判定できない。
...
**抑止の条件（軸2 が確定分類を返したときのみ抑止。fail-open 回避は「不確定時のみ net 維持」で達成）**:
D7（AC-33）は**ordinary でも Medium への格下げ**を要求するため、抑止は safe-zone だけでなく ordinary でも
必要。一方、軸2 が分類できない不確定ケースで net を外すと fail-open になる。よって発生源 floor は
**軸2 が「上げる」方向には合成（max）し、「下げる」のは軸2 が確定分類を返したときに限る**:
- `ZoneOrdinary`（解決済み・非 critical・非 safe）→ ①〜③を抑止し **Medium**（D7/AC-15/AC-33 の格下げ）。
- `ZoneSafe`（per-operand `Trusted`）→ ①〜③を抑止し **Low**。
- `ZoneCritical` → 軸2 自身が High（net は冗長だが残してよい）。
- `ZoneUnresolved` / 解決エラー → **①〜③の High net を残す**（軸2 が分類できない＝唯一の fail-open リスク箇所。
  ここだけ冗長 High を維持）。
- 抑止は**ロケーション定義 applet** かつ軸2 が `applies=true` のときに限る。`find -exec`/`rsync --delete` 等
  「引数による破壊/実行」は間接実行・arg 軸が担い、①〜③の安全網は非ロケーション applet には残す。
...
**`Validator` のメソッド** `(*Validator).EvaluateOutputSecurityRisk(path, workDir string) (runnertypes.RiskLevel, error)`
（free function ではなく receiver 必須）は、**出力キャプチャ（`output_file`）の宛先**を分類する
（Critical/High/Low に加え、非該当パスは **Medium** を既定で返す）。軸2 はこれと**目的が異なる**（軸2 は任意の
...
| safe-zone（Low） | WorkDir 配下 **および現在ユーザの `$HOME` 配下**を Low（`user.Current()` を実行時に読む） | WorkDir/専用 temp のみ・per-operand Trusted 充足時（**`$HOME`・出力先親は safe-zone にしない**。AC-17(b)） |
...
- 追加: `LocationDefinedRisk(names, args, ZoningInput{WorkDir, SafeZone, CriticalPaths})` を呼び、`applies` の
  とき `LocationResult.Level` を `addDimension` で max 合成し、`LocationResult.Operands` を監査へ渡す（§3.11）。
- 改修（**§3.8 の条件付き 3 系統抑止を適用**）: ロケーション定義 applet で**軸2 が確定分類を返したとき**、
  ①`IsDestructiveFileOperation`→High、②`CoreutilsCommandRisk`→High、③profile の `DestructionRisk`→High を
  寄与させない——`ZoneOrdinary`→Medium、`ZoneSafe`(per-operand Trusted)→Low（D7/AC-33 の格下げ）。
  `ZoneUnresolved`/解決失敗のみ①〜③の High net を残す（fail-open 回避）。`find -exec`/`rsync --delete` 等
  「引数による破壊/実行」判定は間接実行・arg 軸に残す。
- `ZoningInput` は `RuntimeCommand`（`EffectiveWorkDir`）と `security.Config`（`SystemCriticalPaths`・
  `ResolveSafeZone`）から組み立てる（§3.4）。
...
- **denial を説明可能にする監査フィールド（必須・キャリアを設計に含む）**: 軸2 の per-operand 記録は
  `LocationResult.Operands`（`[]OperandZone`、§3.3）として evaluator から運ばれ、`RiskAssessment` に
  構造化フィールドを足して `normal_manager.go` の監査エントリ（`RiskAuditEntry`）へ記録する:
  各 `OperandZone{Index, Raw, Resolved, Zone, MatchedCritical, Trusted, UnresolvedErr}`。**level と理由コード
  だけでは運び切れない**（現行 assessment/audit にこの per-operand キャリアが無いため新設が必須）。**解決後
  パスと一致 critical prefix が無いと、`$WORKDIR/out` が symlink で `/etc` を指して High になった理由を運用者が
  再現解析できない**（3 時の denial がデバッグ不能）。解決結果＝最重要の診断情報であり破棄しない。
...
- **降格の安全性と TOCTOU の正直な限界**（最重要）: 軸2 の Low 降格は AC-17(a)〜(d) をすべて満たすときのみ
  ...
  **Low 降格は「最良努力の評価時判断」であり、ハッシュ＋fd バインドと同等の保証ではない**。一次防御
  （allowlist+ハッシュ）はバイナリのみを対象とし**オペランドパスは対象外**である点も明記する。
  **重要な帰結**: Trusted 条件(2)は「run-as（＝脅威モデル上の非信頼主体）から書込不可」を要求するため、
  **run-as 自身が書ける一般的な workdir は Trusted にならず Low 降格は発生しない（→ ordinary=Medium）**。
  Low が成立するのは管理者が用意した非ユーザ書込の信頼ディレクトリに限られ、実運用では稀。D7 の主目的
  （rm/dd の過剰 High 解消）は **ordinary→Medium の格下げ（§3.8）で達成**され、Low はその上の限定最適化。
...
- **shadow / audit-only モード（新規・1 リリース）**: 新ルールで分類し、**旧ルールとの差分（旧→新、newly-deny、
  ...
  新ルールへ切替える。
- **旧分類器を破壊的に置換しない（shadow の前提）**: shadow は旧→新の両方を同時計算するため、**名集合・次元を
  in-place で書き換えると旧分類が失われ shadow が成立しない**。実装は旧経路（`EvaluateRisk` の現行ルール）を
  shadow 期間中**保持**し、新経路を並走させて両結果を比較する（feature flag で enforce 側を選択）。enforce を
  新へ切替えた後に旧経路を撤去する。これにより P1（軸1 名集合再編）等の「移設」は、shadow 期間は**追加**で
  行い、撤去は切替後とする。
...
- **3 系統抑止のゲート**: `ZoneSafe&Trusted`→rm/dd 等が **Low**、`ZoneOrdinary`→**Medium**（D7/AC-33 の
  ordinary 格下げ＝①〜③も抑止）、**`ZoneUnresolved`/解決エラーのみ ①〜③の High が残る**ことを表明
  （§3.8、fail-open 回帰防止。`rm /srv/app/cache.dat`=Medium を必ず含める）。
