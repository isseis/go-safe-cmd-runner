...
`detectPclntabOffset` は pclntab の magic 値から Go バージョンを判定し、
サポート対象外（Go 1.26 未満、magic = `0xfffffff1` 以外）の場合は
offset = 0 を返すのではなく、`ErrUnsupportedPclntabVersion` エラーを `ParsePclntab` に伝播させること。
誤動作（誤った offset 適用）を避けるためにフェイルオープンではなくフェイルクローズとする。

### 3.2 非機能要件

- **サポート対象 Go バージョン: 1.26+ のみ**（`CurrentPCLnTabMagic = 0xfffffff1`）
  - Go 1.25 以前のバイナリは `ErrUnsupportedPclntabVersion` エラーとして扱う
  - 誤検出による誤動作を避けるため、サイレントな `offset = 0` フォールバックは行わない
...
| AC-5 | Go 1.26 未満のバイナリ（magic ≠ `0xfffffff1`） | `ErrUnsupportedPclntabVersion` を返す（誤動作防止のため明示的エラー）|
