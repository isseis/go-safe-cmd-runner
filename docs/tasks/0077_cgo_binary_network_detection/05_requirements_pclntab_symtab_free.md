...
| AC-5 | Go 1.18–1.25 CGO バイナリ | `detectPclntabOffset` が offset = C_startup_size を検出し、`fn.Entry + offset` が ELF シンボルテーブル（.symtab）上の関数 VA と一致すること（案 B が Go 1.18–1.25 バイナリでも動作することを確認）|
