...
- `SyscallAnalyzer` は `socket`(#198) を検出できず `HasNetworkSyscalls: false`、`IsHighRisk: true` を返した
  ...
  - 原因（Pass 1 の追記）: `knownSyscallImpls` に登録された名称（`"internal/runtime/syscall/linux.Syscall6"`）が実際のシンボル名（`"internal/runtime/syscall.Syscall6.abi0"`）と一致しないため、この関数内の `SVC #0` が除外されず `unknown:indirect_setting` と判定される
  - 原因（Pass 2）: `syscall.socket` 内の `BL syscall.RawSyscall` 呼び出しが `GoWrapperResolver` によって解決されなかった原因は未特定（実装フェーズでの調査が必要）
...
- [x] 同バイナリに対して `elfanalyzer.SyscallAnalyzer` を実行した結果（成功/失敗・原因）が確認されており、本文書のセクション 8「未解決事項」に記録されていること
  ...
  - arm64 環境での Pass 1（直接スキャン）は `unknown:indirect_setting` 判定（`knownSyscallImpls` シンボル名不一致による除外漏れを含む）、Pass 2（GoWrapperResolver）は未機能（原因未特定）
...
- [x] Pass 1 修正（`knownSyscallImpls` の arm64 シンボル名追加）と Pass 2 修正（`BL syscall.RawSyscall` の未解決原因の調査・修正）により `HasNetworkSyscalls: true` が返るようになること（実装フェーズで検証）
  - **実測結果（arm64）**: `HasNetworkSyscalls: true`、`socket`(#198) が `go_wrapper` メソッドで検出 ✅
  - Pass 1 修正: `knownSyscallImpls` のシンボル名を `"internal/runtime/syscall.Syscall6.abi0"` から `"internal/runtime/syscall.Syscall6"` に修正（pclntab は ABI サフィックスなしのシンボル名を使用するため）
  - Pass 2: `BL syscall.RawSyscall` の解決が成功し、`socket`(#198) が `go_wrapper` として検出された
  - 残存課題: `control_flow_boundary` による `unknown` syscall が存在するため `IsHighRisk: true` は残っている（§4.2 の安全方向フェイルセーフ）
...
- [x] **GoWrapperResolver arm64 対応**:
  1. **Pass 1 の修正（完了）**: `knownSyscallImpls` のシンボル名を `"internal/runtime/syscall.Syscall6.abi0"` から `"internal/runtime/syscall.Syscall6"` に修正。pclntab は ABI サフィックスなしのシンボル名を使用するため、`.abi0` サフィックス付きのエントリではマッチしなかった。修正後、`SVC #0` (0x402cec) が `IsInsideWrapper()` で正しく除外され、`unknown:indirect_setting` エントリが消えた。
  2. **Pass 2 の修正（完了）**: Pass 1 修正により `IsInsideWrapper` が機能するようになったことで、Pass 2（GoWrapperResolver）も `syscall.socket` 内の `BL syscall.RawSyscall` を `go_wrapper` として解決できるようになった。`socket`(#198) が `HasNetworkSyscalls: true` で検出されることを確認。
  - 残存: `control_flow_boundary` による `unknown` syscall が存在するため `IsHighRisk: true` は残っている。これは §4.2 の安全方向フェイルセーフの範囲内であり、現時点では許容する。
