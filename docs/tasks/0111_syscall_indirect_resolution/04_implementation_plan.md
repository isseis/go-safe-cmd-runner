...
### フェーズ2 チェックリスト（細分化）
- [x] P2-1: 分岐解決対象を x86 直接 syscall 経路に限定するガードを実装
- [x] P2-2: 後方スキャン窓の命令列から最小 CFG（ノード/エッジ/前任者）を構築
- [x] P2-3: 条件分岐の合流点で値マージ（同値のみ確定）ルールを実装
- [x] P2-4: レジスタコピー連鎖と即値代入を CFG 上の遡及探索で統合
- [x] P2-5: 値衝突・未解決時は既存互換で `unknown:indirect_setting` を維持
- [x] P2-6: predecessor 単一路ケースのユニットテストを追加
- [x] P2-7: predecessor 複数同値ケースのユニットテストを追加
- [x] P2-8: predecessor 複数競合ケースのユニットテストを追加
- [x] P2-9: 既存フェーズ1テストが回帰しないことを確認
- [x] P2-10: `make fmt`/`make test`/`make lint` を実行し成功を確認
