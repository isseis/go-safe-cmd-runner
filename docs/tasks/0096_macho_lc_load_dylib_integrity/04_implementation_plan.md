...
- `CurrentSchemaVersion` を 13 → 14 に上げ、旧記録に対しては dynlib 検証のみをスキップして後方互換を維持する
...
| 拡張対象 | `fileanalysis`, `filevalidator`, `verification`, `dynlibanalysis`, `cmd/record` |
| スコープ外 | `dynlibAnalyzer` → `elfDynlibAnalyzer` リネーム、`dynlibanalysis` → `elfdynlib` パッケージリネーム（別タスクで対応） |
...
- `computeFileHash(fs safefileio.FileSystem, path string) (string, error)`:
  ...
  - `dynlibanalysis.computeFileHash` と同一ロジック（循環依存回避のため重複定義）

> **`computeFileHash` の重複について**: `dynlibanalysis.computeFileHash` と同一ロジックだが、YAGNI の観点で本タスクでは重複を許容する。将来の共通ユーティリティ切り出しは別途検討。
...
internal/dynlibanalysis/errors.go          # 拡張（エラーメッセージ汎用化）
...
- `CurrentSchemaVersion`: 13 → 14
  ...
  - `runner` は旧記録に対して dynlib 検証のみスキップし実行を継続する（後方互換）
...
##### 2.3.2 `dynlibanalysis/errors.go` のエラーメッセージ汎用化
...
> **根拠**: 既存の `dynlibAnalyzerFactory` / `SetDynLibAnalyzer` パターンと同一。
...
3. **後方互換性**: schema 13 の記録で Mach-O バイナリの実行が許可されること
...
##### 2.4.4 `dynlibanalysis/errors.go` 変更の影響確認
...
**対策**: `Store.Update` は旧スキーマ（`Actual < Expected`）を上書き許可する既存実装で対応済み。`runner` は旧スキーマ記録に対して dynlib 検証のみスキップし実行を許可する（後方互換）。README・CHANGELOG に移行手順を明記。
