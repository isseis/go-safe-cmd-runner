...
- [x] `MachODynLibAnalyzer.Analyze` が LC_LOAD_DYLIB なし Mach-O で `(nil, nil, nil)` を返すこと（`TestAnalyze_SingleArchMachO_NoDeps`）
- [x] `LibEntry` に `SOName`（インストール名）, `Path`（解決済みフルパス）, `Hash`（`"sha256:<hex>"`）が正しく記録されること
- [x] LC_LOAD_DYLIB の解決失敗で `record` が失敗し、何も永続化されないこと（`TestAnalyze_StrongDepResolutionFailure`）
- [x] LC_LOAD_WEAK_DYLIB の解決失敗でスキップして継続されること（`TestAnalyze_WeakDepSkipped`）
- [x] dyld shared cache ライブラリ（システムプレフィックス + ファイル不在）がスキップされること
- [x] dyld shared cache のみの依存では `DynLibDeps` が nil であること
- [x] 未知 `@` トークンで `AnalysisWarning` が生成され、`record` は継続すること（`TestAnalyze_UnknownAtToken_Warning`）
- [x] 間接依存が再帰的に解決・記録されること（`TestAnalyze_IndirectDeps`）
- [x] 間接依存の `@rpath` 解決が各 `.dylib` 自身の `LC_RPATH` を使って行われること（`TestAnalyze_IndirectDeps_RpathFromDylib`）
- [x] 循環依存で無限ループしないこと（`visited` セット）（`TestAnalyze_CircularDeps`）
- [x] 再帰深度超過時にエラーで `record` が失敗すること（`TestAnalyze_RecursionDepthExceeded`）
- [x] Fat バイナリでネイティブアーキテクチャのスライスが選択されること（`TestOpenMachO_FatBinary_MatchingSlice`, `TestAnalyze_FatBinary_MatchingSlice`）
- [x] Fat バイナリで一致スライスがない場合に `ErrNoMatchingSlice` が返ること（`TestOpenMachO_FatBinary_NoMatchingSlice`, `TestAnalyze_FatBinary_NoMatchingSlice`）
- [x] `HasDynamicLibDeps` が単一アーキテクチャ Mach-O + 非 dyld-cache 依存で `(true, nil)` を返すこと（`TestHasDynamicLibDeps_SingleArch_NonSharedCacheDep`）
- [x] `HasDynamicLibDeps` が Fat バイナリ + 非 dyld-cache 依存で `(true, nil)` を返すこと（`TestHasDynamicLibDeps_FatBinary_NonSharedCacheDep`）
- [x] `HasDynamicLibDeps` が Mach-O + dyld-cache のみで `(false, nil)` を返すこと
- [x] `HasDynamicLibDeps` が非 Mach-O で `(false, nil)` を返すこと
- [x] テスト用 Mach-O フィクスチャの説明が `testdata/README.md` に記述されていること（実バイナリは `manager_macho_test.go` の `findNonDyldCacheMachOBinary` で実行時生成）
- [x] 全ユニットテストがパスすること
- [x] `make lint` / `make fmt` がパスすること
...
- [x] `record --force` で Mach-O の `DynLibDeps` が更新されること（`TestRecord_Force_MachO_UpdatesDynLibDeps` in `validator_macho_darwin_test.go`）
- [x] 既存の `ContentHash` / `SymbolAnalysis` / `SyscallAnalysis` が変更されないこと
- [x] `CurrentSchemaVersion` に依存する既存テストが更新されていること（テストは `CurrentSchemaVersion` 定数を動的参照するため自動追従）
- [x] 全テストがパスすること
- [x] `make lint` / `make fmt` がパスすること
...
- [x] `hasMachODynamicLibraryDeps` が Mach-O + 非 dyld-cache 依存で `(true, nil)` を返すこと
- [x] `hasMachODynamicLibraryDeps` が非 Mach-O で `(false, nil)` を返すこと
- [x] `verifyDynLibDeps` が DynLibDeps あり + ハッシュ一致で `nil` を返すこと
- [x] `verifyDynLibDeps` が DynLibDeps あり + ハッシュ不一致で `ErrLibraryHashMismatch` を返すこと
- [x] `verifyDynLibDeps` が DynLibDeps なし + 動的 Mach-O（schema ≥ 14）で `ErrDynLibDepsRequired` を返すこと
- [x] `verifyDynLibDeps` が DynLibDeps なし + 動的 Mach-O（schema < 14）で `SchemaVersionMismatchError` を返すこと（実行ブロック）
- [x] `verifyDynLibDeps` が DynLibDeps なし + 非 Mach-O + 非 ELF で `nil` を返すこと
- [x] 統合テスト: `record` → `runner` 正常フローが通ること（`TestVerify_MachOWithDynLibDeps` in `manager_macho_test.go`）
- [x] 統合テスト: ライブラリ改ざん検出が動作すること（`TestVerify_MachOLibraryTampered` in `manager_macho_test.go`）
- [x] 統合テスト: 旧スキーマ（schema 13）の記録で dynlib 検証がスキップされること（`TestVerify_MachOOldSchema`）（注: 実装では旧スキーマを「ブロック」ではなく「スキップ」として扱う設計変更が行われた）
- [x] 統合テスト: `DynLibDeps` 未記録検出（新スキーマ）が動作すること（`TestVerify_MachONoDynLibDeps` in `manager_macho_test.go`）
- [x] 統合テスト: dyld shared cache のみの Mach-O バイナリが許可されること（`TestVerify_MachODyldCacheOnly` in `manager_macho_test.go`）
- [x] 既存の ELF `DynLibDeps` 検証テストが全てパスすること
- [x] 既存の `ContentHash` 検証テストが全てパスすること
