...
- **Backward Compatible Schema Migration**: `CurrentSchemaVersion` を 13 → 14 に上げる。`Store.Update` は旧記録（schema 13, `Actual < Expected`）を新規記録として上書き可能とし、`runner` は旧記録に対して dynlib 検証のみをスキップして実行を継続する（後方互換）
...
flowchart TD
    ...
    SchemaError["future schema: エラー<br/>→ 実行ブロック"]
...
- [ ] `runner` 検証の統合テスト（正常系・改ざん検出・後方互換性・dyld shared cache のみの許可）
- [ ] 既存の ELF テストの全パス確認
