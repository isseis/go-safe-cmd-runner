...
- **Schema Migration**: `CurrentSchemaVersion` を 13 → 14 に上げる。`Store.Update` は旧記録（schema 13, `Actual < Expected`）を新規記録として上書き可能とし、`record` 再実行で自動移行される。`runner` は旧記録（schema 13）に対してエラーを返し実行をブロックする（不完全なレコードによる検証すり抜けを防止）
- **Zero External Dependencies**: `otool` 等の外部コマンドに依存せず、Go 標準ライブラリ（`debug/macho`）のみを使用する
- **DRY**: 既存の汎用 dynlib 検証部品（`dynlibanalysis.DynLibVerifier` と共有エラー型）・`fileanalysis.LibEntry`（エントリ型）・`safefileio`（安全なファイル読み取り）を再利用する
- **Naming Clarity**: フォーマット固有の解析器・リゾルバーはパッケージ名とフィールド名に対象形式を含める。汎用部品にのみ `dynlibanalysis` という中立名を使う
...
```
internal/
├── elfdynlib/                           # RENAMED: 既存 ELF 動的ライブラリ解析パッケージ
│   ├── analyzer.go                      # DynLibAnalyzer: ELF の DT_NEEDED 依存解決 + ハッシュ計算
│   ├── resolver.go                      # LibraryResolver: soname → ファイルシステムパス解決
│   └── errors.go                        # ELF 解決エラー型
│
...
│
├── filevalidator/
│   └── validator.go                     # SaveRecord: MachODynLibAnalyzer の呼び出し追加
│                                        # SetMachODynLibAnalyzer: 新規 setter
│
├── verification/
...
└── dynlibanalysis/
    ...
    └── errors.go                        # 形式非依存の整合性検証エラー
```

> **NOTE**: 現行実装の `internal/dynlibanalysis` は実態として ELF 固有の解析ロジックを含む。Mach-O 対応を機に、設計上は `elfdynlib`（ELF 固有）と `dynlibanalysis`（形式非依存の検証・共有エラー）に責務分離する。
...
既存の `SaveRecord` は `Store.Update` コールバック内で ELF 用 `elfDynlibAnalyzer.Analyze()` を呼び出している。同コールバック内に Mach-O 用解析を追加する：
...
type Validator struct {
    // ... 既存フィールド ...
    elfDynlibAnalyzer   *elfdynlib.DynLibAnalyzer       // ELF 用（既存、命名整理後）
    machoDynlibAnalyzer *machodylib.MachODynLibAnalyzer // Mach-O 用（新規）
}

// SetELFDynLibAnalyzer は ELF 用 DynLibAnalyzer を注入する。
func (v *Validator) SetELFDynLibAnalyzer(a *elfdynlib.DynLibAnalyzer)

// SetMachODynLibAnalyzer は Mach-O 用 DynLibAnalyzer を注入する。
...
//          // stale データを残さないため、毎回 dynlib 関連フィールドを初期化
...
> **NOTE**: ELF 解析と Mach-O 解析は排他的（ELF バイナリは Mach-O でなく、その逆も同様）。ただし Mach-O 解析のスキップ条件を「既存 record に `DynLibDeps` が残っていること」に依存させてはならない。ファイルが ELF → Mach-O に差し替わった場合でも `--force` で stale な ELF `DynLibDeps` が残らないよう、各回の `record` で dynlib 関連フィールドを初期化してから再解析する。

> **Naming Policy**: `elfDynlibAnalyzer` / `machoDynlibAnalyzer` のようにフィールド名にも形式名を含める。`dynlibVerifier` のような中立名は形式非依存コンポーネントに限定する。
...
| 状態 | 処理 |
|------|------|
| `SchemaVersionMismatchError.Actual < Expected`（旧 schema 13） | エラーを返す（`record` 再実行を要求）。不完全なレコードによる検証すり抜けを防止する |
...
| 旧スキーマ記録による検証すり抜け | スキーマバージョン | `Actual < Expected` → エラー・実行ブロック、`Actual > Expected` → エラー・実行ブロック |
...
flowchart TD
    ...
    Start(["runner: dynlib 検証開始"])
    NoteHash["前段で ContentHash 検証は完了済み"]
    LoadRecord["記録ファイルをロード"]
    SchemaErr{"SchemaVersionMismatchError?"}
    SchemaError["スキーマ不一致: エラー<br/>→ 実行ブロック（record 再実行を要求）"]

    CheckDynLib{"DynLibDeps<br/>記録あり?"}
    CheckELF{"ELF with<br/>DT_NEEDED?"}
    CheckMachO{"Mach-O with non-dyld-cache<br/>deps?"}
    NoDynOK["静的/非対象/<br/>dyld cache のみ"]
    ...
    Start --> NoteHash
    NoteHash --> LoadRecord
    LoadRecord --> SchemaErr
    SchemaErr -->|"No"| CheckDynLib
    SchemaErr -->|"Yes"| SchemaError
    ...
    class Start,NoteHash,LoadRecord,HashLib process;
    class SchemaError,DynRequired,HashLibFail highRisk;
    class NoDynOK,AllOK safe;
```

### 4.3 ファイル読み取りの安全性

ライブラリのハッシュ計算時は `safefileio.SafeOpenFile()` を使用してストリーミングで読み取る（ELF 版と同一）。パス正規化は `record` 時に `filepath.EvalSymlinks` → `filepath.Clean` の順で適用し、その結果を `LibEntry.Path` に保存する。

本タスクでは `DynLibVerifier` は変更しないため、`runner` は記録済みの正規化済みパスをそのまま利用する。この前提により、シンボリックリンク由来の偽不一致を防ぐ。
...
| `runner` 実行時 | **現状はクリアしない（未対応）** | パスハイジャック攻撃防止のためクリアが望ましいが、現行コードでは `DYLD_*` の除去は未実装。対策は別途実装予定である |

現時点の `internal/runner/executor/environment.go` には `DYLD_*` を除去してから
`runner` を実行する処理は入っていない。そのため、本設計書では「既存動作」とは
扱わず、未対応事項として明示する。
## 6. エラーハンドリング設計
...
- [ ] stale な `DynLibDeps` / `AnalysisWarnings` を残さない再記録テスト
...
- [ ] 既存 ELF 用フィールドを `elfDynlibAnalyzer` へリネーム
- [ ] `Validator.SetELFDynLibAnalyzer` の追加
- [ ] `Validator` に `machoDynlibAnalyzer` フィールド追加
- [ ] `Validator.SetMachODynLibAnalyzer` の追加
...
- [ ] `runner` 検証の統合テスト（正常系・改ざん検出・旧スキーマ拒否・dyld shared cache のみの許可）
- [ ] 既存の ELF テストの全パス確認
- [ ] `make fmt` / `make test` / `make lint` パス確認
...
| 旧スキーマ Mach-O 記録 | 旧スキーマ（schema 13）は `SchemaVersionMismatchError` で実行がブロックされること |
| schema 14 Mach-O で non-dyld shared cache 依存ありかつ `DynLibDeps` なし | 実行がブロックされること |
| schema 14 Mach-O で dyld shared cache のみ | 実行が許可されること |
...
| 依存元 | 依存先 | 依存内容 |
|-------|-------|---------|
| `filevalidator` | `elfdynlib` | `Validator.SaveRecord` 内で `DynLibAnalyzer.Analyze` を呼び出し |
...
| `verification` | `dynlibanalysis` | `DynLibVerifier.Verify` と共有整合性エラー（形式非依存） |
...
flowchart LR
    ...
    T0074 -->|"DynLibDeps フィールド定義<br/>DynLibVerifier の再利用<br/>ELF DynLibAnalyzer の BFS 設計"| T0096
...
| `CurrentSchemaVersion` 変更の影響 | 高 | 全管理対象バイナリの `record` 再実行が必要（README に明記）。旧スキーマ（schema 13）は `runner` が実行をブロックするため、再実行前はコマンドが実行不可となる |
