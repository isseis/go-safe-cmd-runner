...
**flush を通らない終了経路**: 本設計が flush を保証するのは、01_requirements.md 調査結果3 が特定した正常終了経路のみである。`cmd/runner/main.go` の `runTOCTOUCheck` における panic、`run` の中で回復されない panic、および flush 実行中に 2 回目の SIGINT を受けた場合（`signal.NotifyContext` の解除後は既定のシグナル処理でプロセスが終了する）は、キューに残った通知を失う。これらはいずれも異常終了であり、ログファイルと stderr への同期出力は済んでいるため、Slack 通知の欠落は診断の妨げにならない。この限界は利用者向け文書に記載する。
...
先頭境界は 2 種類を用意し、キーを 2 群に分けて使い分ける。

| 群 | 対象キー | 先頭境界 | 意図 |
|---|---|---|---|
| 群 A | `password`、`api_key` | テキストの先頭、または英数字以外の文字 | 語として具体的で、通常の文章に現れても機密の文脈である可能性が高い |
| 群 B | `key`、`token`、`secret` | `_`、`-`、`.`、`"`、`'` のいずれか | 一般的な英単語であり、診断メッセージの散文中に頻出する |
| 群 C | `_PASSWORD`、`_TOKEN`、`_KEY`、`_SECRET` | 課さない | キー自身が英数字以外の文字で始まり、環境変数名の接尾辞としての一致を意図している（`DB_KEY=x`） |
...
**上限**: エントリ数に上限（256）を設ける。上限に達した後は、キャッシュに載っていないパターンを毎回コンパイルする（現行と同じ挙動）。`Config` は公開フィールドを持つ構造体であり、`KeyValuePatterns` を実行時に差し替える呼び出し方を禁じていないため、キーの空間を有限と仮定しない。現状の本番構成では `KeyValuePatterns` は `DefaultKeyValuePatterns()` の 13 キーのみであり、生成されるパターン数は上限に遠く及ばない。
...
| 異常終了経路で未送信通知が失われる | 中 | 2.3 に列挙し、限界として文書化する |
...
**矢印の意味**: A → B は「A の次に B を実行する」ことを表す。菱形は分岐条件である。`ValueDetector.Mask` はキーごとではなく、全キーの処理が終わった後に 1 回だけ適用される。この順序は、構造化された key=value の組を先に精密に隠し、その後で残りのテキストに含まれる裸の秘密を拾うという既存の設計意図によるものであり、変更しない。
...
- **到達保証**: 永続キューやプロセス再起動をまたぐ再送は、本タスクのスコープ外である。必要になった場合、送信キューを永続化層に差し替える形で拡張できる。
...
| AC-23 | 2.3 プロセス終了時のデータフロー、3.4.8 |
| AC-24 | 3.4.2 優先度付きキュー、3.4.5 flush 中の送信規則 |
