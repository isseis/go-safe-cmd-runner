...
**新たに置換される範囲（意図した変更）**: 群 B のキーが引用符付きで現れる構造化データは、秘密でなくても置換される。たとえば AWS CLI の JSON 出力に含まれる `"key": "us-east-1"` は `"key": "[REDACTED]"` になる。AC-09 は「秘密を含まないテキストに対し、新たに置換される範囲が生じない」ことを求めているが、この 1 ケースは例外として受け入れる。理由は、JSON/YAML のフィールド名として `key` / `token` / `secret` が使われている箇所は、秘密を格納する用途である可能性が実際に高く、AC-02（JSON 形式の捕捉）を満たすためにはこの形を捕捉せざるを得ないためである。この例外は 7.1 のテストで固定し、利用者向け文書にも記載する。
