...
**`=` を含むキーの規則の帰属**: `Authorization=` のようにキーが `=` を含む場合の規則（キーの直後の非空白列を置換する）は、生成する正規表現 `(?i)(<エスケープ済み>)(\S+)` と置換処理が接頭辞規則と完全に一致する。したがって**独立した規則としては持たず、`PatternKindPrefix` に統合する**。「呼び出し側が区切りまで含めて指定したので、区切りや引用符の解釈を持ち込まない」という規則の性質は、`Bearer ` が空白を、`password=` が等号を、それぞれ自分で持っているという同じ性質であり、両者を分ける根拠はない。以降の 3.2.2〜3.2.6 は、キー規則（`PatternKindKey`）についての設計である。
