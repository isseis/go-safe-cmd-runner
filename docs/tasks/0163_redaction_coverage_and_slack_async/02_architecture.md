...
**永続キューを導入しない理由**: プロセスの再起動をまたぐ再送は 01_requirements.md でスコープ外とされている。加えて、キューをディスクへ永続化すると、redact 済みとはいえ Slack へ送る本文をログファイルとは別のファイルに二重に置くことになり、保護すべきファイルが 1 つ増える。上記のとおり内容自体はログファイルに残るため、永続キューが救うのは「配送そのもの」だけであり、対価に見合わない。強制終了下でも配送を確実にしたい運用では、`GSCR_SLACK_SYNC=1`（3.4.10）で同期送信に戻す選択肢がある。
...
`performKeyValueRedaction` はキーの形により 3 経路に分岐する。本設計はこの分岐を維持し、第 3 経路（`performKeyValuePatternRedaction`）のみを拡張する。

統合しない理由は次の 2 点である。第 1 に、`Authorization: ` 経路は「行末まで置換し、`Bearer`/`Basic` のスキーム名を保持する」という第 3 経路とは異なる終端規則を持ち、統合すると 1 つの生成規則に 2 種類の終端規則を同居させることになる。第 2 に、統合は AC-06・AC-07 が守るべき既存挙動を書き換えるリスクを持ち込むだけで、本タスクが解消すべき取りこぼしは第 3 経路にしか存在しない（YAGNI）。

第 3 経路はさらに、キー自身が `=` を含むかどうかで 2 つの規則に分かれる。`Authorization=` のようにキーが `=` を含む場合の規則（キーの直後の非空白列を置換する）は**変更しない**。この形は呼び出し側が区切りまで含めてキーとして指定したものであり、区切りや引用符の解釈を持ち込む余地がないためである。以降の 3.2.2〜3.2.6 は、キーが `=` を含まない場合の規則についての設計である。
...
**群の判定規則**: `Config.KeyValuePatterns` は公開フィールドであり、利用者が任意のキーを追加しうる。群は列挙ではなくキー文字列からの導出で決まる。`RedactText` が 1 つのキーを処理する際、次の順に判定する。最初に一致した分岐で確定し、どのような文字列も必ずいずれかに落ちる。

1. キーが `:` を含む → コロン経路（3.2.1、本節の群分けは適用されない）
2. キーが空白を含む → 空白経路（3.2.1、同上）
3. キーが `=` を含む → 第 3 経路の既存規則（3.2.1、同上）
4. キーの先頭が英数字以外 → **群 C**
5. キーが「一般語キー一覧」に完全一致（大文字小文字は区別しない） → **群 B**
6. 上記以外 → **群 A**
...
エスケープされた引用符（`"a\"b"`）も対象外とする。これは正規表現で表現できないからではなく（RE2 でも表現可能である）、対象とする入力（コマンド出力、設定パースエラー、コマンドライン引数）でこの形が現れる頻度に対して、パターンの複雑さと誤検出の検証コストが見合わないためである（YAGNI）。なお、開き引用符と閉じ引用符が同種であることを要求するには後方参照が必要であり、RE2 はこれを持たない。本設計は `"` 用と `'` 用の選択肢を分けて書くことでこの制約を回避する。
...
**新たに置換される範囲（AC-09 の除外規定に該当する変更）**: 群 B のキーが引用符付きで現れる構造化データは、秘密でなくても置換される。たとえば AWS CLI の JSON 出力に含まれる `"key": "us-east-1"` は `"key": "[REDACTED]"` になる。これは AC-09 が明示的に対象から除いている形（短く一般的なキーが、引用符で囲まれた構造化データのフィールド名として現れる場合）であり、本設計が独自に置く例外ではない。AC-09 が挙げる根拠のとおり、AC-02（JSON 形式の捕捉）を満たすには「キー名側の引用符とコロンを伴う形」を捕捉せざるを得ず、キー名だけでは値が秘密か否かを判別できない。したがって境界規則を強めてこの形を落とすことは、AC-02 を壊すため採らない。加えて、JSON/YAML のフィールド名として `key` / `token` / `secret` が使われている箇所は、秘密を格納する用途である可能性が実際に高い。この結果は 7.1 のテストで固定し、利用者向け文書にも記載する。

#### 3.2.7 正規表現のキャッシュ

`RedactText` は呼び出しのたびに `KeyValuePatterns` の全キーについて正規表現をコンパイルしている（D2 L-1）。本タスクでパターンが複雑化し、かつ `RedactText` は長いコマンド出力に対して呼ばれるため、コンパイル結果をキャッシュする。

**格納場所**: キャッシュは既存の `compileRedactionRegex` の内側に置く。この関数は 3 経路すべての共通のコンパイル地点であり、ここに置けば colon 経路と space 経路も同じ恩恵を受ける。キーは生成された正規表現の文字列とする。
...
**上限**: エントリ数に上限（256）を設ける。上限に達した後は、キャッシュに載っていないパターンを毎回コンパイルする（現行と同じ挙動）。`Config` は公開フィールドを持つ構造体であり、`KeyValuePatterns` を実行時に差し替える呼び出し方を禁じていないため、キーの空間を有限と仮定しない。現状の本番構成では `KeyValuePatterns` は `DefaultKeyValuePatterns()` の 12 キーのみであり、生成されるパターン数は上限に遠く及ばない。
...
**適用順序**: `Mask` は各パターンを直前の結果に対して順に適用する。追加パターンは既存 7 種の**後**に、`githubPAT` → `slackPrefixToken` → `jwt` → `slackWebhookURL` の順で適用する。既存パターンを先に走らせることで、既存の検出結果と置換後の見た目が変わらないことを保証する。`slackWebhookURL` を最後に置くのは、URL 中のパス文字列が他のパターンに部分一致した場合でも、ホスト名を保持した読みやすい形が最終結果になるようにするためである。
...
ホスト名は `hooks.slack.com` の固定パターンとし、`SlackHandlerOptions.AllowedHost` からは取らない。`internal/redaction` は `internal/logging` に依存しておらず、依存を追加すると層構造が逆転するためである。既定の `AllowedHost` は `hooks.slack.com` であり、固定パターンで実運用の大半を覆える。プロキシ等で別ホストを `AllowedHost` に設定している構成では値形式検出は働かないが、`sanitizeErrorForLog` による既存の防御は残る。この限界は利用者向け文書に記載する。
...
type slackSender struct {
    // fields omitted; see the shared-state table in 3.4.6
}
...
なお同期モード（3.4.10）はこの契約の対象ではない。同期モードでも `NewSlackHandler` は `slackSender` を生成し、キューとワーカーだけを持たない形にするため、送信機構は nil にならない。
...
この窓を塞ぐため、ワーカーは**終了状態の観測とキャンセル関数の登録を同じロックの下で一体に行う**。すなわち、1 件を取り出した後に書き込みロックを取り、その 1 回の臨界区間の中で、終了状態（受付停止フラグ・終了モード・flush 期限、3.4.6）を読み、そこから送信用コンテキストを決め、そのキャンセル関数を共有状態へ格納してからロックを解放する。コンテキストの決め方は次のとおりである。
...
**送信中の 1 件の中断**: `Flush` と `Close` は、書き込みロックを取って受付停止フラグと終了状態を設定した後、同じロックの下で保持しているキャンセル関数（非 nil のとき）を呼ぶ。これにより、実行時デッドライン（40 秒）で作られた送信中のコンテキストが即座にキャンセルされ、残りの処理は flush 期限（15 秒）の内側で進む。ワーカー側は 1 件を取り出すたびに、書き込みロックを取って「終了状態の観測」と「キャンセル関数の格納」を 1 つの臨界区間で行い（3.4.5）、その送信が終わったら同じロックの下で nil に戻す。取り出しと登録を別々の臨界区間に分けないことが、`Flush` からも終了要求からも届かない 1 件が生じる窓を消す。
...
**`slackSender` 自体は生成する**: 送信に必要な状態（`webhookURL`、`httpClient`、`backoffConfig`、`failureLogger`、`sendTimeout`）は 3.5 のとおりすべて `slackSender` が持ち、`SlackHandler` 側には残さない。加えて 3.4.3 は、送信機構が nil のハンドラを「メッセージを構築せず、送信もせず、nil を返す」ものと定めている。したがって同期モードで `slackSender` の生成まで省くと、`GSCR_SLACK_SYNC=1` が「同期送信する」どころか「何も送らない」設定になってしまう。同期モードで省くのはキューとワーカーだけである。ドライラン（3.4.9）が送信機構そのものを生成しないのとは、この点で異なる。ドライランは送信自体を行わないモードであるため、送信の手段を持つ必要がない。
...
この配置の帰結として、`SlackHandler` は `slackSender` を経由せずに送信することができない。同期モード（3.4.10）で `slackSender` を生成する（キューとワーカーだけを持たない）のはこのためである。`highPriority` / `normal` の 2 本のチャネルと、ワーカーに関わる状態（終了要求チャネル、ワーカー終了通知、`inFlightCancel`）は、同期モードでは未使用のゼロ値のままとなる。
...
| キャッシュを通じた情報の混線・無限成長 | 低 | キャッシュが保持するのはコンパイル済み正規表現のみで、置換対象のテキストもプレースホルダーも保持しない。エントリ数には上限を設ける（3.2.7） |
...
flowchart TD
    ...
    S(["RedactText 開始"]) --> NEXT{"未処理のキーがある?"}
    NEXT -->|"はい"| BR{"キーの形は?"}
    ...
    NEXT -->|"いいえ"| VD["ValueDetector.Mask"]
    VD --> E(["置換後のテキスト"])

    class KV,FORM,VD enhanced
    class COLON,SPACE,KEEP process
```

**矢印の意味**: A → B は「A の次に B を実行する」ことを表す。菱形は分岐条件である。図中の 2 つの菱形（キーの形、キー自身が等号を含むか）は、3.2.4 の判定規則の 1〜3 に対応する。規則の 4〜6（群 A・群 B・群 C の判定）は「選択肢 V3 → V2 → V1 を適用」の内部で、キーごとに先頭境界を決めるために行われる。`ValueDetector.Mask` はキーごとではなく、全キーの処理が終わった後に 1 回だけ適用される。この順序は、構造化された key=value の組を先に精密に隠し、その後で残りのテキストに含まれる裸の秘密を拾うという既存の設計意図によるものであり、変更しない。
...
この状態遷移は、ワーカーを持つ送信機構、すなわち非同期モードのものだけに適用される。同期モード（3.4.10）の送信機構はキューもワーカーも持たず、送信は `Handle` の中で完結するため、`Flush` / `Close` は受付停止フラグを立てるだけで「送信終了」へ直接移る。
...
| 群の判定規則 | 既定の 12 キーが 3.2.4 の判定規則で意図した群に落ちること。および、一覧にないキー（`passphrase`）を `KeyValuePatterns` へ追加すると群 A として扱われること | AC-05 |
| 既存挙動の非退行 | `Authorization: Bearer xxx`、`Bearer xxx`、`Basic xxx`、キー自身が等号を含む場合 | AC-06, AC-07, AC-08 |
...
| キャッシュ | 複数 goroutine から同時に `RedactText` を呼んでも結果が同じで競合しないこと | AC-32 |
...
| フェーズ | 内容 | 対応 AC |
|---|---|---|
| Phase 1 | 正規表現キャッシュの導入（挙動を変えないリファクタリング） | 前提 |
...
- **一般語キー一覧の拡張**: 3.2.4 の一覧に語を足せば、その語は群 A から群 B へ移り、散文中での過剰 redaction が減る。過剰 redaction の報告を受けた語を追加する運用を想定している。逆に、利用者が語ごとに群を指定できる設定項目は設けない。必要になった時点で `KeyValuePatterns` を文字列のリストから群を持つ構造へ変える拡張は可能だが、現時点では要求がない（YAGNI）。
...
| AC-18 | 3.4.2、6.2 |
| AC-19 | 3.4.5 デッドライン（`GSCR_SLACK_SEND_TIMEOUT`） |
