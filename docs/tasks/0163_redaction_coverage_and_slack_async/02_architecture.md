...
**残存リスク（群 B の YAML 取りこぼし）**: 上記の裏返しとして、群 B のキー（`key` / `token` / `secret`）は**引用符のない YAML では捕捉されない**。`token: ghp_xxx` のように行頭・インデント直後・空白の直後に現れる形は、厳しい先頭境界を満たすものが直前にないため V2 に一致しない。YAML は JSON に次いで秘密を格納する形式として一般的であり、この取りこぼしは実害を伴いうる。
...
**規則**: どの選択肢もキーをリテラルとして必要とする。したがってテキストがキーを大文字小文字を無視して含まないならば、そのキーの正規表現は一致しえない。`RedactText` はテキストを 1 度だけ ASCII 小文字化し、各キーについて `strings.Contains` で存在を確かめてから正規表現を走らせる。あるキーが実際に置換を行ったときだけ小文字化をやり直す（置換は稀であり、通常は 1 回で済む）。

**ASCII に限定する理由**: Go の正規表現は Unicode の規則で大文字小文字を畳み込む。U+212A KELVIN SIGN は `(?i)k` に、U+017F LATIN SMALL LETTER LONG S は `(?i)s` に一致するが、ASCII 小文字化はこれらを変換しない。そのため非 ASCII を含むテキストに事前判定をかけると、`paſſword=s3cr3t`（`(?i)password` は一致する）を「一致しえない」と誤判定し、**redaction を弱める**。非 ASCII を 1 バイトでも含むテキストでは事前判定を行わず、従来どおり全キーの正規表現を走らせる。これらのルーンは ASCII のテキストには出現しえないため、ASCII に限定すれば誤判定の可能性は原理的に消える。

**効果**: 拡張前（main）17.0 µs / 13.6 µs に対し、事前判定ありで 15.6 µs / 5.85 µs（`BenchmarkRedactText` / `BenchmarkRedactText_NoSensitiveData`）。いずれも拡張前を下回る。

**副次的な挙動の変化**: キーが存在しないテキストでは、そのキーの正規表現をコンパイルしなくなる。コンパイル失敗時のフェイルセキュア（`RedactionFailurePlaceholder`）はキーが出現するテキストでのみ発火するようになるが、そもそも一致しえないキーが出力を潰さなくなるだけであり、秘密の露出には結びつかない。
