...
- **AC-09**: 秘密を含まないテキスト（例: `--timeout=30`、`keyboard: qwerty`、`/usr/local/key/path`）に対し、F-001 の拡張によって新たに置換される範囲が生じない。過剰 redaction の範囲が拡張前と同一であることをテストで固定する。ただし、`key` / `token` / `secret` のような短く一般的なキーが、引用符で囲まれた構造化データ（JSON/YAML）のフィールド名として現れる形（例: `"key": "us-east-1"`）は、この不変条件の対象から除く。AC-02 が `"password": "secret"` の捕捉を求めている以上、「キー名側の引用符とコロンを伴う形」を捕捉対象から外すことはできず、キー名だけでは値が秘密か否かを判別できないためである。除外の対象と根拠は 02_architecture.md に記載し、除外されたケースの置換結果をテストで固定する。
...
- **AC-36**: TOML の `slack_allowed_host` に `hooks.slack.com` 以外のホスト（Mattermost 等の Slack 互換サービス）を設定した構成で、そのホスト配下の webhook URL のパス部分が置換される。ホスト名は置換後も残る。
- **AC-37**: `slack_allowed_host` が設定されていない構成（Slack 通知を使わない場合）で、値形式検出の結果が本タスクによる拡張の前と一致する。

（AC-36・AC-37 は F-004 以降の項目を確定させた後に追加したため、番号は文書末尾からの連番となっている。）
...
**Acceptance Criteria**:
- **AC-33**: `docs/user/security-risk-assessment.ja.md` の「値形式検出」の検出対象一覧に、F-003 で追加した形式が反映されている。「限界」の記述に、F-001 の検討事項で採らなかった側の残存リスク（引用符なしで空白を含む値の扱い）が追記されている。英語版は `/mktrans` で反映する。
