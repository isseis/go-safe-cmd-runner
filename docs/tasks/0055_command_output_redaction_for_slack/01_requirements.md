...
- NFR-4.3：テストカバレッジは 90% 以上を維持すること（新規追加・変更されたコードおよび RedactingHandler モジュールに適用する。既存コード全体への適用は任意とする）
...
1. **ダウンタイム**：
   - **通常デプロイ**: サービスを停止しないローリングアップデート方式で実施するため、計画的なダウンタイムは発生しない。
   - **ロールバック時**: 重大な問題が発見された場合、迅速なロールバックを実施する。ロールバック処理中に一時的なサービス瞬断が発生する可能性がある。
...
| リスク | 確率 | 影響度 | 対策 |
|--------|------|--------|------|
| 無限再帰によるスタックオーバーフロー | Low | High | 再帰深度の制限（上限 10、詳細はアーキテクチャ設計書参照）を設ける |
