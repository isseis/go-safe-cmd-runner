...
- [x] `internal/common/filesystem.go` に `resolveMode` 型を追加
- [x] `ResolvedPath` に `mode` フィールドを追加
- [x] `NewResolvedPath` が `resolveModeFull` を設定するよう修正
- [x] `NewResolvedPathParentOnly` が `resolveModeParentOnly` を設定するよう修正
- [x] `IsParentOnly() bool` メソッドを追加
- [x] 既存テストを更新し、必要に応じて新規テストを追加
...
const (
    resolveModeFull       resolveMode = iota + 1 // NewResolvedPath が設定
...
- [x] `internal/safefileio/safe_file.go` の `safeWriteFileCommon` に
      `filePath.IsParentOnly()` チェックを追加
- [x] `internal/safefileio/safe_file.go` の `safeAtomicMoveFileWithFS` に
...
- [x] `NewResolvedPath` 生成値を `SafeWriteFile` に渡したとき
      `ErrInvalidFilePath` になるテストを追加
- [x] `NewResolvedPath` 生成値を `SafeWriteFileOverwrite` に渡したとき
      `ErrInvalidFilePath` になるテストを追加
- [x] `NewResolvedPath` 生成値を `SafeAtomicMoveFile` の `srcPath` / `dstPath` に
      渡したとき `ErrInvalidFilePath` になるテストを追加
- [x] `SafeReadFile` は `NewResolvedPath` / `NewResolvedPathParentOnly` の両方で
      正常動作することを確認
