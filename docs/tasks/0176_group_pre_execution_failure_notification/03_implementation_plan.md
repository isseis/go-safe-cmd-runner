...
- [x] `internal/logging/slack_handler_test.go` に `TestBuildPreExecutionError_InterpolationContract` を追加する。改行・制御文字・書式制御文字（`U+202E`）を引用せずに含む `error_message` と、500 byte を超える `error_message` を RedactingHandler 経由のレコードで与え、`buildPreExecutionError` の `Error Message` が 1 行で、制御文字と書式制御文字を含まず、`common.Interpolate` の上限以下であることを `assertDisplaySafeProperties` で検証する（AC-18）。各本文が redaction で置き換わらずに残ることも確認し、プレースホルダだけで性質が満たされる空振りを防ぐ。`%q` で引用済みのパスを含む本文の行は置かない。引用済みの本文は印字可能な ASCII だけからなり、ビルダーの補間を外しても通るため、ビルダーを検証できない（`%q` の引用そのものは Phase 3 の `TestExecuteGroup_PreExecutionStageErrors` が固定する）。既存の `redactedPreExecutionErrorMessage` は本文を引数に取る `redactedPreExecutionErrorField` を呼ぶ形にして共有する。
...
- AC-18 は 2 層で確認する。Phase 3 のテストが、#1/#3 の原因の文言に生の改行が入ることと、#6/#7 のパスが `%q` で引用されること（引用が効く経路）を固定する。ビルダーが引用を受けない入力を 1 行・上限内に収めることは、`internal/logging` の `TestBuildPreExecutionError_InterpolationContract` が固定する。引用済みの入力は印字可能な ASCII だけからなり、ビルダーの補間の有無によらず性質を満たすため、ビルダー側の行は置かない。両者を組み合わせて初めて端から端の性質が成り立つ。
