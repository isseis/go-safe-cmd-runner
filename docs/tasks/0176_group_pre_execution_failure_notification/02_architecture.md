...
- **引用と補間契約の組み合わせ（AC-18）。** 改行と代表的な制御文字・書式制御文字（例: U+202E）を含むコマンドパスで、コマンド検証を失敗させる。#6 は `ResolvePath` がエラーを返すモック、#7 は `VerifyCommandDependencies` がエラーを返すモックで起こす。`Error()` のパスが `%q` で引用され、改行・制御文字・書式制御文字がエスケープ表記になることを検証する。`%q` の出力は印字可能な ASCII だけからなるため、ビルダーの補間の有無によらず 1 行で制御文字を含まない。したがって引用済みの本文をビルダーに通す検査は置かず、ビルダーの検査は上の引用を受けない入力で行う。上限は引用の有無によらずビルダーの補間が担う。
