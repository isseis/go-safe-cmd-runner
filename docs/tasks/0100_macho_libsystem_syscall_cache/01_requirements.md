...
### 1.4 dyld shared cache 対応方針

macOS 11 (Big Sur) 以降の通常環境では `libsystem_kernel.dylib` はファイルシステム上
に個別ファイルとして存在しない。この状況でシンボル名単体一致のみをフォールバックとして
用いる設計（当初の「段階 1」案）は、ライブラリの実測 syscall 番号を取得できず、
本タスクの目的（ハードコードに依存しない実測値キャッシュ）をほぼ達成できない。

そのため、本タスクでは dyld shared cache からの抽出を **スコープに含める**。

**優先順位 1 — ファイルシステム**: `libsystem_kernel.dylib` がファイルシステム上に
存在する場合、直接開いてエクスポート関数を逆アセンブルし、キャッシュを構築する。

**優先順位 2 — dyld shared cache 抽出**: ファイルシステム上に存在しない場合
（macOS 11+ の通常環境）、`blacktop/ipsw` の `pkg/dyld` パッケージを用いて
dyld shared cache から `libsystem_kernel.dylib` を抽出し、同じ関数単位解析を適用する。

**最終フォールバック**: dyld shared cache からの抽出も失敗した場合（異常環境）、または
libSystem 系ライブラリが `DynLibDeps` に含まれていない場合のみ、シンボル名単体一致
（FR-3.4）にフォールバックする。
...
{
  "schema_version": 1,
  "lib_path": "/usr/lib/system/libsystem_kernel.dylib",
...
**dyld shared cache から抽出した場合の `lib_hash`**: 抽出したライブラリのバイト列を
SHA-256 でハッシュした値を使用する。ファイルシステム上のパスが存在しないため、
dyld shared cache ファイル全体ではなく抽出結果のバイト列をハッシュすることで、
ライブラリの実質的な変化（macOS アップデート後の新キャッシュ生成）を検出できる。
...
2. `libSystem.B.dylib` が `DynLibDeps` に含まれているが、`libsystem_kernel.dylib` が
   ...
   - `libSystem.B.dylib` がファイルシステム上に存在しない場合（dyld shared cache 内）：
     ...
     ファイルが存在すればそのパスで解析を進める。存在しなければ
     dyld shared cache 抽出（FR-3.1.6）を試みる
3. libSystem 系ライブラリが `DynLibDeps` に含まれていない場合：最終フォールバック
   （FR-3.4）を適用する

**dyld shared cache 内のケース**: 上記いずれの方法でも `libsystem_kernel.dylib` が
ファイルシステム上に存在しない場合（macOS 11+ の通常環境）は、dyld shared cache
からの抽出（FR-3.1.6）を試みる。抽出に成功した場合はそのバイト列を使用して解析を
継続する。抽出にも失敗した場合は最終フォールバック（FR-3.4）を適用する。

#### FR-3.1.6: dyld shared cache からの `libsystem_kernel.dylib` 抽出

`libsystem_kernel.dylib` がファイルシステム上に存在しない場合（macOS 11+ 通常環境）、
以下の手順で dyld shared cache から抽出する。

**dyld shared cache のパス**: 以下の順で試行し、最初に存在するファイルを使用する。

1. `/System/Library/dyld/dyld_shared_cache_arm64e`
2. `/System/Library/dyld/dyld_shared_cache_arm64`

**ライブラリの抽出**: `blacktop/ipsw` の `pkg/dyld`
（`github.com/blacktop/ipsw/pkg/dyld`）を使用して dyld shared cache を解析し、
インストール名 `/usr/lib/system/libsystem_kernel.dylib` に対応するイメージを
メモリ上に展開する。

**`lib_hash` の決定**: FR-3.1.4 の補足に従い、抽出したバイト列の SHA-256 ハッシュを
使用する。

**`lib_path` の決定**: キャッシュファイルの `lib_path` には
`/usr/lib/system/libsystem_kernel.dylib`（dyld shared cache 内のインストール名）
を使用する。

**失敗時の挙動**: 以下のいずれかの場合は最終フォールバック（FR-3.4）を適用し、
エラーにはしない。ただし `slog.Info` レベルのログを出力する。

- dyld shared cache ファイル（上記 2 パスとも）が存在しない
- `libsystem_kernel.dylib` のイメージが dyld shared cache 内に見つからない
- 展開・解析に失敗した
...
4. 存在しない場合（dyld shared cache 内）：dyld shared cache からの抽出（FR-3.1.6）
   を試みる。抽出成功時はステップ 3 と同様にキャッシュ参照・生成を行う。抽出失敗時は
   最終フォールバック（FR-3.4）を適用する
...
| ライブラリがファイルシステム上に存在しない（dyld shared cache 内） | dyld shared cache からの抽出（FR-3.1.6）を試みる。抽出失敗時のみ最終フォールバック（FR-3.4）を適用。エラーなし |
...
1. `libsystem_kernel.dylib` がファイルシステム上に存在せず、かつ dyld shared cache
   からの抽出（FR-3.1.6）にも失敗した場合
2. libSystem 系ライブラリが `DynLibDeps` に含まれていない場合
...
最終フォールバック（シンボル名単体一致）は追加のファイル I/O を伴わないため、
...
- [ ] dyld shared cache 内にのみ `libsystem_kernel.dylib` が存在する環境（macOS 11+）で
  `record` を実行した際、抽出・解析を経てキャッシュファイルが生成されること
...
- [ ] `libsystem_kernel.dylib` がファイルシステム上に存在せず、かつ dyld shared cache
  からの抽出にも失敗した場合、シンボル名単体一致にフォールバックすること
...
### 6.2.1 dyld shared cache 抽出のユニットテスト

| テストケース | 検証内容 |
|-------------|---------|
| `dyld_shared_cache_arm64e` が存在する場合に `libsystem_kernel.dylib` が抽出されること | 正常系 |
| 抽出したバイト列の SHA-256 が `lib_hash` として使用されること | ハッシュ確認 |
| `dyld_shared_cache_arm64e` が存在せず `arm64` も存在しない場合にフォールバックが発動すること | 不在時の動作 |
| dyld shared cache 内に `libsystem_kernel.dylib` が見つからない場合にフォールバックが発動すること | 異常系 |
...
| テストケース | 検証内容 |
|-------------|---------|
| ライブラリ非存在かつ dyld shared cache 抽出失敗時にフォールバックが発動すること | 条件確認 |
...
| テストケース | 検証内容 | 前提条件 |
|-------------|---------|---------|
| macOS で `record` した動的 Mach-O バイナリの `SyscallAnalysis` にネットワーク syscall が検出されること | エンドツーエンド（dyld shared cache 経由） | macOS arm64 |
| dyld shared cache 抽出失敗時に最終フォールバックが発動し `SyscallAnalysis` にネットワーク syscall が検出されること | フォールバック確認 | macOS arm64 |
...
| ライブラリ | 用途 | 備考 |
|-----------|------|------|
| `github.com/blacktop/ipsw/pkg/dyld` | dyld shared cache のパースと `libsystem_kernel.dylib` イメージの抽出 | macOS 環境でのみ使用。Linux ビルドでは build tag で除外する |
...
| 最終フォールバック | シンボル名単体一致 | dyld shared cache 抽出も失敗した異常環境向けの最終手段 |
| macOS syscall テーブル | Linux テーブルとは別に定義 | BSD syscall 番号は Linux と異なる |
| スキーマバージョン変更 | 不要 | フィールド追加なし、既存フィールドの値追加のみ |
| dyld shared cache 対応 | 本タスクのスコープに含める | macOS 11 以降ではファイルシステム上に `libsystem_kernel.dylib` が存在しないため、フォールバックのみでは実測値キャッシュの目的を達成できない。`blacktop/ipsw` の `pkg/dyld` を使用して抽出・解析する |
