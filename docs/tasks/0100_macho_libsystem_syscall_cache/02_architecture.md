...
macOS 11 以降は `libsystem_kernel.dylib` がファイルシステム上に存在しないため、
dyld shared cache からの抽出（`blacktop/ipsw/pkg/dyld`）を主経路とする。
...
// arm64 以外のアーキテクチャは UnsupportedArchitectureError を返す。
...
type LibSystemKernelSource struct {
    // Path is the filesystem path (non-empty when on filesystem).
    Path string
...
// Returns nil if:
//   - libSystem 系ライブラリが DynLibDeps に含まれない（フォールバック適用）
//   - dyld shared cache からの抽出も失敗（フォールバック適用）
...
type MachoLibSystemAdapter struct {
    ...
    fs           safefileio.FileSystem  // ResolveLibSystemKernel に渡す
...
//   - libSystem 系ライブラリが dynLibDeps に含まれていない（FR-3.4.1 条件2）
//   - dyld shared cache からの抽出も失敗した場合（FR-3.4.1 条件1）
//
// Returns (nil, nil) immediately when dynLibDeps が空 or libSystemCache が nil.
...
2. 結果が `nil` の場合（libSystem 非存在または dyld shared cache 抽出失敗）:
   - `slog.Info` でフォールバック理由をログ出力（FR-3.4.3）
...
func (v *Validator) analyzeLibSystemImports(
    ...
    // machoanalyzer.NormalizeSymbolName でアンダースコアプレフィックスを除去する。
    // （例: "_socket" → "socket"、"_socket$UNIX2003" → "socket"）
    // NormalizeSymbolName は本タスクで machoanalyzer.normalizeSymbolName を
    // エクスポートしたもの（§8 の変更対象ファイル一覧参照）。
...
// ...LoadSyscallAnalysis → switch で svcResult に代入...
...
**既存の `Match()`（変更不要）**:
```go
// ELF 版で引き続き使用。DeterminationMethod = "immediate" を固定設定する。
