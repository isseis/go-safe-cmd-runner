...
### AC-11: filevalidator の統合解析結果ストア移行

- [ ] 既存の HashManifest 形式（旧形式）のハッシュファイルが存在する場合、新しい FileAnalysisRecord 形式（新形式）へ自動移行されること
- [ ] 新形式への移行後、旧形式ファイルが削除されること
- [ ] 新形式でハッシュ検証が正常に動作すること
- [ ] 移行時に既存の syscall 解析結果（存在する場合）が保持されること
