...
Go 標準ライブラリ `debug/elf` および準公式パッケージ `golang.org/x/arch/x86/x86asm` を使用して ELF バイナリの機械語を直接デコードし、ネットワーク関連のシステムコール（`socket`, `connect` 等）が含まれるかどうかを事前に検出する。解析結果はファイルのハッシュ値と共にキャッシュし、runner 実行時にはキャッシュを参照することで実行時コストを最小化する。
...
| キャッシュ | 事前解析の結果をファイルのハッシュ値と共に保存したもの |
...
`syscall` 命令から逆方向に最大 N 命令（N は設計フェーズで決定、目安: 5）を走査し、`eax`/`rax` レジスタを最初に変更している命令を探す。その命令が即値設定であれば、システムコール番号として抽出できること。
...
FR-3.1.3 の逆方向スキャンにおいて、以下のいずれかに該当する場合、そのときは即座に判定せず **FR-3.1.6 (Go syscall ラッパーの解析)** を試行すること。FR-3.1.6 でも解決できない場合に限り、番号不明として **high risk** と判定すること：
...
4. **判定**:
    ...
    - 呼び出し元でも即値が特定できない（変数渡し等）場合は、High Risk と判定する。
    - シンボルテーブルが存在しない（stripされている）場合は、本解析はスキップし、FR-3.1.3/3.1.4 の結果（High Risk）に従う。
...
**スキーマバージョンについて**: キャッシュ読み込み時に、保存されたスキーマバージョンとプログラムが期待するスキーマバージョンを比較し、一致しない場合はキャッシュを無効とすること。これにより、解析ロジックの修正・改善や検出パターンの追加を行った際に、古いキャッシュが使われ続けることを防ぐ。

#### FR-3.2.2: 保存先

キャッシュファイルは既存の `filevalidator` と同じディレクトリに保存すること。ファイル名の生成には既存の `HybridHashFilePathGetter` の仕組みを再利用または拡張すること。

**注記**: ハッシュ検証ファイルと syscall 解析キャッシュファイルの区別方法（ファイル名のプレフィックス、JSON の `format` フィールド等）は設計フェーズで決定する。

#### FR-3.2.3: runner 実行時のキャッシュ利用

runner 実行時には以下の手順でキャッシュを利用すること：

1. 実行ファイルのハッシュ値を計算する
2. キャッシュファイルを検索する
3. キャッシュのスキーマバージョンがプログラムの期待するバージョンと一致することを確認する
4. キャッシュのハッシュ値と一致する場合、記録済みの解析結果を利用する
5. キャッシュが存在しない、スキーマバージョンが不一致、またはハッシュ値が一致しない場合は「不明」として扱う

#### FR-3.2.4: キャッシュ不在時の動作

キャッシュが存在しない場合、runner 実行時に `objdump` による解析は行わない。解析結果は「不明」として扱い、既存の判定ロジックにフォールバックすること。
...
3. **3rd**: ELF 機械語 syscall 解析キャッシュ（本タスク、事前解析済みの場合のみ）
...
#### NFR-4.1.1: 実行時のキャッシュ参照

runner 実行時のキャッシュ参照はハッシュ計算とファイル読み込みのみであり、実行時コストを最小限に抑えること。

#### NFR-4.1.2: 事前解析のパフォーマンス

`objdump` による事前解析は大きなバイナリに対して相応の時間を要することを許容する。ただし、解析の進捗状況をユーザーに表示すること。

### 4.2 セキュリティ

#### NFR-4.2.1: キャッシュの改竄検知

キャッシュファイルに記録されたハッシュ値と実行ファイルの実際のハッシュ値を比較することで、実行ファイルの改竄を検知すること。キャッシュのハッシュ値が一致しない場合はキャッシュを無効とすること。

#### NFR-4.2.2: キャッシュファイルの安全な読み書き

キャッシュファイルの読み書きは既存の `safefileio` パッケージを使用し、シンボリックリンク攻撃などに対する保護を維持すること。
...
- [ ] ELF バイナリの実行可能セクションから `syscall` 命令を正しく検出できること
- [ ] `syscall` 命令から逆方向にスキャンし、`eax`/`rax` への即値設定を抽出できること
- [ ] 即値設定命令と `syscall` 命令の間に `eax`/`rax` に無関係な命令が挟まっていても正しく抽出できること

### AC-2: ネットワーク関連 syscall の判定

- [ ] `socket`(41), `connect`(42) 等のネットワーク関連 syscall 番号が検出された場合、ネットワーク操作ありと判定されること
- [ ] ネットワーク関連以外の syscall 番号のみが検出された場合、ネットワーク操作なしと判定されること

### AC-3: 間接設定の high risk 判定

- [ ] スキャン範囲内で `eax`/`rax` を変更する命令が即値設定でない場合（レジスタ間 `mov`、メモリロード等）、high risk と判定されること
- [ ] スキャン範囲内に `eax`/`rax` を変更する命令が見つからない場合、high risk と判定されること
- [ ] スキャン中に制御フロー命令に到達した場合、high risk と判定されること
- [ ] high risk の判定理由が「syscall 番号が静的に決定不可能」であることが記録されること

### AC-4: キャッシュの保存と読み込み

- [ ] 事前解析結果がスキーマバージョンおよびハッシュ値と共にキャッシュファイルに保存されること
- [ ] キャッシュファイルが既存の filevalidator と同じディレクトリに保存されること
- [ ] runner 実行時にキャッシュファイルが正しく読み込まれること

### AC-5: キャッシュの整合性検証

- [ ] 実行ファイルのハッシュ値とキャッシュのハッシュ値が一致する場合、キャッシュの解析結果が利用されること
- [ ] 実行ファイルが変更された（ハッシュ不一致の）場合、キャッシュが無効化されること
- [ ] キャッシュのスキーマバージョンがプログラムの期待するバージョンと異なる場合、キャッシュが無効化されること
# AC-10: Go syscall ラッパーの解決

- [ ] `syscall.Syscall` 等の標準ラッパー関数を使用している単純な Go プログラム（net パッケージ使用等）おいて、High Risk（不明）とならずに正しくネットワーク操作の有無を判定できること
- [ ] ラッパー関数の呼び出し元でシステムコール番号が即値指定されているケースを検出できること

##
### AC-6: キャッシュ不在時の安全な動作

- [ ] キャッシュが存在しない場合、runner がエラーにならず正常に動作すること
- [ ] キャッシュ不在時は解析結果が「不明」として扱われ、既存のロジックにフォールバックすること

### AC-7: 非 ELF ファイルのエラーハンドリング

- [ ] 非 ELF ファイル（スクリプト、他フォーマットのバイナリ等）に対して事前解析を実行した場合、適切なエラーまたはスキップが行われること
- [ ] 対象外アーキテクチャ（非 x86_64）の ELF ファイルに対して、適切なエラーまたはスキップが行われること

### AC-8: フォールバックチェーンの統合

- [ ] `commandProfileDefinitions` に含まれるコマンドは従来通りリストベースの判定が適用されること
- [ ] リストにないコマンドに対して、ELF `.dynsym` 解析（0069）→ syscall 解析キャッシュ（本タスク）の順でフォールバックが動作すること
...
FR-3.1.6 へフォールバック (テストでは High Risk)
- `mov (%rsp), %eax` + `syscall` のバイト列 → メモリロード、FR-3.1.6 へフォールバック (テストでは High Risk)
- `syscall` のみ（スキャン範囲内に `eax`/`rax` 変更命令なし） → High Risk と判定
- `jmp` + `mov $0x29, %eax` + `syscall` のバイト列 → 制御フロー境界を超えた即値設定は無視、High R出（間に無関係な命令を挟むケース）
...
- ハッシュ一致時のキャッシュ利用
- ハッシュ不一致時のキャッシュ無効化
- スキーマバージョン不一致時のキャッシュ無効化
- 不正な JSON キャッシュファイルの処理
...
2. 事前解析を実行
3. キャッシュファイルが生成されることを検証
4. runner 実行時にキャッシュが利用されることを検証

テスト環境に `gcc` が存在しない場合はテストをスキップすること。

**注記**: 本実装は外部ツール（`objdump` 等）に依存しないため、`gcc` さえあれば統合テストを実行可能である。

## 6. 設計上の制約と限界ただし、Go の標準的なラッパー関数に関しては FR-3.1.6 により呼び出し元解析を行い、可能な限り解決を試みる。
...
4. **大規模バイナリ**: 大きなバイナリのデコードは時間を要する。事前解析 + キャッシュ方式により、この影響は事前解析時に限定される。
5. **Stripされたバイナリ**: シンボル情報が削除（strip）されたバイナリでは、ラッパー関数の特定（FR-3.1.6）ができないため、syscall 命令周辺の解析のみとなり、多くのケースで High Risk と判定される可能性がある。これは「Security First」として許容す
...
2. **間接的なレジスタ設定**: `mov $imm, %eax` 以外のパターン（レジスタ間 `mov`、メモリからのロード、計算結果等）では syscall 番号を静的に決定できない。これは high risk として扱う。特に Go ランタイムの汎用 `Syscall6` ラッパーはスタックから syscall 番号をロードするため、常に high risk と判定される。
3. **逆方向スキャンの限界**: 線形デコードではベーシックブロック境界を厳密に検出できないため、制御フロー命令の簡易検出で代替する。コンパイラの最適化により即値設定命令と `syscall` 命令の間に他の命令が挿入されるケースは、スキャン幅 N で対応するが、N を超える距離の場合は high risk と判定される。
4. **大規模バイナリ**: 大きなバイナリのデコードは時間を要する。事前解析 + キャッシュ方式により、この影響は事前解析時に限定される。
