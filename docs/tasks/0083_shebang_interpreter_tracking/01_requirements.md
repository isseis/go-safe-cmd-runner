...
改行文字は `\n` のみを認識する（Linux カーネル準拠）。`\r\n` 改行のスクリプトはサポートしない。`\r` がトークンに混入した場合の具体的な挙動はトークン位置によって異なる（例: `#!/bin/sh\r\n` ではインタープリタパスに `\r` が含まれ絶対パスとして不正になるが、`#!/bin/sh -e\r\n` では引数側に `\r` が含まれインタープリタパス自体は一見有効に見える）が、いずれも record 時にエラーとして扱う。
...
| ID | 基準 |
|----|------|
| AC-1 | `#!/bin/sh` を含むスクリプトを record すると、`/bin/sh` を `filepath.EvalSymlinks` で解決した実体パスの独立 Record が作成される |
| AC-2 | `#!/bin/bash -e` を含むスクリプトを record すると、`/bin/bash` を `filepath.EvalSymlinks` で解決した実体パスの独立 Record が作成される（`-e` は無視） |
| AC-3 | `#!/usr/bin/env python3` を含むスクリプトを record すると、`/usr/bin/env` を `filepath.EvalSymlinks` で解決した実体パスと、解決済みの `python3` バイナリの独立 Record が作成される |
| AC-4 | スクリプトの `Record` に `shebang_interpreter` フィールドが保存される |
| AC-5 | `#!/usr/bin/env python3` の場合、`shebang_interpreter.interpreter_path` が `/usr/bin/env` を `filepath.EvalSymlinks` で解決した実体パス、`shebang_interpreter.command_name` が `python3`、`shebang_interpreter.resolved_path` が解決済みパスになる |
| AC-6 | `#!/bin/sh` の場合、`shebang_interpreter.interpreter_path` が `/bin/sh` を `filepath.EvalSymlinks` で解決した実体パスとなり、`command_name` および `resolved_path` は省略される |
...
| AC-21 | `#! /bin/sh`（`#!` 直後にスペース）を含むスクリプトを record すると、`/bin/sh` を `filepath.EvalSymlinks` で解決した実体パスの独立 Record が作成される |
