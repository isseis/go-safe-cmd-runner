...
- **詳細:**
  ...
  3. ファイルのSHA-256ハッシュ値を計算する。
  4. ハッシュファイル名を生成し、ハッシュ保存ディレクトリに書き込む。
  5. 既存のハッシュファイルがある場合は、パス衝突をチェックする。
...
- **詳細:**
  ...
  3. 対応するハッシュファイルから記録済みのハッシュ値と元のファイルパスを読み込む。
  4. 記録されたパスと現在のパスが一致することを確認する。
  5. 2つのハッシュ値を比較し、結果（一致/不一致）を返す。
...
- **詳細:**
  1. 入力として、検証対象のファイルパスを受け取る。
  2. ファイルパスを検証する。
  3. ファイルパスからハッシュファイル名を生成し、フルパスを返す。

#### 機能ID: F-004
- **機能名:** 安全なファイル読み込み
- **概要:** セキュリティを考慮した安全なファイル読み込み機能を提供する。
- **詳細:**
  1. O_NOFOLLOW フラグを使用してシンボリックリンクを追跡しない。
  2. ファイルサイズ制限（128MB）を設ける。
  3. TOCTOU攻撃を防ぐためファイルディスクリプタベースの検証を行う。
...
- **詳細:**
  1. O_NOFOLLOW フラグを使用してシンボリックリンクを追跡しない。
  2. O_EXCL フラグを使用して既存ファイルへの意図しない上書きを防ぐ。
  3. パスコンポーネントにシンボリックリンクが含まれていないことを確認する。
