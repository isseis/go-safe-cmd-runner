...
// the go:build test helpers (newGoRunCmdWithHashDir, recordHash, setupTempConfig)
// that that untagged file cannot reference.
func TestE2E_ValidRunIDIsAdopted(t *testing.T) {
	const wantRunID = "backup-20260805-143000"
	...
	cmd := newGoRunCmdWithHashDir(t, hashDir, "-config", configFile, "-dry-run",
		"-dry-run-detail", "summary", "-log-dir", logDir, "-run-id", wantRunID)
