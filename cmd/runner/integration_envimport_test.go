	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.wantErr {
				...
				_, err = config.ExpandCommand(&cfg.Groups[0].Commands[0], runtimeGroup, runtimeGlobal, common.NewUnsetTimeout(), nil)
				assert.Error(t, err, "should fail when env_import var not in allowlist")
			} else {
				// Test success case
				envPriorityTestHelper(t, tt.systemEnv, tt.configTOML, tt.expectVars)
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(cfg.Groups) > 0 {
				...
				if len(cfg.Groups[0].Commands) > 0 {
					runtimeCmd, err := config.ExpandCommand(&cfg.Groups[0].Commands[0], runtimeGroup, runtimeGlobal, common.NewUnsetTimeout(), nil)
