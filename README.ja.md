...
- **ELFバイナリ静的解析**: ELFバイナリの `.dynsym` セクションを解析し、実行前にネットワーク機能・危険なシスコール・動的ライブラリ依存関係を検出
- **シェバンインタープリタ追跡**: `record` コマンドがスクリプトのシェバン行に記載されたインタープリタを自動記録し、実行時にインタープリタの差し替え攻撃を検出
...
- **ELFバイナリ静的解析**: ELFバイナリの `.dynsym` セクションを解析してネットワーク機能（socket/connect シンボル）、危険なシスコールパターン（PROT_EXEC 付き mprotect/pkey_mprotect）、動的ライブラリ依存関係を検出
- **シェバンインタープリタ追跡**: `record` コマンドがスクリプトのシェバン行（`#!/bin/sh` の直接指定形式、`#!/usr/bin/env python3` の env 形式を両方サポート）を自動検出・記録する。実行時に `runner` がインタープリタのハッシュを検証し、env 形式の場合は `record` 時と同じパスに解決されることを確認する
...
- **Slack統合**: コマンドグループの実行結果と実行前エラーのリアルタイム通知（`slack_allowed_host` によるWebhook URLホスト検証付き）
...
- **自動リスク評価**: コマンドの性質（Axis-1）とオペランドのパス信頼ゾーン（Axis-2）を組み合わせたリスク分類
- **設定可能な閾値**: コマンドごとのリスクレベル制限
- **自動ブロック**: 高リスクコマンドの自動ブロック
- **リスク監査ログ**: 許可・拒否の判断根拠をリーズンコードおよびオペランドゾーン情報とともに記録
- **リスクカテゴリ**:
  - **低**: 基本操作（ls、cat、grep）、または安全ゾーン（`/tmp`・自動生成作業ディレクトリ）への書き込みを伴う操作
  - **中**: 通常パスへのファイル変更（cp、mv）、システム変更（mount、crontab）、`systemctl status`/`show` などの読み取り専用サブコマンド
  - **高**: パッケージ管理（apt、yum、dpkg 等）、信頼境界パス（`/etc`、`/usr`、`/lib`、`/boot`、`/var`、`/sbin`、デバイスノードなど）への書き込み操作、破壊的操作
