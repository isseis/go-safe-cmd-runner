...
`record` または `verify` がルートとして実行され、ファイル読み取り権限チェックの基準UID として `SUDO_UID` から基準UID を使用する場合、その UID は使用前にユーザーデータベース上に存在することが確認されます。存在しないまたは解決不可能な `SUDO_UID` 値を持つ呼び出しは、未検証の値を無言で採用する代わりに直ちに失敗します。
...
両方のコマンドは、起動時に権限チェック基準UID を一度だけ解決します。`SUDO_UID` が採用され、実 UID と異なる場合、構造化ログ（`log/slog`、つまり標準エラー）に対して一度だけプロセスごとに警告が発行されます。採用 UID、実 UID、および参照されるユーザーデータベースを名付けます。`sudo` の下では、これは通常の場合であるため、`sudo record` または `sudo verify` の実行ごとに1つのそのような警告を期待してください。これは、権限チェックが使用した UID を記録し、障害ではありません。
...
- フェイルセーフ動作：解析エラーはポテンシャルネットワーク操作として扱われます
...
**新しいフラグ:**
- `--show-sensitive`: 機密環境変数値を明るみに出す（注意して使用）
  - デフォルト：機密値はマスク
...
- タイムアウト設定は更良な制御のためにヌル可能整数を使用するようになりました
